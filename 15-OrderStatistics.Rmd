...
# Order statistics {#OrderStatisticsChapter}
...
:::{.definition #OrderStat name="Oder statistics"}
Order statistics refer to the values in a sample arranged in increasing order. 
...
Important order statistics in a sample of size\ $n$ include the minimum value $X_{(1)}$, the maximum value $X_{(n)}$, and the median.\index{Median}
...
$$
  ...
  \end{cases}
$$
In some circumstances, the median for even\ $n$\ uses a slightly different formula.
:::


The *range*\index{Range} and the inter-quartile range (IQR)\index{Inter-quartile range} are derived from order statistics.
The range is defined as $X_{(n)} - X_{(1)}$.
...
Since the observations $X_1, X_2, \dots X_n$ are all independent and identically distributed (iid),\index{iid} the probability that *all* observations are less than or equal to some value\ $x$ is
$$
   F_{X_{(n)}}(x) = [F_X(x)]^{n}
$$
for $x \in R_X$.
...
plot( f_Max(x, n = 10) ~ x, 
     ...
     las = 1,
     axes = FALSE,
     main = expression( bold( atop(Probability~density~"function",
                              of~italic(X)[(10)]) )),
...
plot( F_Max(x, n = 10) ~ x, 
     ...
     col = ColourOpaque, 
     las = 1,
     main = expression( bold( atop(Distribution~"function",
                              of~italic(X)[(10)]) ) ),
     xlab = expression(italic(X)[(10)]),
...
hist(samples2, 
     ...
     xlim = c(0, 1),
     las = 1,
     main = expression(bold(Distribution~of~italic(X)[(2)])),
...
hist(samples6, 
     ...
     xlim = c(0, 1),
     las = 1,
     main = expression(bold(atop(Distribution~of~italic(X)[(6)]),
                            "("*the~maximum~value*")")),
...
The PDF of $T_{(2)}$ can be found using *convolution* (Sect.\ \@ref(Convolutions)):\index{Convolution}
$$
  f_{T_{(2)}}(t) = \int_0^t f_{E_1}(x) f_{E_2}(t - x)\, dx
$$
where $f_{E_1}(x) = 4\lambda \exp(-4\lambda x)$, and $f_{E_2}(x) = 3\lambda \exp(-3\lambda x)$.
This approach can be extended to *any* $k$th failure out of\ $n$ components.


## Order statistics for discrete random variables {#OrderStatisticsDiscrete}
...
Let $X_1, X_2, \dots, X_n$ be iid from a discrete distribution with probability mass function
$$
   p_X(x) = \Pr(X = x),
$$
...
```{r OrderStatsDiscreteDefine, echo=FALSE, fig.align="center", fig.cap="Explaining the meaning of $F_X(b^-)$.", fig.width=8, fig.height=4, out.width="90%"}
...
axis(side = 2,
     at = c(1, 0),
     las = 1,
     label = c( expression(italic(F)[italic(X)](italic(b))),
...
text(x = -0.25 - Gap,
     y = 0.9,
     label = expression(italic(F)[italic(X)](italic(b))),
     pos = 2)
...
text(x = 0.25 + Gap,
     y = 0,
     label = expression(italic(F)[italic(X)](italic(b^"\u2013"))), #\u2013 is en-dash; \u2014 is em-dash
     pos = 4)
...
:::{.example #DiscreteOrderStatsDiceMin name="Rolling a fair die: minimum"}  
Suppose a fair die is rolled $n = 10$ times, and the number\ $X$ that appears on each roll is recorded.
Then $X \sim\text{Unif}(1, 6)$ and so
$$
  p_X(x)
  = 
  1/6 \qquad \text{for $x = 1, 2, \dots 6$}
  \qquad
  \text{and}
  \qquad
  F_X(x)
...
$$
  ...
  \text{for $x = 1, \dots, 6$.}
$$
These can be computed in **R**, then plotted (the dots in Fig.\ \@ref(fig:OrderStatsDiscreteExample), left panel):
...
More generally, for the $k$th order statistic\ $X_{(k)}$ for a discrete random variable\ $X$,  let $X_1, X_2, \dots, X_n$ be iid from a discrete distribution with
\begin{align*}
  p_X(x)   &= \Pr(X = x);\\
...
:::{.definition #OrderStatsDistnDiscrete name="Distribution of an order statistic: discrete rv"}
...
:::{.example #DiscreteOrderStatsDiceAny name="Rolling a fair die: general order statistic"}  
Continuing Example\ \@ref(exm:DiscreteOrderStatsDiceMin), where a standard six-sided die is rolled $10$\ times.
...
These can be computed in **R**, then plotted (the dots in Fig.\ \@ref(fig:OrderStatsDiscreteExample), right panel):

```{r OrderStatsDiscrete10thSim}
### SIMULATED 
...
```{r OrderStatsDiscrete10thTheory}
### SIMULATED 
# Simulation, using  rolls  from the last Example
Order_Stats_7 <- apply( rolls,
                        MARGIN = 1, # i.e., across rows
...
```{r OrderStatsDiscreteExample, echo=FALSE, fig.align="center", fig.cap="Left: the distribution of the first order statistic (the minimum roll), in sets of $10$\\ rolls. Right: the distribution of the $7$th order statistic, in sets of $10$\\ rolls. In both cases, the bars show the values from $1\\,000$ simulated sets of $10$\\ rolls, and the solid dots are the theoretical values.", fig.width=8, fig.height=4, out.width="90%", results='hide', echo=FALSE}
...
### Graph
min_Y <- -0.05  # Allows room for dots to fully appear near y=0
out <- barplot(table(min_Roll)/num_Sims,
               xlim = c(0, 7),
               ylim = c(min_Y, 0.95),
               main = expression( bold(atop(The~distribution~of~the~minimum,
                                       roll~"in"~sets~of~10~rolls)) ),
               xlab = "Minimum roll in set of 10 rolls", 
               ylab = "Proportion",
               col = ColourLight,
               las = 1)
...
<<OrderStatsDiscrete10thTheory>>
<<OrderStatsDiscrete10thSim>>
out <- barplot(table(Order_Stats_7)/num_Sims,
               xlim = c(0, 7),
               ylim = c(min_Y, 0.45),
               main = expression( bold(atop(The~distribution~of~the~seventh,
                                       order~statistic~"in"~sets~of~10~rolls) ) ),
               xlab = expression(italic(X)[(7)]~"in"~sets~of~10~rolls), 
               ylab = "Proportion",
               col = ColourLight,
               las = 1)
points( x = out,
...
## Quantiles and order statistics {#QuantilesOrderStats}

So far, order statistics have been defined.
A similar, but different, concept is that of *quantiles*.\index{Quantiles}
...
**R** can also find quantiles,\index{Quantiles} using the `quantile()` function.
`quantile(x, probs)` computes the order statistics specified as probabilities (called *quantiles*)\index{Quantiles} defined in `prob`.
Generally, the *exact* quantiles cannot be found, and are approximated.
**R** offers *nine* different methods for approximating quantiles that cannot be exactly defined [@hyndman1996sample].
:::


:::{.example #RQuantiles name="Quantiles in R"}
...
Selected answers appear in Sect.\ \@ref(AnswersOrderStatistics).


::::{.exerciseList}
:::{.exercise #OrderStatisticsUniform}
Prove that the probably density function for the order statistics in the case of the continuous uniform distribution, shown in Eq.\ \@ref(eq:ContinuousUniformPMForder), is a valid density function.
...
Prove that the expected value and variance of the order statistics in the case of the continuous uniform distribution are
$$
...
Find the distribution of the order statistics for a sample of size\ $n$ drawn from the continuous uniform distribution defined on $[-1, 1]$.
:::


:::{.exercise #OrderStatisticsPDF2}
Find the distribution of the order statistics for samples of size\ $n$ drawn from the continuous distribution
$$
...
Suppose a sample of\ $n$ independent values is taken from the exponential distribution $\text{Exp}(\lambda)$ (Sect.\ \@ref(ExponentialDerivation)).
...
Consider Equation\ \@ref(eq:OrderStatisticPMF), which gives the $k$th order statistic for a discrete random variable.

1. Show that the expression given for the probability mass function for the *minimum* value is given by using $k = 1$ in Equation\ \@ref(eq:OrderStatisticPMF).
1. Use Equation\ \@ref(eq:OrderStatisticPMF) to find a formula for the probability mass function for the *maximum* value.
:::
...
2. Consider the discrete random variable\ $X$, where
  $$
    ...
    \end{cases}
  $$
  Show that the above expressions and Equation\ \@ref(eq:OrderStatisticDFdiscrete) are equivalent for the case $n = 3$ and finding the second order statistic $X_{(2)}$.
:::
...
You record the number of times you have to select a number before you have success (i.e., a larger number that the initially-selected number). 

More precisely:

1. Let $X\sim\text{Unif}(0, 1)$, your initial number.
2. You then draw $Y_1, Y_2, \dots \overset{\text{iid}}{\sim} \text{Unif}(0, 1)$, until you get a value $Y_n > X$.
...
:::{.exercise #OrderFibreCable}
A fibre cable is composed of $n = 5$ independent strands. 
The breaking strength of each strand\ $X$ follows an exponential distribution with mean $100\kgs$ (so that $X \sim \text{Exp}(1/100)$). 
The cable fails as soon as the *first* strand breaks.

1. Identify the order statistic that represents the cable's breaking strength.
2. Find the distribution of $X_{(1)}$.
3. Use **R** to find the probability the cable breaks under a load of $10\kgs$.
:::


:::{.exercise #OrderFloods}
A levee is designed to withstand the maximum annual water level. 
Suppose the maximum water levels (in metres) over the next $10$\ years are independent and follow a Uniform distribution $U(0, 15)$.

1. What is the CDF of the $10$-year maximum, $X_{(10)}$?
2. Calculate the probability that the $10$-year maximum exceeds $14\ms$.
3. In **R**, simulate $1\,000$ decades and find the empirical median of $X_{(10)}$.
:::


:::{.exercise #OrderSequencing}
A sequencer reads a DNA fragment $n$\ times to ensure accuracy. 
Let\ $X_i$ be the quality score of the $i$-th read. 
To filter out noise, the lab uses the **median** score of three reads ($X_{(2)}$). 
Assume $X_i \sim U(0, 100)$.

1. Write the general PDF formula for the $k$-th order statistic from a sample of size\ $n$.
2. Specifically, find the PDF of $X_{(2)}$ for $n = 3$.
3. Use **R** to plot the density of $X_{(2)}$.
:::


:::{.exercise #OrderSensors}
A satellite has three redundant temperature sensors. 
The system remains operational as long as at least two sensors provide a reading below $80^0$C. 
Let\ $X_i$ be the temperature read by sensor\ $i$, where $X_i \sim \text{Normal}(\mu=75, \sigma=5)$.

1. Express the *system success* event in terms of an order statistic.
2. Calculate the probability of success using the **R** function `pnorm()` and the binomial logic (or order statistic CDF).
:::
::::
