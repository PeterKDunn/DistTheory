...
The change of variable method requires find the pre-image, then accounting for the how transformation redistributes probability (through counting in the discrete case; through the Jacobian in the continuous case).


|                | **Discrete** | **Continuous** |
|----------------|--------------|----------------|
| **Univariate** | Sum $p_X$ over pre-images of $y$ | Substitute inverse; multiply by $\|J\|$ |
| **Bivariate**  | Sum $p_{\mathbf{X}}$ over pre-images of $(y_1, y_2)$ | Substitute inverses; multiply by $\|J\|$ (det of matrix) |
...
```{r bivariate-discrete, echo=FALSE, fig.width = 9, fig.height = 5}
n1 <- 10; n2 <- 8; p <- 0.4
m_vals <- 0:max(n1, n2)
