...
```{r TwoTransforms, echo=FALSE, fig.align="center", fig.cap="Three transformations: a non-one-to-one transformation (left panel), and two one-to-one transformation (centre and right panels).", fig.height=4, out.width='100%', fig.width=7}
par(mfrow = c(1, 3))
...
lines( y2 ~ x2,
       col = plotColour,
       lwd = 3)
lines(x = c(min(x2), min(x2)),
      y = c(0, min(y2)),
      lty = 3)
lines(x = c(max(x2), max(x2)),
      y = c(0, max(y2)),
      lty = 3)
...
Since $\mathcal{R}_Y = \{0, 1, 2\}$ the mapping is *not* one-to-one:\index{Transformation!not one-to-one} the event $Y = 0$ occurs if $X = 3$, the event $Y = 1$ occurs if $X = 2$ or $X = 4$, and the event $Y = 2$ occurs if $X = 1$ or $X = 5$.
