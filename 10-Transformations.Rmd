...
Let $X \sim \text{Geometric}(p)$ (number of trials until first success, support $\{1, 2, 3, \ldots\}$), so
$$
  \Pr(X \leq x) = 1 - (1-p)^x, \quad x = 1, 2, 3, \ldots
$$
Define $Y = X - 1$ (number of **failures** before the first success, support $\{0, 1, 2, \ldots\}$). For $y \in \{0, 1, 2, \ldots\}$:
$$
  F_Y(y) = \Pr(Y \leq y) = \Pr(X - 1 \leq y) = \Pr(X \leq y + 1) = 1 - (1 - p)^{y + 1}.
$$
The PMF is
$$
  p_Y(y) = F_Y(y) - F_Y(y-1) = (1-p)^y p, \quad y = 0, 1, 2, \ldots
$$
This is the $\text{Geometric}_0(p)$ parameterisation (failures before first success), confirming that a simple shift of support corresponds to a reparameterisation within the same family.
:::


```{r discrete-univariate, fig.width = 9, fig.height = 4.5, echo=FALSE}
p_geom <- 0.3
x_vals <- 1:20
y_vals <- 0:19

# CDFs
F_X <- pgeom(x_vals - 1, prob = p_geom)   # P(X <= x), X is trials-until-success
F_Y <- pgeom(y_vals,     prob = p_geom)   # P(Y <= y), Y = X - 1

par(mfrow = c(1, 2), mar = c(4, 4, 3, 1))

# CDF of X
plot(x_vals, F_X, type = "s", col = "#2c7bb6", lwd = 2,
     xlab = "x", ylab = "F(x)",
     main = expression("CDF of X ~ Geometric"[1]*"(0.3)"),
     ylim = c(0, 1), las = 1)
points(x_vals, F_X, pch = 16, col = "#2c7bb6", cex = 0.9)
grid(col = "grey85", lty = 1)

# CDF of Y = X - 1
plot(y_vals, F_Y, type = "s", col = "#d7191c", lwd = 2,
     xlab = "y", ylab = "F(y)",
     main = expression("CDF of Y = X - 1 ~ Geometric"[0]*"(0.3)"),
     ylim = c(0, 1), las = 1)
points(y_vals, F_Y, pch = 16, col = "#d7191c", cex = 0.9)
grid(col = "grey85", lty = 1)
...
$$
  ...
  = \frac{y^{1/2 - 1} e^{-y/2}}{2^{1/2}\,\Gamma(1/2)}, \quad y > 0.
$$
This is the $\chi^2(1)$ distribution\index{Chi-squared distribution} (i.e.\ $\text{Gamma}(1/2,\, 1/2)$).
:::


```{r continuous-univariate, echo=FALSE, fig.width = 9, fig.height = 4.5}
y_seq <- seq(0.001, 8, length.out = 500)

# Derived pdf via CDF differentiation
# f_Y(y) = phi(sqrt(y)) / sqrt(y)  where phi is standard normal pdf
f_derived <- dnorm(sqrt(y_seq)) / sqrt(y_seq)
f_chi2    <- dchisq(y_seq, df = 1)
...
plot(x_seq, x_seq^2, type = "l", col = "#7b2d8b", lwd = 2,
     xlab = "x", ylab = "y = x^2",
     main = expression("Transformation Y = X"^2),
     las = 1)
abline(h = 2, lty = 2, col = "grey50")
segments(c(-sqrt(2), sqrt(2)), c(0, 0), c(-sqrt(2), sqrt(2)), c(2, 2),
         lty = 2, col = "#d7191c")
text(0, 2.3, expression(sqrt(y)), cex = 0.85, col = "#d7191c")
grid(col = "grey85", lty = 1)

# Derived pdf vs chi-squared(1)
hist(Y_sim, breaks = 150, freq = FALSE, xlim = c(0, 8),
     col = "grey85", border = "white",
     main = expression("pdf of Y = X"^2 * " ~ " * chi^2 * "(1)"),
     xlab = "y", las = 1)
lines(y_seq, f_chi2, col = "#d7191c", lwd = 2.5)
legend("topright", legend = expression(chi^2 * "(1) pdf"),
       col = "#d7191c", lwd = 2, bty = "n")
grid(col = "grey85", lty = 1)
