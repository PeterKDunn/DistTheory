...
\reading{\DGS, Section~2.4, page~79}
...
The value of $p^{(2)}_{ij}$ can be interpreted as the element of the $i$th row and the $j$th column of the matrix $\bv{P}^2$. 
In, general, the term $p^{(m)}_{ij}$ will be the $ij$th entry in the matrix $\bv{P}^m$, which is called the *m-step transition matrix*.
...
This final summation is recognized as the $j$th component of $\bv{\nu} \bv{P}$.
After $n$ steps the distribution will be given by $\bv{\nu} \bv{P}^n$.
...
For Model I, the following R code shows the long term behaviour by examining $\bv{P}^n$ as $n$ increases:
...
It is clear that the system is periodic (period = 2), as all the even powers of $\bv{P}$ are the same, as well as the odd.
...
P8
```
In this case, *all* the rows of the matrix $\bv{P}^n$ converge to the same values. 
You can confirm the *any* initial vector $\bv{\nu}$ will converge
to the distribution given by the rows of $\bv{P}^{\infty}$; ie, $(2/7, 3/7, 2/7) = \bv{\nu}\bv{P}^{\infty}$.
...
If our Markov chain has the property that *for some* $m$, the matrix $\bv{P}^m$ has all of its elements strictly positive, then the matrix is *regular*. 
Further, there exists a *limiting distribution* given by the rows of the matrix
\[
   \lim_{m\to\infty}\bv{P}^m.
\]
