...
This number of kicks landed\ $X$ can be modelled using a binomial distribution with probability\ $\theta$, but the probability of a successful kick\ $\theta$ may vary (depending on the conditions, where the kick is taken from, who is taking the kick, fatigue, and so on), such that $\theta \sim \text{Beta}(4, 4)$.
:::
...
Hence, the distribution $f_X(x; \theta)$ is effectively a *conditional* distribution $f_{x\mid\Theta}(x)$, and the parameter\ $\Theta$ is effectively a random variable.

The *marginal* distribution of\ $X$ (i.e., $f_X(x)$) can be found from Def.\ \@ref(def:BVContMarginal) if\ $X$ and\ $\Theta$ are continuous:
\begin{equation}
   f_X(x)
   = \int f_{X \mid \Theta}(x)\,f_\Theta(\theta)\,d\theta.
   (\#eq:marg1)
\end{equation}
For\ $X$ and\ $\Theta$ discrete, Def.\ \@ref(def:BVDiscreteMarginal) yields
\begin{equation}
   p_X(x) 
   = \sum_\theta p_{X \mid \Theta}(x)\,p_\Theta(\theta).
   (\#eq:marg2)
\end{equation}
The mixed cases can be dealt with similarly: 
\begin{equation}
   f_X(x) 
   = \sum_\theta f_{X}(x)\,p_\Theta(\theta)
   (\#eq:marg3)
\end{equation}
for\ $X$ continuous and\ $\Theta$ discrete, and
\begin{equation}
   p_X(x)
   = \int p_{X \mid \Theta}(x)\, f_\Theta(\theta)\,d\theta
   (\#eq:marg4)
\end{equation}
...
The score\ $X$ achieved by a student on an examination is normal with mean\ $\Theta$ and standard deviation $\sigma = 5$; that is, for some *given* value of\ $\Theta$, marks are distributed as $X \sim N(\Theta, 5^2)$.
...
From Eq.\ \@ref(eq:marg1), the marginal PDF of\ $X$ is
\begin{align}
  f_X(x)
  &= \int f_{X \mid \Theta}(x)\,f_\Theta(\theta)\,d\theta\notag\\
...
Then\ $X \sim \text{Poisson}(\Theta)$ for some given value of\ $\Theta$.
...
$$
From Eq.\ \@ref(eq:marg2), the marginal PMF of\ $X$ is
\begin{align}
  p_X(x)
  &= \sum_\theta p_{X \mid \Theta}(x)p_\Theta(\theta)\notag\\
  &= 0.9 \frac{\exp(-10)\,10^x}{x!} + 0.1 \frac{\exp(-20)\,{20^x}}{x!}\quad\text{for $x = 0, 1, 2, \dots$}.
...
From Eq.\ \@ref(eq:marg4), the marginal PDF of\ $X$ is
\begin{align}
  p_X(x)
  &= \int p_{X \mid \Theta}(x)\,f_\Theta(\theta)\,d\theta\notag\\
...
\end{align}
where the integral can be simplified by seeing it is a Beta function (see Eq.\ \@ref(eq:BetaFunction)) with parameters $x + 4$ and $14 - x$, so that $\int_0^1 \theta^{x + 3}(1 - \theta)^{10 - x + 3}\,d\theta = \Gamma(x - 4)\Gamma(14 - x)/\Gamma(18)$.
Then:
$$
  p_X(x)
...
```{r RandomMarginal, echo=FALSE, out.width='100%', fig.width = 7.0, fig.height=2.25, fig.align="center", fig.cap="Marginal distributions"}
par( mfrow = c(1, 3),
     mar = c(5, 2, 4, 2) + 0.1)
...
Suppose that\ $X$ and\ $Y$ are discrete random variables, where $p_Y(y)$ is the probability function for\ $Y$ and $p_{X \mid Y}(x)$ is the conditional PMF for\ $X$ given\ $Y$.
Then the conditional PMF of\ $Y$ given\ $X$ is
\begin{align}
  p_{Y \mid X}(y)
  &= \frac{p_{X, Y}(x, y)}{p_X(x)} \\
  &= \frac{p_{X \mid Y}(x)\,p_Y(y)}{\sum_y p_{X \mid Y}(x)p_Y(y)}.
  (\#eq:Bayesdis)
\end{align}
...
\begin{align}
   ...
   &= \frac{f_{X, Y}(x, y)}{f_X(x)}
    = \frac{f_{X \mid Y}(x)\, f_Y(y)}{\int_y f_{X \mid Y}(x)\, f_Y(y)\,dy}
    = \frac{f_{X \mid Y}(x)\,f_Y(y)}{f_X(x)};
   (\#eq:Bayescts)\\
  p_{Y \mid X}(y)
  &= \frac{f_{X, Y}(x, y)}{p_X(x)}
   = \frac{f_{X \mid Y}(x)\,p_Y(y)}{\sum_y f_{X \mid Y}(x)\,p_Y(y)}
   = \frac{f_{X \mid Y}(x)\,p_Y(y)}{p_X(x)};
  (\#eq:Bayesdiscts)\\
  f_{Y \mid X}(y, x)
  &= \frac{f_{X, Y}(x, y)}{p_X(x)}
   = \frac{p_{X \mid Y}(x)\,f_Y(y)}{\int_y p_{X \mid Y}(x)\,f_Y(y)\,dy}
   = \frac{p_{X \mid Y}(x)\,f_Y(y)}{p_X(x)}.
  (\#eq:Bayesctsdis)
\end{align}
<!-- \begin{align} -->
<!--    f_{Y \mid X}(y, x) -->
...
In all cases, Eqs.\ \@ref(eq:Bayesdis) to\ \@ref(eq:Bayesctsdis), the left-hand side shows that the distribution is a function of\ $y$, given the observed data\ $X$.
...
\begin{align*}
   f_{\Theta|X}(\theta \mid x)      
   &\propto f_{X|\Theta}(x)      \times f_\Theta(\theta)\\
   \text{Posterior distn} 
   &\propto \text{Sampling distn} \times \text{Prior distn}.
\end{align*}
Here:
...
The distribution for\ $\Theta$ is assumed *prior* to seeing the data (i.e., $f_\Theta(\theta)$), and is then adjusted with the knowledge of the data\ $X$ to produce the *conditional* (or *posterior*) distribution $f_{\Theta|X}(\theta)$.
...
* The data\ $X$ are then observed.
* Using this extra available information, the description of\ $\theta$ is updated. 
  This is the *conditional* distribution $f_{\Theta \mid X}(\theta)$, called the *posterior* distribution in this context.


:::{.example #ExamScores3 name="Exam scores"}
(Follows Examples\ \@ref(exm:RandomPar1) and\ \@ref(exm:RandomParExam).)
The conditional PDF of the mean score\ $\Theta$, given an observed exam score $X = x$, follows directly from Eq.\  \@ref(eq:Bayescts) on substituting Eq.\ \@ref(eq:exam2):
\begin{align}
  f_{\Theta \mid X}(\theta)
  &= \frac{f_{X|\Theta}(x)\,f_\Theta(\theta)}{f_X(x)}\notag\\
...
* After seeing an exam score $x$, the adjusted (*posterior*) distribution of the mean score is $\Theta\mid (X=x) \sim N(0.8x + 12, 4.472^2)$.
...
```{r PosteriorExam, echo=FALSE, fig.align="center", out.width='100%', fig.height=2.25, fig.width=7, fig.cap="Exam scores: The prior distribution for the mean $\\Theta$ (left panel) and the posterior distribution when $x = 50$ centre panel) and when $x = 70$ (right panel). The dashed grey lines in the two right plots shows the prior distribution."}
par( mfrow = c(1, 3),
     mar = c(5, 2, 4, 2) + 0.1)
...
plot(yA ~ xx,
     ...
     main = expression(bold(Prior~prob.~of~Theta)),
     axes = FALSE)
axis(side = 1,
     at = seq(0, 100, 
              by = 50))
...
plot(yA ~ xx,
     ...
     xlim = range(xx),
     xlab = expression(Mean~Theta),
     main = expression(bold(atop("Posterior distn", 
                            when~italic(x)==50))),
     axes = FALSE)
axis(side = 1,
     at = seq(0, 100, 
...
(Follows Examples\ \@ref(exm:RandomPar1) and\ \@ref(exm:RandomParAccidents).)
From Eq.\ \@ref(eq:RandomParAccidents) and Eq.\ \@ref(eq:Bayescts), the conditional probabily function of the mean number of accidents per day\ $\Theta$ given\ $X$ is 
\begin{align}
  p_{\Theta \mid X}(\theta)
  &= \frac{f_{X \mid \Theta}(x)\,f_\Theta(\theta)}{p_X(x)}\notag\\
  &= \begin{cases}
        \displaystyle \frac{0.9\frac{\exp(-10)10^x}{x!}} {0.9 \frac{\exp(-10)10^x}{x!} + 0.1 \frac{\exp(-20){20^x}}{x!}} & \text{for $\theta = 10$}\\[6pt]
        \displaystyle \frac{0.1\frac{\exp(-20)20^x}{x!}}
     {0.9 \frac{\exp(-10)\,10^x}{x!}+ 0.1 \frac{\exp(-20)\,{20^x}}{x!}} & \text{for $\theta = 20$}.
  \end{cases}\notag\\
  &= \begin{cases}
        \displaystyle \frac{0.9 \exp(-10)\,10^x} {0.9 \exp(-10)\,10^x + 0.1 \exp(-20)\,{20^x}} & \text{for $\theta = 10$}\\[6pt]
        \displaystyle \frac{0.1 \exp(-20)\,20^x}
     {0.9 \exp(-10)\,10^x + 0.1 \exp(-20)\,{20^x}} & \text{for $\theta = 20$}.
   (\#eq:accidents3)
  \end{cases}
\end{align}
...
```{r PostPosteriorAccidents, echo=FALSE, out.width='100%', fig.height=2.25, fig.width=5.5, fig.align="center", fig.cap="Vehicle crashes: The prior distribution for $\\Theta$ (left panel) and the posterior distribution when $x = 8$ crashes (centre panel) and when $x = 17$ crashes (right panel). The grey crosses in the two right plots shows the prior distribution."}
...
(Follows Examples\ \@ref(exm:RandomPar3) and\ \@ref(exm:RandomParGoalKicking).)
From Eq.\ \@ref(eq:marg4), the conditional PDF of\ $\Theta$ given\ $X$ is
$$
  f_{\Theta \mid X}(\theta)
  = \frac{p_{X \mid \Theta}(x)\, f_\Theta(\theta)}{p_X(x)}
$$
where $X \mid \theta \sim \text{Bin}(n = 10, p = \theta)$ so that
$$
  p_{X \mid\Theta}(x) = \binom{10}{x}\theta^x (1 - \theta)^{10 - x},
$$
and where $p_X(x)$ is given in Eq.\ \@ref(eq:goal2), and $\Theta \sim \text{Beta}(4, 4)$.
...
```{r PostPosteriorKicks, echo=FALSE, out.width='100%', fig.height=2.25, fig.width=5.5, fig.align="center", fig.cap="Goal kicking: The prior distribution for $\\theta$ (left panel) and the posterior distribution when $x = 8$ (i.e., lands 8 kicks out of 10; centre panel) and when $x = 1$ (i.e., lands 1 kick out of 10; right panel). The grey vertical lines correspond to the distribution means. The dashed grey lines in the two right plots show the prior distribution."}
...
plot(yA ~ xx,
     ...
     ylab = "Prob. function",
     main = expression(bold(Prior~prob.~theta) ))
axis(side = 1,
     at = c(0, 0.5, 1))
...
Applying meaning to the conditional distribution of parameters, such as those in Eqs.\ \@ref(eq:exam3), \@ref(eq:accidents3) and\ \@ref(eq:goal3), has historically been controversial in statistics.
...
The prior and posterior distribution combinations in Table\ \@ref(tab:PriorPosteriorTable)) and conjugate distributions.
...
Conjugates[2, ] <- c("$X \\sim \\text{Gamma}(\\alpha, \\beta)$",
                     "$\\beta \\sim \\text{Gamma}(\\alpha_0, \\beta_0)$",
                     "$\\beta \\mid X \\sim \\text{Gamma}(\\alpha_0 + n\\alpha, \\beta_0 + \\sum x)$")
Conjugates[3, ] <- c("$X \\sim \\text{Poisson}(\\lambda)$",
                     "$\\lambda \\sim \\text{Gamma}(\\alpha_0, \\beta_0)$",
                     "$\\lambda \\mid X \\sim \\text{Gamma}(\\alpha_0 + \\sum x, \\beta_0 + n)$")
...
Bayesian statistics is generally computational, as many of the integrals are difficult to compute or even impossible to compute.
...
1. Determine the *posterior* probability $f_{\theta\mid y}(\theta)$, and identify the distribution in terms of known distributions (including parameters).
