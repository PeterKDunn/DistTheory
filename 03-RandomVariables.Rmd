...
2. $\displaystyle \sum_{t \in R_X}  p_X(t) = 1$ where\ $R_X$ is the range of\ $X$; that is, the probability function covers the probability for all possible sample points in the sample space.
3. $p_X(t) = 0$ if\ $t \notin R_X$.
...
Then, let\ $X$ be the random variable 'the *larger* of the two numbers chosen', so that $R_X = \{2, 3, 4, 5\}$.
...
```{r DFCont, echo=FALSE, fig.align="center", fig.cap="The probability function (left) and the distribution function (right) for\\ $V$.", fig.width=7, out.width='90%', fig.height=3.5}
p <- 0.4
x <- seq(0, 4, 
         length = 100)
fy <- (1 - p) * exp( - x)
...
As seen from the graph of the distribution in Example\ \@ref(exm:TossingHeads2), the values of\ $x$ where a 'jump' in $F_X(x)$ occurs are the points in the range space, and the probability associated with a particular point in\ $R_X$ is the 'height' of the jump there.
That is,
\begin{equation}
...
The *quantile function* $Q_X(p)$ does the opposite of the distribution function.
The *quantile function* $Q_X(p)$ is the *smallest* value\ $x$ such that $F_X(x) \ge p$ for given values of\ $p$.
More formally,
\begin{equation}
  Q_X(p) = \inf\{x \in \mathbb{R} \mid F_X(x) \ge p\}.
  (\#eq:QuantileFunction)
\end{equation}
where 'inf' is the 'infimum', the greatest number that is less than or equal to every number in the set. 
...
When\ $X$ is a *continuous* random variable, this becomes an equality, and we can write
$$
   Q_X(p) = F_X^{-1}(p).
$$
The quantile function is defined for all $p\in [0, 1]$.
...
$$
   ...
   \end{cases}
$$
The quantile function\ $Q(p) = F^{-1}_X(p)$ is the solution to $p = 1 - \exp(-2x)$, which is
$$
   Q(p) =
   F_X^{-1}(p) = 
      -\frac{1}{2}\log(1 - p)
...
Consider the value $p = 0$; the value of $Q_X(0)$ is the *smallest* value of\ $x$ for which $p = 0$.
This corresponds to $x = 10$.
In fact, $x = 10$ is the smallest value of\ $x$ for which $p = 0$ up to $p = 0.1$.
So,
$$
  Q_X(p) = 10\quad\text{for $0 < p \le 0.1$}.
$$
Similarly, $x = 11$ is the smallest vaue of\ $x$ for which $0.1 < p \le 0.4$,
Continuing:
$$
  Q_X(p) = 
  \begin{cases}
...
For example, $Q_X(0.3) = 10$, and $Q_X(0.4) = 11$.
:::



```{r QuantileFnDiscrete, echo=FALSE, fig.align="center", fig.cap="Left: the distribution function of $X$. Right: the probability mass function of $X$.", fig.width=8, fig.height=4, out.width='100%'}
par(mfrow = c(1, 2))
...
yQ <- c(0, 5,  10,  11,  15, 17, 18)
xQ <- c(0, 0, 0.1, 0.4, 0.9,  1,  1)
...
Many statistical distributions are generated from existing distributions.
Many of these connections are explored later.
These connection can also be shown using computer simulation.

As an example, the chi-squared distribution\index{Chi-squared distribution} is related to the normal distribution,\index{Normal distribution} and is discussed later (XREF: SECT).
Suppose a random variable\ $Z$ has a standard normal distribution: $Z\sim N(0, 1)$.
Then, a chi-squared distribution (with one *degree of freedom*) is the distribution of\ $Z^2$: 
...
Furthermore, the sum of\ $k$ independent standard normal distributions is a chi-squared distribution with $k$\ dgrees of freedom:



```{r}
X <- matrix( data = rnorm(n = 5000,
                          mean = 0, 
                          sd = 1),
             nrow = 1000,
             ncol = 5)
