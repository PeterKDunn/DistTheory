...
The *range space* for a random variable $X$ is often denoted $R_X$. where $R_X\in\mathbb{R}$.
:::
...
* 'The random variable\ $X$' (e.g., 'the sum of the roll of two dice')...
* '... takes the value\ $3$ in some outcome of the random process'.

Once a random variable has been defined, events\index{Events} can be defined in terms of its values.
For example, the event 'The total is four' comprises the three outcomes
$$
  (1, 3), (2, 2), (3, 1),
$$
...
$$
  \{s \in S \mid X(s) = 4\},
$$
and is usually written more simply as $X = 4$.
Similarly, the event 'The total is less than 5', a subset of the sample space\ $S$, corresponds to the event 
$$
  \{s \in S \mid X(s) \le 5\},
$$
usually written as $X\le 5$.
...
$$
   R_Y = \{ Y(s) \mid s\in S\} = \{2, 3, \dots, 12\}.
$$
:::


:::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
*Random variables* are different from *variables* used in (for example) algebra.
...
$$
   p_X(x) = \Pr(X = x).
$$
The function $p_X(x)$ is called the *probability function* of\ $X$.
:::
...
Let\ $X$ be the random variable 'the *larger* of the two numbers chosen'; then $R_X = \{2, 3, 4, 5\}$.
...
```{r ContinuousPDF, echo=FALSE, fig.align="center", fig.cap="The probability function for\\ $W$.", fig.height=3.5, fig.width=5, out.width='55%'}
plot( x = c(-1, 2),
      y = c(0, 2),
      type = "n",
      las = 1,
...
plot(x = c(-1, 2),
     ...
     type = "n",
     las = 1,
     main = expression(paste("The probability distribution of ", italic(W))),
     xlab = expression(paste("Values of the rv ", italic(W))),
     ylab = expression(paste("The probability function ", italic(p)[italic(W)](italic(w))))
)
polygon(x = c(0, 0.5, 0.5, 0),
        y = c(0, 1, 0, 0),
        col = "skyblue")
lines(x = c(-0.5, 0, 1), y = c(0, 0, 2), lwd = 2)
points(1, 2, pch = 1)
points(1, 0, pch = 19)
'

# Set up 2-panel layout
par(mfrow = c(1, 2), mar = c(4, 4, 2, 1))  # Adjust margins if needed
...
polygon(x = c(0, 0.5, 0.5, 0), y = c(0, 1, 0, 0), col = "skyblue")
lines(x = c(-0.5, 0, 1), y = c(0, 0, 2), lwd = 2)
points(1, 2, pch = 1)
points(1, 0, pch = 19)
...
plot.new()
text_lines <- strwrap(plot_code, width = 55)
n_lines <- length(text_lines)

# Position text from top down
# Right: display the code as multi-line text
text_lines <- strwrap(plot_code, width = 60)
n <- length(text_lines)
y_positions <- seq(1, 0, length.out = n)  # top to bottom spacing

for (i in seq_along(text_lines)) {
  text(x = 0, y = y_positions[i], labels = text_lines[i],
