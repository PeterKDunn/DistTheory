...
The set of all possible pairs of outcomes, the *joint range* (or *support*) of $(X, Y)$, denoted $\mathcal{R}_{X,Y}$, is a subset of the Euclidean plane $\mathbb{R}^2$.
Each outcome $(X(s), Y(s))$ (usually written as $(X, Y)$) is represented as a point $(x, y)$ in this plane. 
As with univariate distributions, discrete and continuous random vectors must be distinguished to determine how probabilities are assigned to these points.
...
More formally, we write $(X, Y): S \to \mathbb{R}^2$.
:::


:::{.example #BVDiscreteEG name="Bivariate discrete"}
Consider a random process where:

1. *two* coins are tossed, and\ $X$ is the number of heads that show on the two coins; and
2. *one* die is rolled repeatedly, and\ $Y$\ is the number of rolls needed to roll a `r knitr::include_graphics("Dice/die6.png", dpi=2000)`.
...
Let $(X, Y)$ be a $2$-dimensional random variable.
The *joint probability mass function* (or joint PMF), denoted $p_{X, Y}(x, y)$, assigns a probability to each pair of outcomes $(x_i, y_j)$ such that $p_{X, Y}(x_i, y_j) = \Pr(X = x_i \cap Y = y_j)$.
...
:::{.definition #BivariatePDF name="Continuous probability density function"}
The *joint probability density function* (joint PDF) function of the pair of continuous random variables\ $X$ and\ $Y$ is a function $f_{X, Y}(x, y)$ such that:
...
```{r echo=FALSE, fig.align="center", out.width="100%", fig.width=9, fig.height=3.75, fig.cap="The joint PMF (left), a joint PDF (right)."}
# Set up a 1x3 layout
par(mfrow = c(1, 2),
    mar = c(4, 4, 3, 1))

# 1. DISCRETE JOINT (e.g., Two Dice)
x_disc <- 1:6
y_disc <- 1:6
z_disc <- matrix(1/36, 
                 nrow = 6, 
...
# Use 'symbols' to create a "bubble plot" where size = probability
plot(expand.grid(x_disc, y_disc), 
     pch = 19, 
     cex = 2, 
     col = "steelblue",
     xlab = expression(italic(x)),
     ylab = expression(italic(y)),
     main = "Discrete joint PMF",
     xlim = c(0, 7), 
     ylim = c(0, 7), 
     las = 1)
grid()

# 2. CONTINUOUS JOINT (Bivariate Normal)
x_cont <- seq(-3, 3, 
              length = 50)
y_cont <- seq(-3, 3, 
              length = 50)

# Create a simple 2D Gaussian surface
z_cont <- outer(x_cont, 
                y_cont, 
                function(x, y) (1/(2*pi)) * exp(-(x^2 + y^2)/2))
...
A *mixed joint distribution* occurs when one variable (say $X$) is discrete and the other ($Y$) is continuous.
...
To fully understand the mixed joint distribution case, *conditional distributions* must first be studied (Sect.\ \@ref(ConditionalDistributions)), so this case is deferred until later (Sect.\ \@ref(MixedJointPF))
...
Then, for example,
$$
  \Pr( \{X = 1, Y = 0, 1\} ) = \frac{12}{47} + \frac{6}{47} + 0 = \frac{18}{47}.
$$
:::
...
The above conditional probability density functions satisfy the requirements for a univariate PDF; that is, $f_{X \mid Y}(x \mid y) \ge 0$ for all\ $x$ and $\int_{\mathcal{R}_X} f_{X\mid Y}(x\mid y)\,dx = 1$.
...
To interpret the conditional distribution, for example $f_{X \mid Y = y}(x \mid y)$, consider slicing through the surface $f_{X, Y}(x, y)$
...
$$p_{X\mid Y}(x \mid Y=0) = \frac{p_{X,Y}(x,0)}{p_Y(0)} = \frac{p_{X,Y}(x,0)}{36/47}$$
giving $p(0\mid Y=0) = 6/36$, $p(1\mid Y=0) = 12/36 = 1/3$, $p(2\mid Y=0) = 18/36 = 1/2$.
...
$$
  f_{X,Y}(x, y_j) = f_{X \mid Y}(x \mid y_j) \cdot p_Y(y_j)
$$
which automatically satisfies both conditions above.

The probability of any event is then given by:
$$
...
row.names(EG1) <- c("$x = 1$",
                    "$x = 2$",
                    "$x = 3$")
EG1.caption <- "The bivariate distribution function for $X$ and $Y$."
if( knitr::is_latex_output() ) {
  knitr::kable(EG1,
               format = "latex",
               booktabs = TRUE,
...
Then,
\begin{align*}
  f_{Y\mid X = 1}(y\mid X = 1) &= 1\quad\text{for $0 < y < 1$};\\
  f_{Y\mid X = 2}(y\mid X = 2) &= 0.5\quad\text{for $0 < y < 2$};\\
  f_{Y\mid X = 3}(y\mid X = 3) &= 1\quad\text{for $1 < y < 2$}.
\end{align*}
...
Then,
\begin{align*}
  f_{Y\mid X = -1}(y\mid X = -1) &= 0.5\quad\text{for $-1 < y < 1$};\\
  f_{Y\mid X =  1}(y\mid X = 1)  &= 0.25\quad\text{for $-2 < y < 2$}.
\end{align*}
