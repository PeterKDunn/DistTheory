...
This chapter discusses the two-variable (bivariate) case.
...
These situations are handled by combining the summation and integration approaches of Defs.\ \@ref(def:BivariatePMF) and\ \@ref(def:BivariatePDF).

In this case, the relationship is described using a set of conditional density functions rather than a single joint PDF $f(x,y)$, because $X$ cannot be integrated. 
Instead, the total probability is still\ $1$ by using a hybrid approach:
$$
  \sum_{x \in \mathcal{R}_X} \int_{-\infty}^{\infty} f_{Y|X}(y|x) \Pr(X = x) \, dy = 1.
$$

:::{.example name="Insurance claims"}
Suppose\ $X$ indicates if a person has a car accident ($X = 1$ with probability\ $p$; $X = 0$ with probability $1 - p$). 
If $X = 1$, the claim amount\ $Y$ follows a continuous distribution
$$
  f_{Y}(y) = \exp(-y)\quad\text{for $y > 0$}.
$$
The joint behaviour is described by:

* $\Pr(X = 0) = 1 - p$ (a discrete point mass at $X = 0$); and
* $f_{X, Y}(1, y) = p \cdot  \exp(-y)$ for $y > 0$ (a continuous "slice" along the line $x = 1$)
:::


```{r echo=FALSE, fig.align="center", out.width="100%", fig.width=9, fig.height=3.75, fig.cap="The joint PMF (left), a joint PDF (center), and a mixed joint distribution (right)."}
# Set up a 1x3 layout
par(mfrow = c(1, 3),
    mar = c(4, 4, 3, 1))
...
# 3. MIXED JOINT (Discrete X, Continuous Y)
# We plot two separate density 'slices' for X=0 and X=1
y_seq <- seq(-4, 8, 
             length = 200)
dens0 <- dnorm(y_seq, 
               mean = 0, 
...
dens1 <- dnorm(y_seq, 
               mean = 3, 
               sd = 1.5) * 0.5 # P(X=1) = 0.5

plot(x = y_seq, 
     y = dens0, 
     type = "n", 
     ylim = c(0, 0.25),
     xlab = expression(italic(Y)~"(continuous)"), 
     ylab = "Density", 
     main = expression(bold(Mixed~"("*italic(X)~discrete*")") ), 
     las = 1)
...
plot( x = c(0, 1),
      ...
      ylab = expression(italic(y)),
      type = "n",
      main = expression(bold(The~region~where~italic(f)~is~defined)),
      las = 1)
polygon( x = c(0, 1, 0, 0),
         y = c(0, 1, 1, 0),
         col = plotColour)
...
$$
  ...
  =
  \begin{cases}
    0.4   & \text{for $0\le x \le 1$}\\
    0.3   & \text{for $0\le x \le 2$}.
  \end{cases}
$$
...
The joint PMF can be defined in\ **R**:\index{matrix()@\texttt{matrix()}}
...
P <- matrix(c(0.1, 0.2,
            ...
            nrow = 3, 
            byrow = TRUE,
            dimnames = list(x = x_vals, 
                            y = y_vals))
P
sum(P)          # Verify that the sum of probabilities is 1
```

The marginal distribution can be found as follows:\index{rowSums()@\texttt{rowSums()}}\index{colSums()@\texttt{colSums()}}

```{r}
rowSums(P)      # Marginal distribution of X
colSums(P)      # Marginal distribution of Y
```

The conditional distributions can also be found; for example, to find $\Pr(X = x \mid Y = 1)$:
...
The probabilities in the joint PMF are not the sameL $X$ and\ $Y$ are *not* independent.


### Continuous bivariate distributions

Consider the joint PDF in Example\ \@ref(exm:MixedRV).
...
We can verify that this is a valid joint PDF using `integrate()` twice, once in the $x$-direction and once in the $y$-direction:\index{integrate()@\texttt{integrate()}}

```{r}
# Verify integrates to 1 (integrate over y first, then x)
...
Marginal distribution can be found using `integrate()` also:\index{Vectorize()@\texttt{Vectorize()}}

```{r}
# Marginal of X at a specific value, say x = 0.3
integrate(function(y) fXY(0.3, y), 
          lower = 0.3, 
          upper = 1)$value

# Marginal of X as a function
...
We can simulate from a joint distribution too, using the factorisation $f_{X,Y}(x, y) = f_{X\mid Y} (x \mid y) \cdot p_Y(y)$.
Consider the mixed joint probability function in Example\ \@ref(exm:MixedRV).
We can simulate from this distribution:\index{sample()@\texttt{sample()}}\index{ifelse()@\texttt{ifelse()}}


```{r}
n <- 10000                  # Number of simulations
Y <- sample(c(1, 2),        # Select from the numbers 1 and 2
            size = n,       # Select this many 
            replace = TRUE, # Select *with* replacement
            prob = c(0.4, 0.6))  # Select according to these probabilities
X <- ifelse(Y == 1,               # If Y = 1:
            runif(n, 0, 1),       # then: f(x | Y = 1) = U(0,1)
            runif(n, 0, 2))       # If Y = 0: f(x | Y = 2) = U(0,2)
```

The marginal distribution is close to the $(0.4, 0.6)$ expected:

```{r}
# Verify marginal of Y
table(Y) / n
```
