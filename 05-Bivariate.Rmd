...
As with univariate distributions, we must distinguish between discrete and continuous random vectors to determine how probabilities are assigned to these points.
...
\begin{align}
  p_{X, Y}(x, y) &\geq 0 \quad \text{for all } (x, y) \in \mathcal{R}{X,Y} \\
  \sum{j} \sum_{i} p_{X, Y}(x_i, y_j) &= 1
...
When both $X$ and $Y$ are continuous, we describe their behavior using a joint probability density function (joint PDF). 
...
Joint PDFA function $f_{X,Y}(x, y)$ is a joint PDF if it satisfies:

* Non-negativity: $f_{X,Y}(x, y) \ge 0$ for all $(x, y) \in \mathbb{R}^2$.
* Total Volume: The total volume under the surface must be 1:
$$
  \int_{-\infty}^{\infty} \int_{-\infty}^{\infty} f_{X,Y}(x, y) \, dx \, dy = 1.
$$
To find the probability that $(X, Y)$ falls within a specific region $A$ in the plane, we calculate the volume above that region:
$$
...
For example, imagine\ $X$ is a "Treatment Indicator" (0 for Control, 1 for Medication) and $Y$ is "Recovery Time.
Then, $X$ has a probability mass function (PMF) and\ $Y$ has a probability density function (PDF) that might look different depending on whether $X = 0$ or $X = 1$.

In this case, the relationship is described using a set of conditional density functions. 
We don't have a single joint PDF $f(x,y)$ because $X$ cannot be integrated. 
Instead, we satisfy the "sum to 1" rule using a hybrid approach:
$$
  \sum_{x \in \mathcal{R}_X} \int_{-\infty}^{\infty} f_{Y|X}(y|x) P(X=x) \, dy = 1.
$$

:::{.example name="Insurance claims"}
Suppose\ $X$ indicates if a person has a car accident ($X=1$ with prob $p$; $X=0$ with prob $1-p$). 
If $X=1$, the claim amount $Y$ follows an exponential distribution.
The "joint" behavior is described by:

* $P(X=0, Y=0) = 1-p$ (A discrete point mass at the origin)
* $f_{X,Y}(1, y) = p \cdot \lambda e^{-\lambda y}$ for $y > 0$ (A continuous "slice" along the line $x=1$)
:::
