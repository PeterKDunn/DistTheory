...
:::{.theroem #LinearCombVariance name="Variance of a linear combvination"}
If $X_1, X_2, \dots, X_n$ are random variables and $a_1, a_2,\ldots a_n$ are any constants then
\[
...
\begin{align*}
     ...
     & = \sum_i a^2_i\text{E}(X_i-\mu_i)^2 + 2{\sum\sum}_{i < j}a_i a_j\text{E}(X_i - \mu_i)X_j - \mu_j)\quad\text{using Theorem \@ref(thm:ExpLinear}\\
...
Because of its importance this special case is called a corollary of Theorems \@ref(thm:ExpLinear) and \@ref(LinearCombVariance).
...
\end{align*}
Now Theorems \@ref(thm:ExpLinear) and \@ref(LinearCombVariance) can be expressed in vector form.
...
Using $H$ to refer to heights of husbands and $W$ to the heights of wives, the sample statistics are shown below (and where the estimate f the correlation is $+0.364$):
:
