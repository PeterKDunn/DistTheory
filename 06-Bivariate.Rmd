...
* compute of conditional mean and variance for a bivariate normal distribution.
:::
...
As in the one-dimensional case, distinguish between discrete and continuous random variables is necessary.
...
Let $X$ be the number of heads that show on the two coins, and $Z$ the number of rolls needed to roll a `r knitr::include_graphics("Dice/die6.png", dpi=2000)`.\smallskip

$X$ is discrete with $R_X = \{0, 1, 2\}$.
$Z$ is discrete with a countably infinite range space $R_Z = \{ 1, 2, 3, \dots\}$.\smallskip

The range space is $R_{X\times Z} = \{ (x, z): 0 \le x \le 2, z = 1, 2, 3, \dots\}$.
:::


The discrete case (for example, see Sect. \@ref(MultinomialDistribution)) and continuous case (Sect. \@ref(BVNormalDistribution)) are considered separately below.
...
and a table (Table \@ref(tab:Bivar1)).\smallskip
...
```{r Bivar1graph, echo=FALSE, warning=FALSE, message=FALSE, webgl=TRUE, fig.cap="A bivariate probability function", fig.align="center", out.width='80%',fig.width=4}
pmf1 <- data.frame(
...
`r if (knitr::is_latex_output()) {
   '(Fig. \\@ref(fig:BVDiscrete3)).'
} else {
   '(see below).'
}`
The *volume* under the surface is one.\smallskip
...
Consider a random process where *two* coins are tossed, and *one* die is rolled (simultaneously).
Let $X$ be the number of heads that show on the two coins, and $Y$ the number on the die.\smallskip
...
Consider the bivariate discrete distribution which results when two dice are thrown. \smallskip
...
The probabilities in Table \@ref(tab:BVTwoDice) are $\Pr(X = x, Y = y)$ for the $(x, y)$ pairs in the range space.\smallskip
...
On a randomly selected day, let $X_1$ be the proportion of time the ATM is in use (at least one customer is being served or waiting to be served), and $X_2$ is the proportion of time the walk-up window is in use.\smallskip
...
The probability function is given in Table \@ref(tab:JointPF).\smallskip
...
#                       "$x \\text{less than} 1$",
#                       "$x \\text{less than} 2$")
Bivar2Table[, 1] <- c("$x$ less than, equal to  $0$",
                      "$x$ less than, equal to  $0$",
...
\]
So $\displaystyle f_Y(y) = \frac{1}{6}(2 + y)$, for $0 \leq y \leq 2$.\smallskip
...
Similarly for the other rows.\smallskip
...
\]
Both these conditional density functions are density functions (verify!).\smallskip
...
plot(fx(y, x) ~ y,
     ...
     lwd = 2,
     ylim = c(0, 1),
     main = expression(Conditional~distn~of~italic(Y)~"|"~italic(X)==0.1) )
grid(nx = NA)
...
plot(fx(y, x) ~ y,
     ...
     lwd = 2,
     ylim = c(0, 1),
     main = expression(Conditional~distn~of~italic(Y)~"|"~italic(X)==0.9) )
grid(nx = NA)
...
`r if (knitr::is_latex_output()) {
   '(Fig. \\@ref(fig:BivarConditionalInterpretation)).'
} else {
   '(see below).'
}`
...
```{r BivarConditionalInterpretation, webgl=TRUE, echo=FALSE, fig.cap="A bivariate distribution, sliced at $Y =  c$, showing the conditional distribution of $X$ when $Y = 0.5$", fig.align="center", out.width='100%', fig.width = 7}
...
Consider again the experiment in Example \@ref(exm:BVDiscreteEG). 
...
Independence is, however, obvious from the description of the experiment (Example \@ref(exm:BVDiscreteEG)), and is easily seen from Table \@ref(tab:JointPF).
:::
...
\]
the random variables $X_1$ and $X_2$ are *not independent*.\smallskip
...
Determine $\text{E}(X + Y)$; i.e., the mean of the number of heads plus the number showing on the die.\smallskip
...
\begin{align*}
   ...
   &= \frac{6}{5} \int_0^1 \frac{x^3y}3 + \frac{x^2 y^3}2\biggm|_{x = 0}^{x = 1}\,dy\\
   &= \frac{6}{5} \int_0^1 \frac y3+\frac{y^3}2\,dy\\
   &= \frac{6}{5} \left(\frac{y^2}6 + \frac{y^4}8\right)\biggm|_0^1\\
   &= \frac{6}{5} \left(\frac16 + \frac18\right) = \frac7{20}.
\end{align*}
...
This theorem is no surprise after seeing Theorem \@ref(thm:ExpectationLinear), but powerful and useful. 
...
Theorem \@ref(thm:ExpTwoRV) naturally generalises to the expected value of a *linear combination of random variables*.
...
Computing the covariance is involved: $\text{E}(X)$, $\text{E}(Y)$, $\text{E}(X Y)$ need to be computed, and so the joint and marginal distributions of $X$ and $Y$ are needed.
...
* If the units of measurements change, the numerical value of the covariance changes, but the numerical value of the correlation stays the same.
   (For example, if $X_1$ is changed from kilograms to grams, the numerical value of the correlation will not change in value, but the covariance will.)
...
The joint pf for $(X_1, X_2)$ is shown in Table \@ref(tab:JointRship).\smallskip
...
But $X_1$ and $X_2$ *are certainly related* as $X_2$ was explicitly defined as a function of $X_1$.\smallskip
...
Suppose the random variables $X_1, X_2, X_3$ have respective means 1, 2, and 3, respective variances 4, 5, and 6, and covariances $\text{Cov}(X_1, X_2) = -1$, $\text{Cov}(X_1, X_3) = 1$ and $\text{Cov}(X_2, X_3) = 0$.\smallskip

Consider the random variables $Y_1 = 3X_1 + 2X_2 - X_3$ and $Y_2 = X_3 - X_1$. 
Determine $\text{E}(Y_1)$, $\text{E}(Y_2)$, $\text{var}(Y_1)$, $\text{var}(Y_2)$ and $\text{Cov}(Y_1,Y_2)$.\smallskip
...
Given a random sample of $8$ individuals, what is the probability that there will be $3$ each of types O and A and $1$ each of types B and AB?\smallskip
...
```{r BivarNormalPDF, webgl=TRUE, echo=FALSE, fig.cap="A bivariate normal distribution", fig.align="center", out.width='100%'}
mu <- c(0, 0)
...
```{r BivarNormalHTML, webgl=TRUE, echo=FALSE, fig.cap="A bivariate normal distribution", fig.align="center", out.width='80%'}
mu <- c(0, 0)
...
The scatterplot of the heights is shown in Fig. \@ref(fig:HtsPlots) (right panel).\smallskip
...
The pdf for the bivariate normal distribution for the heights of the husbands and wives could be written down in the form of \@ref(eq:bivarnormalpdf) for the values of $\mu_H$, $\mu_W$, $\sigma^2_H$, $\sigma^2_W$ and $\rho$ above, but this is tedious.\smallskip

Given the information, what is the probability that a randomly chosen man in the UK in 1980 who is $173$ centimetres tall had married a woman taller than himself?\smallskip
...
So far, parameters describing a model or population have been assumed to be fixed values.
...
The mean $\mu$ is not constant, but varies (due to the specific questions that appear on the exam, the amount of study completed, etc.), and follows an approximately normal distribution.
:::



:::{.example #RandomPar2 name="Vehicle crashes"}
Motor vehicle crashes occur in a town according to a Poisson distribution with a mean rate of 10 per day when fine, and 20 per day when wet.
About 10% of days are wet.
:::


:::{.example #RandomPar3 name="Goal kicking"}
Let $\theta$ be the probability that a rugby league goal-kicker lands a goal.
This number of kicks landed $X$ can be modelled using a binomial distribution, but the probability of a successful kick $\theta$ may vary (depending on the conditions, where the kick is taken from, and so on).
:::


In each of these examples, a distribution $f_{X}(x; \theta)$ describes the data, where the parameter $\Theta$ itself has a distribution, $f_\Theta(\theta)$.
...
The *marginal* distribution of $X$ can be found from Def. \@ref(def:BVContMarginal) if $X$ and $\Theta$ are continuous:
...
The parameters that describe the distribution of $\Theta$ are called *hyperparameters*.


:::{.example #RandomParExam name="Exam scores"}
The score $X$ achieved by a student on an exam is normal with mean $\Theta$ and standard deviation $\sigma = 5$.
The mean $\Theta$ varies with a normal distribution with two hyperparameters: the mean 60 and standard deviation 10. 
...
Then, $X\sim N(60, 11.18^2)$ (see Fig. \@ref(fig:RandomMarginal), left panel). 
(Note that $11.18 = \sqrt{5^2 + 10^2}$.)\smallskip
...
Then $X \sim \text{Pois}(M)$, where mean parameter $M$ has the distribution by $\Pr(M = 10) = 0.9$ and $\Pr(M = 20) = 0.1$, or

\[
   p_M(m) = 0.1^{(m - 10)/10} 0.9^{(20 - m)/10}\quad\text{for $m = 10, 20$}.
\]
...
Because of this, sometimes these expressions in \@ref(eq:Bayesdis) to \@ref(eq:Bayesctsdis) are written as proportionalities rather than equalities by omitting the denominator (the normalising constant):

\begin{align*}
   f_{Y|X}(y \mid x)      &\propto f_{X|Y}(x\mid y)      \times f_Y(y)\\
   \text{Posterior distn} &\propto \text{Sampling distn} \times \text{Prior distn}.
\end{align*}
This shows that a distribution for $Y$ is assumed *prior* to seeing the data (i.e., $f_Y(y)$), and is then adjusted with the knowledge of the data $X$ to produce the *conditional* (or *posterior*) distribution $f_{Y|X}(y\mid x)$.

In this context:

* The original, or *unconditional* distribution $f_\Theta(\theta)$ models our prior beliefs about the parameter $\theta$.
  This is called the *prior* distribution (modelling our beliefs about the distribution of study times *prior* to seeing the data).
...
\begin{align}
  ...
  \exp\left[ -\frac{1}{2}\left(\frac{\theta - (0.8x + 12)}{4.472}\right)^2 \right]
   (\#eq:exam3)
\end{align}
after some algebraic manipulation; i.e., $\Theta | (X = x)\sim N(0.8x + 12, 4.472^2)$.\smallskip

This is the distribution of the mean score, given an observed value of $x$.
The initial (*prior*) assumption about the mean score (Example \@ref(exm:RandomParExam)) was that $\Theta \sim N(60, 10^2)$.
After seeing an exam score, this is adjusted to $\Theta\mid X\sim N(0.8x + 12, 4.472^2)$:
...
* If observed exam score is less than $60$ (e.g., $x = 50$), the original expectation is adjusted down, since $0.8\times 50 + 12 = 52$.
* If observed exam score is greater than $60$ (e.g., $x = 70$), the original expectation is adjusted up, since $0.8\times70 + 12 = 68$.
...
```{r PosteriorExam, echo=FALSE, fig.align="center", out.width='100%', fig.height=2.25, fig.width=4.75, fig.cap="Exam scores: The prior distribution for the mean (left panel) and the posterior distribution when $x = 50$ centre panel) and when $x = 70$ (right panel)."}
par( mfrow = c(1, 3),
     mar = c(5, 2, 4, 2) + 0.1)
...
plot(yy ~ xx,
     ...
     xlim = range(xx),
     xlab = "Mean",
     main = expression(Posterior~distn:~italic(x)==50),
     axes = FALSE)
axis(side = 1,
     at = seq(0, 100, 
              by = 50))
...
plot(yy ~ xx,
     ...
     xlim = range(xx),
     xlab = "Mean",
     main = expression(Posterior~distn:~italic(x)==70),
     axes = FALSE)
axis(side = 1,
     at = seq(0, 100, 
              by = 50))
...
From \@ref(eq:RandomParAccidents) and \@ref(eq:Bayescts), the conditional pf of $M$ given $X$ is 

\begin{align}
  p_{M \mid X}(\mu, x)
  &= \frac{f_{X \mid M}(x, \mu)f_M(\mu)}{p_X(x)}\notag\\
  &= \begin{cases}
        \displaystyle \frac{0.9\frac{e^{-10}10^x}{x!}} {0.9 \frac{e^{-10}10^x}{x!} + 0.1 \frac{e^{-20}{20^x}}{x!}} & \text{for $\mu = 10$}\\[6pt]
        \displaystyle \frac{0.1\frac{e^{-20}20^x}{x!}}
     {0.9 \frac{e^{-10}10^x}{x!}+ 0.1 \frac{e^{-20}{20^x}}{x!}} & \text{for $\mu = 20$}.
  \end{cases}\notag\\
  &= \begin{cases}
        \displaystyle \frac{0.9 e^{-10}10^x} {0.9 e^{-10}10^x + 0.1 e^{-20}{20^x}} & \text{for $\mu = 10$}\\[6pt]
        \displaystyle \frac{0.1 e^{-20}20^x}
     {0.9 e^{-10}10^x + 0.1 e^{-20}{20^x}} & \text{for $\mu = 20$}.
   (\#eq:accidents3)
  \end{cases}
\end{align}
\smallskip

After observing the number of crashes, the prior belief is adjusted accordingly:

* If we observe 10 crashes, the prior belief is adjusted to
...
\[
  ...
  \end{cases}
\]
* If we observe 20 crashes, the prior belief is adjusted to
...
```{r PostPosteriorAccidents, echo=FALSE, out.width='100%', fig.height=2.25, fig.width=4.75, fig.align="center", fig.cap="Vehicle crashes: The prior distribution for $M$ (left panel) and the posterior distribution when $x = 10$ (centre panel) and when $x = 20$ (right panel)."}
...
plot(yy ~ xx,
     ...
     xlim = c(5, 25),
     ylim = c(0, 1),
     xlab = expression(Mean~M),
     ylab = "Prob. function",
     main = expression(Prior~prob.~M) )
points(yy ~ xx,
       pch = 19)
axis(side = 1,
     at = c(10, 20))
...
plot(yy ~ xx,
     ...
     xlab = expression(Mean~M),
     ylab = "Prob. function",
     main = expression(Posterior~prob.:~italic(x)==10 ) )
points(yy ~ xx,
       pch = 19)
axis(side = 1,
     at = c(10, 20))
...
plot(yy ~ xx,
     ...
     xlab = expression(Mean~M),
     ylab = "Prob. function",
     main = expression(Posterior~prob.:~italic(x)==20) )
points(yy ~ xx,
       pch = 19)
axis(side = 1,
     at = c(10, 20))
...
\begin{align}
  ...
  &= \frac{17!\theta^{x + 3} (1 - \theta)^{13 - x}}{(x + 3)! (13 - x)!}
   (\#eq:goal3)
\end{align}
which corresponds to a $\text{Beta}(4 + x, 14 - x)$.
...
```{r PostPosteriorKicks, echo=FALSE, out.width='100%', fig.height=2.25, fig.width=4.75, fig.align="center", fig.cap="Goal kicking: The prior distribution for $\\theta$ (left panel) and the posterior distribution when $x = 8$ (centre panel) and when $x = 1$ (right panel). The grey vertical lines correspond to the distribution means."}
...
plot(yy ~ xx,
     ...
     xlab = expression(Probability~theta),
     ylab = "Prob. function",
     main = expression(Posterior~prob.:~italic(x)==8 ) )
axis(side = 1,
     at = c(0, 0.5, 1))
abline(v = (4 + x)/(18),
       col = "grey")
...
plot(yy ~ xx,
     ...
     xlab = expression(Probability~theta),
     ylab = "Prob. function",
     main = expression(Posterior~prob.:~italic(x)==1) )
axis(side = 1,
     at = c(0, 0.5, 1))
abline(v = (4 + x)/(18),
       col = "grey")
...
The algebra involved in deriving posterior distributions is, except in special cases (Table \@ref(tab:ConjugateTable)), intractable and so numerical methods are necessary (including simulation techniques).


```{r ConjugateTable, echo=FALSE}
Conjugates <- array( dim = c(4, 3))
...
Monthly rainfall, for example, is commony modelled using gamma distributions (for example, @husak2007use).
...
cat("Rainfall exceeding 1200mm:",
    sum(MRain > 1200) / 1000 * 100, "%\n")
```
...
cat("Rainfall exceeding 1200mm:",
    sum(MRain2 > 1200) / 1000 * 100, "%\n")
```
Using simulation, it is also easy to simulate the impact of the scale parameter $\beta$ varying also, suppose with a normal distribution mean of 200 and variance of 9 (Fig \@ref(fig:RainfallSim), right panel):
...
MRain3 <- rgamma( 1000,
                  shape = rexp(1000, rate = 1/2),
                  scale = rnorm(1000, mean = 200, sd = 3))

cat("Rainfall exceeding 1200mm:",
    sum(MRain3 > 1200) / 1000 * 100, "%\n")
```

```{r echo=FALSE, RainfallSim, fig.width=6, fig.height = 3, out.width='100%', fig.align="center", fig.cap="KK"}
par( mfrow = c(1, 3))
...
:::{.exercise #DiscreteXY}
Discrete rvs $X$ and $Y$ have the joint pf shown in Table \@ref(tab:TableXY).
Determine:
...
The random variable $A$ has mean $3$ and variance $2$. 
The random variable $B$ has mean $4$ and variance $3$. 
...
:::{.exercise}
In studies of human reproduction [@paul2005testing], the number of menstrual cycles until pregnancy is denoted $Y$, where $Y$ is often modelled using a geometric distribution: $Y\sim \text{Geom}(p)$, where $p$ is the pre-cycle conception probability.
$p$ is usually considered constant for each couple, but can vary between couple with a beta distribution: $p\sim \text{Beta}(m, n)$.

1. Determine the marginal distribution of $Y$ (in terms of [beta-functions](thm:BetaFunctionProperties)).
1. Suppose $m = 1.2$ and $n = 2$.
   Compute the probability that couples take more than 3 cycles to fall pregnant.
:::
...
* For each rainfall event, say $i$ for $i = 1, 2, \dots N$, the *amount* of rain in event $i$, say $Y_i$, is modelled using a gamma distribution with parameter $\alpha$ and $\beta$.
...
1. Use simulation to produce one month of rainfall.
1. Repeat for 1000 simulations, and ploit the distribution.
1. What type of random variable is the monthly rainfall: discrete, continuous, or mixed? 
   Explain.
1. How often does the month have exactly zero rainfall?
1. What is the mean monthly rainfall?
1. What is the mean monthly rainfall in months where rain is recorded?
:::


:::{.exercise}
Show that the negative binomial distribution can be generated as follows:
Suppose $X \sim \text{Pois}(\lambda)$, where $\lambda\sim\text{Gamma}$.
:::
...
:::{.exercise}
For what value of $k$ is $f(x,y) = kxy$ (for $0 \le x \le 1$; $0 \le y \le 1$, a joint pdf?
...
1. Find the correlation coefficient between $U$ and $V$.
1. Under what conditions on $\sigma_x$, $\sigma_y$ and $\sigma_z$ are $U$ and $V$ uncorrelated?
:::
  

:::{.exercise}
Supposes $(X, Y)$ has joint probability function given by
...
:::{.exercise}
The random variables $X_1$, $X_2$, and $X_3$ have means $\mu_1 = 5$, $\mu_2 = 3$ and $\mu_3 = 6$, standard deviations $\sigma_1 = 2$, $\sigma_2 = 3$ and $\sigma_3 = 4$.
...
Show that the distribution of $Z_1 + Z_2 + \cdots + Z_n$ has the gamma distribution $\text{Gam}(n, \beta)$.
:::
...
*Daily* rainfall, however, is ore difficult to model as some days have exactly zero rainfall, and on some days a continuous amount of rainfalls; i.e., daily monthly is a *mixed* random variable (Sect. \@ref(RVsMixed)).
