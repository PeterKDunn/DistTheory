...
A typical graph of the bivariate normal surface above the $x$--$y$ plane is shown in Fig. \@ref(fig:BivarNormal).
...
```{r BivarNormal, webgl=TRUE, echo=FALSE, fig.cap="A bivariate normal distribution", fig.align="center", out.width='80%'}
...
Consequently the marginal distribution of $X$ can be found from \@ref(eq:BivariateProbFunctionDiscrete) and Def. \@ref(def:BVContMarginal) provided $X$ and $\Theta$ are continuous; i.e.,
\begin{equation}
   f_X(x)
   = \int f_{X|\Theta}(x,\theta)f_\Theta(\theta)\,d\theta.
   (\#eq:marg1)
\end{equation}
For $X$ and $\Theta$ discrete, \@ref(eq:BivarXYdiscrete) and \@ref(eq:BivariateMarginal) yield
\begin{equation}
   p_X(x) 
...
\begin{align}
  ...
  &= \frac{\Gamma(8)}{\Gamma(4)\Gamma(4)}\binom{10}{x}\int_0^1 \theta^{x + 3}(1 - \theta)^{n - x + 3}\,d\theta\notag\\
...
From \@ref(eq:Bayescts) and \@ref(eq:RandomParAccidents), the conditional pf of $M$ given $X$ is 
\begin{align}
  p_{M|X}(\mu,x)
  &= \frac{f_{X|M}(x,\mu)f_M(\mu)}{p_X(x)}\notag\\
  &= \begin{cases}
     \frac{0.9\frac{e^{-\mu}\mu^x}{x!}} {0.9 \frac{e^{-10}10^x}{x!}+ 0.1 \frac{e^{-20}{20^x}}{x!}} & \text{for $\mu = 10$}\\
     \frac{0.1\frac{e^{-\mu}\mu^x}{x!}}
     {0.9 \frac{e^{-10}10^x}{x!}+ 0.1 \frac{e^{-20}{20^x}}{x!}} & \text{for $\mu = 20$}\label{EQ:accidents3}
  \end{cases}
\end{align}
:::
