...
This chapter begins discussing the two-variable (bivariate) case, and then more than two variables (Sect. \@ref(MultivariateExtensions)).
...
```{r Bivar1graph, echo=FALSE, warning=FALSE, message=FALSE, webgl=TRUE, fig.cap="A bivariate probability function", fig.align="center", out.width='80%',fig.width=6}
...
In **R**, the multinomial distribution functions have the form `[dr]multinom(size, prob)` where `size`${}= n$ and `prob`${} = (p_1, p_2, \dots, p_k)$ for $k$ categories.
...
```{r BivarNormalPDF, webgl=TRUE, echo=FALSE, fig.cap="A bivariate normal distribution with means of 0, variances of 1, and a correlation coefficient of 0.5.", fig.align="center", out.width='100%'}
mu <- c(0, 0)
...
Because of this, sometimes these expressions in \@ref(eq:Bayesdis) to \@ref(eq:Bayesctsdis) are written as proportionalities rather than equalities by omitting the denominator (the normalising constant); also, write $\Theta$ as the paameter of interest:
...
\end{align*}
This shows that a distribution for $\Theta$ is assumed *prior* to seeing the data (i.e., $f_\Theta(\theta)$), and is then adjusted with the knowledge of the data $X$ to produce the *conditional* (or *posterior*) distribution $f_{\Theta|X}(\theta\mid x)$.
