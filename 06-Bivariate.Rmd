...
Motor vehicle crashes per day occur in a town according to a Poisson distribution with mean $\theta$.
...
This number of kicks landed $X$ can be modelled using a binomial distribution with probability $\theta$, but the probability of a successful kick $\theta$ may vary (depending on the conditions, where the kick is taken from, who is taking the kick, and so on), such that $theta \sim \text{Beta}(4, 4)$.
:::
...
So the *prior* belief abour $\Theta$ is that 
\[
  p_\Theta(\theta) = 
  \begin{cases}
...
Conjugates[5, ] <- c("",
                     "",
                     "where $\\mu_P = \\displaystyle \\frac{1}{1/\\sigma_0^2 + n/\\sigma^{2}} \\left(\\frac{\\mu _0}{\\sigma _0^2} + \\frac{\\sum_{i = 1}^n x_{i}}{\\sigma^2}\\right)$")
Conjugates[6, ] <- c("",
                     "",
                     "and $\\sigma_P^2 = \\left( \\frac{1}{\\sigma_0^2} + \\frac{n}{\\sigma^2} \\right)^{-1}$")
...
Repeat Exercise \@ref(exr:AandBsimple), but with $\text{cor}(A, B) = 0.2$.
:::
