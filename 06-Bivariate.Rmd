...
PCSM <- read.csv("/Users/pdunn2/Documents/Git-Projects/DistTheory/Data/PlaneCreekSugarMill/IDCJAC0001_033059_Data12.csv",
                 na.strings = "null")
PCSMjan <- PCSM$Jan
...
:::{.exercise}
Generate Tweedie from $N\sim\text{Pois}(\lambda$) and so on.
:::


:::{.exercise}
Show that the negative binomial distribution can be generated as follows:
Suppose $X \sim text{Pois}(\lambda)$, where $\lambda\sim\text{Gamma}$.
:::
...
1. Hence evaluate $\Pr(X \le (3/8), Y \le (5/8) )$.
:::
