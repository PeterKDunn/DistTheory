...
Consider the following discrete distribution where probabilities $\Pr(X = x, Y = y)$ are shown as a graph (Fig. \@ref(fig:Bivar1graph)) and a table (Table \@ref(tab:Bivar1)).\smallskip
...
To interpret the conditional distribution, for example $f_{X \mid Y = y}(x \mid y)$, consider slicing through the surface $f_{X, Y}(x, y)$ with the plane $y = c$ say, for $c$ a constant (Fig. \@ref(fig:BivarConditionalInterpretation)). 
...
For example:
\[
   f_{Y_1}(y_1) = 1 + 2y_1 - 3y_2^2\quad\text{for $0 < y_1 < 1$} CHECK???
\]
...
A typical graph of the bivariate normal surface above the $x$--$y$ plane is shown in
`r if( knitr::is_latex_output() ) {
   'Figs. \\@ref(fig:BivarNormalPDF).'
} else {
   'Fig. \\@ref(fig:BivarNormalHTML).'
}`
...
\]
Recall the interpretation of the conditional distribution of $X$ given $Y = y$ (Sect. \@ref(ConditionalDistributions)) and note the shape of this density in Fig. 
`r if( knitr::is_latex_output() ) {
   'Figs. \\@ref(fig:BivarNormalPDF).'
} else {
   'Fig. \\@ref(fig:BivarNormalHTML).'
