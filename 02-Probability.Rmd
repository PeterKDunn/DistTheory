...
1. **Demonstration of theory.**
   Simulation can be used to visually demonstrate theory (e.g., Sect.\ \@ref(SimulationCoinToss)).
...
# Compute P(Heads) after each toss
Prop_Heads <- cumsum(Tosses == "H") / Toss_Number  # cumsum() is the 'cumulative sum'
...
```{r}
Results <- replicate(num_Reps, sample(Deck,   # Sample from the  Deck  of cards
                                      size = 5,  # A hand of *five* cards 
                                      replace = FALSE)  ) # Without replacement
Results[, 1:4]  # Show the first four columns; each column is one hand of five cards

# For each Hand (i.e., column), count how many  Hearts
num_Hearts <- colSums(Results == "Hearts")

# Show the first four hands, and the count of Hearts:
rbind(Results[, 1:4], 
      num_Hearts[1: 4])

# Count how mnay have *more than 3* hearts
more_Than_3_Hearts <- sum( num_Hearts >= 3 )      # Count how many have at least 3 Hearts 
```
...
```{r}
# Print the result
prob_more_Than_3_Hearts <- more_Than_3_Hearts / num_Reps
cat("The prob. estimate is", round( mean(prob_more_Than_3_Hearts), 
                                    4), "\n") # Round to four decimal places
...
*Sensitivity analysis*\index{Sensitivity analysis} refers to the process of checking how (and how much) the results of a problem or model change when the inputs or assumptions are adjusted: how *sensitive* the results are to those inputs.
