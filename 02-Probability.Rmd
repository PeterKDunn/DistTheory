...
## Statistical computing {#ProbStatisticalComputing}
...
Suppose\ $10$ people are invited to a party, and sit at a round table.

Alice and Bob refuse to sit next to each other. (ex-partners?)

What is the probability that a random seating satisfies this constraint?

This is difficult (but not impossible) to compute exactly: the number of circular permutations would need to be counted, while excluding adjacent pairs.
This becomes a bit messy (???).


Simulation approach:

Randomly permute 10 names.

Check if Alice and Bob are neighbors.

Repeat 100\,000 times and compute fraction valid.

```{r}
set.seed(123)
n_sim <- 100000  # number of random seatings

# Names of friends
friends <- c("Alice","Bob","Carol","Dave","Eve","Frank","Grace","Hank","Ivy","Jack")

# Function to check if Alice and Bob are neighbors in a circular table
neighbors_ok <- function(arrangement) {
  n <- length(arrangement)
  # Circular adjacency: first and last are neighbors too
  for(i in 1:n) {
    if ((arrangement[i] == "Alice" && arrangement[(i %% n) + 1] == "Bob") ||
        (arrangement[i] == "Bob"   && arrangement[(i %% n) + 1] == "Alice")) {
      return(FALSE)
    }
  }
  return(TRUE)
...
# Run simulation
valid_count <- sum(replicate(n_sim, neighbors_ok(sample(friends))))

# Estimated probability
probability <- valid_count / n_sim
probability
```
...
However, is also easy to simulate in **R** to check the answer:
...
For this scenario, we could vary the probabilty that each patients os a no-show, and se what impact this has on the probability that the doctor runs late (e.g., 10%, 15%, 20%).
...
# Test different no-show probabilities
no_show_probs <- c(0.10,  # Look at three different no-show probabilities 
                   0.15, 
                   0.20)

sapply(no_show_probs, 
       function(p) mean(replicate(num_Sims, 
                                  simulate_Day(p))))
```
