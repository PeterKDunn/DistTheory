...
2. Confirm results.  
   Simulation can confirm (not *prove*) counter-intuitive results (such as the Monty Hall problem (Sect.\ \@ref(Gameshow)), the birthday paradox (Exercise\ \@ref(exr:BirthdayProblem)), or non-transitive dice (Exercise\ \@ref(exr:NonTransitiveDice)).
3. Answers tedious or complex situations.  
   Simulation can make tedious, difficult or complex computations easier, such as "What is the probability that at least cards 3 are hearts in a randomly-drawn hand of five cards?".
   Simulation gives quick approximations that bypass heavy combinatorial formulas.
4. Check answers and reasoning. 
...
5. Sensitivity analysis.  
   Simulation allows easy tweaking or changing of parameters to see the impacts on the results. (REF; EXAMPLE?)

An example of each is given below.


SECTION ON HOW MANY? WHY LARGER n GIVES BETTER ANSWERS (and point to sampoling chap!)

set.seed need and so on


### Simulation for demonstration of theory
...
### Simulation to confirm results


### Simulation to answers tedious or complex situations
...
num_Sims <- 5000  # Set the number of simulations
...
Now, we can run numerous simulations, using the **R** function `replicate()` (which repeatedly evaluates **R** code a specified number of times; in this case, the expression `check_If_Neighbours( sample(People) )` which checks if the given permutation of people has Anh and Barb seated as neighbours):

```{r}
# Run simulation
# Summing works, because  TRUE  is treated as 1, FALSE  is treated as 0
invalid_Count <- sum(replicate(num_Sims, 
...
### Simulation to check answers and reasoning
...
However, it is easy to miscount, or to not allow for certain restructions.
...
What’s the probability the clinic runs late?

For this scenario, we could vary the probability that each patients os a no-show, and se what impact this has on the probability that the doctor runs late (e.g., 10%, 15%, 20%).
In addition, we could change teh number of patients scheduled each day to see what the impact is (e.g., $18$ or\
$22$ patients).

First we set up teh scenario:


```{r}
set.seed(8091) # For reproducibility

num_Sims <- 5000  # number of simulated days
daily_Patients <- 20
```
...
```{r}
# Create an R function to simulate hwat happens on a day
simulate_Day <- function(no_Show_Prob = 0.15) {
  Arrivals <- rbinom(daily_Patients, 
                     1, 
                     1 - no_Show_Prob)  # 1 means "shows up"
  sum(Arrivals) > 15  # TRUE if clinic runs late
}
```
...
```{r}
no_Show_Prob <- 0.10 
mean(replicate(num_Sims, 
               simulate_Day(no_Show_Prob) ) )
...
Clearly, the greater the no-show probability, the smaller the chance of running late.
