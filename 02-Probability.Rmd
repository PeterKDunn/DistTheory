...
ppv_sim <- if (total_positives > 0) {
  true_positives / total_positives } else {
    NA
  }
ppv_sim
```
...
```{r}
cat("Prob (cancer | positive test) :", ppv_sim, "\n")
```
...
To do this (for\ $5\,000$ replications), effectively the above code is repeated and the result of interest stored from each replication (in `ppv`):

```{r}
num_Reps <- 5000 # The number of replications
ppv <- array( dim = num_Reps) # Create an array to hold the info we need
...
for (i in 1:num_Reps){
  ...
  # Estimated Prob(cancer | positive test)
  ppv_sim[i] <- if (total_positives > 0) {
    true_positives / total_positives } else {
      NA
    }
}
...
Simulation, however, can be used.
The strategy is:
...
Then we simulate numerous simulations of a deal of $5$\ cards, and count how many have at least $3$\ hearts:
...
A better estimate would be found using a larger number of simulations.
...
Then we run the simulation for the various no-show probabilities:
...
Then we print (and plot; see Fig.\ \@ref(fig:probLate)) the results:
...
2. Use a computer simulation to estimate the probabilities to determine the best strategy.
   *Hint:* remember the crucial information: the host of the show who knows where the car is, and opens one of the doors *not* selected by the contestant, *and reveals a goat*. 
:::
