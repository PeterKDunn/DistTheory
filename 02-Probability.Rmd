...
A collection of simple events is sometimes called a *compound event*.
:::
...
A *combination* is an *unordered* selection of elements (without replacements).\index{Combinations}
:::
...
In many cases, computer *simulation* can be used to help with computing probabilities.
...
Using a computer can be useful for many reasons when working with probabilities:

1. **Demonstration of theory.**
   Simulation can be used to demonstrate theory (e.g., Sect.\ \@ref(SimulationCoinToss)).
   Sometimes the theory can be difficult to understand, and a simulation can bridge the gap between formulas and reality.
2. **Confirmation and validation of results.**
...
4. **Checking answers, intuition and reasoning.**
...
Now, after each toss, the probability of obtaining a head using all the available information was computed after each toss:
...
Each time `runif()` is run, different random numbers are allocated to each personin the population.
...
This is like 'So you’ve basically 'flipped a biased coin' for each patient, where the chance of landing on 'cancer' is\ $2$%.

So we can obtain a list of whether each person has cancer or not.
...
To do this (for\ $5\,000$ replications), effectively the above code is repeated and the result of interest stored from each replication (in `cancer_Given_Pos`):

```{r}
num_Reps <- 5000 # The number of replications
cancer_Given_Pos <- array( dim = num_Reps) # Create an array to hold the info we need
...
# Now we replicate the scenario  num_Reps  times:
for (i in 1:num_Reps){
  # Allocate a random number between 0 and 1 to each patient
  patient_Probs <- runif( population_Size)
  cancer_Result <- patient_Probs < cancer_Prob
...
In Example\ \@ref(exm:BreastCancer), the probability was given as\ $0.1327$, which compares favourably with this simulation answer.
...
* Repeat this numerous times:
  * Randomly permute\ $10$ seating arrangements.
...
# Name the 10 people: use the 'initials' of the ten people
...
```{r}
# Function to check, for any given seating arrangement, if
# Anh and Barb are neighbours.
#
...
check_If_Neighbours <- function(Arrangement) { 
  ...
  # Loop over the people in the (extended) Arrangement.
  for (i in 1:n) {
    # Check if Anh and Barb  are neighbours:
    if (extended_Arrangement[i] == "A" &&  # This checks if A, then B
        extended_Arrangement[i + 1] == "B") are_Neighbours <- TRUE
    if (extended_Arrangement[i] == "B" &&  # This checks if B, then A
...
Now, we can run numerous simulations, using the **R** function `replicate()` (which repeatedly evaluates **R** code a specified number of times); in this case:

```{r}
# Run simulation
# Summing works, because  TRUE  is treated as 1, FALSE  is treated as 0
invalid_Count <- sum(replicate(num_Reps, 
                               check_If_Neighbours( sample(People))) )
   # sample(People) makes a random arrangement of the 10 people
```


In this code, the expression `check_If_Neighbours( sample(People) )` checks if the given permutation of people has Anh and Barb seated as neighbours.
This is then replicated `num_Reps` times, and adds up how many times they are seated together.
Then, print the result:

```{r}
# Estimated probability
prob <- invalid_Count / num_Reps
cat("Prob. of an UNacceptable seating arrangement: approx. ", round(prob, 4), "\n")
```
...
This question is not too difficult to answer using the counting methods of Sect.\ \@ref(CombsAndPerms); you should show that the answer is about\ $0.0927$.
However, it is easy to miscount.
...
Then we simulate numerous simulations of a deal of $5$\ cards using `replicate()`, and count how many hands have at least $3$\ hearts:
...
results <- replicate(num_Reps, {
  ...
  sum( Hand == "Hearts") >= 3      # Count how many have at least 3 Hearts 
})
```
...
```{r}
# Print the result
cat("The probability estimate is", round( mean(results), 4), "\n")
...
If more than\ $15$ patients show up, the clinic runs late.
...
For this scenario, we could vary the probability that each patients os a no-show, and see what impact this has on the probability that the doctor runs late (e.g., $10$%; $15$%; $20$%).
...
num_Reps <- 50000  # number of simulated days
daily_Patients <- 20
```
...
simulate_Day <- function(no_Show_Prob = 0.15, 
                         num_Patients = 20,
                         patient_Limit = 15) {
  ...
  # sum()  works, as R uses that TRUE = 1, and FALSE = 0
  number_Miss <- sum( patient_Probs < no_Show_Prob )

  running_Late <- ( (num_Patients - number_Miss) > patient_Limit) 
...
no_Show_List <- seq(0.01, 0.20,
                    by = 0.025)

# Find the mean of the TRUE and FALSE values returned.
# This works because  R  treats TRUE as 1, FALSE as 0
prob_Late <- array(dim = length(no_Show_List))

for (i in (1:length(no_Show_List)) ){ # For each probability:
  
  num_Days_Late <- sum(replicate(num_Reps, 
                                 simulate_Day(no_Show_List[i]) ) )
  prob_Late[i] <- num_Days_Late / num_Reps
}
```


Then\ resukts then can be printed (and plotted; see Fig.\ \@ref(fig:probLate)):

```{r plotProbLate, fig.show='hide'}
round(prob_Late, 4)

plot(prob_Late ~ no_Show_List,
     ...
     ylab = "Probability that a day runs late")
```
...
These dice are *non-transitive* (Miwin's dice)\index{Miwin's dice}; that is, in the long run, Die\ A beats Die\ B, Die\ B beats Die\ C... but Die\ C also beats Die\ A.
