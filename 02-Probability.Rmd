...
Using a computer can be useful for many reasons when working with prpbabilities:

1. Simulation can be used to demonstrate theory (e.g., Sect.\ \@ref(SimulationCoinToss)).
2. Simulation can confirm (not *prove*) counter-intuitive results (such as the Monty Hall problem (Sect.\ \@ref(Gameshow)), the birthday paradox (Exercise\ \@ref(exr:BirthdayProblem)), or non-transitive dice (Exercise\ \@ref(exr:NonTransitiveDice)).
3. Simulation can make tedious, difficult or complex computations easier, such as "what's the chance of getting at least 2 reds before 3 blues?") counting methods may be tedious or impossible. ???? (POOR EXAMPLE; REF, EXAMPLE?)
   Simulation gives quick approximations that bypass heavy combinatorial formulas.
4. Simulation can be used to check answers and reasoning.
   Analytic solutions can be verified by comparing to simulation results.
5. Simulation allows easy tweaking or changing of parameters to see the impacts on the results. (REF; EXAMPLE?)
...
* Die\ A: The six sides are labelled:  2 \qquad 2 \qquad 4 \qquad 4 \qquad 9  \qquad 9 
* Die\ B: The six sides are labelled:  1 \qquad 1 \qquad 6 \qquad 6 \qquad 8 \qquad 8
* Die\ C: The six sides are labelled:  3 \qquad 3 \qquad 5 \qquad 5 \qquad 7 \qquad 7
...
Use a computer simulation to confirm these results, and find the probabilities of each die winning against the other two.



birthday paradox, or non-transitive dice (SIMULATION EXERCISES!)).
:::
