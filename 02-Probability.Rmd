...
'
<em>S</em> = {<span class="HTcircle">H</span>, <span class="HTcircle">H</span>,
   <span class="HTcircle">H</span>, <span class="HTcircle">T</span>
   <span class="HTcircle">T</span>, <span class="HTcircle">H</span>
   <span class="HTcircle">T</span>, <span class="HTcircle">T</span>},
   '
}`
where
`r if (knitr::is_latex_output()) {
...
`r if (knitr::is_latex_output()) {
   ...
   '
   <em>A</em> = {<span class="HTcircle">H</span>, <span class="HTcircle">H</span>,
   <span class="HTcircle">T</span>, <span class="HTcircle">H</span>;
   '}`
notice that $A \subset S$.
...
:::{.example #ElementaryEvents2 name="Elementary events"}
...
```{r EventsFandG, echo=FALSE, out.width = '50%', fig.width=7, fig.align="center", fig.cap="Events $E$ and $G$"}
colourE <- rgb(0, 0, 255, 
               max = 255, 
               alpha = 125)
colourG <- rgb(0, 255, 0, 
...
Events $A$ and $B$ are *mutually exclusive* if, and only if, $A\cap B = \varnothing$; that is, they have no elements in common.
:::
...
Consider throwing a cricket ball, where the distance of the throw is of interest (Example\ \@ref(exm:CricketBallDefine)), so that $D$ is the set of possible distances: $D = \{ d \in \mathbb{R} \mid d > 0 \}$.

We can define these two events:  
\begin{align*}
   B_1:&\quad \text{Throw a cricket ball at least $40\ms$}  &&=  [40, \infty);\\
   B_2:&\quad \text{Throw a cricket ball less than $50\ms$} &&= (0, 50).
\end{align*}
The notation for $B_1$ means that a distance of\ $40\ms$ is included in $B_1$.
The notation for $B_2$ means that distances of $0\ms$ and\ $50\ms$ are *not* included in\ $B_2$.

Then:  
\begin{align*}
   B_1 \cap B_2   &= \text{Throw the ball at least $40$m but less than $50\ms$};\\
   B_1 \cup B_2   &= \mathbb{R}_+;\\
   B_1^c &= \text{Throw the ball less than $40\ms$}.
\end{align*}
:::
...
That is, how likely it is to observe any of the various events ('sets') defined on the sample space.
...
## Probability axioms {#ProbabilityAxioms}
...
Consider a sample space $S$ for a random process, and an event\ $A$ in $S$.
...
These axioms can sometimes help to assign probabilities, but their main purpose is to formally define the rules that apply to probabilities, however these numbers are assigned.
...
* **Bounds**:  
  $0 \le \Pr(A)\le 1$; that is, probabilities are numbers between one and one inclusive for any event\ $A$.
* **Empty sets**:  
  $\Pr(\varnothing) = 0$; that is, the probability of the empty set is zero.
* **Monotonicity**:  
  If $A\subseteq B$, then $\Pr(A) \le \Pr(B)$; that is, if Event\ $A$ is wholly contained within\ $B$, it cannot have a larger probability that $\Pr(B)$.
* **Complements**:  
  $\Pr(A^c) = 1 - Pr(A)$; that is, the probability that event\ $A$ *does not* happens is\ $1$ minus the probability that it *does* happen.
* **Addition**:  
  $\Pr(A_1 \cup A_2) = \Pr(A_1) + \Pr(A_2) - \Pr(A_1 \cap A_2)$, a more result than the third axiom.

All of these can be proven, simply based on the three axioms and the definitions that have been presented so far.
...
By definition, the empty set $\varnothing$ contain no points; hence $\varnothing \cup A = A$ for any event $A$.
Also, $\varnothing\cap A = \varnothing$, as $\varnothing$ and $A$ are [mutually exclusive](#def:MutuallyExclusive) (that is, they have no elements in common).
...
Importantly: 'odds' and 'probability' are not the same, and the three axioms define the rules that all probabilities must follow.
:::
...
* finding probabilities using classical probability (Sect.\ \@ref(ClassicalProb)), when the simple events in the sample space are equally likely (i.e., there is no reason to suspect one outcome is more likely that any other); and
* *estimating* probabilities using relative-frequency (Sect.\ \@ref(EmpiricalApproach)), when trials can be repeated many times.
...
For a discrete sample space, where all outcomes in the sample space are equally likely (i.e., there is no reason to suspect one outcome is more likely that any other), the probability of an event\ $E$ is defined as
$$
  \Pr(E) 
  = \frac{|E|}{|S|} 
...
A probability of\ $0$ is assigned to an event that *never* occurs (i.e., $E$ corresponds to the empty set), and\ $1$ to an event that is *certain* to occur (i.e., $E$ corresponds to the universal set).
...
However, outcomes are rarely equally likely; the probability of rain tomorrow is not always the same as the probability of no rain tomorrow.
...
Events defined on a uncountably infinite sample space do not have elements that can be counted (Sect.\ \@ref(InfiniteSets)), so different means are needed to compute probabilities in these situations.I
In fact, the probability of  observing any specific, individual outcome has probability\ $0$.

For continuous sample spaces, a probability is not based on counting elements, but on a *density function*.
A *probability density function* (or PDF) over the sample space quantifies the concentration of probability in different regions of the sample space.

For example, if we consider the heights of adult females (Example\ \@ref(exm:HeightsOfPeople)).
Every female has a height, so the sample space\ $S$ of the heights of all females can be defined as:
$$
   S = \{ x \in \mathbb{R} \mid 50 < x < 300\},
$$
where $x$ is the height in centimetres, and we have assumed no height greater than $300\cms$ or less than $50\cms$. 
Since this is the sample space\ $S$, this represents a probability of one (by the third axiom of probability (Sect.\ \@ref(ProbabilityAxioms)).
That is, the total probability over\ $S$ is one; that is, every adult female is represented somewhere within\ $S$.

Now consider how that total probability of one could be distributed to various ranges of heights.
Now, the probability of finding an adult female with a height less than $75\cms$ (or\ $29\inches$) is basically impossible. 
Almost no probability will be concentrated below\ $75\cms$.

The probability of finding an adult female with a height less than $150\cms$ (or 4ft 11in) is unlikely, but is possible.
Thus, only a little of the probability will be concentrated below\ $150\cms$.

Likewise, the probability of finding an adult female with a height greater than $300\cms$ (or 9ft 10in) is practically impossible.
Almost no probability will be concentrated above\ $300\cms$.

The probability of finding an adult female with a height greater than $175\cms$ (or 5ft 9in) is unlikely but not impossible; only a little of the probability will be concentrated above\ $175\cms$.

In contrast,  the probability of finding an adult female with a height between\ $150\cms$ and\ $175\cms$ is *very* high; almost all of the probability will be concentrated between\ $150\cms$ and\ $175\cms$. 
We can draw a curve that represents this concentration of probability (Fig.\ \@ref(fig:HeightsConcentrations), left panel).
This figure shows how the probability is allocated, or *distributed*, over the sample space.

Rather than dividing the sample space into just five large intervals of height, a finer division of heights could be used; Fig.\ \@ref(fig:HeightsConcentrations), right panel) uses the same ideas but with $10\cms$\ intervals of heights.
This representation shows that most of the probability is concentrated\ $155\cms$ to $165\cms$, suggesting that finding an adult female with a height in this range is concentrates a lot of the probability.

As the intervals become smaller, the grapoh showing the allocations of probability become smoother.
This smooth curve is called a *probability density function* (or PDF): it shows the density (or concentration) of probability over various range of the sample space.




```{r HeightsConcentrations, echo=FALSE, fig.align="center", fig.cap="Allocating the concentration of probability over regions of the sample space.", fig.width=8, fig.height=5, out.width="90%"}
par( mfrow = c(1, 2) )
...
text_Adjust <- c(23, 23, 15, 23, 15)
Divisions <- c(divNever1, divRare1, divOK1, 
               divOK2, divRare2, divNever2)

text(x = mean( c(Divisions[1], 0)) - text_Adjust[1],
     y = 0.1,
     srt = 90,
     pos = 4,
     cex = 0.9,
     col = "grey",
     labels = "Very low prob. concentration")
text(x = mean( c(Divisions[6], 400)) - text_Adjust[5],
     y = 0.1,
     srt = 90,
     pos = 4,
     cex = 0.9,
...
text(x = mean( Divisions[3:4]) - text_Adjust[3],
     ...
     cex = 0.9,
     col = "grey",
     labels = "High prob. concentration")
...
With continuous sample spaces, probabilities are assigned to *intervals* (corresponding to the event\ $E$) of the sample space, and these probabilities are described using a real-valued *probability function*, say $f_X(x)$.
