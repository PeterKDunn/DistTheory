...
Having laid a foundation by studying sets, the idea of probability can be developed.
...
The possible outcomes of a random process is called the *sample space*.
In the context of probability, the universal set is the *sample space*.
...
If a sample space is a finite set, or a countably infinite set, the sample space is called *discrete*.
...
The term *disjoint* is used for *sets* (Sect.\ \@ref(RelationshipsBetweenSets)), whereas *mutually exclusive* is used when referring to *events*.
...
We can define these two events (whose cardinality is *uncountably infinite*):  
\begin{align*}
...
## Axiomatic probabilities {#AssignProbDiscrete}

Usually, we are interested in how likely it is for various outcomes from a random experiment to occur. 
That is, how likely it is to observe any of the various elements of the sample space.
...
But what values should a probability take?
...
A rigorous foundation for probability is found by using a set of fundamental rules, called the *Axioms of Probability*.
...
3. *Additivity*: If $A_1$, $A_2$, $\dots$ form a sequence of mutually exclusive events in $S$, then
$$
...
These axioms can sometimes help to assign probabilities, but their main purpose is to formally define the rules that apply to probabilities.
...
Consider proving that $\Pr(\varnothing) = 0$.
...
Other important rules that can be derived from these axioms, and hence apply to *all* probabiities, are given below, whenre event\ $A$ is defined in a sample space\ $S$:

* All probabilities are between zero and one inclusive: $0 \le \Pr(A)\le 1$.
* $\Pr(\varnothing) = 0$.
* WHAT ELSE??
...
Having seen these axioms, and the rules that follow from them, we can now consider *how* to determine probabilities.
...
### Classical probability {#AssignProbDiscrete}
...
Events defined on a uncountably infinite sample space do not have elements that can be counted (Sect.\ \@ref(InfiniteSets)), so different means are needed to compute probabilities in these situations.
