...
We only prove the empty theorem result: $\Pr(\varnothing) = 0$.\index{Empty set}
...
Subjective probabilities can be used for discrete or continuous sample space, though probabilities can only be allocated to regions for continuous sample spaces.
...
Assume that a sample space\ $S$ for the [random process](#def:RandomProcess)\index{Random process} has been constructed, an event\ $A$ has been identified, and its probability, $\Pr(A)$, has been determined.
...
## Tools for computing probabilities {#ComputingProbs}
...
colnames(BreastTable) <- c("Positive test result",
...
Venn diagrams\index{Venn diagrams} can be useful when there are two events, sometimes three, but become unworkable for more than three.
...
### Enumerating the sample spaces {#Counting}

Sometimes the most convenient way to compute probabilities is to list the sample space.
This only works for sample spaces with a small number of discrete elements, and works best when each of the outcomes in the sample space are equally likely (so that the classical approach to probability\index{Probability, classical probability} can be applied; see Sect.\ \@ref(ClassicalProb)).
...
When the number of elements in the sample space is large, principles and formulae exist which aid counting, some of which are described in the following sections (Sect.\ \@ref(CombsAndPerms)).
...
This is called the *multiplication principle*.\index{Multiplication principle}
...
## Combinations and permutations {#CombsAndPerms}

Combinations and permutations are two ways of counting the number of elements in a finite, discrete sample space.
In *combinations*,\index{Combinations} the order in which the elements are selected is *not* important.
In *permutations*,\index{Permutations} the order in which the elements are selected is important.

:::{.example #CombsAndPerms name="Combinations and permutations"}
...
Consider selecting a random six-letter password from the set of lower-case letters only: '`a`', '`b`', \dots '`z`'.
...
EXPLAIN!!!!!!!!!!!!!!
The number of *combinations* of size\ $r$ that can be drawn from the sample space\ $S$ is
$$
   ^nC_r = \frac{n(n - 1)\ldots (n - r + 1)}{r!} = \frac{n!}{(n - r)!\,r!} = \frac{^n P_r}{r!}
$$
when elements are removed from the sample space once they have been selected (this is called 'selecting without replacement').\index{Selecting!without replacement}

(SHOULD I HAVE LOOKED AT THE CASE $r=n$ FIRST?)
...
#### Combinations selecting with replacement {#CombinationSelectWithReplacement}
...
### Permutations with replacement {#PermutationSelectWithReplacement}

Consider a finite, discrete sample space\ $S$ with\ $n$ elements.
Suppose we wish to count the number of *permutations* of size\ $r$ that can be drawn from\ $S$, when selected items *can* be reselected ('*with* replacement').\index{Selecting!with replacement}
There are\ $n$ options for the first item selected; there are also\ $n$ options for the second item selected, since the element selected first can be re-selected.
The same idea applies for all\ $r$ elements.
Therefore, the number of permutations of size\ $r$, when selection is *with* replacement, is (using the *multiplication principle*; Sect.\ \@ref(MultiplicationPrinciple)):
$$
   n^r.
$$
...
Suppose a company requires users to select a password of eight characters using any of the\ $26$ lower-case letters.
Since the order of the letters is important in a password, *permutations* are relevant.
In addition, characters can be re-used (so, for example, the letter `r` can appear more than once).
There are
$$
  26^8 = 208\,827\,064\,576
$$
possible passwords that can be selected.
:::
...
### Permutations without replacement {#PermutationSelectWithoutReplacement}
...
Notice that once an elements is selected, it cannot be selected again; this is called *selecting without replacement*.
\index{Selecting!without replacement}

We find (using the multiplication principle; Sect.\ \@ref(MultiplicationPrinciple))\index{Multiplication principle} that there are $n(n - 1)(n - 2)\ldots 2 \times 1$ different ways to order the\ $n$ elements. 
...
There are\ $n$ options for the first item selected. 
However, there are only\ $n - 1$ options for the second item selected, since the element selected first *cannot* be re-selected.
The same idea applies for all\ $r$ elements.
Therefore, the number of permutations of size\ $r$, when selection is *without* replacement, is (using the *multiplication principle*; Sect.\ \@ref(MultiplicationPrinciple)):
$$
...
For any given values\ $n$ and\ $r$, the number of permutations is never smaller than the number of combinations (as is clear from Eq.\ \@ref(eq:Permutation)).
This is because any given combinations have many per mutations, since the order is important.
