...
Consider selecting a random password from the set of all lower-case letters only ('`a`', '`b`', \dots '`z`'), of *exactly* six characters in length.

There are $26$\ choices for the first character, and $26$\ choices for the second characters, and so on,
...
More generally, though, *permutations* and *combinations* are needed when selection are made *without replacement*: that is, one an elements is selected, it cannot be selected again.\index{Selecting!without replacement}

Permutations and combinations are used to count when selections are made from a fixed number of elements, without replacement,
If the order of selection *is* important, we use *permutations*.\index{Permutations}
If the order of selection *is not* important, we use *combinations*.\index{Combinations}
...
Counting the number of outcomes can also be achieved by listing the possible outcomes in other ways.
...
The second element must then be chosen, which can be done in $(n - 1)$ ways (since it cannot be the same as the first), using the multiplication rule.
...
Therefore, the number of permutations of size\ $r$, when selection is *without* replacement, is (using the *multiplication rule*; Def.\ \@ref(def:MultiplicationRule)):
$$
...
So we can state that the number of combinations of $r$\ elments drawn form $n$\ elements, where order is not important, is
$$
   ^nC_r = \frac{^nP_r}{r!} = \frac{\text{number of permutations}}{\text{number of equivalent re-arrangements of that permutation}}.
$$
...
The two important counting methods are:

* *permutations*,\index{Permutations} where the order in which the elements are selected *is* important.
* *combinations*,\index{Combinations} where the order in which the elements are selected *is not* important.
