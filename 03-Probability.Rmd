...
Probability is a way of describing how likely it is for things to occur: will it rain tomorrow? will my team win on the weekend?
...
While the outcome of any instance of a random process is unknown, the possible outcomes are known.
...
If a sample space is an uncountably infinite set, the sample space is called *continuous*.
...
However, if rain does fall, the *exact* amount cannot be recorded exactly; this is the continuous component.
...
Simple events are usually not of great interest; events of interest usually contain many elements of the sample space.
...
However, a different language is usually used, to indicate that events are real outcomes, whereas sets are for describing structures more widely and abstractly (Table\ \@ref(tab:CompareLanguage)).
...
The two *events* are therefore *mutually exclusive*.\index{Mutually exclusive}
:::
...
colourE <-ColourOpaque
colourG <- ColourVeryLightOpaque
...
plot( x = c(0, 1),
      ...
      ylab = "",
      xlab = "",
      main = expression(bold(Events~italic(E)~and~italic(G))),
      axes = FALSE)
...
```{r CricketBallSpace, out.width='100%', fig.width=8, echo=FALSE, fig.height=3.5, fig.align="center", fig.cap="The two events\\ $B_1$ and\\ $B_2$ defined for throwing a cricket ball, the sample space\\ $D$, and three events defined with\\ $B_1$ and\\ $B_2$. When the bar representing the event is open at an end, the indicated value is \\emph{not} included in the region."}
...
# x-label
mtext("Distance thrown with cricket ball (in m)",
      side = 1,
      line = 0)
...
The probability\index{Probability} of an event $E$ occurring can be denoted as $\text{P}(E)$, $\text{Pr}(E)$, $\text{Pr}\{E\}$, $\Pr[E]$, or using other similar notation.
:::
...
Other ways also exists to quantify the likelihood of an event occurring.
...
In Example\ \@ref(exm:CountingElements) where a coin is flipped three times, the multiplication rule could be used.
...
With this definition, many formulas apply for all valid choices of\ $n$ and\ $r$.
:::
...
:::{.example #PermAndCombSizes name="Permutations and combinations"}
...
plot( x = c(0, 1),
      ...
      ylab = "",
      xlab = "",
      main = expression(bold(The~probabilities~of~Events~italic(A)~and~italic(B))),
      axes = FALSE)
...
text(x = 0.85,
     y = 0.9,
     labels = expression(Pr*"("*italic(A)*")"==0.4) )
arrows(x0 = 0.85,
       x1 = 0.71,
       y0 = 0.87,
       y1 = 0.76,
...
text(x = 0.85,
     y = 0.10,
     labels = expression(Pr*"("*italic(B)*")"==0.3) )
arrows(x0 = 0.85,
       x1 = 0.78,
       y0 = 0.15,
       y1 = 0.25,
...
plot( x = c(0, 1),
      ...
      ylab = "",
      xlab = "",
      main = expression(bold(The~probabilities~"in"~each~section)),
      axes = FALSE)
...
text(x = 0.86, 
     y = 0.92, 
     label = expression(atop(Pr*"("*italic(A)*union()*italic(B)*")"^c,
                             phantom(.)==0.4)))
...
To understand how to use tree diagrams requires a study of *conditional probability*, which we do next.
...
A consequence of Def.\ \@ref(def:ConditionalProb) is the following theorem.\index{Probability rules!general multiplication rule}
...
\begin{align*}
     \Pr(A\cap B)
     &= \Pr(A)\cdot \Pr(B \mid A)\\
     &= \Pr(B)\cdot \Pr(A \mid B).
\end{align*}
:::
...
Three events\ $A$, $B$ and\ $C$ are *mutually independent* if, and only if, all of these are true:
\begin{align*}
...
The concepts introduced in this section allow us to determine the probability of an event using the *event-partitioning approach*, which we now discuss.\index{Sample space!partitioning}
...
\begin{align*}
   \Pr(A) 
   &= \Pr(A \mid B_1) \cdot \Pr(B_1) + \Pr(A \mid B_2) \cdot \Pr(B_2) + \ldots \\
   & \qquad {} + \Pr(A \mid B_k) \cdot \Pr(B_k).
\end{align*}
...
$$
  \Pr(A) = \sum_{J = 1}^6 \Pr(A\cap B_j) = \sum_{J = 1}^6  \Pr(A\mid B_j)\cdot \Pr(B_k).
$$
For example, $\Pr(A\mid B_6) = 1$ from Fig.\ \@ref(fig:Partitioning)).


```{r Partitioning, echo=FALSE, out.width='60%', fig.width=8, fig.align="center", fig.cap="Partitioning the sample space into six parts ($B_1$ to\\ $B_6$), and event\\ $A$."}
plot(0, 0, 
     type = "n", 
     xlim = c(0, 10), 
     ylim = c(0, 6), 
...
Then
$$
   \Pr(B_i \mid A) = \frac{\Pr(B_i) \cdot \Pr(A \mid B_i)}
                          {\displaystyle \sum_{j = 1}^k \Pr(B_j)\cdot\Pr(A \mid B_j)}
$$
for $i = 1, 2, \dots, k$.
...
In effect, the theorem takes a conditional probability of the form $\Pr(A\mid B_i)$ and can 'reverse' the conditioning.
...
colourCancer <- ColourLightOpaque
colourPos  <- ColourVeryLightOpaque
...
plot( x = c(0, 1),
      ...
      ylab = "",
      xlab = "",
      main = expression(bold(Events~italic(C)~and~italic(C)^c)),
      axes = FALSE)
...
plot( x = c(0, 1),
      ...
      ylab = "",
      xlab = "",
      main = expression(bold(Events~italic(D)~and~italic(D)^c)) ,
      axes = FALSE)
...
plot( x = c(0, 1),
      ...
      ylab = "",
      xlab = "",
      main = expression(bold(Events~italic(C)~and~italic(D))) ,
      axes = FALSE)
...
4. **Performing sensitivity analysis.**
   Simulation allows easy tweaking or changing of important values to see the impacts on the results (e.g., Sect.\ \@ref(SimulationSensitivityAnalysis)).
...
The **R** function `replicate()` allows an **R** expression to be repeated ('replicated') a given number of times, and often proves useful for creating simulations.\index{R@\textbf{R}}\index{replicate()@\texttt{replicate()}}
...
People with an allocated random below the value of `cancer_Prob` are deemed to have cancer.\index{function()@\texttt{function()}}\index{runif()@\texttt{runif()}}
...
Now, we can simulate over many replications:\index{mean()@\texttt{mean()}}\index{sd()@\texttt{sd()}}\index{set.seed()@\texttt{set.seed()}}\index{replicate()@\texttt{replicate()}}
...
Then we use the **R** function `sample()` to take a 'sample' of heads and tails (with replacement!) for each replication:\index{sample()@\texttt{sample\index{head()@\texttt{head()}}()}}
...
After each toss, the probability of obtaining a head using all the available information up to that toss was computed:\index{cumsum()@\texttt{cumsum()}}
...
The results can be plotted (Fig.\ \@ref(fig:SimTosses)):\index{plot()@\texttt{plot()}}\index{abline()@\texttt{abline()}}
...
First, set up the scenario:\index{set.seed()@\texttt{set.seed()}}
...
Then, we create an **R** function to determine if Anh (who is `A`) and Barb (who is `B`) are next to each other:\index{function()@\texttt{function()}}\index{length()@\texttt{length()}}\index{c()@\texttt{c()}}\index{any()@\texttt{any()}}

```{r}
# The function takes a seating arrangement as input, given as (for example) 
...
In this case:\index{sum()@\texttt{sum()}}\index{replicate()@\texttt{replicate()}}
...
The command first creates a random arrangement of people (i.e., `sample(People)`), and then checks if the given permutation of people has Anh and Barb seated as neighbours (using our function `check_If_Neighbours()`).
This is replicated `num_Reps` times (using `replicate()`), and adds up (using `sum()`) how many times they are seated together.
Then, the result can be printed:\index{round()@\texttt{round()}}
...
A sensitivity analysis may consider how the final results (i.e., $\Pr(\text{Has cancer} \mid \text{Positive test})$) changes depending on different values of the inputs.

For example, what happens if the sensitivity of the test improves, from\ $0.75$ to\ $0.90$?\index{set.seed()@\texttt{set.seed()}}\index{replicate()@\texttt{replicate()}}\index{mean()@\texttt{mean()}}\index{sd()@\texttt{sd()}}
...
Suppose a squad consists of seven batters, five bowlers, two all-rounders and one wicket-keeper.

1. Find the number of teams possible if the *playing team* consists of five batters, four bowlers, one all-rounder and one wicket-keeper.
...
1. Two of the children legally need car seats, so two car seats are fixed in the vehicle (i.e., they cannot be moved to different seats) and each child has their 'own' seat.
...
:::{.exercise #ProbFaceRecognition}
An international airport deploys an automated facial recognition (AFR) system to screen passengers walking past a checkpoint against a database of known security threats. 
Based on benchmark evaluations of "in the wild" facial recognition systems (such as NIST FRVT studies) and security watchlist data, assume the following parameters:

* *Prevalence:* The probability that a randomly scanned passenger is on the watchlist is $P(W) = 1 \times 10^{-5}$ (1 in 100,000).
* *Sensitivity (true positive rate):* If a passenger is on the watchlist, the system correctly generates an alert with probability $P(A \mid W) = 0.85$ (85%).
* *False Positive Rate:* If a passenger is **not** on the watchlist, the system mistakenly generates an alert with probability $P(A \mid W^c) = 0.001$ (0.1%, or 1 in 1,000).

Use this information to answer these questions.

1. State the *Law of Total Probability* formula for $P(A)$, the marginal probability that a randomly scanned passenger triggers an alert, and calculate its numerical value.
2. Calculate the *Positive Predictive Value (PPV)*, $P(W \mid A)$: given that the camera triggers an alert for a passenger, what is the posterior probability that this passenger is actually on the watchlist?
3. Briefly explain the intuition behind why $P(W \mid A)$ is so low despite the system having an 85% sensitivity and a 99.9% specificity (the "False Positive Paradox" / "Base Rate Fallacy").
4. Write a short **R** code snippet using Bayes' theorem to compute $P(W \mid A)$ for a vector of prior probabilities $P(W) \in [10^{-6}, 10^{-2}]$.
:::


:::{.exercise #ProbAITextDetectors}
A university introducing automated academic integrity software uses an AI text classifier (such as GPTZero or Turnitin) to screen student essays for AI generation. 
Based on realistic independent benchmarks of AI detectors on non-native or heavily edited writing:

* *Prevalence:* In a specific course, it is estimated that only 5% of submitted essays are actually AI-generated without permission ($P(AI) = 0.05$).
* *Sensitivity (true positive rate):* If an essay is fully AI-generated, the tool correctly flags it with probability $P(Flag \mid AI) = 0.85$ (85% sensitivity).
* *False Positive Rate:* If an essay is human-written, the tool mistakenly flags it as AI-generated with probability $P(Flag \mid Human) = 0.04$ (4% FPR, or 96% specificity).

Use this information to answer these questions.

1. State the *Law of Total Probability* formula for $P(Flag)$, the marginal probability that a randomly scanned essay is flagged by the detector, and compute its value.
2. Calculate the *Positive Predictive Value (PPV)*, $P(AI \mid Flag)$: given that an essay triggers a flag, what is the posterior probability that the student actually used AI?
3. Calculate the *Negative Predictive Value (NPV)*, $P(Human \mid Flag^c)$: given that an essay is *not* flagged, what is the probability that it was genuinely written by a human?
4. Briefly explain why $P(AI \mid Flag)$ is surprisingly low despite the detector having high sensitivity (85%) and high specificity (96%).
5. Write a short **R** code snippet that computes $P(AI \mid Flag)$ across a sequence of prior probabilities $P(AI) \in [0.01, 0.50]$ and plots the resulting posterior curve.
:::
