...
* *Prevalence:* The probability that a randomly scanned passenger is on the watchlist is $\Pr(W) = 1 \times 10^{-5}$ ($1$ in $100\,000$).
* *Sensitivity (true positive rate):* If a passenger is on the watchlist, the system correctly generates an alert with probability $\Pr(A \mid W) = 0.85$ (or $85$%).
* *False Positive Rate:* If a passenger is **not** on the watchlist, the system mistakenly generates an alert with probability $\Pr(A \mid W^c) = 0.001$ ($0.1$%, or $1$ in $1\,000$).

Use this information to answer these questions.

1. State the *Law of Total Probability* formula for $\Pr(A)$, the marginal probability that a randomly scanned passenger triggers an alert, and calculate its numerical value.
2. Calculate the *Positive Predictive Value (PPV)*, $\Pr(W \mid A)$: given that the camera triggers an alert for a passenger, what is the posterior probability that this passenger is actually on the watchlist?
3. Briefly explain the intuition behind why $\Pr(W \mid A)$ is so low despite the system having an $85$% sensitivity and a $99.9$% specificity (the "False Positive Paradox" / "Base Rate Fallacy").
4. Write a short **R** code snippet using Bayes' theorem to compute $\Pr(W \mid A)$ for a vector of prior probabilities $\Pr(W) \in [10^{-6}, 10^{-2}]$.
:::
...
* *Prevalence:* In a specific course, it is estimated that only 5% of submitted essays are actually AI-generated without permission ($\Pr(A) = 0.05$).
* *Sensitivity (true positive rate):* If an essay is fully AI-generated, the tool correctly flags it with probability $\Pr(F \mid A) = 0.85$ (85% sensitivity).
* *False Positive Rate:* If an essay is human-written, the tool mistakenly flags it as AI-generated with probability $\Pr(F \mid H) = 0.04$ (4% FPR, or 96% specificity).

Use this information to answer these questions.

1. State the *Law of Total Probability* formula for $\Pr(F)$, the marginal probability that a randomly scanned essay is flagged by the detector, and compute its value.
2. Calculate the *Positive Predictive Value (PPV)*, $\Pr(A \mid F)$: given that an essay triggers a flag, what is the posterior probability that the student actually used AI?
3. Calculate the *Negative Predictive Value (NPV)*, $\Pr(H \mid F^c)$: given that an essay is *not* flagged, what is the probability that it was genuinely written by a human?
4. Briefly explain why $\Pr(A \mid F)$ is surprisingly low despite the detector having high sensitivity (85%) and high specificity (96%).
5. Write a short **R** code snippet that computes $\Pr(A \mid F)$ across a sequence of prior probabilities $\Pr(A) \in [0.01, 0.50]$ and plots the resulting posterior curve.
:::
