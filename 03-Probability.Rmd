...
\begin{align*}
   B_1 &= \{b \in D \mid b \ge 40\} = [40, 150)\quad\text{(i.e., throw a cricket ball at least $40\ms$)};\\
   B_2 &= \{b \in D \mid b < 50\} = (0, 50)\quad\text{(i.e., throw a cricket ball less than $50\ms$)}.
\end{align*}
...
Based on real scenarios (such as NIST FRVT studies) and security watchlist data, assume the following parameters:

* *Prevalence:* The probability that a randomly scanned passenger is on the watchlist is $\Pr(W) = 1 \times 10^{-5}$ ($1$\ in\ $100\,000$).
* *Sensitivity:* If a passenger is on the watchlist, the system correctly generates an alert with probability $\Pr(A \mid W) = 0.85$.
* *False positive rate:* If a passenger is *not* on the watchlist, the system mistakenly generates an alert with probability $\Pr(A \mid W^c) = 0.001$ ($0.1$%, or $1$\ in\ $1\,000$; i.e., $99.9$% specificity).

Use this information to answer these questions.

1. Use the *Law of total probability* to compute the value of $\Pr(A)$, and explain what this value *means*.
...
3. Briefly explain the why $\Pr(W \mid A)$ is so low, despite the system having an $85$% sensitivity and a $99.9$% specificity.
4. Write some **R** code to use Bayes' theorem to compute $\Pr(W \mid A)$ for a range of prior probabilities $\Pr(W) \in [10^{-6}, 10^{-2}]$.
   Plot the resulting posterior curve.
...
Based on realistic independent benchmarks of AI detectors on non-native or heavily edited writing, assume the following parameters::

* *Prevalence:* In a specific course, 5% of submitted essays are estimated to be AI-generated without permission ($\Pr(A) = 0.05$).
* *Sensitivity:* If an essay is fully AI-generated, it is correctly flagged by the tool with probability $\Pr(F \mid A) = 0.85$.
* *False Positive Rate:* If an essay is human-written, the tool mistakenly flags it as AI-generated with probability $\Pr(F \mid H) = 0.04$ (i.e., $96$% specificity).

Use this information to answer these questions.

1. Use the *Law of total probability* to compute the value of $\Pr(F)$, and explain what this value *means*.
2. Calculate the *Positive Predictive Value (PPV)*, $\Pr(A \mid F)$: given that an essay triggers a flag, what is the probability that the student actually used AI?
...
5. Write some **R** code that computes $\Pr(A \mid F)$ for a range of prior probabilities $\Pr(A) \in [0.01, 0.50]$.
   Plot the resulting posterior curve.
:::
