...
About 10% of the time, the distance is between 50m and 60m; 20% further than 40m; 50% of the time further than 30m; and 100% of the time further than 20m.
...
Then, so that the total area is one, we find that $k = 1/1000$.
...
That is:
\[
   \Pr(L | R) = \frac{\Pr(L \cap R)}{\Pr(R)} = \frac{\Pr(L)}{\Pr(R)} = \frac{`r round(Prob30, 4)`}{`r round(Prob0, 4)`} = `r round(CProb30, 3)`.  
\]
