...
* What is the chance that a medical patient will suffer from a dangerous side-effect?
...
Essentially, a random process produced data when the outcome is unknown.
Examples of simple *random processes* include tossing a coin, or rolling a die.

The outcomes of the random process are best understood in terms of *sets*.


::: {.definition #Sets name="Sets"}
A *set* of *elements* is denoted using capital letters: $A$.

The distinct *elements* in the set, if they exist, are denoted by lower-case letters, and are shown to belong to a set by enclosing them in braces:
\[
   A = \{ a_1, a_2, a_3, a_4\}.
\]
:::
...
* Finite, such as the set of the states of Australia.
* Countably infinite, such as the set of integers.
* Continuous (or uncountable), such as the set of real numbers.

Sets can be described in various ways too:

* By listing the individual elements.
  After rolling a single die, event $R$ is $R = \{2, 4, 6\}$.
* By describing the elements.
  Event $A$ may be 'States belonging to the United States of America'. 
* By describing a pattern.
  Event $N$ is the set of all even numbers: $N = \{2, 4, 6, \dots\}$.
* By describing regions.
  Event $H$ is the set 'the heights of humans'.
 
Having defined a *set*, we can now define the *sample space*.


::: {.definition #SampleSpace name="Sample space"}
A *sample space* (or *event space*, or *outcome space*) for a random process is a set of all possible outcomes, usually denoted by $S$, $\Omega$ or $U$ (the 'universal set').
...
:::{.definition #EmptySet name="Empty sets"}
...
So far, most of sample spaces used in the examples have been *discrete*, but this is not necessary.
...
In Example \@ref(exm:RollDie), the sample space on which events $E$ and $G$ were defined is discrete.
There are exactly six outcomes.
:::


The idea of a *countably infinite* number of distinct points sounds a little strange... an example helps illustrate.


:::{.example #RollingTill6 name="Rolling a die to obtain a 6"}
...
A *continuous sample space* is a sample space where any interval contains a infinite number of points.
:::

:::{.example #ContinuousSampleSpaceThrow name="Throwing a cricket ball"}
...
No upper limit exists for the distance the ball can be thrown, though clearly some parts of the sample space are extremely unlikely to be observed.
Furthermore, the distance cannot be specified *exactly*.
...
We can define sets containing just some of those elements of the sample space.
...
\[
   A = \{ (HH), (TH)\}.
\]
The event 'the set of outcomes with *three* heads' is the [*null set*](#def:NullSet), as there are no sample points with three heads.
:::
...
Then:
\begin{align*}
   B_1 \cap B_2 &= \{\text{Throw the ball between 40m and 50 m}\};\\
   B_1 \cup B_2 &= \mathbb{R}_+;\\
   \overline{B_1} &= \{\text{Throw the ball less than 40m}\}.
\end{align*}
:::

```{r CricketBallSpace, echo=FALSE, fig.height=3, fig.align="center", fig.cap="The two events defined for throwing a cricket ball"}
plot( x = c(-10, 70),
      y = c(0, 0),
      ylim = c(-30, 30),
      type = "l",
...
Notation | Event                     | Set
---------|---------------------------|--------------------
$M$      | 'Obtain a Head on Toss 1' | $\{(HT), (HH)\}$
$N$      | 'Obtain a Tail on Toss 1' | $\{(TT), (TH)\}$
...
To formalise the notion of the chance of an event occurring, the chance is assigned a number, called a *probability*. 
We denote the probability of an event $E$ occurring as $\text{Pr}(E)$.

::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
Be aware of variations in notation!

The probability of an event $E$ occurring can be denoted as $\text{P}(E)$, $\text{Pr}(E)$, or other similar notation.
:::
...
This method is appealing as it aligns with the idea of [proportions or percentages](#EmpiricalApproach) as numbers between 0 and 1.
...
Odds compare how often an event is likely to occur, to how often the event is likely to *not occur*.
...
\]
$n$ needs to be very large--and teh sample random--to compute probabilities accurately, and approximate probabilities can only ever be found.
...
To determine an accurate probability, matches must be struck, and then can no longer be sold.

As another example, consider the probability that the airbag in a car will prevent a serious injury to the driver
...
For the classical approach, random process with $n$ *equally-likely* outcomes, are all assigned probability $1/n$.
...
The goal is to correctly pick the seven numbers that will be drawn at random.
...
In the discrete case, the probability of observing an element of the sample space must *sum* to one; for a continuous sample space, the equivalent statement involves integration.
That is, over the whole sample space, the probability function must have an integral of one:
\[
   \int_S f(x)\,dx = 1.
\]
This is like the second of the three [axioms of probability](#AxiomaticApproach).
These axioms still apply, so we also have:

* integration over any region of the sample space must never produce a negative value;
  this implies $f_X(x)\ge0$ for all values of $x$.
...
Knowing the extra information in event $B$ has changed your calculation of $\Pr(A)$.
:::


::: {.example #Rainfall name="Rainfall"}
In Example \@ref(exm:ChanceRain), the chance of obtaining rain during April in Charleville was computed to be $0.85$ using the empirical approach.

However, the probability of April rainfall may depend on the value of the Southern Oscillation Index (SOI).
...
\begin{align*}
   \text{For SOI Phase 1:}\quad
   & \Pr(\text{rain} \mid \text{Phase 1}) = 0.56;\\
   \text{For SOI Phase 2:}\quad
   & \Pr(\text{rain} \mid \text{Phase 2}) = 0.96.
\end{align*}
:::

:::{.example}
Need continuous example.COuld use ainfall $R$.
eg $\Pr(R>20 | R >0$ compared to $\Pr(R>20)$.
:::
...
RainTable[3, ] <- as.character( round(RainTable[1, ] / RainTable[2, ], 2) )
# This turn all integers to text to retain as integers
...
:::{.example}
COND PROB example. REUSE above, with notation.
:::
...
\[
   \Pr(\text{Compiled} | \text{Females}) = 63/84 = 0.75.
\]

For males, the probability of complying with the isolation request is
\[
   \Pr(\text{Compiled} | \text{Males}) = 36/48 = 0.75.
\]
...
The two events $E_1$ and $E_2$ are *independent* because,  whether or not a head occurs on one of the tosses, the probability of a head occurring on the other is still $1/2$... otherwise coins must have memories or can talk to each other...
...
Express the event of interest, say event $A$, as a composition (unions and/or intersections), then apply the laws of probability to find $\Pr(A)$.
...
The second of these is often referred to as the events $B_i$ being *exhaustive*.
...
\begin{align*}
   \Pr(A)
   &= \Pr(A|B_1)\times \Pr(B_1) +  \Pr(A|B_2)\times \Pr(B_2) + {}\\
   &\quad \Pr(A|B_3)\times \Pr(B_3) + \Pr(A|B_4)\times \Pr(B_4) + {} \\
   &\quad \Pr(A|B_5)\times \Pr(B_5) +  \Pr(A|B_6)\times \Pr(B_61)\\
...
Notice that the right-hand side includes conditional probabilities of the form $\Pr(A|B_i)$, while the left-hand side contains the probability $\Pr(B_i|A)$.
In effect, it takes a conditional probability and can 'reverse' the conditioning.

Because of this, Bayes' theorem has many uses, as it uses conditional probabilities that are easy to find or estimate (such as $\Pr(A \mid B_j$), to compute a probability that is *not* easy to find or estimate (such as $\Pr(B_i \mid A)$).
...
* Sensitivity: $\Pr(D | C)$; and
* Specificity: $\Pr(\overline{D} | \overline{C})$.
...
* $\Pr(D | C ) = 0.75$ (and so $\Pr(\overline{D} | C) = 0.25$);
* $\Pr(\overline{D} | \overline{C}) = 0.90$ (and so $\Pr(D | \overline{C}) = 0.10$).

Furthermore, about 2% of women under 40 will get breast cancer [@houssami2003sydney]; that is, $\Pr(C) = 0.02$ (and so $\Pr(\overline{C}) = 0.98$).

For this study, the probabilities $\Pr(D|C)$ are easy to find: women who are *known* to have breast cancer have a mammogram, and we record whether the result is positive or negative.

But consider a women under 40 who gets a mammogram.
When the results are returned, her interest is whether they have breast cancer, *given* the test results; for example $\Pr(C | D)$.
In other words: If the test returns a positive result, what is the probability that she actually has breast cancer?

That is, we would like to take probabilities like $\Pr(D|C)$, that can be found, and determine $\Pr(C | D)$, which is of interest in practice.

Using [Bayes' Theorem](#Bayes):
\begin{align*}
   \Pr(C | D) 
   &= \frac{\Pr(C) \times \Pr(D | C)}
            {\Pr(C)\times \Pr(D | C) + \Pr(\overline{C})\times \Pr(D | \overline{C}) }\\
   &= \frac{0.02 \times 0.75}
...
This partly explains why mammograms for women under 40 are not commonplace: most women who get a positive test results actually do not have breast cancer.
...
Typically, the correct answer can be found using many different methods, but one method is often easier to apply than others in a given situation.
...
Consider using a tree diagram to describe the breast cancer information from Example \@ref(exm:BreastCancer), as shown in Fig. \@ref(fig:BreastTree).
By following each 'branch' of the tree, we can compute, for example:
\[
   \Pr(C \cap D) = 0.02 \times 0.075 = 0.015;
\]
that is, the probability that a woman has a positive test **and** breast cancer is about 0.015.
But compare:
\[
   \Pr(\overline{C} \cap D) = 0.98 \times 0.10 = 0.098;
\]
...
:::{.example #HatData name="Wearing hats"}
...
A **tree diagram** (Fig. \@ref(fig:HatTree)) can be used to summarise the situation.
'Step 1' is identifying the sex of the person.
'Step 2' is then identifying whether they are wearing a hat. 
Here:
\begin{align*}
   \Pr(\text{Wear hat} | \text{Male}) 
   &= 0.205\\
   \Pr(\text{Wear hat} | \text{Female}) 
   &= 0.060
\end{align*}
The probability that a person wears a hat depends on the sex of the person.
:::
...
When there are two variables of interest, tables may be a convenient way of summarizing the information.
...
The breast-cancer data in Example \@ref(exm:BreastCancer) can also be compiled into a table.
The data provided can be used (Table \@ref(tab:BreastTable)) to produces a final table of probabilities for each of the four outcomes in the sample space (Table \@ref(tab:BreastTable2)).
:::
...
BreastTable[1, ] <- c("0.75 x 0.02",
                      "0.25 x 0.02",
                      "0.02")
BreastTable[2, ] <- c("0.10 x 0.98",
                      "0.90 x 0.98",
                      0.98)
...
```{r BreastTable2, echo=FALSE}
BreastTable2 <- array(dim = c(2, 3))

colnames(BreastTable2) <- c("+ive test",
                           "-ive test",
                           "Total")
rownames(BreastTable2) <- c("Has breast cancer",
                           "Does not have breast cancer")
BreastTable[1, ] <- c("0.015",
                      "0.005",
                      "0.02")
BreastTable[2, ] <- c("0.098",
                      "0.882",
                      0.98)
knitr::kable(BreastTable,
             escape = FALSE,
...
:::{.example #HatData2 name="Wearing hats"}
The data used in Example \@ref(exm:HatData) can also be compiled into a table (Table \@ref(tab:HatTable)).
...
plot( x = c(0, 1),
      ...
      ylab = "",
      xlab = "",
      main = "The probabilities in each section",
      axes = FALSE)
...
::: {.example}
Consider again the hat data (Example \@ref(exm:HatData); Example \@ref(exm:HatData2)).

Define these events:
\begin{align*}
   M:{} &\text{The person is male};\\
   H:{} &\text{The person is wearing a hat}
\end{align*}
A Venn diagram could be constructed to show the sample space (???????) (Fig. \@ref(fig:HatVenn)).
:::
...
In [Oz Lotto](#exm:Lotto), players select seven number from 47, and try to match these with seven randomly selected numbers.
The order in which the seven numbers are selected in not important.
...
c(WinByNotSwitching, WinBySwitching) / 1000
```
...
* $\Pr{A\cup B}=\Pr{A} + \Pr{B} - \Pr{A\cap B} = 0.56$.
* $\Pr{\overline{A}\cap B} = \Pr{B} - \Pr{A\cap B} = 0.26$.
* $\Pr{\overline{A}\cup\overline{B}} = \Pr{\overline{A}} + \Pr{\overline{B}} - \Pr{\overline{A}\cap\overline{B}} = 0.76$ or $1-\Pr{A\cap B} = 0.76$.
* $\Pr{A\mid B} = \Pr{A \cap B}/\Pr{B} = 0.24/0.50 = 0.48$.
* Since $\Pr{A}\times \Pr{B} = 0.3\times 0.5 = 0.15\ne \Pr{A\cap B}$, $A$ and $B$ are not independent events.
...
* All real numbers are possible values for $a$, $b$ and $c$; hence $S=\{ (a,b,c)\mid -\infty<a<\infty, -\infty<b<\infty, -\infty<c<\infty\}$.
