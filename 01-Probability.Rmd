...
*Distribution theory* is about describing random variables using *probability*.
...
The elements of a set $A$ are usually denoted by lower-case letters, and are shown to belong to a set by enclosing them in braces:
...
* By *enumerating* the individual elements, either directly or establishing a pattern.
* By *describing* what is common to all elements of the set.
* By using a *rule*.
 
 
::: {.example #DefiningSets name="Sets"}
After rolling a single die, the set $B$ can be defined by listing its elements individually: $B = \{2, 4, 6\}$. 
$B$ is a finite set.\smallskip

The set $N$ of odds numbers could be defined by enumeration, by giving a *pattern*: $N = \{1, 3, 5, 7, 9, \dots\}$.
$N$ is countably infinite.\smallskip

Set $A$ can be defined by describing the elements: $A = \left\{\right.$*Countries whose names begin and end with the letter 'A'*$\left.\right\}$ (whose elements include, for example, 'Austria', 'Antigua and Barbuda' and 'Albania').
$A$ is finite.\smallskip

The set $R$ refers to the real numbers $x$, and can be defined by a *rule*: $R = \{ x \mid x > 0\}$, where the notations means 'the set $R$ consists of elements $x$ such that all values of $x$ are greater than zero'.
...
A *sample space* (or *event space*, or *outcome space*) for a random process is a set of all possible outcomes from a random process, usually denoted by $S$, $\Omega$ or $U$ (for the 'universal set').\smallskip

When appropriate, a single element of $S$ is called an *element* or *sample point*.
The sample points are usually denoted $s_1$, $s_2$, and so on.\smallskip
:::
...
\[
   \Omega = \{ d \mid d > 0 \}.
\]
The sample space could also be denoted $\mathbb{R}_{+}$, commonly-used notation to represent the positive real numbers.
...
$\Omega$ is countable infinite (and sometimes called *continuous*).
:::
...
The 6 could appear on the first roll, or the second, or the third.\smallskip
...
Example \@ref(exm:SampleSpaceContinuous) considered the distance a cricket ball can be thrown $d$, with the sample space $\Omega = \{d \mid d > 0\}$, or $\Omega \equiv \mathbb{R}_{+}$, where $\mathbb{R}_{+}$ represents the positive real numbers.\smallskip
...
Between a distance of 35.71m and 35.72m are an infinite number of possible values (in theory; in practice we may round to the nearest centimetre, for example).
The sample space $\Omega$ is *continuous*.\smallskip
...
\[
   S = \{(HH), (HT), (TH), (TT) \},
\]
where $H$ represents tossing a head and $T$ represents tossing a tail, and the pair lists the result of each toss in order.\smallskip
...
For a discrete sample space, an *elementary event* (or a *simple event*) is an event with one sample point, that cannot be decomposed into smaller events.\smallskip
...
\[
   T\cap D = \{ 6 \};\quad\text{and}\quad T\cup D = \{2, 3, 4, 6\};\quad\text{and}\quad \overline{D} = \{1, 3, 5\}.
\]
:::
...
Events $A$ and $B$ are *mutually exclusive* if, and only if, $A\cap B = \emptyset$; that is, they have no elements in common.\smallskip
...
To test the effectiveness of the vaccine, the data in Table \@ref(tab:Polio) were collected.\smallskip
...
Consider using a computer to simulate large numbers of coin tosses; here, **R** is used to simulate 500 tosses.\smallskip
...
While the result of any single toss is unpredictable, we see the general pattern emerging: Heads occur about half the time (as expected).
:::
...
Now the running probabilities can be plotted:
...
Take care: errors are frequently made by failing to list all the sample points in $S$, or failing to list *equally-likely* sample points.

Methods of counting the points in a sample space are discussed in Sect. \@ref(Counting).
...
Listing the sample space is difficult as [the number of options is very large](#exm:CombinationOzLotto) for selecting seven numbers from 47 (without replacement).
:::
...
Many farmers could give a subjective estimate of the probability based on their experience and the conditions on their farm.\smallskip

Using the classical approach to determine the probability is not possible.
While two outcomes are possible---it *will* rain, or it *will not* rain---these are almost certainly not *equally likely*.\smallskip
...
For every event $A$ (a subset of $S$), a number $\Pr(A)$ can be assigned which is called the *probability* of event $A$.\smallskip
...
While this may appear 'obvious', it is not one of the three axioms.\smallskip
...
About 10% of the time, the distance is between 50m and 60m; 20% of the time further than 40m; 50% of the time further than 30m; and 100% of the time further than 20m.\smallskip

With this information we have some idea of the probabilities that my throw goes for certain ranges of distance (Fig. \@ref(fig:BallThrow), left panel).\smallskip

For these areas to represent probabilities, though, the *total* area must sum to one.
Each rectangle has a width of 10m, and we can define the vertical-axis steps as shown as a distance of $k$ apart.\smallskip
...
Then, you would compute $\Pr(A) = 1/6$ (using the [classical approach](#ClassicalApproach)).\smallskip

However, suppose I provide you with extra information:
I tell you that event $B$ has already occurred, where $B$ is the event 'the number rolled is even'.\smallskip
...
However, the probability that the number is a six is now $\Pr(A\mid B) = 1/3$.\smallskip
...
If you are told that someone falls out of an airborne plane with no parachute, the probability that they die is very high.\smallskip

Then, consider the probability $\Pr(F\mid D)$.
If you are told that some has died, the cause is very unlikely to be a fall from an airborne plane.\smallskip
...
Event $L$ occurs `r L` times in the `r numYears` years of data, while event $R$ occurs `r R` times.\smallskip

Using the relative frequency approach with the [BoM](http://www.bom.gov.au) data, the probability of obtaining *more than* 30mm in April is approximately

\[
   \Pr(L) = \frac{`r L`}{`r Number`} = `r round(Prob30, 3)`. 
\]
However, the *conditional probability* of receiving more than 30mm, *given* that some rainfall was recorded, is approximately:
...
\]
Otherwise the events are *dependent* (or *not independent*).
:::
...
\begin{align*}
   ...
   = &\Pr(A \mid B_1)\times \Pr(B_1)\quad +  \quad\Pr(A \mid B_2)\times \Pr(B_2) \quad+ {}\\
     &\Pr(A \mid B_3)\times \Pr(B_3)\quad + \quad\Pr(A\mid B_4)\times \Pr(B_4) \quad+ {} \\
     &\Pr(A \mid B_5)\times \Pr(B_5)\quad +  \quad\Pr(A\mid B_6)\times \Pr(B_6)\\
...
If a 'two-course meal' consists of one main plus one dessert, a total of $5\times 3 = 15$ meal combinations are possible.
:::
...
Some notation for permutations include $nPr$, $^nP_r$, $P_r^n$ or $P(n,r)$.
:::
...
Suppose a company requires users to select a password of 8 unique characters using only lower-case letters.
...
\[
   ^nC_r = \binom{n}{r} = \frac{n(n - 1)\ldots (n - r + 1)}{r!} = \frac{n!}{(n - r)!\,r!} = \frac{^n P_r}{r!}.
\]
...
Notation for combinations include $nCr$, $^nC_r$, $C_r^n$, $\binom{n}{r}$ or $C(n,r)$.
:::
...
* the function `sample()` takes a list of elements from those given, with or without replacement.
:::
...
2. Define the event $Z$: 'the equation has no real roots'.
:::
...
Each student in the sample then selects one card at random from the prepared pile of $N$ cards, and answers 'Yes' or 'No' to the question 'Is the statement on the selected card true or false?' without divulging which statement is on the card. 

Let $\Pr(Y)$ be the probability that a student answers 'Yes', and $\Pr(D)$ the probability that a student chosen at random has used an illegal drug.
Assume that each student answers truthfully.

1. Find $\Pr(Y)$ in terms of $\Pr(D)$, $m$ and $N$, stating any conditions required on $m$ and $N$.
1. Suppose that, in a sample of 400 students, 175 answer 'Yes'.
   Determine $\Pr(Y)$.
1. Estimate $\Pr(D)$ from the expression found above, given that $N = 100$ and $m = 25$.
:::
...
1. What is the probability the first card is an **Ace**?
...
:::{.exercise}
Compute the probability that, for a group of $N$ randomly selected individuals, *at least two* have the same birthday. 
...
2. What is the probability that the second player receives exactly one pair, given that the first player received exactly one pair: Larger or smaller than your previous answer? 
   Explain.
3. What is the probability that the initial five cards include exactly one three-of-a-kind?
:::
...
:::{.exercise #CombinSimplify}
Without using any technology, compute the value of $\displaystyle \frac{{}^{25} C_8}{{}^{25} C_6}$.
:::

:::{.exercise #CombinSolve}
Show that $n\left( {}^{n - 1}P_{r - 1} \right) =  {}^n P_r$.

Hence, solve (without using technology)

\[
   \frac{ {}^n C_1 \times {}^{6n}C_6}{ {}^{7n}C_7}
   = \frac{91}{190}.
\]
:::


:::{.exercise #CombinSolve2}
Solve without technology:

\[
   \frac{ {}^7 P_{r + 1}}{ {}^{7}C_r} = 10.
\]
:::
