...
:::{.example #SampleSpace name="Sample space"}
...
Consider observing the outcome on a single roll of a die (Example \@ref(exm:SampleSpace)).
...
\[
   S = \{(H, H), (H, T), (T, H), (T, T) \},
\]
where $H$ represents tossing a head; $T$ represents tossing a tail; and the pair $(\cdot, \cdot)$ list the result of each toss in order.
...
Notation | Event                     | Set
---------|---------------------------|--------------------
$M$      | 'Obtain a Head on Toss 1' | $\{(H, T), (H, H)\}$
$N$      | 'Obtain a Tail on Toss 1' | $\{(T, T), (T, H)\}$
...
A *continuous sample space* is a sample space where any interval contains a inifnite number of points.
...
Using data supplied by the Queensland Department of Primary Industries (QDPI) from 1 Jan 1889 to 21 July 2002, the maximum temperatures recorded for January on 3534 days WHERE????.
...
plot(PropHeads,
     ...
     type = "l",       # Draw a "l"ine rather than just "p"oints
...
CarDoor <- sample(1:3,         # Could be behind Door1, 2 or 3
