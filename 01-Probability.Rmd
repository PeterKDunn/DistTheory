...
The commute from home to university or work would always take the same length of time;
weather predictions would always be accurate;
you would know exactly what lotto numbers would be drawn on the weekend...
...
In most scientific, engineering and business applications, situations exist where being certain is almost impossible, and so probability is necessary:
...
Simple *processes* examples include tossing a coin, or rolling a die.

The concepts of *sample space* and *sample point* are important for understanding probability.

::: {.definition}
A *set* of points is denoted using capital letters: $A$.

The *elements* (or points) of the set are denoted by lower-case letters, and are shown to belong to a set by enclosing them in brackets:
\[
   A = \{ a_1, a_2, a_3, a_4\}.
\]
:::
...
OLD STUFF, moved here. Still needed??
...
* the **intersection** of sets $A$ and $B$, written as $A\cap B$, is the set of points that $A$ and $B$ have in common.
...
```{r IntersectionUnionSubset, echo=FALSE, fig.height = 6.5, fig.align="center", fig.cap="NEEDS FIXING!!!Subsets, union and intersection of sets. The rectangle represents the universal set $S$."}
...
plotrix::draw.circle(x = 0.4, 
                     ...
                     radius = 0.2,
                     col = colourA)
# Now plot the other circle in white to remove the intersection.
# Then replot it, so the intersection has same shading
plotrix::draw.circle(x = 0.6,
                     y = 0.5,
                     radius = 0.2,
                     col = "white")
...
## Discrete and continuius sample spaces
...
:::{.example #Discrete SS name="Discrete sample space: Countably infinite"}
GIVE EXAMPLE!!
:::
...
Using the classical approach, the six *equally-likely outcomes* are a {1}, {2}, {3}, {4}, {5} and {6}, so the chance of rolling each should be $1/6 = 0.166\dots$.
...
What is the probability that the number is a {2}?

Without the extra information, the chance of obtaining a {2} is $1/6$ as the sample space here has six (equally likely) outcomes:
$S = \{ {1}, {2}, {3}, {4}, {5}, {6}\}$.
...
We write $\Pr({2} \mid \text{even number rolled}) = 1/3$.
:::
