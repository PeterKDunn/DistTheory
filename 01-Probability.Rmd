...
Simple examples of simple *random processes* include tossing a coin, or rolling a die.
...
$B$ can also be described by listing the elements: $B = \{$`r if (knitr::is_latex_output()) {
   '\\Largedice{2}'
} else {
...
::: {.example #DefiningSets name="Sets"}
We could write $\mathbb{Q} = \{m/n \mid m, n \in \text{$Z$ and $n\ne 0$}\}$.
...
The notation for $B_2$ means that a distance of $50$\ m is *not* included in $B_2$.
...
gradientToUse <- seq(b1limit2 - delta, 
                     b1limit2,
                     length = 20)
...
#polygon(x = c(b1limit2 - delta, b1limit2, b1limit2, b1limit2 - delta),
#        y = c(b1lo, b1lo, b1hi, b1hi),
#        col = "red", #colsR,
#        border = NA) # No borders
...
Using the relative frequency approach with the [Bureau of Meteorology](http://www.bom.gov.au) data, the probability of obtaining *more than* 30mm in April is:
\[
...
**R** can be used to verify (not *prove*) theoretical results obtained.
