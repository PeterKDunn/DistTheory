...
\[
   A = \{ a_1, a_2, a_3, a_4\}.
\]
If the elements $a_j$ is in the set $A$, we write $a_j \in A$.
:::
...
* By *enumerating* the individual elements, either directly or using a pattern.
...
\[
   \Omega = \{ d \mid d > 0 \}.
\]
The sample space could also be denoted $\mathbb{R}_{+}$, commpnly used to represent the positive real numbers.
...
1. Is ~~the sex  of the student~~ whether the student is a First-Nations student approximately independent of the type of school, for female students?
1. Is ~~the sex  of the student~~ whether the student is a First-Nations student approximately independent of the type of school, for male students?
...
1. What is the probability the second card is an **Ace**?
   (This part is independent of the first question.)
...
\[
  1 - \left(\frac{365}{365}\right) \times \left(\frac{364}{365}\right) \times \left(\frac{363}{365}\right) \times \left(\frac{365 - n + 1}{365}\right).
\]
...
2. What is the probability that the second player receives exactly one pair, given that the first player received exactly one pair: Higher or lower than the first player?
