...
The concepts of *sample space* and *sample point* are very important.
...
Definition \@ref(def:RandomExperiment) in the next section captures the essentials.
...
Can we use another term rather than "EXPERIMENT"? It contradicts definitions used elsewhere in stats!!!
...
$\overline{A}$ (known as the *complement* of $A$) occurs if $A$ does *not* occur.
:::
...
This method is that it cannot always be used.
For example, suppose a company wishes to determine the probability that matches in a given box will light.
To determine an accurate probability, matches must be struck, and then can no longer be used, which may defeat the purpose of the experiment.
...
Rather than using probabilitis, we would also use *odds*:
\begin{align*}
...
Someone rolled a die 20\,000 times [@wolf1882] and counted the number of times each face came up (Table \@ref(tab:WolfDice)).
...
'Subjective' probabilities are estimated by considering all the relevant issues that may impact the probability  (and may, for example, be based on mathematical models that incorporate information from numerous inputs).
Depending on how these other issues are considered and combined, different individuals (or models) may give different subjective probabilities.
...
The three axioms of probability are (NEEDS BOX)
...
3. If $A_1$, $A_2$, $\dots$ form a sequence of pairwise mutually exclusive events in $S$, then
   ...
   \]
   (that is, for *mutually exclusive* events, the probability of the *union* of events is the sum of    the individual probabilities).
