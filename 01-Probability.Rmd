...
Example \@ref(exm:SampleSpaceContinuous) considered the distance a cricket ball can be thrown, with the sample space $\Omega = \mathbb{R}_{+}$, where $\mathbb{R}_{+}$ represents the positive real numbers.
...
\begin{align*}
   E_1:&\quad \text{Roll a `r knitr::include_graphics("Dice/die1.png", dpi=2000)`}; & E_2:&\quad \text{Roll a `r knitr::include_graphics("Dice/die2.png", dpi=2000)`};\\
   E_3:&\quad \text{Roll a `r knitr::include_graphics("Dice/die3.png", dpi=2000)`}; & E_4:&\quad \text{Roll a `r knitr::include_graphics("Dice/die4.png", dpi=2000)`};\\
   E_5:&\quad \text{Roll a `r knitr::include_graphics("Dice/die5.png", dpi=2000)`}; & E_6:&\quad \text{Roll a `r knitr::include_graphics("Dice/die6.png", dpi=2000)`}.\\
\end{align*}

Consider the event $T$, numbers divisible by 3 and $D$, numbers divisible by 2. 
$T$ and $D$ are compound events:
\[
   T = \{`r knitr::include_graphics("Dice/die3.png", dpi=2000)`, `r knitr::include_graphics("Dice/die6.png", dpi=2000)` \}
   \quad
   \text{and}
   \quad
   D = \{`r knitr::include_graphics("Dice/die2.png", dpi=2000)`, `r knitr::include_graphics("Dice/die4.png", dpi=2000)`, `r knitr::include_graphics("Dice/die6.png", dpi=2000)` \}.
\]
:::
...
For these events:
\begin{align*}
   T\cap D &= \{`r knitr::include_graphics("Dice/die6.png", dpi=2000)` \};\\
   T\cup D &= \{`r knitr::include_graphics("Dice/die2.png", dpi=2000)`, `r knitr::include_graphics("Dice/die3.png", dpi=2000)`, `r knitr::include_graphics("Dice/die4.png", dpi=2000)`, `r knitr::include_graphics("Dice/die6.png", dpi=2000)`\}.
\end{align*}
:::
...
When a standard die is tossed, the sample space comprises six equally-likely outcomes: 
\[
   S = \{ {1},{2},{3},{4},{5},{6}\}.
\]
...
```{r BreastVenn, echo=FALSE, fig.height=7, out.width = '90%', fig.align="center", fig.cap="The Venn diagram for the breast cancer example"}
par( mfrow = c(2, 2))
...
:::{.exercise #CompareMeansStress}
Consider the study in Sect. \@ref(CompSimulation), where the *number* of patients with a higher beta-endorphin concentration were compared at the two time points.

Use a computer simulation to compare the *means* at the two time points.
First, compute the overall mean of the two groups combined, and each group separately.
Then, using similar idea to those used in Sect. \@ref(CompSimulation), compare how often the two means will differ by such an amount by chance alone.

NOT SURE HOW THAT WORKS WITH JUST PROB., DIFFERENT STUDY??
:::
