...
RainTable[1, ] <- as.character(c(9, 24, 12, 27, 24)
RainTable[2, ] <- c(16, 25, 15, 28, 29)
RainTable[3, ] <- as.character( round(RainTable[1, ] / RainTable[2, ], 2) )
# Turn numbers to text to retain as integers
RainTable[1, ] <- as.character( round(RainTable[1, ]) )
RainTable[2, ] <- as.character( round(RainTable[2, ]) )
...
Consider the second.
If you are told that some has died, it is very unlikely the the cause is a fall from aa airborne plane.
...
The second of these is often referred to as the events $Bi$ being *exhaustive*.
...
The reason for this is explained LATER (TREE).
:::
...
This explains the surprisingly result in Example \@ref(exm:BreastCancer): Because breast cancer is so uncommon in younger women, the number of *false* positives overwhelms the number of *true* positives.
:::
