...
* apply the concept of conditional probability and independence.
...
This definition for a *random process* is a little vague, but the intention is to define a process where data are produced.
...
ALL CONFUSING! A SAMPLE **SPACE** IS A SET DENITED S. THEN S IS THE UNIVERSAL *SET*. Combine,
...
An *elementary event* (or a *simple event*) is an event that cannot be decomposed into smaller events.
It is an event with one sample point.
:::
...
The sample space (the set of all possible outcomes, or the *universal set*) could be listed as:
\[
   S = \{(H, H), (H, T), (T, H), (T, T) \},
\]
...
\[
   A = \{ (H, H), (T, H)\}.
\]
The event 'the set of outcomes with *three* heads' is the *null set*, as there are no sample points with three heads.
:::
...
Events $A$ and $B$ are *mutually exclusive* if, and only if, $A\cap B = \emptyset$; that is, there are sample points in common.
...
\begin{align*}
   M: \text{Obtain a Head on Toss 1}
   &= \{ (T, T), (H, H)\}\\
   N: \text{Obtain a Tail on Toss 1} 
   &= \{ (T, T), (T, H)\}
\end{align*}
...
CLARIFY DIFF BETWEEN SETS AND EVENTS??
...
A *continuous sample space* is a sample space that...
:::
...
Listing the sample space is difficult as there are a very arge number of options for selecting seven numbers from 47.
:::
...
Then, $\Pr(A) = 1/6$ (using the classical approach).
...
:::{.example #MumpsIndpendence name="Independence"}
...
\[
   \Pr(\text{Compiled} | \text{Females}) = 63/(63 + 21) = 0.75.
\]

For males, the probability of complying with the isolation request is
\[
   \Pr(\text{Compiled} | \text{Males}) = 36/(36 + 12) = 0.75.
\]
...
Also, the probability that a student isolated (ignoring their sex) is:
\[
   \Pr(\text{Student isolated}) = \frac{99}{132} = 0.75.
\]
Complying with the isolation request is independent of the sex of the student.
:::

```{r MumpsIsolation, echo=FALSE}
MumpsTable <- array( dim = c(2, 2))

MumpsTable[1, ] <- c(63, 21)
MumpsTable[2, ]<- c(36, 12)
...
Obtaining one excludes the possibility of the other... so whether one occurs *depends* on whetehr the other has occurred.
...
A tree diagram (see Fig. \@ref()) is just one of several tools that can assist in calculating and manipulating probabilities.
...
```{r HatTree, echo=FALSE, fig.align="center", fig.cap="Tree diagram for the the hat-wearing data"}
