...
Set $A$ can be defined by describing the elements: $A = \{\null$ Countries whose names begin and end with the letter 'A'$\null}\}$ (whose elements include, for example, 'Austria', 'Antigua and Barbuda' and 'Albania').
...
```{r SimTosses, echo=TRUE, fig.cap = "A simulation of tossing a fair coin $500$ times. The probability of getting a head is computed from the data after each toss.", fig.width = 4, fig.height=3.5}
...
1. Compare the actual factorials with the Stirling approximations for $n = 1, \dots, 10$.
...
A random process consists of selecting two points at random on the square.
...
:::{.exercise}
For sets $A$ and $B$, show:

* that $A\cup (A\cap B) = A$ and
...
* What is the probability that the initial five cards include exactly one pair?
* What is the probabiity that the second player receives exactly one pair, given that the first player received exactly one pair?
* What is the probability that the initial five cards include exactly one three-of-a-kind?
:::
...
:::{.exercise}
Without using a calclator, compute the value of $\displaystyle \frac{^25 C_8}{^25 C_6}$.
:::
