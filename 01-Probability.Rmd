...
Probability is one of the tools used to describe this unpredictability.
...
This definition defined a process where data are produced, when the outcome is unknown.
...
The *elements* in the set are denoted by lower-case letters, and are shown to belong to a set by enclosing them in braces:
\[
   A = \{ a_1, a_2, a_3, a_4\}.
\]
:::
...
This is usually denoted by $S$, $\Omega$ or $U$ (the 'universal set').
...
Consider observing the outcome on a single roll of a die.
...
Having defined the sample space, we can define sets containing just some of those elements of the sample space.
...
The [sample space](#def:SampleSpace) (the set of all possible outcomes, or the *universal set*) could be listed as:
\[
   S = \{(HH), (HT), (TH), (TT) \},
\]
where $H$ represents tossing a head; $T$ represents tossing a tail; and the pair lists the result of each toss in order.
...
An *elementary event* (or a *simple event*) is an event with one sample point, that cannot be decomposed into smaller events.
:::
...
An event $A$ *occurs* on a particular trial of a [random process](#RandomProcess) if the outcome of the trial is one of the sample points in $A$.
:::
...
:::{.proof}
These may be proved using the rules of probability, or using Venn diagrams.
:::
...
Typically. the correct answer canbe found using many different methods.
...
This explains the surprisingly result in Example \@ref(exm:BreastCancer): Because breast cancer is so uncommon in younger women, the *false* positives (0.098) overwhelms the *true* positives (0.015).
:::
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
By definition, $0! = 1$.
By making this defintion, many formulas in combinatorics (that follow) remain valid for all valid choices of parameters values.
:::
...
The probability of picking the correct seven numbers is
\[
...
If the biologist needs to select six of the 20 mice at $3$ weeks, how many ways are there to select the mice?
