...
* uncountably infinite (infomrally, there are too many elements to count).
...
The set $N$ of odds numbers could be defined by the enumeration, by giving a *pattern*: $N = \{1, 3, 5, 7, 9, \dots\}$.
$N$ is countably infinite.\smallskip

Set $A$ corresponding to the 'States belonging to the United States of America' can be defined by describing the elements: $A = \{ \text{Alabama}, \dots \text{Wyoming}\}$.
...
Example \@ref(exm:SampleSpaceContinuous) considered the distance a cricket ball can be thrown $d$, with the sample space $\Omega = \{d | d > 0\}$, or $\Omega \equiv \mathbb{R}_{+}$, where $\mathbb{R}_{+}$ represents the positive real numbers.\smallskip
...
In particular, **R** can be used to verify theoretical results obtained.
...
### The gameshow (Monty Hall) problem {#Gameshow}
...
A more complete solution is given later (see Exercise ???Somewhere???).
...
:::{.exercise #WalkScore}
Use @mypapers:dunnsmyth:glms Exercise 12.9 about Walk Score (mixed), redone of course.
:::
...
Model this process using **R** and use your simulation to estimate the probability that there are *no consecutive numbers* (in any order) amongst the numbers selected.
:::
