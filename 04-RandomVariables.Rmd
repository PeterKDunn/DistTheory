...
In many applications (particularly in medical research, engineering, and insurance) we are more interested in the probability that a subject 'survives' beyond a certain time than the probability that an event has already occurred.
The survival function, denoted $S_X(x)$, is defined as the probability that the random variable\ $X$ exceeds a value $x$:
\begin{equation}
  S_X(x) = P(X > x)
  (\#eq:SurvivalFunction)
\end{equation}
...
* Direction: While $F_X(x)$ is non-decreasing (moving from\ $0$ to\ $1$), $S_X(x)$ is non-increasing. 
  It starts at $S_X(-\infty) = 1$ and falls toward $S_X(\infty) = 0$.
* Interpretation: 
  In reliability engineering, $S_X(x)$ is often called the Reliability Function, $R(x)$, representing the probability that a component is still functioning at time $x$.
  
Discrete vs. Continuous: 

* For continuous variables, $P(X > x) = P(X \ge x)$, so the boundary point doesn't change the value.
...
Suppose the time $X$ (in years) until a specific LED bulb fails follows an exponential distribution with a mean of $2$\ years ($\lambda = 0.5$)
...
$$
  S_X(x) = 1 - (1 - e^{-0.5x}) = e^{-0.5x}
$$
At $x = 0$, the bulb has just started. 
$F_X(0) = 0$ (probability of failing instantly is zero) and $S_X(0) = 1$ ($100$% chance it has survived to the start).
As $x \to \infty$, $F_X(x)$ approaches\ $1$, while $S_X(x)$ approaches\ $0$. 
Eventually, every bulb 'dies'.
The point where $F_X(x) = S_X(x) = 0.5$ is the median of the distribution.
:::
...
# Add a legend
legend("right", 
       legend = c("Distribution Fn F(x)", 
                  "Survival Fn S(x)"),
       col = c("steelblue", "firebrick"), 
       lwd = 3, 
       bty = "n")

# Highlight a specific point: Median (where F(x) = S(x) = 0.5)
median_val <- qexp(0.5, 
                   rate = lambda)
abline(h = 0.5, 
       lty = 2, 
...
Let $X$ be the number of rolls until the first '6'. 
This follows a Geometric distribution where the probability of success is $1/6$.
The survival function $S_X(x)$ tells us the probability that it takes more than $x$ rolls to see a '6'.

In the discrete case, the definition of the survival function requires precision regarding the boundaries

Distribution Function: $F_X(x) = P(X \le x)$. 
It includes the probability of $X$ being exactly $x$.

Survival Function: $S_X(x) = P(X > x)$. 
It excludes the probability of $X$ being exactly $x$.

Note: Because $S_X(x)$ excludes the point $x$, if you are at a "jump" point (like $x=1$ roll), the survival probability $S_X(1)$ is the probability that it takes 2 or more rolls.
:::
...
F_x <- pgeom(x - 1, prob = p)   # P(X <= x) for Geometric
S_x <- 1 - pgeom(x - 1, prob = p)  # P(X > x)

plot(x, F_x, type = "s", lwd = 3, col = "steelblue",
     xlab = "Number of Rolls", ylab = "Probability",
     main = "CDF vs. Survival Function (Geometric)", las = 1,
     ylim = c(0, 1))
lines(x, S_x, type = "s", lwd = 3, col = "firebrick")
legend("right", legend = c("Distribution Fn F(x)", "Survival Fn S(x)"),
       col = c("steelblue", "firebrick"), lwd = 3, bty = "n")
...
* $Q_X(0.02) = 10$.
* $Q_X(0.3) = 11$.
* $Q_X(0.4) = 11$, because\ $11$ is the *smallest* value such that $F_X(x) \ge 0.4$. 
  While $F_X(15) = 0.9$ (which is also $\ge 0.4$), $11$ is the infimum.
* $Q_X(1) = 17$.
...
If the distribution has a clear 'end' (like the discrete example ending at\ $17$), $Q(1)$ is just that end point. 
If the distribution has a 'tail' that goes on forever (like the exponential), $Q(1)$ is infinity.
...
There is a\ $50$% chance the event happens instantly (so that $x = 0$), and a $50$% chance it follows an exponential distribution. 
...
The resulting quantile function (Fig.\ \@ref(XMixedPlots), right panel) is:
$$
  Q_X(p) = 
  \begin{cases} 
...
We use `runif()` to generate a target quantile [DEFINE THAT WORK IN PREV SECTION] between\ $0$ and\ $1$ (i.e, a probability $p^*$ on the $y$-axis (i.e., the range of $F_X(x)$).
...
Many\ **R** functions assume the random variable is denoted by\ `x`, so the PDF is defined as a function of\ `x`:
...
integrate(fX,
          ...
          upper = 1)   # Upper limit of integration
```
...
The DF can also be defined (using `Vectorize()` is an **R** quirk that avoids writing loops):
```{r}
