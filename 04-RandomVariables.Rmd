...
\begin{equation}
  ...
  (\#eq:SurvivalFunction)
\end{equation}
Since $F_X(x) = P(X \le x)$, the survival function is simply the complement of the distribution function:
$$
  S_X(x) = 1 - F_X(x)
$$
...
For continuous variables, $P(X > x) = P(X \ge x)$.
...
The DF is defined such that $F_X(x) = P(X \le x)$, and so includes the probability that\ $X$ is exactly\ $x$.
However, the survival function, defined as $S_X(x) = P(X > x)$, *excludes* the probability that\ $X$ is exactly the value\ $x$.
...
:::{.exercise}
Consider the mixed variable $Y$ where $P(Y=0) = 0.2$ and the rest is continuous.
