...
When finding the larger of two numbers, the order is not important, so combinations\index{Combinations} are appropriate, and the number if possible outcomes is therefore 
$$
  \binom{5}{2} = 10.
$$
...
Importantly, the distributiom functions is defined is  for *all* real numbers.
...
If the distribution has a tail that goes on forever (like the random varianle\ $X$ in Example\ \@ref(exm:QuantileFnContinuous)), $Q(1)$ is infinity.
