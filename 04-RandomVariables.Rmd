...
# Add a legend
legend("right", 
       legend = c(expression(Distribution~fn~italic(F)*"("*italic(x)*")"), 
                  expression(Survival~fn~italic(S)*"("*italic(x)*")") ),
       lwd = 2,
       lty = c(1, 2),
       bty = "n")
...
We use `runif()` to generate a target quantile between\ $0$ and\ $1$ (i.e, a probability $p^*$ on the $y$-axis (i.e., the range of $F_X(x)$).
Then, move horizontally to the curve of the distribution function; then, drop down to the $x$-axis to find the corresponding value $x^*$.

Mathematically, this 'dropping down to the $x$-axis' is what the quantile function does:
$$
   x^* = Q_X(p^*).
$$
