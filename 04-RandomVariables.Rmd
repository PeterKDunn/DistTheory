...
The DF can be found using the function `cumsum()`, th **cum**ulative **sum**ation:
...
Many\ **R** functions assume the random variable is denoted by\ $x$, so the PDF is defined as:
...
```{r}
integrate(fX, 0.2, 0.6)
```

The CDF IS TRICKIER:
```{r}

FX <- function(x) {integrate(fX, 0, x)}
```

Then,

```{r}
FX(0.6)$value - FX(0.2)$value
```
