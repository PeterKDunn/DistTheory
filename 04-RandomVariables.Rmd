...
```{r RVmapping, echo=FALSE, out.width='85%', fig.width=6.5, fig.align="center", fig.cap="A random variable\\ $X$ is a function that maps each outcome in the sample space\\ $S$ to a real number."}
par(mar = c(2, 1, 3, 1), 
    bg = "white")
plot.new()
...
theta <- seq(0, 2 * pi, 
             length.out = 300)
col_s    <- ifelse(knitr::is_html_output(), "#2C6FAC", "black")
col_arr  <- ifelse(knitr::is_html_output(), "#C0392B", "black")
col_line <- "black"
...
sp <- data.frame(
  ...
  lab = c("s[1]","s[2]","s[3]","s[4]","s[5]","s[6]","s[7]")
)
points(sp$x, 
       sp$y, 
       pch = 19, 
...
The *support*\index{Support} of\ $X$ is the subset of $\mathcal{R}_X$ consisting of values assigned non-zero probability or density.
:::
...
Each element\ $s$ of\ $S$ can be assigned to exactly one real number (in this case, to exactly one *integer*):
\begin{align*}
  (1, 1) &\mapsto X = 1;\\
  (1, 2) &\mapsto X = 2;\\
  (1, 3) &\mapsto X = 3;\\
...
$$
$\mathcal{R}_X$ only $18$\ elements, fewer than $|S| = 36$, since several pairs give the same product (as shown above for $X = 12$).
...
Here, the *domain*\index{Domain} is $S = \mathbb{R}$ and the *range*\index{range} is $\mathcal{R}_X = \mathbb{R}$, since $X(s) = s$ for every $s\in S$.
...
```{r PDFParameter, echo=FALSE, fig.align="center", fig.cap="The PDF for the rv\\ $T$ with parameter\\ $\\alpha$.", out.width='80%', fig.width=7, fig.height=3.5}
alpha <- 1
...
$$
In other words, $75$% of the probability occurs at or below $x = 0.6931...$.
...
```{r, RNos, echo=FALSE, fig.align="center", fig.cap="Selecting a random number from a distribution, using the distribution function (left) or quantile function (right).", out.width='60%', fig.width=5, fig.height=3.75}
# CDF:        F(x) = (x-1)^3 / 27  for 1 < x < 4
# Quantile:   Q(p) = 1 + (27p)^(1/3)
...
p_star <- 0.6
x_star <- 1 + (27 * p_star)^(1/3)   # About 3.24
...
# Dot on curve
points(p_star, x_star, pch = 19, cex = 1.1)
...
Random numbers from the distribution can be generated using\ **R** using the idea shown in Fig.\ \@ref(fig:RNos)).:
...
$$
   ...
   \end{cases}.
$$
In **R**, we define the values of\ $X$ where probabilities are non-zero:\index{c()@\texttt{c()}}
...
$$
   f_W(w) = 
      2w \qquad \text{for $0 < w < 1$}.\\
$$
The PDF can be expressed in **R** as a function.
The `integrate()` function in\ **R**, which we use below, assumes the random variable is denoted by\ `x`, so the PDF is defined as a function of\ `x` rather than\ `w`:\index{function()@\texttt{function()}}
...
The DF can also be defined (using `Vectorize()` is an **R** construct to avoids writing loops):\index{Vectorize()@\texttt{Vectorize()}}
```{r}
FX <- Vectorize(function(x) {
        integrate(fX,                  # Integrate f(x)... 
                  lower = 0,           # ... from 0 to x
...
$$
  Q_X(p) =  \left\lceil
            ...
            \right\rceil
$$
where $\lceil x \rceil$ is the *ceiling* function: the smallest integer not less than\ $x$ (e.g., $\lceil\pi\rceil = 4$).
