...
```{r RVmapping, echo=FALSE, out.width='90%', fig.width=7, fig.align="center", fig.cap="A random variable\\ $X$ is a function that maps each outcome in the sample space\\ $S$ to a real number."}
par(mar = c(2, 1, 3, 1), 
    bg = "white")
plot.new()
...
Importantly, the distribution functions is defined is  for *all* real numbers.
...
The same ideas apply for continuous random variables, where integrations are used rather than summations.
...
The distribution function for mixed random variables follow the same ideas as above.
...
### Recovering the probability functiin from the DF {#PFfromDF}
...
As seen from the graph of the distribution in Example\ \@ref(exm:TossingHeads2), the values of\ $x$ where a 'jump' in $F_X(x)$ occurs are the points in the range, and the probability associated with a particular point in\ $\mathcal{R}_X$ is the 'height' of the corresponding jump.
That is,
\begin{equation}
...
In many applications (particularly in medical research, climatology, engineering, and insurance), the probability that a subject 'survives' beyond a certain time is of more interest than the probability that an event has already occurred (i.e., the distribution function).
...
The distribution function is $F_X(x) = 1 - e^{-x/2}$, so the survival function is
$$
  S_X(x) = 1 - (1 - e^{-x/2}) = e^{-x/2}.
$$
At $x = 0$, the bulb has just been turned on.
$F_X(0) = 0$ (probability of failing instantly is zero) and $S_X(0) = 1$ ($100$% chance the bulb has survived to the start).
As $x \to \infty$, $F_X(x)$ approaches\ $1$, while $S_X(x)$ approaches\ $0$. 
Eventually, every bulb 'dies'.
:::
...
For example, the $0.9$-quantile (or the $90$% quantile) is a value with (at least) $90$% of the distribution below or at the value. 
The median is the $0.5$-quantile---half the distribution (i.e., $50$%) on each side of the median.
...
p_vals <- c(0.1, 0.4, 0.9, 1.0)

# 1. Distribution Function Plot
plot(NA, 
     xlim = c(8, 18), 
     ylim = c(0, 1), 
     xlab = expression(italic(x)), 
...
p_starts <- c(0, 0.1, 0.4, 0.9)
p_ends   <- c(0.1, 0.4, 0.9, 1.0)
...
If the distribution has a clear end (like the random variable\ $X$ in Example\ \@ref(exm:QuantileFnDiscrete) ending at\ $17$), $Q(1)$ is just that end point. 
...
Random numbers are generated using distribution functions.
Computer software can generally provide random numbers between between\ $0$ and\ $1$, which correspond to a probability $p^*$ (i.e., the range of $F_X(x)$).
Then, the quantile function is used to find the corresponding value $x^*$:
$$
   x^* = Q_X(p^*)
$$
...
```{r, RNos, echo=FALSE, fig.align="center", fig.cap="Selecting a random number from a distribution, using the distribution function (left) or quantile function (right).", out.width='80%', fig.width=7, fig.height=4}
...
Random numbers from the distribution can be generated using\ **R**:
...
Then, $\Pr(0.2 < W < 0.6)$ can be found using the distribution function also:
