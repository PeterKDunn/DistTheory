...
$$
   ...
   \end{cases}.
$$
In **R**, we define the values of\ $X$ where probabilities are non-zero:
...
The DF can be found using the function `cumsum()`, the **cum**ulative **sum**mation:
...
The `integrate()` function in\ **R**, which we use below, assumes the random variable is denoted by\ `x`, so the PDF is defined as a function of\ `x` rather than\ `w`:
...
Then, the PDF can be plotted (Fig.\ \@ref(fig:RDistributions), left panel) using:
...
As required, the total area under the curve is\ $1$:
...
The DF can also be defined (using `Vectorize()` is an **R** construct to avoids writing loops):
```{r}
