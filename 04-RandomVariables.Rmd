...
```{r ProbDistributionCoin, echo=FALSE, fig.align="center", fig.width=4,fig.height=3.5,fig.cap="The probability function for\\ $H$, the number of heads on two tosses of a coin.", out.width="45%"}
plotDiscrete( -1:3,
...
The $p$-quantile\index{Quantile} of a distribution is a value\ $x$ such that a fraction\ $p$ of the distribution lies at or below\ $x$.
...
```{r, RNos, echo=FALSE, fig.align="center", fig.cap="Selecting a random number from a distribution, using the distribution function (left) or quantile function (right).", out.width='60%', fig.width=5, fig.height=4}
...
:::{.exercise #Exceedance}
In meteorology and climatology, rainfall is often graphed using rainfall survival functions [@dunn2001bootstrap] $S_X(x)  = $ - F_X(x)$, where\ $X$ represents the rainfall.
...
:::{.exercise}
Using the discrete random variable $X$ from earlier (where $x = \{10, 11, 15, 17\}$ and $F = \{0.1, 0.4, 0.9, 1.0\}$)

1. Calculate $Q(0.05)$ manually.
2. Calculate $Q(0.4)$ manually.
3. Calculate $Q(0.41)$ manually.

Then, use the `quantile()` function in\ **R** to verify.

Why does $Q(0.4)$ and $Q(0.41)$ give different results? 

:::

:::{.exercise}
Consider the mixed variable $Y$ where $\Pr(Y=0) = 0.2$ and the rest is continuous.
...
:::{.exercise}
Look at the PDF $f_X(x) = 2x$ for $0 < x < 1$.
