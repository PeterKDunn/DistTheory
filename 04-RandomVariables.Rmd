...
A *random variable*\index{Random variables} is a function that assigns a real number to each outcome\ $s$ in the sample space\ $S$.
A random variable\ $X$ maps $S\to\mathbb{R}$, and the value assigned to an outcome $s\in S$ is written\ $X(s)$.
:::


```{r RVmapping, echo=FALSE, out.width='90%', fig.width=6.5, fig.align="center", fig.cap="A random variable\\ $X$ is a function that maps each outcome in the sample space\\ $S$ to a real number."}
par(mar = c(2, 1, 3, 1), 
    bg = "white")
plot.new()
...
The *domain*\index{Domain} of a random variable is the sample space\ $S$, and the *range*\index{Range} (or *support*,\index{Support} or *value set*)\index{Value set} is the set of real numbers taken by a function.

The *range* for a random variable\ $X$ is often denoted\ $\mathcal{R}_X$, where $\mathcal{R}_X\subseteq\mathbb{R}$.
...
As with PMFs, when a PDF depends on one or more parameters\ $theta$,\index{Parameter} this is made explicit by writing $f_X(x; \theta)$ for one parameter or $f_X(x; \theta_1, \theta_2, \dots)$ for multiple parameters, where the semicolon separates the variable\ $x$ from the parameters.
