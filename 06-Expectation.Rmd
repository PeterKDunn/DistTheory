...
\begin{align*}
     ...
     & = \sum_x x\,e^{xt} p_X(x)\\
     \frac{d^2 M_X(t)}{dt^2} 
     & = \sum_x x^2\,e^{xt} p_X(x),
\end{align*}
and, in general, for each positive integer\ $r$:
$$
   \frac{d^r M_X(t)}{dt^r} = \sum_x x^re^{xt} p_X(x). 
$$
On setting $t = 0$,
\begin{align*}
...
\begin{align*}
  ...
  &= \exp(-\lambda) \exp(\lambda e^t)\\
  &= \exp(\lambda(e^t - 1))
\end{align*}
where Eq.~\@ref(eq:Exponential) has been used.
Thus,
$$
  K_X(t) = \log M_X(t) = \lambda(e^t - 1).
$$
...
$$
Compare with the MGF $M_X(t) = \exp[\lambda(e^t - 1)]$: the substitution $t \to it$ is all that is needed.
:::
...
$$
  M_X(t) = \left(\frac{0.4}{1 - 0.6e^t}\right)^3.
$$
...
The CGF of a random variable\ $X$ is $K_X(t) = \lambda(e^t - 1)$.
