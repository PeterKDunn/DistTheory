...
plot(x, d5, 
     ...
     ylab = "Density",
     axes = FALSE,
     main = expression( atop(bold(Uniform~distribution),
                             bold(mu==2)) ) )
axis(side = 1)
box()
...
For a *continuous* random variable\ $X$, the approach is more involved, and so is deayed until Sect.\ \@ref(CharacteristicFunction).
\index{Moment generating function|)}
...
$$
This follows immediately from $K_{X+Y}(t) = K_X(t) + K_Y(t)$ (which itself follows from $M_{X+Y}(t) = M_X(t)M_Y(t)$ for independent random variables\ $X$ and\ $Y$).
...
:::{.example name="Poisson distribution cumulants"}
If $X \sim \text{Poisson}(\lambda)$ then $M_X(t) = \exp[\lambda(e^t - 1)]$, so
$$
  K_X(t) = \lambda(e^t - 1).
$$
Differentiating $r$ times and evaluating at $t = 0$ gives $\kappa_r = \lambda$ for all $r \geq 1$.
All cumulants of the Poisson distribution are equal to $\lambda$.
In particular, $\kappa_1 = \kappa_2 = \lambda$: the mean and the variance are equal for a Poisson distribution.
:::
\index{Cumulant generating function|)}
...
$$
  \varphi_X(t) = \sum_x \exp(itx) \Pr(X = x).
$$
...
:::{.example name="Poisson distribution characteristic function"}
If $X \sim \text{Poisson}(\lambda)$, then the characteristic function is
$$
...
\begin{align*}
  f_X(x) 
  &= \frac{1}{2\pi} \int_{-\infty}^{\infty} \exp(-itx) \frac{\lambda}{\lambda - it}  \, dt\\
  &= \lambda \exp( -\lambda x)
\end{align*}
provided $x < 0$, after some mathematical manipulation.
Later, this will be identified as an [exponential distribution](#ExponentialDistribution)\index{Exponential distribution}.
:::
