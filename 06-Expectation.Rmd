...
:::{.example name="Normal distribution characteristic function"}
If $X \sim N(\mu, \sigma^2)$, then teh characteristic function is
$$
...
comparison_df <- data.frame(
  Property = c("Definition", "Always exists?",
               "Uniquely determines distribution?",
               "Recovers moments by differentiation?",
               "Additive for independent sums?",
               "Useful for proving CLT?",
               "Values"),
  MGF = c("$E(e^{tX})$, $t \\in \\mathbb{R}$",
          "No — fails for heavy-tailed distributions",
          "Yes, when it exists",
          "Yes: $E(X^r) = M^{(r)}(0)$",
          "Yes: $M_{X+Y} = M_X M_Y$",
          "Indirectly",
  ...
  CGF = c("$\\log M_X(t)$",
          ...
          "Rarely used",
          "Real-valued"),
  CF  = c("$E(e^{itX})$, $t \\in \\mathbb{R}$",
          "Yes — always",
          "Yes — always",
          "Yes, when moments exist",
          "Yes: $\\varphi_{X+Y} = \\varphi_X \\varphi_Y$",
          "Yes — standard proof",
          "Complex-valued"),
...
kable(comparison_df,
      escape = FALSE,
      col.names = c("Property", "MGF $M_X(t)$", "CGF $K_X(t)$", "CF $\\varphi_X(t)$"),
      caption   = "Comparison of moment generating, cumulant generating, and characteristic functions")
```


The practical message is straightforward: use the **MGF** when it exists and moments are the primary interest; use the **CGF** when working with cumulants or sums of independent variables; use the **characteristic function** when dealing with heavy-tailed distributions or when a general proof (such as the CLT) is required.
...
:::{.exercise #InvertIndependetCharacteristic]
...
If $E|X|^r < \infty$, show that $\varphi_X$ is $r$ times differentiable and
$$
