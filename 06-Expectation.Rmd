...
An important result that follows is *Jensen's inequality*,\index{Jensen's inequality} which applies for convex and concave functions.
...
Similarly, the function $g(x) = \log x$ is *strictly concave* over the interval $\{I: x > 0\}$ (Fig.\ \@ref(fig:ConvexConcave), right panel), because
$$
  g''(x) = -\frac{1}{x^2} < 0
$$
...
Since $g(\cdot)$ is convex, it lies *above* every tangent line (see Fig.\ \@ref(fig:ConvexConcave), left panel) at any point\ $x_0$.
