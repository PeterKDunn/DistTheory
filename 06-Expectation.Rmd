...
$$
   \operatorname{E}[X] = 
     \sum_{x_i\in D} x_i \, p_X(X = x_i) + \int_S x \, f_X(x) \, dx,
$$
...
$\operatorname{E}[X]$ is well-defined only if $\operatorname{E}[\,|X|\,] < \infty$. 
...
By Def.\ \@ref(thm:ExpectationFunction) with $g(X) = aX + b$,
$$
...
plot(d3 ~ x,
     ...
     xlab = expression(italic(x)),
     ylab = "Prob. density",
     main = expression( atop(bold(Light~tails*":"~leptokurtic),
                             gamma[1] > 0) ) ) 
lines( d2 ~ x,
       col = "grey")
axis(side = 1)
