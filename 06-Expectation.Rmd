...
In more usual terms, the mathematical expression of a random variable is its *mean*.
...
If $\int |x|f(x)\,dx = \infty$, the expectation does not exist (see Example\ \@ref(exm:MGFDoesNotExist)).
:::
...
$$
   p_T(t) = 0.5 \qquad \text{for $t = 0$ or $t = 1$}.
$$
...
<!--
:::{.example name="Normal distribution cumulants"}
If $X \sim N(\mu, \sigma^2)$ then $M_X(t) = \exp(\mu t + \tfrac{1}{2}\sigma^2 t^2)$, so
$$
  K_X(t) = \mu t + \tfrac{1}{2}\sigma^2 t^2.
$$
This is a second-degree *polynomial* in\ $t$.
Reading the coefficients: $\kappa_1 = \mu$, and $\kappa_2 = \sigma^2$.
Importantly, $\kappa_r = 0$ for all $r \geq 3$.

This is a characterisation of the normal distribution: *a distribution is normal if and only if all cumulants of order\ $3$ and above are zero*.
The degree of departures of higher cumulants from zero is a way to measure non-normality of a distribution.
:::
-->
...
<!--
:::{.example #NormalCharFn name="Normal distribution characteristic function"}
If $X \sim N(\mu, \sigma^2)$, then the characteristic function is
$$
  \varphi_X(t) = \exp\!\left(i\mu t - \tfrac{1}{2}\sigma^2 t^2\right).
$$
This can be verified by substituting $t \to it$ into the MGF.
:::
-->

:::{.example name="Characteristic function"}
...
So, we *know* a relationship exists between\ $X$ and\ $Y$ (but the relationship is non-linear). 
...
\end{equation*}
so $\text{Corr}(X, Y) = 0$. 
But\ $X$ and\ $Y$ are *certainly related*, because\ $Y$ was explicitly defined as a function of\ $X$.
...
$$
To find $f_{Y \mid X = x}(y)$, first $f_X(x)$ is needed:
$$
