...
### Cumulants and the cumulant function {#CumulantFunctions}
...
The *cumulant generating function* (CGF)\index{Cumulant generating function} of a random variable\ $X$ is defined as the natural logarithm of the MGF, wherever the MGF exists:
$$
  K_X(t) = \log M_X(t).
$$
Since $M_X(0) = 1$, then $K_X(0) = 0$.

The CGF can be used to generate the *cumulants*\index{Cumulants} of a distributions.

The coefficients in the Maclaurin expansion of $K_X(t)$ are called **cumulants**:

$$
  K_X(t) = \sum_{r=1}^{\infty} \kappa_r \frac{t^r}{r!}
$$
so that the $r$-th cumulant is
$$
  \kappa_r = K_X^{(r)}(0) = \left.\frac{d^r}{dt^r} K_X(t)\right|_{t=0}
$$
The first four cumulants have direct interpretations:

| Cumulant | Expression in terms of moments | Interpretation |
|----------|-------------------------------|----------------|
| $\kappa_1$ | $\mu_1' = E(X)$ | Mean |
| $\kappa_2$ | $\mu_2' - (\mu_1')^2 = \text{Var}(X)$ | Variance |
| $\kappa_3$ | $\mu_3$ (third central moment) | Related to skewness |
| $\kappa_4$ | $\mu_4 - 3\mu_2^2$ (fourth central moment minus $3\sigma^4$) | Related to excess kurtosis |

where $\mu_r = E[(X - \mu)^r]$ denotes the $r$-th central moment.
...
$$
This follows immediately from $K_{X+Y}(t) = K_X(t) + K_Y(t)$ (which itself follows from $M_{X+Y}(t) = M_X(t)M_Y(t)$ for independent $X, Y$).
...
$$
This is a *polynomial* of degree 2 in\ $t$.  
Reading off the coefficients: $\kappa_1 = \mu$, $\kappa_2 = \sigma^2$, and $\kappa_r = 0$ for all $r \geq 3$.

This is in fact a characterisation: **a distribution is normal if and only if all cumulants of order 3 and above are zero**.  Departures of higher cumulants from zero measure non-normality in a precise sense.
:::
...
In particular, $\kappa_1 = \kappa_2 = \lambda$, which corresponds to the familiar mean-equals-variance property.
:::


```{r cgf-illustration, message=FALSE, warning=FALSE}
library(tidyverse)
theme_set(theme_bw(base_size = 13))

# Illustrate CGF vs MGF for N(0,1)
t_vals <- seq(-2, 2, length.out = 300)

cgf_df <- tibble(
  t   = t_vals,
  MGF = exp(0.5 * t_vals^2),           # N(0,1): M(t) = exp(t^2/2)
  CGF = 0.5 * t_vals^2                  # K(t) = t^2/2
) |>
  pivot_longer(-t, names_to = "Function", values_to = "value")

ggplot(cgf_df, aes(x = t, y = value, colour = Function)) +
  geom_line(linewidth = 1.1) +
  coord_cartesian(ylim = c(0, 8)) +
  labs(title = "MGF and CGF for N(0, 1)",
       x = "t", y = "Value",
       caption = "Note: CGF = log(MGF) is simply t²/2 for the standard normal") +
  theme(legend.position = "top")
```

The CGF inherits the main limitation of the MGF: it need not exist for all distributions.
...
### Characteristic function {#CharacteristicFunctions}

The MGF $M_X(t) = E(e^{tX})$ uses a real-valued argument $t$.
The *characteristic function* replaces $t$ with $it$ where $i = \sqrt{-1}$, giving
$$
  \varphi_X(t) = E(e^{itX}) = E(\cos tX) + i\, E(\sin tX), \quad t \in \mathbb{R}
$$
Because $|e^{itX}| = 1$ for all real $t$ and $X$, the expectation always exists; no moment conditions are required.

The characteristic function of a random variable $X$ is
$$
  \boxed{\varphi_X(t) = E\!\left(e^{itX}\right), \quad t \in \mathbb{R}}
$$
...
$$
The characteristic function is sometimes preferred over the MGF and CGF.

**Always exists.** $|\varphi_X(t)| \leq E|e^{itX}| = 1$ for all $t$, so
$\varphi_X(t)$ is well-defined and bounded for every distribution.

**Uniqueness.** The characteristic function uniquely determines the
distribution: if $\varphi_X(t) = \varphi_Y(t)$ for all $t \in \mathbb{R}$, then
$X$ and $Y$ have the same distribution.  This is the analogue of the uniqueness
theorem for MGFs, but holds without any moment restrictions.

**Inversion.** The density (if it exists) can be recovered from the
characteristic function by the **inversion formula**:
$$
  f(x) = \frac{1}{2\pi} \int_{-\infty}^{\infty} e^{-itx} \varphi_X(t)\, dt
$$

**Independence and convolution.** If $X \perp Y$ then
$$
  \varphi_{X+Y}(t) = \varphi_X(t)\cdot \varphi_Y(t)
$$
exactly as for MGFs.

**Moments.** If $E|X|^r < \infty$ then $\varphi_X$ is $r$ times differentiable and
$$
  E(X^r) = \frac{1}{i^r} \varphi_X^{(r)}(0)
$$

**Symmetry.** $X$ is symmetric about zero (i.e. $X \overset{d}{=} -X$) if and only if $\varphi_X(t)$ is real-valued for all $t$.
...
$$
This can be verified by completing the square in the exponent of the Fourier
integral, or by substituting $t \to it$ into the MGF.
:::


:::{.example name="Cauchy distribution characteristic function"}
The Cauchy distribution has density $f(x) = \{\pi(1 + x^2)\}^{-1}$ and has **no finite moments**; specifically, $\operatorname{E}|X|$ does not exist, so the MGF is nowhere finite.  
Nevertheless, the characteristic function is well-defined:
$$
  \varphi_X(t) = e^{-|t|}
$$
This is a clean, closed-form expression that carries full distributional
information despite the non-existence of moments.
:::
...
```{r cf-illustration}
# Visualise the real part of the characteristic function for several distributions
...
cf_mat <- data.frame(
  ...
  N01    = exp(-0.5 * t_seq^2),
  N04    = exp(-2   * t_seq^2),
  Cauchy = exp(-abs(t_seq)),
  Laplace = 1 / (1 + t_seq^2)
)

# Convert to long form manually
cf_long <- reshape(cf_mat,
                   varying   = c("N01", "N04", "Cauchy", "Laplace"),
                   v.names   = "Re_cf",
                   timevar   = "Distribution",
                   times     = c("N(0,1)", "N(0,4)", "Cauchy", "Laplace"),
                   direction = "long")

# Plot
dists   <- c("N(0,1)", "N(0,4)", "Cauchy", "Laplace")
colours <- c("steelblue", "firebrick", "forestgreen", "darkorchid")

plot(NA, xlim = c(-4, 4), ylim = c(-0.1, 1.05),
     xlab = "t",
     ylab = expression(Re(varphi(t))),
     main = "Real part of characteristic function")
mtext("All distributions shown are symmetric, so the CF is real-valued",
      side = 3, line = 0.3, cex = 0.85)
abline(h = 0, lty = 2, col = "grey50")

for (i in seq_along(dists)) {
  sub <- cf_long[cf_long$Distribution == dists[i], ]
  lines(sub$t, sub$Re_cf, col = colours[i], lwd = 2)
}

legend("topright", legend = dists,
       col = colours, lwd = 2, bty = "n")
```
...
5. The text states that distributions with *negative* excess kurtosis (*platykurtic* distributions)\index{Playtkurtic}
