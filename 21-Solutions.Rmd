...
Write $d$ as $\Pr(\text{used drugs})$ and $y = \Pr(\text{Answers `yes'})$.
...
\begin{align*}
   ...
   &= (1 - d) \times\left(\frac{N - m}{N}\right) + d\times\frac{m}{N}.
\end{align*}
...
\]
2. $Y = 175/400 = 0.4375$.
3. $\displaystyle d = \frac{yN - N + m}{2m - N}$, so plugging in $N = 100$, $m = 25$ and $y = 175/400$ gives $d = 0.625$.
:::
...
1. $4/52 \approx 0.07692308$.
1. $3/51 \approx 0.05882353$. FIX!!! This is secnd card ACE,but first card may or maty not be an ACE.
1. Be careful:
...
\end{align*}
1. We can select any of the 52 cards to begin.
...
1. Probability a bit less.
1. Using the above logic again:
...
```{r fig.align="center", fig.cap="The DF for $f(z)$"}
F1 <- function(x){
...
Using the area of triangles, the 'height' is $2/27$ as shown below.
...
Then, 
$\Pr(S > 20 \mid S > 10) = \Pr( (S > 20) \cap (S > 10) )/\Pr(S > 10) = \Pr(S > 20)/Pr(S > 10)$.
This is about 0.263.
:::
...
1. Write as $p(x) = \log_{10}(1 + x) - \log_{10}x$; then the sum is
  ...
  \[
    (\log_{10}(2) - \log_{10} 1) 
    + (\log_{10} 3 - \log_{10} 2)
    + (\log_{10} 4 - \log_{10} 3) + \dots
    + (\log_{10} 10 - \log_{10} 9)
  \]
and things cancel, leaving $\log{10}10 = 1$.
1. DF:
:::
...
:::{.answer}
**Answer to Exercise \@ref(exr:CountingBirds).**

**ASSUMING ERROR IN PAPER**:
Write $k=r$ and $1-p=(m/k)$ in out parameterisation.

So then:

* in ponderosa pine stand (riparian sites): $m = 0.11$ and $k = 0.275$: $r = 0.275$ and $p = 0.6$
* in lodgepole pine stands (again, riparian sites): $m = 0.04$ and $k = 0.275$: $r = 0.275$ and $p = 0.8545$.


$\Pr(\text{fewer than 2 birds})$ means observing 0 birds or 1 bird.

$\Pr(\text{fewer than 2 birds} \mid \text{1 or more birds seen})$ means observing 0 birds or 1 bird.
...
plot( y = c(0, 1),
      ...
      type = "n",
      las = 1,
      main = "Two negative binomial distributions",
      xlab = expression(italic(x)),
      ylab = "Prob function")
points(pf1 ~ x, 
       pch = 1)
...
pnbinom(1, # Ponnderosa pine stand
        size = 0.275,
        p = 0.6 )
pnbinom(1, #lodgepole pine stand
        size = 0.275,
        p = 0.8545)

dnbinom(1, # Ponnderosa pine stand
        size = 0.275,
        p = 0.6 ) / 
  (1 - dnbinom(0, # Ponnderosa pine stand
               size = 0.275,
               p = 0.6 ) )
dnbinom(1, #lodgepole pine stand
        size = 0.275,
        p = 0.8545) / 
  (1 - dnbinom(0, #lodgepole pine stand
               size = 0.275,
               p = 0.8545) )
...
```{r}
dpois(10, lambda = 12.5)
dpois(6, lambda = 2.5 * 3) * dpois(4, lambda = 2.5 * 2)
```

For part 3: The number of errors occurring in the 'overlap minute' could be 0, 2, \dots 6:

* 6 errors in overlap minute: 
 $\Pr(\text{0 errors in first 2 minutes})\times{}$ 
 $\Pr(\text{6 errors in overlap minute})\times{}$
 $\Pr(\text{0 errors in final 2 minutes})$ 
* 5 errors in overlap minute: 
 $\Pr(\text{1 error in first 2 minutes})\times{}$ 
 $\Pr(\text{5 errors in overlap minute})\times{}$
 $\Pr(\text{1 error in final 2 minutes})$ 
...
$m = \frac{\mu}{\sigma^2}\left( \mu(1 - \mu) - \sigma^2\right)$;
...
1. $\text{E}(X) = \alpha \beta \approx 38.4$ and $\text{var}(X) = \alpha \beta^2 \approx 54.54$. 
1. $\Pr(C > 30 \mid C < 50) = \Pr(30 < C < 50) / \Pr(C < 50)\approx 0.0044$.
:::
...
plot( y = dgamma(x, shape = alpha, rate = beta),
      x = x,
      xlab = expression(italic(x)),
      ylab = "Prob. fn",
      type = "l",
      lwd = 2)
plot( y = pgamma(x, shape = alpha, rate = beta),
      x = x,
      xlab = expression(italic(x)),
      ylab = "Dist. fn",
      type = "l",
...
( pgamma(50, shape = alpha, rate = beta) - pgamma(30, shape = alpha, rate = beta) ) /
  pgamma(50, shape = alpha, rate = beta)
```
...
$\text{CV} = \frac{\sqrt{a\beta^2}}P{\alpha\beta} = 1/\sqrt{\alpha}$, which is constant.
:::
...
1. With $V$, the largest 20% correspond to $V = 0.4940853$, so that $S = 9.934938$; the largest 20% are longer than 9.93 minutes.
:::
...
qbeta(0.8, shape1 = 0.287, shape2 = 0.926)
```
...
1. $\text{E}(X) = \alpha\beta = 3.76$ days.
   $\text{var}(X) = \alpha\beta^2 = 12.75$ days^2^.
...
1 - pgamma(4, shape = alpha, scale = beta)
1 - pexp(4, rate = 1/beta)

qgamma(0.95, shape = alpha, scale = beta)
qexp(0.95, rate = 1/beta)
```
:::
...
1. About 25.2%.
2. $Pr(T > 7 | T > 5) = \Pr(T > 7)/\Pr(T > 5)$ or about 33.8%.
3. About 7:30pm.
...
2. County A: 
   ...
   * $\text{var}(X) = 0.01196957$ or about 1.2%.  
   County B: 
   * $\text{E}(X) = m / (m + n) = 0.274$ or about 27.4%.
   * $\text{var}(X) = 0.02939085$ or about 2.9%.
...
qbeta( 0.5, shape1 = 11.52, shape2 = 4.75)
qbeta( 0.5, shape1 = 3.85, shape2 = 3.65)
```
:::
...
# Tchebyshev: AT LEAST this proportion
k <- 2
1 - 1/k^2
```
:::
...
1. This corresponds to the cell $X = 1, Y = 2$:  the answer is $1/40$. 
1. $\Pr(X + Y \le 1) = \Pr(X = 0, Y = 0) + \Pr(X = 0, Y = 1) + \Pr(X = 1, Y = 0) = 1/2$.
1. $\Pr(X > Y) = \Pr(X = 1, Y = 0) = 1/4$.
1. Write:
...
1. $13 + 4 = 17$; 2. $5 + 2 = 7$; 3. $(2\times 13) - (3\times 4) = 14$; $(2^2\times 5) + (-3)^2\times 2) = 38$.
:::
...
1. $17$ ; 2. $5 + 2 + 0.2 = 7.2$; 3. $-6$; $(2^2\times 5) + ((-3)^2\times 2) + (2\times -3\times 0.2) = 36.8$.
:::
...
1. With the sampling distribution $Y\mid p\sim \text{Geom}(p)$ and prior distribution $p\sim\text{Beta}(m, n)$, the posterior distribution is $p\sim\text{Beta}...$
https://en.wikipedia.org/wiki/Conjugate_prior
1. With $m = 1.24$ and $n = 2$, 
:::
