...
```{r WaitingTimes, echo=FALSE, fig.width=5, fig.align="center", fig.cap="Waiting times"}
flo <- function(x){
  (4/81) * x - (14/81)
}
fhi <- function(x){
...
```{r Solition, echo=FALSE, fig.align="center", fig.caption="The Soliton distribution", fig.width=5}
pSoliton <- function(K){
  y <- array( dim = K)
  Kseq <- 2:K
...
\begin{align}
  f_X(x) 
  &= \int_{-\infty}^{\infty} MX(it) ]exp(-itx)\, dt \\ 
...
t <- seq(-10, 2, # Recall: t only from -infty to lambda (= 2)
         length = 1000)
...
1. We find that $\displaystyle \int_0^1\!\!\!\int_{x^2}^1 (1 - x)y\, dy\, dx = 7/30$, so that $k = 30/7$.
2. $25/28$.
3. First, $f_X(x) = 15(x - 1)(x^4 - 1)/7$, and then $\Pr(X > 0.5) = 183/896$.
:::
