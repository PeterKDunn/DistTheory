...
We write $X\sim N(\mu, \sigma^2)$ (though sometimes in non-theoretical work the notation used is $X\sim N(\mu, \sigma)$).
:::


Some authors use the notation $X\sim N(\mu,\sigma)$ so it is wise to check each article or book for the notation used.
...
\begin{equation}
  ...
  (\#eq:ErrorFunction)
\end{equation}
which appears in numerous places, is commonly tabulated, and usually available in most computer packages.
:::
...
Specifically, $\Phi(z)$ corresponds to the function `pnorm(z)`.
:::
