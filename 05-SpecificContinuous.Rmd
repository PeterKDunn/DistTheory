...
Consider a [Poisson process](PoissonDerivation), where events occur at an average rate of $\lambda$ events per unit time (e.g., $\lambda = 3$ per minute).
Then, in a length of time $t$, the number of occurrences $Y$ would have an average rate of $\lambda t$ (e.g., in 6 minutes, we'd expect an average rate of $3\times 6 = 18$ occurrences).
So the *number* of occurrences in the time $\lambda t$ (say $Y$) would be modelled with a Poisson distribution with mean $\lambda t$, with probability function  
\begin{equation}
  p_Y(y) = \frac{\exp(-\lambda t) (\lambda t)^y}{y!},
   (\#eq:PoissonPMF2)
\end{equation}
from the Poisson probability function (Sect. \@ref(PoissonDefinition)).

Let $X$ be the *time* at which the first event occurs.
Suppose *zero* occurrences (i.e., $Y = 0$) happen in the interval $[0, T]$. 
This is the same as saying that the *first* occurrence must happen *after* time $T$.
...
text( x = 10,
      y = 0.5,
      pos = 3,
      labels = expression(italic(t)) )

# Locate events that occur
locateEvents <- c(0.25,
                  0.35, 
                  0.42,
                  0.55, 
                  0.78) * 10
numEvents <- length(locateEvents)

points( x = locateEvents,
        y = rep(0, length = numEvents),
        pch = 19)
...
# Time T
arrows( x0 = 0.8 * locateEvents[1],
        x1 = 0.8 * locateEvents[1],
        y0 = 3,
        y1 = 0,
        angle = 15,
        length = 0.15)
text(x = 0.8 * locateEvents[1],
     y = 3,
     pos = 3,
     labels = expression(italic(T)))
...
Since the mean rate of occurrences is $\lambda t$, then the probability of zero events in the time interval $[0, T]$ is  
\[
  \Pr(Y = 0)
  = \Pr(X > T) 
...
Since $\Pr(X < t) = 1 - \Pr(X > t)$ is the *distribution function* for $X$, we can write   
\[
  F_X(x) = 1 - \exp(-\lambda t);
\]
differentiating, the probability density function of $X$ is  
\[
  f_X(x) = \lambda \exp(-\lambda x) \quad \text{for $x > 0$}.
\]
...
::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
In **R**, the exponential distribution functions have the form `[dpqr]exp(rate)` where `rate`${}= \lambda = 1/\beta$.
:::
...
(sp the *rate parameter* is $\lambda = 1/0.2186 = 4.575$ per second.)
...
\[
   \Pr(T > t) = \Pr(N(t) = 0).
\]
But $N(t) \sim \text{Pois}(\lambda)$ with mean $\text{E}(N(t)) = \lambda t$; hence  
\begin{equation}
   \Pr(T > t) 
   ...
   (\#eq:ExpSurvivor)
\end{equation}
This shows that the df of $T$ is  
\[
   F_T(t)
   = \Pr(T \le t) 
   = 1 - \Pr(T > t)
...
The beta distribution functions have the form `[dpqr]beta(shape1, shape2)`, where `shape1`${}= m$ and `shape2`${}= n$.
:::
