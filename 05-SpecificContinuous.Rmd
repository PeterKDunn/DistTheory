...
```{r Normal, echo=FALSE, fig.align="center", fig.cap="Some examples of normal distributions. The solid lines correspond to $\\sigma = 0.5$ and the dashed lines to $\\sigma = 1$. For the left diagram, $\\mu = -3$; for the right diagram, $\\mu = 2$.", fig.height=3.5, fig.width=7, out.width='80%'}
...
&= \left[ -e^{-\frac 12z^2}\right]^\infty_{-\infty} = 0
\end{align*}
...
:::{.example #ComputingNormalProbs1 name="Computing normal probabilities"}
If the random variable $Z$ is distributed $N(0, 1)$ then,
...
:::{.example #ComputingNormalProbs2 name="Computing normal probabilities"}
...
```{r, AreaUnderNormal, echo=FALSE, fig.align="center", fig.cap="Area under standard normal curve: $z = -1.96$ to $z = 1.96$",fig.width=3.5, fig.height=3.5}
...
@pearson1903laws give the forearms lengths of 140 adult males (in inches).
...
Drawing a picture of the situation is helpful; see Figs. \@ref(fig:NormalForearms) and \@ref(fig:NormalForearmsBackwards).
...
\begin{align*}
   ...
   & = 17.6
\end{align*}
Approximately 15% of men in the study have forearms shorter than about 17.6 inches.
:::




```{r NormalForearms, echo=FALSE, fig.align="center", fig.cap="The forearm lengths (in inches) of 140 adult males. The shaded region corresponds to the proportion of males with a forearm longer than 20.5 inches.",fig.width=7,fig.height=3.5,out.width='80%'}
...
```{r NormalForearmsBackwards, echo=FALSE, fig.align="center", fig.cap="The forearm lengths (in inches) of 140 adult males. The shaded region corresponds to the proportion of males with a forearm longer than 20.5 inches.",fig.width=7,fig.height=3.5,out.width='80%'}
...
Figure~\ref{FG:BinomialCC} shows the upper tail of the distribution near $X = 35$.
...
CCTable.caption <- "Some events and their probabilities for the die-rolling example, computed using the binomial distribution (exact) and the normal approximation using the continuity correction. The accuracy of the approximation is very good."
...
The parameter $\lambda = 1/\beta$ (or $\theta = 1/\beta$) is often used in place of $\beta$.
...
This is indeed the case; see Fig. \@ref(FG:speccont:expon).
...
A lot of data is only defined for non-negative values or for positive values[The phrase *non-negative* permits zero; *positive* explicitly excludes zero.]
...
:::{.theorem #GammaProperties name="Properties of gamma distribution"}
...
Like for the normal distribution, the distribution function of the gamma cannot, in general, be computed without using numerical integration or tables (although see Example \@ref(exm:ElectricalComponent)).
...
If $\alpha$ was, for example $\alpha = 2.5$, integration by parts could not have been done. (CHECK)!
