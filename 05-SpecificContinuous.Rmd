...
The situation can be shown in Fig. \@ref(fig:NormalWalkingSpeed), top left panel.\smallskip
...
Assuming the score has a normal distribution, what is the lowest passing score?\smallskip
...
What is the probability that 35 or more of the group will survive beyond 40 weeks?\smallskip
...
Both $np = 30$ and $n(1 - p) = 70$ are much larger than 5, so this approximation is expected to adequate.\smallskip
...
This change from $X \ge 34.5$ to $Y > 34.5$ is called using the *continuity correction*.\smallskip
...
The random variable $X$ is the number of times a `r knitr::include_graphics("Dice/die1.png", dpi=2000)` appears; then, $X\sim \text{Bin}(n = 100, p = 1/6)$.\smallskip
...
Various probabilities (Table \@ref(tab:ContCorrectionTable)) show the accuracy of the approximation, and the way in which the continuity correction has been used.\smallskip
...
What proportion of time intervals can be expected to be longer than 1 second?\smallskip
...
We can describe the distribution of the time between consecutive events and the distribution of the number of events in one day (24 hours).\smallskip

Since events occur at the mean rate of $\lambda = 5$ events per hour, then the mean time between consecutive events is $\beta = 1/\lambda = 0.2$ hours. 
Hence, the mean number of events in one day is $\mu = 24\times 5 = 120$.\smallskip
...
Then, $T \sim \text{Exp}(\beta = 6)$.\smallskip
...
A similar approach is adopted by [@climate:wilks:1990].\smallskip
...
What is the probability that a component will last for more than three hours?\smallskip
...
If $\alpha$ was, for example $\alpha = 2.5$, integration by parts would not be possible.\smallskip
...
If $X\sim \text{Gam}(\alpha, \beta)$, find the distribution of $Y = kX$ for some constant $k$.\smallskip
...
The retailer has observed that over many weeks the proportion of the available fuel supply sold is well modelled by a beta distribution with $m = 4$ and $n = 2$.\smallskip
