...
\]
then $X$ has a *continuous uniform distribution* and we write $Y\sim U(a,b)$ or $X\sim\unif(a,b)$.
:::
...
```{r ContinuousUniform, echo=FALSE, fig.align="center", fig.caption="The pdf for a continuous uniform distribution $U(a,b)$."}
plot( x = c(1, 7),
      y = c(0, 1),
      type = "n",
      xlab = "x",
...
:::{.example #ContinuousUniform name="Continuous uniform"}
 If $X$ is uniformly distributed on $[-2, 2]$, find $\Pr(|X|>\frac 12)$.
\begin{align*}
     \Pr(|X|>\frac 12 ) 
     &= \Pr{X > \frac 12 }+ \Pr{X < -\frac 12}\\
     &= \int^2_{\frac 12} f(x)\,dx +\int^{-\frac 12}_{-2} f(x)\,dx \text{ where }f(x)=\frac 14\\
     &= \frac 34.
\end{align*}
:::
...
In many ways it is the cornerstone of modern statistical theory as you will see in this and follow-up courses. 
It was first investigated in the 18th century when scientists observed an astonishing degree of regularity in errors of measurement. 
They found that the patterns which they observed could be closely approximated by continuous curves which they referred to as 'normal curves of errors' and attributed to laws of chance.
It has many applications and many natural quantities (such as heights and weights of humans) follow normal distributions.
...
Some texts use the notation $X\sim N(\mu,\sigma)$ so it is wise to check.
...
for (i in (1:2)){
   ...
   plot( x,
         ...
         lwd = 2,
         las = 1,
         main = paste("Normal distribution: mean =", mu[i])
         )
   lines( x,
         dnorm(x, 
               mean = mu[i], 
...
1. $f_X(x)$ is symmetrical about $\mu$
...
The proof that $\int^\infty_{-\infty} f_X(x)\,dx = 1$ is not obvious and relies on first squaring the integral and then changing to polar coordinates.
...
:::{.proof}
The proof of these results is delayed until after the Theorem~\ref{TM:stdnormal}.
:::
...
The proof of \ref{EQN:stdnormal} is given...
...
:::{.proof}
As usual we could prove part 3 and use that result to prove parts 1 and 2.
However, it's instructive to prove parts 1 and 2 directly.
...
\begin{align*}
   ...
   &= 1
\end{align*}
since the first term is zero and the second term makes use of~(\ref{EQN:stdnormal}).
...
\begin{equation*}
     -\frac 1{2}[z^2 -2tz] = -\frac 1{2}((z-t))^2+\frac 12 t^2.
\end{equation*}
...
\begin{equation}
   Z = \frac{X - \mu}{\sigma}.
   \label{EQN:speccontinuous:zscore}
\end{equation}

Theorem~\ref{TM:stdnormal} makes this proof straightforward:
...
\]
This integral cannot be written in terms of standard functions and in general must be evaluated for a particular $x$ numerically (eg using Simpson's rule or the like).

We don't have to do this because all statistical packages have a built-in procedure that evaluates $F_X(x)$ for any $x$.
Also there are tables which tabulate values for the df of the standard normal distribution
\[
...
To compute the probability $\Pr(X > 20.5)$, first transform into a standard normal distribution using Equation~(\ref{EQN:speccontinuous:zscore}), and then use the tables.
\begin{align*}
   \Pr(X > 20.5)
...
It is often useful to draw a picture of the situation; see Fig. \@ref(NormalForearmsBackwards).
...
To convert this into a forearm length, use Equation~(\ref{EQN:unstandardising}) to get
\begin{align*}
   x^*
   &= z\sigma + \mu \\
...
\begin{align*}
   \Pr(X > x_1) 
   &= 0.75\\
   \Pr(Z > \frac{x_1 - 500}{100}) 
   &= 0.75\\
   \Pr(Z < \frac{500 - x_1}{100}) 
   & = 0.75
\end{align*}
...
In Section~\ref{SC:specdiscrete:binomial}, the binomial distribution was considered.
...
Figures~\ref{FG:specdiscrete:binomial} and~\ref{FG:speccont:normbin}.
...
Figure~\ref{FG:speccont:normbin} shows some picture of various binomial pfs overlaid with the corresponding normal distribution; the approximation is visibly better as the guidelines given above are satisfied.

```{r NormalApprox, echo=FALSE, fig.align="center", fig.cap="The normal distribution approximating a binomial distribution. The guidelines suggest the approximation should be good when $np \\ge 5$ and $n (1 - p) \\ge 5$; this is evident from the pictures. In the top row, a significant amount of the approximating normal distribution even appears when $Y < 0$."}
par( mfrow = c(2, 2))
...
Consider rolling a standard die 100 times, and counting the number of \epsdice{1}'s that appear uppermost.
The random variable $X$ is the number of \epsdice{1}'s; then, $X\sim \text{Bin}(n = 100, p = 1/6)$.
...
Plots of the pdf for various exponential distributions are given in Figure~\ref{FG:speccont:exponential}.

```{r, echo=FALSE, fig.align="center", fig.cap="Exponential distributions"}
x <- seq(0, 12,
         length = 100)
...
Consequently we have that $N\sim\Pois(\mu = 120)$ and  $X\sim\text{Exp}(\beta = 0.2)$ (or, equivalently, $X\sim\text{Exp}(\lambda = 5)$).
...
\Pr{T > t} = \Pr{N(t) = 0}
\]
But $N(t) \sim \Pois(\lambda)$ with mean $\text{E}(N(t)) = \lambda t$.
Hence
\begin{equation}
   \Pr(T > t) 
   = \frac{(\lambda t)^0 e^{-\lambda t}}{0!}
   = e^{-\lambda t}\label{EQ:expsurvivor}
\end{equation}
...
:::{.proof}
Using Definition~\ref{DF:conditional},
\[
   \Pr(T > s + t \mid T > s) 
...
\begin{align*}
   ...
   &= \Pr(T > t)
\end{align*}
where we have used Equation~(\ref{EQ:expsurvivor}).
:::
...
:::{.example #memoryless name="Memoryless propery of exponential distribution"}
...
Example~\ref{EG:explifespan} highlights the notion that an exponential process is ageless, in the sense that the risk of 'mortality' remains constant with age.
...
This is not always a problem, especially when the observations are far from zero; see Example~\ref{EG:speccont:normal:forearm}.
...
\]
then $X$ has a *gamma distribution*, where $\Gamma(\cdot)$ is the *gamma function* (see Section~\ref{SEC:speccont:gamma:gammafn}) and $\alpha, \beta > 0$.
...
Plots of the gamma pdf for various values of the parameters are given in Figure~\ref{FG:speccont:gamma}.
...
```{r GammaDistribution, echo=FALSE, fig.align="center", fig.cap="The pdf of a gamma distribution for various values of $\\alpha$ and $\\beta$"}
...
\begin{align}
   ...
   &= 1\quad \text{(because $\int_0^\infty e^{-y} y^{\alpha-1}\,dy=\Gamma(\alpha)$)}\label{EQN:gammastd}
\end{align}
as it must.
...
\underbrace{\int_0^{\infty}\frac{e^{-x/\beta}x^{(\alpha+1)-1}}{\beta^{\alpha+1}\Gamma(\alpha+1)} \, dx}_{=1}
   = \alpha\beta.
\]
This result follows from using~(\ref{EQN:gammastd}) and Theorem~\ref{TM:gamma}.
...
From the information, $T\sim \Gam(\alpha = 2, \beta = 1)$.
...
If $X\sim \Gam(\alpha, \beta)$, find the distribution of $Y = kX$ for some constant $k$.
...
$B(m, n)$ defined by (\ref{EQN:betadef}) is known as the *beta function* with parameters $m$ and $n$.
...
:::{.theorem}
The beta function (\ref{EQN:betadef}) satisfies the following:
...
* $B(1, 1) = 1$
* $\ds B\left(\frac{1}{2}, \frac{1}{2}\right) = \pi$
* For positive constants $m$, $n$ (not necessarily integers) $B(m, n) = \frac{\Gamma(m) \Gamma(n)}{\Gamma(m + n)}$.
:::

:::{.proof}
To prove this, put $z = 1 - x$ and hence $dz = -dx$ in (\ref{EQN:betadef}). 
We obtain
\begin{align*}
   B(m, n)
...
Put $x = \sin^2\theta$, $dx = 2\sin\theta\cos\theta\,d\theta$ in (\ref{EQN:betadef}).
We have
\[
...
If $X \sim \be(m,n)$ then

* $\text{E}(X) = \ds\frac{m}{m + n}$.
* $\text{var}(X) = \ds\frac{mn}{(m + n)^2(m + n + 1)}$.
* A mode occurs at $x = \ds\frac{m - 1}{m + n - 2}$ for $m, n > 1$.
:::


:::{.proof}
Assume $X \sim \be(m,n)$, then
\begin{align*}
   \mu_r'
   =\text{E}(X^r)
...
\end{align*}
Putting $r = 1$ and $r = 2$ into the above expression, and using the fact that $\text{var}(X) = \text{E}(X^2) - (\text{E}(X))^2$, it is easy to verify that the mean and variance are $\text{E}(X) = \ds\frac{m}{m + n}$ and $\text{var}(X) = \ds\frac{mn}{(m + n)^2(m + n + 1)}$.
