...
The following examples illustrate the use of *the**R**.
...
Thta is, the probability of such an event occurring in the next small interval, whether the failure of a component or the occurrence of an accident, remains constant regardless of the age of the component or the length of time since the last accident. 
...
Instead, the beta distribution is used, for example, tp model percentage cloud cover (and also see Example \@ref(exm:BetaTanks)).
...
Suppose we are seeking women over 175cm tall for a study; what percentage of women are over 173cm tall?
...
Of course, the actual percentage can be computed by working directly with normal distributions, but simulations allows us to consider more complex situations.
For this simple question initially (Fig. \@ref(fig:HeightSimHistograms), right panel):
...
cat("Percentage of 'tall' people that are female:", 
    ...
    '%\n\n')
cat("Variance of hieghts: ", var(htP), "\n")
```
...
:::{.exercise #GammaCV}
Show that the gamma distribution has a constant *coefficient of variation*, where the *coefficient of variation* if the standard deviation divided by the mean.
:::
