...
Consider a [Poisson process](PoissonDerivation), where events occur at a rate of $\lambda$ events per unit time (e.g., $\lambda = 3$ per minute).
Then, in a length of time $t$, the number of occurrences $Y$ would be $\lambda t$ (e.g., in 6 minutes, we'd expect $3\times 6 = 18$ occurrences).
So the number of occurrences in the time $\lambda t$ would be modelled with a Poisson distribution with mean $\lambda t$, with probability function  
\begin{equation}
  p_Y(y) = \frac{\exp(-\lambda t) (\lambda t)^x}{x!},
   (\#eq:PoissonPMF2)
\end{equation}
from the Poisson probability function (Sect. \@ref(PoissonDefinition)).

Suppose we look at time $t = 0$.
Let $X$ be the *time* at which the first event occurs.
Suppose *zero* occurrences (i.e., $Y = 0$) happen in the interval $0 < X < T$. 
This is the same as saying that the *first* occurrence must happen *after* time $T$.

Since the mean rate of occurrences is $\lambda t$, using the argument above, then the probability of zero events in the time interval $\lambda t$ is  
\[
  \Pr(Y = 0)
  = \Pr(X > t) 
  ...
  = \exp(-\lambda t)
\]
using $X = 0$ in \@ref(eq:PoissonPMF2).
