...
2. rite an R code snippet to simulate 100 plots using this hurdle logic.

Answer: 1. $\Pr(X > 0) = 0.25$.2. ifelse(runif(100) < 0.75, 0, rgamma(100, 2, 1)).Section 3: Censored Models (The Boundary Mass)
...
Answer: $P(Y = 1000) = \Pr(X \geq 1000) = e^{-1000\lambda}$.
...
1. What is the theoretical probability that a month has zero fire damage?
2. Write R code to find the mean damage for months where damage was greater than zero.

Answer: 1. $\Pr(N = 0) = e^{-2} \approx 0.135$.2. mean(S[S > 0]) (using the simulation approach).
