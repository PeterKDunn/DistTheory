...
*Censored models*\index{Censored models} (Sect.\ \@ref(CensoredModels)) assumes a single latent\index{Latent variable} (underlying), continuous variable, that is only measurable when it exceeds a threshold.
...
Compound *Poisson-gamma models*\index{Compound Poisson-gamma models} (Sect.\ \@ref(TweedieModels)) generates zeros naturally when a Poisson count of underlying events equals zero.


```{r}
MixedTable <- array( dim=c() )

colnames(MixedTable) <- c("Censored",
                          "Hurdle",
                          "Poisson-Gamma")
...
rownames(MixedTable) <- c("Zeros",
                          ...
                          "Flexibuiity",
                          "Uses")
MixedTable[1, ] <- c("True value unobservable",
                     "Different process",
                     "No events occurred")
MixedTable[2, ] <- c("One model",
                     "Two separate models",
                     "One unified model")
MixedTable[3, ] <- c("Mechanistic",
                     "Very natural",
                     "Requires physical story")
MixedTable[4, ] <- c("Less",
                     "High",
                     "Moderate")
MixedTable[5, ] <- c("Detection limits", 
                     "Healthcare spend",
                     "Insurance, rainfall")
...
In hurdle models, zeros come only from the binary component of the part model.
Zero-inflated distributions mixes a point mass at zero with a distribution that also has support at zero.
This implies two different types of zero (structural vs. sampling). 
:::
...
Censored model assume a distribution for\ $Y$, such as a normal (or other continuous) distribution, over its full range, but the value cannot always be observed exactly.
Commonly, the value of the random variable is only observed when it exceeds some threshold (typically zero). 
Below that threshold, the observed value is recorded as zero.
The zero is recorded because the underlying process did not generate a value large enough to be observed. 
For example, any rainfall less than (say) 0.01cm cannot be recorded accurately, anf is recorded as zero.

Censoring can occur at the lower end of the scale (left censoring, where values below a threshold are unobserved) or the upper end (right censoring, where values above a threshold are unobserved). Right censoring is common in survival analysis; left censoring arises naturally when instruments cannot record below a detection limit.


:::{.definition #Censoring name="Censoring"}
Here's a formal definition that covers both:

A random variable\ $Y$ is censored if its exact value is only observed when it falls within some interval $[L, U]$; otherwise, only the bound that was exceeded is recorded. 

*Left censoring* occurs when there is a finite lower threshold\ $L$ and no upper censoring: values below\ $L$ are unobserved and recorded as\ $L$.
...
*Right censoring* occurs when there is a finite upper threshold\ $U$ and no lower censoring: values above\ $U$ are unobserved and recorded as\ $U$.
...
A Tobit model assumes normal distribution, censored at zero.

The Tobit model is a natural single-process model: zeros are not generated using a separate process, but are simply the left-censored tail of the underlying distribution. 


---
The hurdle model, sometimes called a two-part model, treats zeros and positive values as arising from two entirely separate processes. The first part is a binary model — typically logistic regression — that determines whether an observation clears the "hurdle" into positive territory at all. The second part is a truncated continuous model, such as a log-normal or gamma regression, that describes the distribution of values conditional on being positive. The two parts are estimated independently, which is both a practical convenience and a conceptual feature: different predictors, or the same predictors with different coefficients, can govern the probability of a non-zero outcome and the magnitude of that outcome given it is non-zero. This makes the hurdle model particularly attractive when you have substantive reasons to believe the zero-generating process is distinct from the process driving variation among positive values — for example, whether a household buys alcohol at all versus how much they spend if they do.
---

The Tweedie / Compound Poisson-Gamma Model
The Tweedie model generates the point mass at zero through a physically motivated compound process. Imagine that events arrive according to a Poisson process, and each event contributes an independent gamma-distributed amount to the total. The observed Y is the sum of all contributions. When the Poisson count is zero — no events occur — the total is exactly zero, naturally producing the point mass without any separate modelling of zeros. When at least one event occurs, the sum of gamma random variables gives a continuous positive value. The resulting marginal distribution belongs to the Tweedie family, with a variance-mean relationship of the form Var(Y) = φμ^p for power parameter p ∈ (1, 2). This is a unified single-distribution model, making it elegant and computationally convenient — it fits within the framework of generalised linear models — but it requires the compound Poisson-gamma story to be a plausible description of the data-generating process. Insurance claim totals and rainfall accumulation are natural fits; for data where that physical story does not apply, the model may be harder to justify interpretively.
