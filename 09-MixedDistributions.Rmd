...
The hurdle model is useful when the zero-generating process is believed to be distinct from the process driving variation among positive values; for example, whether a household buys alcohol at all is a different process tha how much a household spends if they do buy alcohol.
...
The model assumes that events arrive according to a Poisson process, and each event contributes an independent gamma-distributed amount to the total.
...
Insurance claim totals and rainfall accumulation are natural fits; for data where that physical story does not apply, the model may be harder to justify interpretively.

:::{.example}
EXAMPLE WHERE THE NARRATIVE FITS
:::
