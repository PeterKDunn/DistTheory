...
Then the *expectation* or *expected value* of $\operatorname{E}[u(X, Y)]$ is
...
The *conditional expected value* or *conditional mean* of a random variable\ $X$ for given $Y = y$ is denoted by $\operatorname{E}[X \mid Y = y]$ and is defined as
\begin{align*}
...
The *conditional variance* of a random variable\ $X$ for given $Y = y$ is denoted by $\operatorname{var}[X \mid Y = y]$ and is defined as
\begin{align*}
...
$\operatorname{var}[X \mid Y = y]$ is typically denoted $\sigma^2_{X \mid Y = y}$.
...
\begin{align*}
     ...
     &= \operatorname{E}\left[ \sum_i a^2_i(X_i - \mu_i)^2 + 2\sum\sum_{i < j}a_i a_j(X_i - \mu_i)X_j - \mu_j)\right]\\
     &= \sum_i a^2_i\operatorname{E}[X_i - \mu_i]^2 + 2{\sum\sum}_{i < j}a_i a_j\operatorname{E}[X_i - \mu_i] (X_j - \mu_j)\\
        %\quad \text{using Theorem\ \@ref(thm:ExpLinear)}\\
     &= \sum_i a^2_i\sigma^2_i + 2{\sum\sum}_{i<j}a_i a_j\operatorname{Cov}(X_i, X_j).
\end{align*}
:::
...
:::{.exercise}
Let\ $X$ and\ $Y$ be the body mass index (BMI) and percentage body fat for netball playerers attending the AIS. 
...
1. the expected percentage body fat of a netball playerer who has a BMI of\ $19$.
:::
...
In Sect.\ \@ref(NegativeBinomialDistribution), different forms of the negative binomial distribution were given: one for\ $Y$, the number of trials until\ $r$ successes were observed, and one for\ $X$, the number of failures until\ $r$ successes were observed.

1. Use that $Y = X + r$ to derive the mean and variance of\ $X$, given that $\operatorname{E}[Y] = r/p$ and $\operatorname{var}[Y] = r(1 - p)/p^2$.
