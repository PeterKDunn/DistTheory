...
The *moment-generating integral* of the normal is
\begin{equation}
  \int_{-\infty}^{\infty}
...
For $\lambda > 0$ and $n = 0, 1, 2, \ldots$:
\begin{equation}
...
The *inversion formula* for recovering the PDF from the characteristic function $\varphi_X(t) = \operatorname{E}[\exp(itX)]$\index{Characteristic function} is
\begin{equation}
  f_X(x)
...
The *Dirichlet integral*, a multivariate generalisation of the beta function, is
\begin{equation}
...
The *order-statistic integral* gives the joint density of the $r$-th and $s$-th order statistics ($r < s$) from a sample of size $n$:
\begin{equation}
...
Whn you start **R**, you are presented with a *prompt* asking you enter an instruction:
