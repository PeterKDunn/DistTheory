...
The time to complete each part of the journey, in minutes, is modelled as follows (NEEDS FIXING!):

* The time to travel Leg\ 1: $T_1 \sim N(\mu = 2, \sigma^2 = 0.3)$.
* The time spent at the traffic lights: $T_{TL} \sim \text{CPG}(\lambda = 3/60, \alpha = 2/60, \beta = 1/60)$ since the traffic light may be green;
* The time to travel Leg\ 2: $T_1 \sim N(mu = 2, \sigma^2 = 0.2)$.
* The time spent at the Stop sign: $T_1 \sim \text{Gamma}(\alpha = 0,7, \beta = 0.3)$.
* The time to travel Leg\ 3: $T_1 \sim N(\mu = 3, \sigma = 0.5)$.
* The time spent waiting to turn right: $T_1 \sim \text{CPG}(\lambda = 8/60, \alpha = 3/60, \beta = 10/60)$.
...
2. Complete $1\,000$ simulations, and hence find an approximation to the density function describing the commute time from Home to Work.
3. From the simulation, estimate $\operatorname{E}(X)$ and $\operatorname{var}(X)$.
:::

```{r echo=FALSE, WorkToHome, fig.width=6, out.width='100%', fig.align="center", fig.cap="Thejourney from Home (H) to Work (W). TL represent traffic lights; SS represents a Stop Sign; TR represents a right turn."}
plot( x = c(-1, 11)* 3,
      y = c(-1, 5),
      asp = 1,
      axes = FALSE,
