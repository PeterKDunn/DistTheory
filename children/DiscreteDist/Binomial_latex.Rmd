
Fig. \@ref(fig:BinomialExamples) shows the probability function for the binomial distribution for various parameter values.


```{r BinomialExamples, echo=FALSE, fig.align="center", fig.cap="The probability function for the binomial distribution for various values of\\ $p$ and\\ $n$.", fig.height = 7, fig.width=7, out.width='95%'}
par( mfrow = c(2, 2))

###  Set up  ###
n <- c(8, 8, 
       15, 15)
prob <- c(0.2, 0.6, 
          0.2, 0.6)


###  Plots  ###
for (i in (1:4)) {
  ni <- n[i]
  probi <- prob[i]
  x <-  0:ni
  y <- dbinom(x, 
              size = ni,
              prob = probi)
  
   plot( x = x,
         y = y,
         xlab = expression(italic(x)),
         ylab = "Probability function",
         main = bquote("Binomial: " ~ italic(n) == .(ni) ~ " and " ~ italic(p) == .(probi)),
         las = 1,
         lty = 3,
         lwd = 2,
         col = "grey",
         type = "h")
  points(x = x,
         y = y,
         pch = 19)
}
```
