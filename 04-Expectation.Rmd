...
$$
For a *mixed* random variable\ $X$ with pdf $f_X(x)$, the expected value is
$$
   \text{E}[X] = 
     \sum_{x_i} x_i \, p_X(x_i) + \int_{-\infty}^\infty x \, f_X(x) \, dx,
$$
where $p_X(x)$ is the probability mass function (pmf) over discrete points\ $x_i$, and $f_X(x)$ is the probability density function (pdf) over the continuous part with pdf $f_X(x)$).
:::
...
The mean is a special case of 'moment about the origin', and the variance is a special case of a 'central moment'.
...
Instead of central moments themselves, using expressions unaffected by a linear transformation of the type $Y = aX + b$ (for constants\ $a$ and\ $b$) are convenient for assessing skewness.
...
In 1986, Moors gave an interpretation of kurtosis.
Let $Z = (X - \mu)/\sigma$, where\ $X$ is a random variable, $\mu$ is the mean and $\sigma$ is the standard deviation.

Now by definition of the kurtosis $\kappa$ and using that $\left[V^{2}\right]=\operatorname {var}[V] + \operatorname {E}[V]^{2}$,
\begin{align*}
  \kappa 
  &= \operatorname{E}[Z^4]\\
  &= \operatorname{var}[Z^2] + \operatorname{E}[Z^2]^2\\
  &= \operatorname{var}[Z^2] + \operatorname{var}[Z]^2\\
  &= \operatorname{var}[Z^2] + 1
\end{align*}
????

The kurtosis can now be seen as a measure of the dispersion of $Z^2$ around its expectation.
Alternatively it can be seen to be a measure of the dispersion of\ $Z$ around $+1$ and $-1$.
$\kappa$ attains its minimal value in a symmetric two-point distribution. 
In terms of the original variable\ $X$, the kurtosis is a measure of the dispersion of\ $X$ around the two values $\mu\pm\sigma$.
...
REPEATED?
A negative value for the excess kurtosis means that the tails are 'heavy', while small values for the excess kurtosis means that the tails are 'light'.
...
```{r KurtosisPlots, fig.cap="Kurtosis"}
par( mfrow = c(2, 2))
...
Any of the six distributions in Fig.\ \@ref(fig:FiveDistributions) meet this criterion (based on the first moment), so this information is not sufficient to uniquely define a distribution.

So, suppose add a extra criterion: in addition, we require $\text{var}[X] = 1$.
Any of the first five distributions in Fig.\ \@ref(fig:FiveDistributions) meet these two criteria (based on the first two moments), so again this information is not sufficient to uniquely define a distribution.

Suppose a third criterion is added: the distribution must be symmetric.
Any of the first four distributions in Fig.\ \@ref(fig:FiveDistributions) meet these three criteria (based on the first three moments); again, this information is not sufficient to uniquely define a distribution.

In general, *all* the moments of a distribution are needed to uniquely define a distribution.
All the moments of a distribution can be succinctly captured in a *moment generating function* (or MGF).


```{r FiveDistributions, echo=FALSE, fig.height=8, fig.width=5, out.width='80%', fig.align="center", fig.cap="Six distributions, all with mean 1 and variance 1. The first four distributions are all symmetric."}
par(mfrow = c(3, 2))
...
# Plot setup
plot(x, d1, type = "l", col = "blue", lwd = 2,
     xlab = "x", 
     ylab = "Density",
     axes = FALSE,
...
plot(x, d2, type = "l", col = "red", lwd = 2,
     xlab = "x", 
     ylab = "Density",
     axes = FALSE,
...
plot(x, d3, type = "l", col = "forestgreen", lwd = 2,
     xlab = "x", 
     ylab = "Density",
     axes = FALSE,
...
plot(y, d8, type = "l", lwd = 2, col = "darkred",
     ylim = c(0, 2),
     axes = FALSE,
...
plot(x, d4, type = "l", col = "purple", lwd = 2,
     xlab = "x", 
     ylab = "Density",
     axes = FALSE,
...
plot(x, d5, type = "l", col = "orange", lwd = 2,
     xlab = "x", 
     ylab = "Density",
     axes = FALSE,
...
## Other measures of location {#FeaturesLocation}
...
The most important measure of location is the mean $\mu = \text{E}(X)$, which has already been defined (Sect.\ \@ref(def:Expectation)).
...
If $X$ is a discrete random variable with probability function defined in Table\ \@ref(tab:ProbDeg1r) (and shown in Fig.\ \@ref(fig:DiscreteMedian)), the distribution is bimodal (i.e., has two modes), with modes at $x = 3$ and $x = 4$.
...
eg1Table[1, ] <- 0:7
eg1Table[2, ] <- c("$\\frac{1}{128}$", "$\\frac{7}{128}$", "$\\frac{21}{128}$", "$\\frac{35}{128}$", "$\\frac{35}{128}$", "$\\frac{21}{128}$", "$\\frac{7}{128}$", "$\\frac{1}{128}$")
...
The *median* is $\nu = 3$ since this is the only value of $X$ that will satisfy Def.\ \@ref(def:Median).
:::
...
eg2Table[1, ] <- 0:6
eg2Table[2, ] <- c("$\\frac{1}{64}$", "$\\frac{6}{64}$", "$\\frac{15}{64}$", "$\\frac{20}{64}$", "$\\frac{15}{64}$", "$\\frac{6}{64}$", "$\\frac{1}{64}$")
...
## Other measures of variability  {#Dispersion}
