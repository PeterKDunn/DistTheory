...
\operatorname{var}[X]
   ...
   &= \operatorname{E}[X^2] - \big(\operatorname{E}[X]\big)^2.
\end{align*}
:::
...
The expected value of $X$ is
$$
   \operatorname{E}[X] = \int_0^2 x.3x(2 - x)4\, dx = 1.
$$
To use the computational formula, also find
$$
   \operatorname{E}[X^2] = \frac{6}{5},
$$
and so $\operatorname{var}[X] = \operatorname{E}[X^2] - \big(\operatorname{E}[X]\big)^2 = 1/5$.
...
The *$r$th moment about the origin*, or *$r$th raw moment*, of a random variable\ $X$ (where\ $r$ is a positive integer) is denoted\ $\mu'_r$ and defined as $\mu'_r = \operatorname{E}[X^r]$.
...
Likewise, 
\begin{align*}
   \mu_2 &= \operatorname{E}\big((y - 6.3)^2 p_Y(y) \big) = 0.1982;\quad{\text{and}}
   \mu_3 &= \operatorname{E}\big((y - 6.3)^3 p_Y(y) \big) = 0.02457.
\end{align*}
...
So, suppose add a extra criterion: in addition, we require $\operatorname{var}[X] = 1$.
...
Then $\mu = \operatorname{E}[X] = 7/2$ and thus the MAD of\ $X$ is  
\begin{align*}
   \operatorname{E}[|X - \mu|] 
   &= \sum |x - \mu| \Pr(X = x)\\
...
2. Find $\operatorname{E}\big[(X + 3)^2\big]$ using the previous answer.
