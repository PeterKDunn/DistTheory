...
out <- barplot(table(min_Roll)/num_Sims,
               xlim = c(0, 7),
               ylim = c(min_Y, 0.95),
               main = expression( atop(The~distribution~of~the~minimum,
...
out <- barplot(table(Order_Stats_7)/num_Sims,
               xlim = c(0, 7),
               ylim = c(min_Y, 0.45),
               main = expression( atop(The~distribution~of~the~seventh,
...
### Exercise 5.1: The Weakest Link (Material Science)
...
3. Use R to find the probability the cable breaks under a load of 10kg.

* **Answer:** 1. $X_{(1)}$ (the minimum).
    2. $X_{(1)} \sim \text{Exp}(n\lambda) = \text{Exp}(5/100)$.
    3. `pexp(10, rate = 5/100)` $\approx 0.393$.



### Exercise 5.2: Flood Protection (Hydrology)
...
2. Calculate the probability that the 10-year maximum exceeds 14 feet.
3. In R, simulate 1,000 "decades" and find the empirical median of $X_{(10)}$.

* **Answer:** 1. $F_{X_{(10)}}(x) = [F_X(x)]^{10} = (\frac{x}{15})^{10}$ for $0 < x < 15$.
    2. $1 - \Pr(X_{(10)} \leq 14) = 1 - (\frac{14}{15})^{10} \approx 0.496$.
    3. `max_levels <- replicate(1000, max(runif(10, 0, 15))); median(max_levels)`

---

### Exercise 5.3: DNA Sequencing (Bioinformatics)
A sequencer reads a DNA fragment $n$ times to ensure accuracy. 
Let $X_i$ be the quality score of the $i$-th read. 
To filter out noise, the lab uses the **median** score of 3 reads ($X_{(2)}$). 
...
* **Answer:** 1. $f_{X_{(k)}}(x) = \frac{n!}{(k-1)!(n-k)!} [F(x)]^{k-1} [1-F(x)]^{n-k} f(x)$.
...
y <- 6 * (100*x - x^2) / 100^3
plot(x, y, type="l", main="Density of Median (n=3) from U(0,100)")
```
...
2. Calculate the probability of success using R's pnorm and the binomial logic (or order statistic CDF).

Answer: 

1. The event is $\{X_{(2)} \leq 80\}$.
2. In R: p_single <- pnorm(80, 75, 5); pbinom(1, 3, p_single, lower.tail=FALSE) + dbinom(3, 3, p_single) (or more simply p_order_2 <- pbeta(p_single, 2, 3-2+1)).
