...
* **Proper subset**:    
  $A$ is a *proper subset* of\ $B$, written $A\subset B$, if $a\subseteq$ and $A\ne B$.
...
```{r IntersectionUnionSubset, echo=FALSE, out.width='80%', fig.height = 8, fig.width=5.5, fig.align="center", fig.cap="Intersection, union, proper subset, complement and set differences. The rectangle represents the universal set, $S$."}
...
::: {.example #DefiningSets4 name="Sets"}
We could define $\mathbb{N}_0 = \{ 0 \} \cup \mathbb{N}$.
:::
...
1. find the set of US states who names end in `a`, **or** start with `a`.
1. find the set of US states who names end in `a`, **and** start with `A`.
1. find the set of US states who names end in `a`, **and** do not start with `A`.
:::
...
:::
(That is, all real numbers strictly between -2 and 2
In words: A  contains all real numbers whose square is less than 4.

Cardinality:
A is an uncountably infinite set (like all open intervals of real numbers).)
)


:::{.exercise #SetBuildeReading2}
...
(B is integers: $B\in\mathbb{Z}$. Countably infinite cardinality: $|B| = |\mathbb{Z}| = \aleph_0$)
