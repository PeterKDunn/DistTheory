...
As above, write $a_1\in A$ to denote that element\ $a_1$, belongs to a set\ $A$.
...
The elements of the set can be defined this way, or by giving the pattern:
$$
...
A set is called *uncountably infinite* if the elements of the set cannot be counted; the elements of the set cannot all be listed in a sequence like $a_1$, $a_2$, $a_3$, $\dots$ without missing some elements.
:::
...
Sets that are uncountably infinite can often can be concisely defined using a rule, using *set-builder notation*.
...
\end{align*}
In this notation, $x$ is a variable representing the elements of the set\ $A$; the colon or vertical bar is read as 'such that', and is followed by the condition for $x$ to belong to the set\ $A$.
...
Using the sets in Example\ \@ref(exm:RelationshipBetweenSets2), $|U| = 10$, $|B| = |E| = 4$.
:::
