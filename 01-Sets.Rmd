...
# Essentials of set theory {#SetTheory}


::: {.objectivesBox .objectives data-latex="{iconmonstr-target-4-240.png}"}
Upon completion of this module you should be able to:

* define sets and describe the elements of sets.
...
A *set* is a well-defined, unordered collection of distinct *elements*, usually denoted using a capital letter: $A$.
:::

The use of 'distinct' means that no elements is repeated; notice that the order of the elements in the set is not relevant.
The phrase 'well-defined' means that is must be clear whether any given object is in the set ('belongs to the set') or is *not* in the set ('does not belong to the set').

The *elements* can be interpreted widely, and can refer to letters, digits, names, standard mathematical operations, numbers, words, animals, people, machine components, plants... or even other sets.
...
These two sets are all equivalent, since the order of the elements does not matter:
\begin{align*}
...
$$
  W = \{ M, E\} = \{ (\text{Echidnas}, \text{Playtpuses}), (1, 3, 5, 7, 9)\}.
$$
...
## Describing the elements of sets {#DescribingElementsOfSets}
...
Whichever method is used, an object must be clearly in a set or not in a set.
...
## Cardinality of sets {#Cardinality}

*Cardinality* refers to the number of elements of a set.
The cardinality of a set can be:
...
In sets with a finite or countably infinite number of elements, a single element of the set is called an *element* or a *sample point*.
...
Define set $A$ as: $A = \left\{\right.$*Countries whose names begin and end with the letter 'A'*$\left.\right\}$.
...
::: {.example #CardinalityOfSets name="Cardinality of sets"}
...
$$
The cardinality of set\ $B$ is *finite*.
...
If the number of elements in a set can be counted, the set is called a finite set.
In other words, a set is finite if it has exactly\ $n$ distinct elements for some non-negative integer\ $n$.
If $n = 0$ (that is, the set contains no elements), the set is the *empty set*.
...
The elements of the set can be described as above, or by giving the pattern:
$$
...
Sets can also be *countably infinite*.
A set is countably infinite if its distinct elements can be counted or listed in principle, but there are an infinite number of elements.
...
$$
  N = \{1, 2, 3, \dots\}.
$$
In other words, the elements can be listed or enumerated in an infinite sequence without missing any.
:::
...
$$
  S = \{1, 2, 3, 4, \dots\}
$$
The cardinality of set\ $S$ is *countably infinite*.
:::
...
$$
  O = \{1, 3, 5, 7, 9, \dots\}
$$
The cardinality of set\ $O$ is *countably infinite*.
:::


### Uncountably infinite sets {#InfiniteSets}

Some sets cannot be described by listing the elements, even using an infinite sequence.
These are called *uncountably infinite* sets.
...
A set is called *uncountably infinite* if the elements of the set cannot be counted.
?????
:::


Sets with uncountably infinite cardinality can often can be concisely defined using a rule, using *set-builder notation*.
(Set-builder notation can be used for sets of any cardinality, but are very useful for uncountably infinite sets.)

Set-builder notation usually has the format:
\begin{align*}
  A &= \{ x \mid \text{some condition on $x$ to belong to the set}\}, \text{or}\\
  A &= \{ x : \text{some condition on $x$ to belong to the set}\}.
\end{align*}
In this notation, $x$ is a variable representing the elements of the set\ $A$; the colon or vertical bar is read as 'such that', and is followed by the condition for $x$ to belong to the set.


:::{.example #SetBuilderNotation name="Set-builder notation"}
Consider the set defined by
$$
   F = \{s \mid \text{names of students whose mark is less than\ $50$\%}\}.
$$

This is read as '$F$ is the set of all $s$ such that the elements\ $s$ are the names of all students with a mark less than $50$%'.

Consider the set defined by
$$
   B = \{x : \text{$x$ is an animal species with two legs}\}
$$
This is read as '$B$ is the set of all\ $x$ such that the elements\ $x$ are all animal species with two legs'.

Consider the set defined by
$$
    V = \{x \mid \text{$x$ is a vowel}\}.
$$
This is read as '$V$ is the set of all\ $x$ such that the elements\ $x$ are all vowels'.

Consider the set defined by
$$
   P = \{y : \text{$y$ is a two-digit integer}\}.
$$
This is read as '$P$ is the set of elements\ $y$, such that the elements\ $y$ are all two-digit integers'.
:::


:::{.example #UncountablyInfiniteSets name="Uncountably infinite sets"}
Define $C$ as the set of 'distances travelled by cars in Europe in one year'.
The cardinality of $C$ is *uncountably infinite*, and can be defined as
$$
  C = \{c \mid c \ge 0 \}.
$$
:::
...
The set $D$ of possible distances is $D = \{ d \mid d \ge 0 \}$.
...
\begin{align*}
  P &= \{(x, y) \mid \text{$0 < x < 1$ and $0 < y < 1$} \} \quad\text{or, equivalently}\\
  P &= \{(x, y) \mid (0 < x < 1)\cap(0 < y < 1) \},
\end{align*}
which defines a unit square on the Cartesian axes.
...
Notice that the *set* itself is denoted $\emptyset$, and the *number* of elements is\ $0$.
:::
...
The *universal set* is the set of all elements that could be considered in a specific context.
...
$$
  U = \{1, 2, 3, 4, 5, 6\}.
$$
Then, the set\ $B$ of rolls greater than\ $4$ would contain the elements
$$
  B = \{5, 6\}.
$$
...
$$
  C = \emptyset.
$$
Notice that we do not write $C = \{\emptyset\}$; the symbol $\emptyset$ itself represents a set (with no elements).
:::
...
* The **intersection** of sets $A$ and $B$, written as $A\cap B$, is the set of elements in *both* $A$ and $B$.
  We say $A$ **and** $B$.
* The **union** of sets $A$ and $B$, written as $A\cup B$, is the combined set of all the elements in either $A$ or $B$, or in both. 
  Usually we just say that $A\cup B$ comprises the points in $A$ **or** $B$.
* $A$ is a **proper subset** of $B$, written as $A\subset B$, if all the elements of $A$ are in $B$. (Set $B$ may have other elements that are not in $A$.)
...
If a universal set\ $S$ is defined, then

* The **complement** of $A$, written $\overline{A}$, is the set of elements in $S$ but *not* in $A$.
:::

Two sets whose intersection is the empty set (i.e., if $A\cap B = \emptyset$ for two sets\ $A$ and\ $B$) the sets are called *disjoint sets*.
...
```{r IntersectionUnionSubset, echo=FALSE, out.width='75%', fig.height = 6, fig.align="center", fig.cap="Intersection, union, proper subset, and complement. The rectangle represents the universal set, $S$."}
...
## Special sets of numbers {#SpecialSetsOfNumbers}
...
We could write $\mathbb{Q} = \{m\div n \mid m, n \in \text{$\mathbb{Z}$ and $n\ne 0$}\}$.
...
$$
  A = \{ x \mid \sqrt{x^2 - 4} \in \mathbb{R}\}.
$$
This means that the set\ $A$ comprises the elements\ $x$ such that the values of $\sqrt{x^2 - 4}$ are real numbers; that is, such that $x^2 - 4 \ge 0$.
This means that
$$
  A = \{ (x \le -2) \cup (x \ge 2)\};
$$
...
Countably infinite sets and uncountably infinite sets both have *infinite* cardinalitry.
...
* if $A$ has countably infinite cardinality then $|A| = \aleph_0$, where $\aleph_0$ ('aleph-null') is the smallest infinite cardinality.
* if $A$ has uncountably infinite cardinality then $|A| = \mathfrak{c}$ where $\mathfrak{c}$ (the 'cardinality of the continuum') is a larger cardinality that $\aleph_0$.
...
We introduce **R** by showing how to operate with sets.
Greater functionality is provided by installing and using the **R** package `sets`.
...
```{r}
DieRolls <- 1:6
RollsEven <- seq(2, 6, by = 2)
RollsDivisibleBy3 <- c(3, 6)
RollsEven
RollsDivisibleBy3
```
...
```{r}
setdiff(RollsEven, RollsDivisibleBy3)
setdiff(RollsDivisibleBy3, RollsEven)
```
...
\begin{align*}
  ...
  P &= \{ \text{Jack}, \text{Queen}, \text{King}, \text{Ace}\}\enskip \text{(i.e., the picture cards)},
\end{align*}
