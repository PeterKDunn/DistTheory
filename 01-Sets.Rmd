...
Theoretically, this is not true, to avoid difficults like that in *Russell's paradox'.
...
Since the elements of uncountably infinite sets can be listed---even with a pattern---the elements of uncountably infinite sets are often concisely defined using *set-builder notation*.
...
\begin{align*}
  P &= \{(x, y)\in \mathbb{R}\times\mathbb{R} \mid \text{$0 < x < 1$ and $0 < y < 1$} \} \quad\text{or, equivalently}\\
...
* **Subset**:    
  $A$ is a *subset* of\ $B$, written $A\subseteq B$, if all the elements of $A$ are in $B$. 
  (Set $B$ may have other elements that are not in $A$.)
  This definition requires that $A$ has *fewer*, or the *same number* of, elements than $B$.

* **Proper subset**:    
  $A$ is a *proper subset* of\ $B$, written $A\subset B$, if all the elements of $A$ are in $B$.
  (Set $B$ may have other elements that are not in $A$.)
...
```{r IntersectionUnionSubset, echo=FALSE, out.width='75%', fig.height = 6, fig.align="center", fig.cap="Intersection, union, proper subset, and complement. The rectangle represents the universal set, $S$. ADD THE SET DIFFERENCE anf FIX COMPLEMENT NOTATION"}
...
\begin{align*}
   ...
   B \setminus E &= \{7, 8, 9\};            &          E\setminus U &= \emptyset.
\end{align*}
:::
...
- define what these sets mean (when written in set buildr notations)
- write these sets using set buider notation

$B=\{x\in\mathbb{R} | \lfloor x \rfloor = x\}$ where (define floor function)

$A = \{ x\in\mathbb{R} | x^2 < 4\}$
$A = \{ x\in\mathbb{Z} | \text{$x$ is prime}\}$
$A = \{ x\in\mathbb{R} | x \ge 2\}$
