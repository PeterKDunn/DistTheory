...
$$
   p_X(x) = \begin{cases}
            ...
            \end{cases}
$$
To find the probability function of $Y = X^2$, first note that $Y = X^2$ is *not* a one-to-one transformation in general, but is over the range of\ $X$ (i.e., for $x = 0, 1, 2, 3$).
...
### Bivariate dscrete case {#ChangeOfVarDiscreteBivariate}
...
$$
  J = \det 
        \begin{bmatrix} 
          \displaystyle \frac{\partial x_1}{\partial y_1} & \displaystyle \frac{\partial x_1}{\partial y_2} \\ 
...
Let\ $X_1$ and\ $X_2$ be indepdent random variables with PDF
$$
...
Since the random variable\ $X_1$ and\ $X_2$ are independent, the joint PDF of is
$$
  f_{X_1, X_2}(x_1, x_2) = \exp(-x_1) \cdot \exp(-x_2) = \exp\left(-(x_1 + x_2)\right) \quad \text{for $x_1, x_2 > 0$}.
$$
Inverting the transformations, we find 
\begin{align*}
  y_1 &= x_1 + x_2\quad\text{and}\\
  y_2 &= x_1 / x_2\text{ so that } x_1 = y_2 x_2.
\end{align*}
Substituting these into the joint PDF for $(X_1, X_2)$ gives 
$$
  y_1 = y_2 x_2 + x_2 = x_2(1 + y_2).
$$
...
$$
\begin{array}{ll}
  \displaystyle \frac{\partial x_1}{\partial y_1} = \frac{y_2}{1+y_2};\quad & \quad\displaystyle \frac{\partial x_1}{\partial y_2} = \frac{y_1(1+y_2) - y_1 y_2}{(1+y_2)^2} = \frac{y_1}{(1+y_2)^2};\\
...
In every case: find the pre-image, then account for how the transformation redistributes probability, through counting in the discrete case, and through the Jacobian in the continuous case.


Distribution Function (CDF) Method
This is your default fallback; it always works in principle. 
You write $F_Y(y) = P(g(X)\le y)F_Y(y) = P(g(X) \leq y) = F_Y(y) = P(g(X)\le y)$, manipulate the inequality, and differentiate. 
Reach for it when the transformation is awkward or non-monotone in a way that makes the CoV Jacobian approach messy, when you only need the marginal distribution of one variable out of a multivariate setup, or when the support of the transformed variable has tricky boundaries that depend on yy
y. 
The downside is that differentiation can get ugly and it gives you less structural insight than the other methods.


## The distribution function method {#DistributonFunctionMethod}

The distribution function (CDF) method is the most broadly applicable technique for
finding the distribution of a transformed random variable. The strategy is always the
same: express the CDF of the new variable directly in terms of probabilities involving
the original variable(s), simplify by inverting inequalities, then---in the continuous
case---differentiate to recover the pdf.

Because it operates at the level of probabilities rather than densities, it handles
non-monotone transformations, awkward supports, and many-to-one mappings without
requiring the special conditions needed by the change of variables approach.
...
4. Differentiate (continuous case) to get the pdf: $f_Y(y) = F_Y'(y)$.
...
Let\ $X$ be a discrete random variable with pmf $p_X(x)$ and CDF $F_X(x) = P(X \leq x)$.
For $Y = g(X)$, the CDF of $Y$ is
$$
  F_Y(y) = P(Y \leq y) = P(g(X) \leq y) = \sum_{\{x:\, g(x) \leq y\}} p_X(x).
$$
The pmf of\ $Y$ is then recovered as
$$
  p_Y(y) = F_Y(y) - F_Y(y^-) = P(Y \leq y) - P(Y \leq y - 1)
$$
where\ $y^-$ denotes the largest value in the support of\ $Y$ strictly less than\ $y$.

In the discrete case the CDF method and the change of variables (pre-image) method are closely related ---the difference is mainly one of presentation. 
...
:::{.example name="Geometric Distribution and Its Shift"}
...
$$
  F_Y(y) = P(Y \leq y) = P(X - 1 \leq y) = P(X \leq y + 1) = 1 - (1-p)^{y+1}.
$$
The pmf is
$$
...
The CDF method is more flexible because it handles non-monotone $g$ without partitioning.


:::{.example name="Distribution of ..."}
Let $X \sim N(0, 1)$ and $Y = X^2$. 
Since $g(x) = x^2$ is not monotone on $\mathbb{R}$, the CoV method requires partitioning; the CDF method is cleaner.
...
$$
This is the $\chi^2(1)$ distribution, i.e.\ $\text{Gamma}(1/2,\, 1/2)$.
:::
...
To find the distribution of $Y = g(X_1, X_2)$ (a scalar function of the pair), compute
$$
  F_Y(y) = P(g(X_1, X_2) \leq y) = \iint_{\{(x_1,x_2):\, g(x_1,x_2) \leq y\}} f_{\mathbf{X}}(x_1, x_2)\, dx_1\, dx_2,
$$
...
:::{.example name="Example: Distribution of $M = \max(X_1, X_2)$ for Independent Uniforms"}
Let $X_1, X_2 \overset{\text{iid}}{\sim} \text{Uniform}(0,1)$ with joint pdf $f_{\mathbf{X}}(x_1, x_2) = 1$ on the unit square. Define $M = \max(X_1, X_2)$.

**CDF:** The event $\{M \leq m\}$ is equivalent to $\{X_1 \leq m \text{ and } X_2 \leq m\}$,
so for $m \in [0,1]$:

$$F_M(m) = P(X_1 \leq m,\, X_2 \leq m) = m \cdot m = m^2.$$

**PDF:** Differentiating,

$$f_M(m) = 2m, \quad m \in [0, 1].$$

This is the $\text{Beta}(2, 1)$ distribution. More generally, for $n$ iid
$\text{Uniform}(0,1)$ random variables, $M = \max(X_1,\ldots,X_n)$ has CDF $m^n$
and pdf $nm^{n-1}$---a $\text{Beta}(n, 1)$ distribution. This extends naturally to
order statistics.
:::
...
Let $(X_1, X_2)$ have joint pmf $p_{\mathbf{X}}(x_1, x_2)$. For $Y = g(X_1, X_2)$, the CDF of\ $Y$ is
$$
  F_Y(y) = P(Y \leq y) = \sum_{\{(x_1,x_2):\, g(x_1,x_2) \leq y\}} p_{\mathbf{X}}(x_1, x_2),
$$
and the pmf is recovered as $p_Y(y) = F_Y(y) - F_Y(y^-)$.

The bivariate discrete CDF method is often the most natural approach when\ $g()$ is a many-to-one function (such as a maximum or minimum) where enumerating pre-images directly would be cumbersome, but the event $\{g(X_1, X_2) \leq y\}$ has a clean geometric or combinatorial description.
...
$$
  F_M(m) = P(X_1 \leq m,\, X_2 \leq m) = F_{X_1}(m) \cdot F_{X_2}(m)
$$
by independence. 
The pmf is then
$$
...
| Setting | Strategy | Differentiate? |
|---|---|---|
| **Discrete univariate** | $F_Y(y) = \sum_{\{x:\,g(x)\leq y\}} p_X(x)$; pmf = $\Delta F_Y$ | No--- difference the CDF |
| **Continuous univariate** | $F_Y(y) = P(g(X)\leq y)$; invert inequality; use $F_X$ | Yes---differentiate |
| **Bivariate continuous** | $F_Y(y) = \iint_{\{g\leq y\}} f_\mathbf{X}$; characterise region | Yes---differentiate |
| **Bivariate discrete** | $F_Y(y) = \sum\sum_{\{g\leq y\}} p_\mathbf{X}$; pmf = $\Delta F_Y$ | No---difference the CDF |
The method is most valuable relative to change of variables when: (i) the transformation
is many-to-one (e.g.\ maximum, minimum, absolute value); (ii) the result is not a
standard named distribution; or (iii) only a marginal distribution is needed from a
