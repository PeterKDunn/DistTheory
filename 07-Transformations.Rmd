...
ADD PROOF OF Theorem\ \@ref{thm:ExpectationFunction}:
Theorem\ \@ref(thm:ExpectationFunction) gave an important theorem involving the expectation of function of a random variable.
Here, we prove this theorem on teh discrete and (partially for???) continuous cases.
...
Note that the event $\{Y = y\}$ is actually the set of all $x$ such that $g(x) = y$.
Substitute: $\Pr(Y = y) = \sum_{x: g(x) = y} \Pr(X = x)$.
Plug this back into the expectation:
$$
  \operatorname{E}[g(X)] = \sum_y y \left( \sum_{x: g(x) = y} \Pr(X = x) \right).
$$
Since $y = g(x)$ for all $x$ in that inner sum, we can move it inside:
$$
  \operatorname{E}[g(X)] = \sum_y \sum_{x: g(x) = y} g(x) \Pr(X = x).
$$
Summing over all $y$, and then all $x$ that map to those $y$, is just the same as summing over all possible $x$.
Result: 
$\operatorname{E}[g(X)] = \sum_x g(x) \Pr(X = x)$. $\square$


For the continuous case, we need to show that  $\int y f(g(X)=y) = \int_x g(x) f(x)$.

Assume $g(x)$ is a strictly increasing, differentiable function (this makes the proof simpler). 
Let $Y = g(X)$.
Find the PDF of $Y$ using the transformation method:
$$
  f_Y(y) = f_X(x) \left| \frac{dx}{dy} \right| \quad \text{where } x = g^{-1}(y).
$$
Use the definition of $\operatorname{E}[Y]$:
$$
