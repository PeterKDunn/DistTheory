...
:::{.example #TranasformBivariate name="Transformation (bivariate)"}
...
```{r Transformation, echo=FALSE, fig.align="center", fig.cap="A monotone increasing transformation function", fig.width=7, fig.height=4, out.width='70%'}
par( mfrow = c(1, 2))
...
A modification of Theorem \@ref(thm:7.1) is used.
...
The relationship $y = u(z) = \frac{1}{2}z^2$ is not strictly increasing or strictly decreasing in $(-\infty, \infty )$ so we can't apply Theorem \@ref(thm:7.1) directly.
...
Example \@ref(exm:Non11Xform) is repeated here using the df method.
...
\begin{align*}
     F_Y(y) 
     & = \Pr(Y\leq y) = \Pr(\frac{1}{2}Z^2\leq y) = \Pr(Z^2\leq 2y)\\
...
Examples \@ref(exm:Non11Xform) and \@ref(exm:TransformB) give rise to the chi-square distribution, which is an important model in statistical theory.
...
Comparison of Definitions \@ref(def:chisq) and \@ref(def:gamma) reveal that $X$ has a $\chi^2(\gamma)$ distribution if and only if $X$ has a gamma distribution with parameters $\beta = 2$ and $\alpha = \nu/2$. 
EXERCISE??

The basic properties of the chi-square follow directly from those of the gamma distribution.

:::{.theorm #ChisqProperties name="Properties of chi-squared distribution"}
If $X\sim\chi^2(\nu)$ then
...
The importance of the chi-square distribution is hinted at in Examples \@ref(exm:7.6) and \@ref(exm:7.6rep), which essentially prove the following theorem.
...
:::{.proof}
Exercise---see Example \@ref(exm:7.6).
:::
...
Since $Z_i \sim \chi^2(1)$, from Theorem \@ref(thm:chiprop)
\[
   M_{Z_i}(t)
   = (1 - 2t)^{-1/2}.
\]
It follows then from Theorem \@ref(thm:mgf2) that $S = \sum_{i=1}^n Z_i^2$ has mgf
\begin{align*}
   M_{S}(t)
