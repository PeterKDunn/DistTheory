...
Given random variables $X$ and $Y$ with range spaces $R_X$ and $R_Y$ respectively, the function $u$ is a *one-to-one transformation* (or mapping) if, for each $y\in R_Y$, there corresponds exactly one $x\in R_X$.
:::
...
The transformation $Y = (X - 1)^2$ is not, in general, a one-to-one transformation.
...
```{r TwoTransforms, echo=FALSE, fig.align="center", fig.cap="Two transformations: a non-one-to-one transformation (left panel), and a one-to-one transformation (right panel)", fig.width=5, out.width='80%', fig.width=7}
par(mfrow = c(1, 2))
...
\]
Then the joint pf of the new (transformed) random variables is
...
In this case, a second (dummy) transformation is used, usually a very simple tranasformation.
...
\]
$Y_2$ is a dummy transform, and is very simple.
...
\[
   ...
   \end{cases}
\]
You should recognise this as the pf of a Poisson random variable (Def. \@ref(def:PoissonDistribution)) with mean $\mu_1 + \mu_2$.
...
In the range $-\infty < z < 0$, $y$ and $z$ are monotonically related. 
...
It follows that if $X\sim N(\mu,\sigma^2)$, then the pdf of $Y = \frac{1}{2} (X - \mu )^2 / \sigma^2$ is also $\text{Gamma}(\alpha = 1/2,\beta = 1)$ since then $(X - \mu)/\sigma$ is distributed as $N(0, 1)$.
:::
...
\[
   f_X(x) = \begin{cases}
            ...
            \end{cases}
\]
To find the pdf of the random variable $Y$ where $Y = X^2$, first see that $1 < y < 9$ and the transfornation is monotonoic over this region.
...
```{r SquaringXform, echo=FALSE, fig.align="center", fig.cap="The transformation $Y = X^2$ when $X$ is defined from $1$ to $3$. The thicker line corresponds to the region where the transformation applies. Note that if $Y < y$, then $2 - \\sqrt{y - 1} < X < 2 + \\sqrt{y - 1}$.", fig.width=4.5, fig.height=4, out.width='100%'}
par( mfrow = c(1, 3))
...
The discrete bivariate random vector $(X_1, X_2)$ has the joint pf
...
1. Determine the joint pf of $(Y_1, Y_2)$.
...
:::{.exercise #Projectile}
Suppose a projectile is fired at an angle $\theta$ from the horizontal with velocity $v$. The horizontal distance that the projectile travels, $D$, is
...
1. Sketch the pdf of $D$ over a suitable range for $v = 12$ and using $g\approx 9.8$.
:::
...
The total load distribution $L$ was then modelled as $L = 0.24S + 0.76B$.
...
The random variable $Y = \log X$ is said to have a *log-normal distribution*.

1. Determine the pdf for the log-normal distribution.
...
1. Find the distribution function, $F_Y(y)$, of the random variable $Y = 4 - X^2$.
1. Hence find the pdf of $Y$, $f_Y(y)$.
:::

:::{.exercise}
If the random variable $X$ has an exponential distributed with mean 1, show that the distribution of $-\log(X)$ has a Gumbel distribution (Eq. \@ref(eq:Gumbel)) with $\mu = 0$ and $\sigma = 1$.
:::
