...
Given a rv $X$ with known distribution, and a function $u$, how do we find the probability distribution of a rv $Y = u(X)$?
...
A concept that is important in this context is that of 1:1 transformations as contained in the following definition.

:::{.definition #OneOneTransformation name="One-to-one transformation"}
Given rv's $X$ and $Y$ with range spaces $R_X,\,R_Y$ respectively, the function $u$ is said to represent a 1:1 transformation (or mapping) if to each $x\in R_X$ there corresponds exactly one $y\in R_Y$.
:::
...
\[
   p_X(x) = \begin{cases}
               {3\choose x}(0.2)^x (0.8)^{3-x} & \text{for $x=0, 1, 2, 3$}\\
               0 & \text{otherwise}
            \end{cases}
\]
...
\[
   p_Y(y) = p_X(\sqrt{y})
   = \begin{cases}
               {3\choose \sqrt{y}}(0.2)^{\sqrt{y}} (0.8)^{3-\sqrt{y}} & \text{for $y=0, 1, 4, 9$}\\
               0 & \text{otherwise}
     \end{cases}
\]
:::
...
:::{.example #Exprain name="Exponential distributions"}
Suppose $\Pr(X = x)$ is as in Example~\ref{EG:7.1} and define $Y = |X - 3|$.
...
\[ 
   \begin{array}{c|ccc}
     y & 0 & 1 & 2\\ \hline \\[-2mm]
     \Pr(Y = y) & 
     \displaystyle{\frac 15} & \displaystyle{\frac 25} &
     \displaystyle{\frac 25}
     \end{array}
\]
:::
...
:::{.example #Exprain name="Exponential distributions"}
Let the two discrete random variables $X_1$ and $X_2$ have the joint pf

\begin{center}
\begin{tabular}{c|ccc}
\botnewrule &\multicolumn{3}{c}{$x_2$}\\
\cline{2-4}
\bothnewrule $x_1$ & $0$ & $1$ & $2$ \\
\hline
\botnewrule
$-1$ & $0.3$ & $0.1$ & $0.1$ \\
$1$  & $0.2$ & $0.2$ & $0.1$ \\
...
The joint pf can then be constructed as follows:

\begin{center}
\begin{tabular}{c|ccccc}
\botnewrule &\multicolumn{5}{c}{$y_1$}\\
\cline{2-6}
\bothnewrule $y_2$ & $-1$ & $0$ & $1$ & $2$ & $3$ \\
\hline
\botnewrule
$-2$ & $0.3$ & $0.1$ & $0.1$ & $0$ & $0$ \\
$2$  & $0$   & $0$   & $0.2$ & $0.2$ & $0.1$ \\
...
\[
   p_{Y_1}(y_1) =
   \begin{cases}
      \ds{\frac{(\mu_1+\mu_2)^{y_1}\exp\bigl(-(\mu_1 + \mu_2)\bigr)}{y_1!}} & \text{for $y_1 = 0, 1, 2, \dots$}\\
      0 & \text{otherwise.}
   \end{cases}
\]
...
:::{.theorem}
If $X$ has pdf $f_X(x), \ x\in R_X$ and $u$ is a strictly increasing or decreasing function for $x\in R_X$ then the rv $Y = u(X)$ has pdf
\[
...
```{r Transformation, echo=FALSE, fig.align="center", fig.cap="A monotone increasing transformation function"}
par( mfrow = c(1, 2))
...
\]
Therefore, $\displaystyle {f_Y(y)=f_X(w(y))\frac{dx}{dy}}$, where $w(y) = u^{-1}(x)$.
...
\end{align*}
Therefore $f_Y(y) = -f_X(w(y))\ds{\frac{dx}{dy}}$.
...
```{r SqrtRt, echo=FALSE, fig.align="center", fig.cap="The square-root transformation"}
x <- seq(0, 20,
         length = 100)
y <- sqrt(x)
...
plot(x = x,
     ...
     lwd = 2,
     las = 1,
     main = "The transformation y = sqrt(x)",
     xlab = "x",
     ylab = "y = sqrt(x)",
     type = "l")
...
:::{.example #Exprain name="Exponential distributions"}
Let rv $X$ be uniformly distributed on $\ds{\left[-\frac\pi 2,\frac \pi 2 \right]}$. 
Find the distribution of $Y = \tan X$ (Fig. \@ref(fig:TanXform)).


```{r TanXform, echo=FALSE, fig.align="center", fig.cap="The tan transformation"}
x <- seq(-pi/2 + 0.1, 
         pi/2 - 0.1,
         length = 100)
y <- tan(x)
...
plot(x = x,
     ...
     xlim = c(-pi/2, pi/2),
     axes = FALSE,
     main = "The transformation y = tan(x)",
     xlab = "x",
     ylab = "y = sqrt(x)",
     type = "l")
axis(side = 2,
...
A modification of Theorem~\ref{TM:7.1} is used.


:::{.example #Exprain name="Exponential distributions"}
Given a random variable $Z$ which is distributed $N(0, 1)$, find the probability distribution of $Y = \frac
12Z^2$.


```{r Non11Xform, echo=FALSE, fig.align="center", fig.cap="A transformation that is not 1:1"}
x <- seq(-5, 5,
         length = 100)
...
The relationship $y = u(z) = \frac 12z^2$ is not strictly increasing or strictly decreasing in $(-\infty,\infty )$ so we can't apply Theorem~\ref{TM:7.1} directly.
However, we can subdivide the range of $z$ and $y$ so that in each portion the relationship is monotonic. 
Now
\[
   {f_Z(z)=\frac{1}{\sqrt{2\pi}}\,e^{-\frac12z^2},\quad-\infty <z<\infty}
\]
...
It follows from this result that if $X$ is $N(\mu,\sigma^2)$, the pdf of $Y = \frac 12 (X - \mu )^2 / \sigma^2$ is also $\Gam(\alpha = 1/2,\beta = 1)$ since then $\frac{X - \mu}{\sigma}$ is distributed $N(0, 1)$.
:::

Note that the probability can only be doubled as in Example~\ref{EG:7.6} if both $Y = u(Z)$ and the pdf of $Z$ are symmetrical about the same point.
...
1. The distribution function of the transformed variable is obtained. 
2. The probability density function is derived by differentiation.
...
```{r SquaringXform, echo=FALSE, fig.align="center", fig.cap="The transformation $Y = X^2$ when $X$ is defined from $1$ to $3$. The thicker line corresponds to the region where the transformation applies. Note that for $Y < y$, $2 - \\sqrt{y - 1} < X < 2 + \\sqrt{y - 1}$."}
...
:::{.example #Exprain name="Exponential distributions"}
Consider the same random variable $X$ as in the previous example, but the transformation $Y = (X - 2)^2 + 1$ (see
Fig. \@ref(fig:SquaringXform2)).




```{r SquaringXform2, echo=FALSE, fig.align="center", fig.cap="The transformation $Y = (X - 2)^2 + 1$ when $X$ is defined from $1$ to $3$. The thicker line corresponds to the region where the transformation applies. Note that for $Y < y$, $2 - \\sqrt{y - 1} < X < 2 + \\sqrt{y - 1}$."}
...
axis(side = 1,
     at = c(1, x0, 2, x1, 3),
     labels = c(1, "2 - sqrt(y - 1)", 2, "2 + sqrt(y - 1)", 3))
box()
```
...
\begin{align*}
   F_Y(y)
   &= \Pr{Y\le y} \qquad\text{(by definition)}\\
   &= \Pr{ (X-2)^2 + 1  \le y} \qquad\text{(since $Y=(X-2)^2 + 1$)}.
\end{align*}
...
:::{.example #Exprain name="Exponential distributions"}
Example~\ref{EG:7.6} is repeated here using the df method.

Given $Z$ is distributed $N(0,1)$ we seek the probability distribution of $Y = \frac 12 Z^2$.
\[ 
   f_Z(z) 
...
Then
\begin{align*}
     F_Y(y) & = \Pr{Y\leq y}=\Pr{\frac 12Z^2\leq y}=\Pr{Z^2\leq 2y}\\
...
2\sqrt{y}}f_Z(-\sqrt{2y})\\[2mm]
     ...
     & = \frac{1}{2y} \left[ \frac{1}{\sqrt{2\pi}}\,e^{-
y}+\frac{1}{\sqrt{2\pi}}\,e^{-y}\right]\\
     & = \frac{e^{-y}y^{-\frac 12}}{\sqrt{\pi}}
\end{align*}
as before.
:::

Obviously, care is needed to ensure the steps are followed logically. 
Drawing diagrams like Figure~\ref{FG:transform:yx2} and~\ref{FG:transform:yx-2} are certainly encouraged. 
...
Examples~\ref{EG:7.6} and \ref{EG:7.6rep} give rise to the chi-square distribution, which is an important model in statistical theory.
...
The chi-square distribution is actually a special case of the gamma distribution.
Comparison of Definitions~\ref{DF:chisq} and~\ref{DF:gamma} reveal that $X$ has a $\chi^2(\gamma)$ distribution if and only if $X$ has a gamma distribution with parameters $\beta = 2$ and $\alpha = \nu/2$. 
EXERCISE??
...
:::{.proof}
See Theorem~\ref{TM:gamma}.
:::

The importance of the chi-square distribution is hinted at in Examples~\ref{EG:7.6} and \ref{EG:7.6rep}, which essentially prove the following theorem.
...
:::{.proof}
Exercise---see Example~\ref{EG:7.6}.
:::


A useful property of the chi-square distribution is that the sum of independent rvs, each with a chi-square distribution, also has a chi-square distribution.
...
Since $Z_i \sim \chi^2(1)$, from Theorem~\ref{TM:chiprop}
\[
   M_{Z_i}(t)
   = (1 - 2t)^{-1/2}.
\]
It follows then from Theorem~\ref{TM:3:mgf2} that $S = \sum_{i=1}^n Z_i^2$ has mgf
\begin{align*}
   M_{S}(t)
