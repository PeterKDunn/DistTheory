...
For the discrete case , we need to show that $\sum_y y P(g(X)=y) = \sum_x g(x) P(X=x)$.

Let $Y = g(X)$. 
By definition, $\operatorname{E}[Y] = \sum_y y P(Y=y)$.
Note that the event $\{Y = y\}$ is actually the set of all $x$ such that $g(x) = y$.
Substitute: $P(Y=y) = \sum_{x: g(x)=y} P(X=x)$.
Plug this back into the expectation:
$$
  \operatorname{E}[g(X)] = \sum_y y \left( \sum_{x: g(x)=y} P(X=x) \right).
$$
Since $y = g(x)$ for all $x$ in that inner sum, we can move it inside:
$$
  \operatorname{E}[g(X)] = \sum_y \sum_{x: g(x)=y} g(x) P(X=x).
$$
Summing over all $y$, and then all $x$ that map to those $y$, is just the same as summing over all possible $x$.
Result: 
$\operatorname{E}[g(X)] = \sum_x g(x) P(X=x)$. $\square$
