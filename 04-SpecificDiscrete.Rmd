...
The probability that the lake will not freeze in these ten years is $\Pr(X = 10) = \binom{10}{10} 0.95^{10} 0.05^{0} \approx 0.599$, or about 60%.
...
\]
and $\text{E}(X) = (1 - p)/p$, $\text{var}(X) = (1 - p)/p^2$, and $M_X(t) = ???$. (CHECK THOSE???)
...
The probability that the system will first fail before day $20$ (that is, $\Pr(Y < 20) = \Pr(Y \le 19)$) can be found by hand, or using a computer.

To use **R**, the alternative parameterisation \@ref(eq:GeometricPMFAlternative) is needed.
Let $X$ be the number of days before the first system failure, so that $X = \{0, 1, 2, 3, \dots\}$ using \@ref(eq:GeometricPMFAlternative).
...
To observe the $r$th success in the $y$th trial, first $y - r$ failures and $r - 1$ successes must have occurred in the first $y - 1$ trials; the $r$th success then happens on the $y$th trial.
There are $\binom{y - 1}{r - 1}$ ways in which to allocate these successes to the first $y - 1$ trials.
...
\begin{align*}
   \text{No. ways}
   &\times \Pr\big(\text{$(y - r)$ failures}\big) \times \Pr(\text{$r$ successes}) \\
   \binom{y - 1}{r - 1} 
...
\begin{equation}
   p_Y(y) = \binom{y - 1}{r - 1}(1 - p)^{y - r} p^r
...
:::{.example #Demonstration name="Negative binomial"}
 A telephone marketer invites customers, over the
phone, to a product demonstration. 
...
Consider finding the probability that the marketer will need to make more than 100
calls to secure ten acceptances. 
Here, a ‘success’ is an acceptance to attend
the demonstration. 
Let $Y$ be the number of calls necessary to secure ten acceptances.
Then $Y$ has a negative binomial distribution 
with $p = 0.15$ and $r = 10$.
The mean number of calls to be made will is $\text{E}(Y) = r/p = 10/0.15 \approx 66.7$.
:::


Often, a different parameterisation for the negative binomial distribution is used (including by **R**), modelling the number of *failures* $x$ which occur in a sequence of independent trials until $r$ successes are reached.
The pf of this reparameterisation is
\begin{equation}
   p_X(y) = \binom{x + r - 1}{y} p^r (1 - p)^x
...
Note that the variables $X$ and $Y$ are related by $X = Y + r$ for constant $r$. 
Hence, this pf is defined for $y = 0$, $1$, $2$, $\dots$.


For the reparameterised version of the negative binomial distribution, Theorem \@ref(thm:NegBinomialProperties) is restated.


:::{.theorem #NegBinomialProperties2 name="Negative binomial (alternative parameterisation) properties}
...
Since the parameterisation in \@ref(eq:NegativeBinomialPMFAlternative) is defined over the non-negative integers, it is used to model count data (as an alternative to the [Poisson distribution](PoissonDistribution)).
...
::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
In **R**, the negative binomial distribution functions are based on the paramaterisation in \@ref(eq:NegativeBinomialPMFAlternative), and have the form `[dpqr]nbinom(size, prob)`, where `prob` is $p$ and `size` is $r$.
:::
...
The pf for the negative binomial distribution for various values of $p$ and $r$ is shown in Fig. \@ref(fig:NegativeBinomial), using the parameterisation in \@ref(eq:NegativeBinomialPMFAlternative).
...
Assuming each call take an average of 5 minutes, how long is the marketer expected to be calling to find ten acceptances?
...
In each parameterisation of the negative binomial distribution, $r$ can be any positive number, not just an integer.
...
\end{align*}
The integral is $\frac{1}{2}$, being half the area under a normal curve (which we see later in Sect. \@ref(Normal)). 
So
\[
   \Gamma(1/2) = \sqrt{\pi}.
\]
...
```{r GammaFunction, echo=FALSE, fig.cap="The gamma function is like the factorial function but has a continuous argument. The line corresponds to the gamma function $\\Gamma(z)$; the solid points correspond to the factorial $(z - 1)! = \\Gamma(z)$ for integer $z$.", fig.align="center", fig.width=5, fig.height=3.75, out.width='80%'}
...
To find the probability that an upgrade will happen within one year, let $D$ be the days on which the system fails for the fifth time.
Then $D\sim \text{NBin}(p = 0.02, r = 5)$, and $D = \{5, 6, 7, \dots\}$.

We seek $\Pr(D < 365)$ or, if using the alternate parameterisation (which is needed if we use **R**) where $X = D - r$, we seek $\Pr(X < 360)$.
Using **R**:
...
To find the the expected number of days between upgrades, use $\text{E}(D) = 5/0.02 = 250$. 
...
NEED to talk about that... How neg bin can be found by not assuming mu is constant.
...
occurring within each trapping
