...
Noticing that $Y\sim\text{Unif}(0, b - a)$ we have, using \@ref(eq:SumNaturalNumbers)
\begin{align*}
  \text{E}(Y)
...
\begin{align*}
   ...
   &= (pe^t + q)^n
\end{align*}
Prove the third result first, and then using it to prove the others, is easier (using of the methods in \@ref(MGFMoments)---try this as an exercise.)
:::
...
The pf for a geometric distribution for various values of $p$ is shown in Fig. \@ref(fig:BinomialPoisson2).
...
\begin{align*}
   \text{No. ways}
   &\times \text{$(y - r - 1)$ failures} \times \text{$r$ successes} \\
   {y - 1 \choose r - 1} 
...
:::{.proof}
Once again it makes sense to prove the third statement iniially, and this is left as an exercise.
:::
...
This is true for the form above or for the parameterization in \@ref(eq:NegativeBinomialPMFAlternative).
It is, however, most common to relax this restriction with the alternative parameterization of \@ref(eq:NegativeBinomialPMFAlternative).
...
For the reparameterised version of the negative binomial distribution, Theorem \@ref(thm:NegBinomialProperties)} becomes:
...
:::{.theorem #GammaProperties name="Gamma function properties"}
...
Bliss [@BIB:Bliss:negbin] gives data from the counts of adult European red mites on leaves selected at random from six similar apple trees; see
