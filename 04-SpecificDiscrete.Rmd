...
Consider tossing a die five times and observing the number of times a `r knitr::include_graphics("Dice/dice1.png", dpi=2000)` is rolled.
The probability of observing three `r knitr::include_graphics("Dice/dice1.png", dpi=2000)` can be found as follows:
In the five tosses, a `r knitr::include_graphics("Dice/dice1.png", dpi=2000)` must appear three times; there are ${5\choose 3}$ ways of allocating on which of the five rolls they will appear.
In the five rolls, `r knitr::include_graphics("Dice/dice1.png", dpi=2000)` must appear three times with probability $1/6$; the other two rolls must produce another number with probability $5/6$.
...
```{r BinomialExamples, echo=FALSE, fig.height = 7, fig.align="center", fig.cap="The pf for the binomial distribution for $p=0.2$ and $p=0.7$; $n=10$ and $n=20$"}
par( mfrow = c(2, 2))
...
```{r PoissonExamples, echo=FALSE, fig.height = 7, fig.align="center", fig.cap="The pf for the Poisson distribution for various values of $\\mu$"}
par( mfrow = c(2, 2))
...
```{r BinomialPoisson, echo=FALSE, fig.height = 7, fig.align="center", fig.cap="The Poisson distribution is an excellent approximation to the binomial distribution when $p$ is small and $n$ is large. The binomial pf is shown using empty circles; the Poisson pf using crosses."}
par( mfrow = c(2, 2))
...
```{r Geometric, echo=FALSE, fig.align="center", fig.cap="The pf for the geometric distribution for $p = 0.2$, $0.3$, $0.5$ and $0.8$"}
par( mfrow = c(2, 2))
...
```{r NegativeBinomial, echo=FALSE, fig.height=7, fig.align="center", fig.cap="The pf for the negative binomial distribution for $p = 0.2$ and $0.7$ and $r = 1$ and $r = 3$"}
par( mfrow = c(2, 2))
xmax <- 15
...
@BIB:Bliss:negbin gives data from the counts of adult European red mites on leaves selected at random from six similar apple trees; see Table~\ref{TB:specdiscrete:negbintrees}.
...
The probability that the next ball is red is $5/9 = 0.556$.
