...
If a discrete rv $Y$ can assume $k$ different and distinct values with *equal* probability, then $Y$ is said to have a *discrete uniform distribution*. 
...
A plot of the pf for a discrete uniform distribution is shown in Fig. \@ref(fig:DiscreteUnform).
...
1. $\text{E}(X) = (a + b)/2$.
2. $\displaystyle \text{var}(X) = \{(b - a)(b - a + 2)\}/12$.
:::
...
Consider tossing a die five times and observing the number of times a \epsdice{1} is rolled.
The probability of observing three \epsdice{1}'s can be found as follows:
In the five tosses, a \epsdice{1} must appear three times; there are ${5\choose 3}$ ways of allocating on which of the five rolls they will appear.
In the five rolls, \epsdice{1} must appear three times with probability $1/6$; the other two rolls must produce another number with probability $5/6$.
So the probability will be
\[
   \Pr(3 ones) = {5\choose 3} (1/6)^3 (5/6)^2 = 0.032,
\]
...
In this case, the *hypergeometric distribution* could be used (see Sect. \@ref(Hypergeometric)).
:::
...
```{r PoissonExamples, echo=FALSE, fig.align="center", fig.cap="The pf for the Poisson distribution for various values of $\\mu$"}
par( mfrow = c(2, 2))
...
Usingf the pmf, computing some probabiliuties, such as $\Pr(X > 101)$, is very tedious.
...
```{r BinomialPoisson, echo=FALSE, fig.align="center", fig.cap="The Poisson distribution is an excellent approximation to the binomial distribution when $p$ is small and $n$ is large. The binomial pf is shown using empty circles; the Poisson pf using crosses."}
par( mfrow = c(2, 2))
...
:::{.theorem}
If $X\sim \text{NB}(r,p)$ with pf (\ref{EQN:specdiscrete:negbin}) then
...
If $X\sim \text{NB}(r,p)$ with pf (\ref{EQN:specdiscrete:negbin2}) then
