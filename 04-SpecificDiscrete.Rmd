...
```{r DiscreteUnform, echo=FALSE, fig.align="center", fig.cap="The pf for the discrete uniform distribution $\\text{Unif}(a, b)$", fig.width=5, fig.height=3.5, out.width='50%'}
x <- c(1:4, 7)
y <- rep(1, length(x))
...
\[
   ...
   \end{cases}
\]
where $\lfloor z \rfloor$ is the *floor function* (i.e., round $z$ to the nearest integer in the direction of $-\infty$).
:::
...
Then $X$ has a *binomial probability distribution* with parameters $n$, $p$ and pf given by  
\begin{equation}
...
To find the variance, use the computational formula $\text{var}(X) = \text{E}(X^2) - \text{E}(X)^2$.
Firstly, to find $\text{E}(X^2)$, write $\text{E}(X^2)$ as $\text{E}[X(X - 1) + X]$ so that $\text{E}[X(X - 1)] + \text{E}(X)$; then:  
\begin{align*}
     \text{E}(X^2) 
...
In **R**, many functions are built-in for computing the pf, df and other quantities for common distributions.
The functions for the binomial distribution have the form `[dpqr]binom(..., size, prob)`, where `prob`${} = p$ and `size`${} = n$.
For example:
...
Let $Y$ be the number of goals made until the shooter first *misses*; in this case a 'success' refers to *missing* the shot with $p = 0.2$.
...
The [binomial distribution](BinomialDistribution) applies in situations where an event may or may not occur in a fixed number of trials, say $n$.
...
\begin{equation}
   ...
   (\#eq:BinomialPMF2)
\end{equation}
with a mean of $\lambda = np$. 
Re-writing \@ref(eq:BinomialPMF2) in terms of the mean $\lambda$:  [
\begin{equation}
   p_X(x; n, p) 
   = 
   ...
   (\#eq:BinomialPMF2Mean)
\end{equation}
Now consider the case $n \to \infty$ (for $x = 0, 1, \dots$ still):  
\begin{align}
   \lim_{n\to\infty}
   p_X(x; n, p) 
   &= 
   ...
   &= \frac{\lambda^k}{k!} 
   \lim_{n\to\infty} \frac{n!}{(n - k)!} \frac{1}{n^k} \left(1 - \frac{\lambda}{n}\right)^{n}  \left(1 - \frac{\lambda}{n}\right)^{-x}.
\end{align}
Now, since the limit of product of functions is equal to product of their limits, each component can be considered in turn.
Looking at $n!/(n - k)! \times 1/n^k$, see that  
\begin{align}
  \lim_{n\to\infty} \frac{n!}{(n - k)!} \times 1/n^k
  &= \lim_{n\to\infty} \frac{n \times (n - 1) \times (n - 2) \times\cdots\times 2 \times 1}
                            {(n - k)\times (n - k - 1) \times \cdots \times 2 \times 1} \frac{1}{n^k}\\
  &= \lim_{n\to\infty}\frac{ \overbrace{n \times (n - 1) \times \cdots\times (n - k + 1)}^{\text{$k$ terms}}}
                           { \underbrace{n \times n \times\cdots \times n\times n}_{\text{$k$ terms}}}\\
  &= \lim_{n\to\infty} \frac{n}{n} \times \frac{n - 1}{n} \times \frac{n - 2}{n} \times\cdots\times \frac{n - k + 1}{n}\\
  &= 1 \times 1\times\cdots\times 1 = 1.
\end{align}
For the next component, see that (using \@ref(eq:ExponentialInfSeries)):  
\[
...
```{r PoissonExamples, echo=FALSE, fig.align="center", fig.cap="The pf for the Poisson distribution for various values of $\\lambda$", fig.height = 7, fig.width=7, out.width='80%'}
par( mfrow = c(2, 2))
...
The pf for the Poisson distribution can be compared to the empirical probabilities computed above; see Table \@ref(tab:BombTable).
...
For example, consider $X\sim \text{Bin}(n = 2000, p = 0.005)$; the pf is  
\[
...
Since $\text{E}(Y) = \text{var}(Y) = \lambda$ for the Poisson distribution, this means the variance is also set to $\sigma^2 = np$.
...
\]
where the parameter is $\lambda > 0$.
This is called the *zero-truncated Poisson distribution*.
In this case, $\text{E}(X) = \lambda + 1$ and $\text{var}(X) = \lambda$ (Ex. \@ref(exr:PoissonZeroTruncated)).
...
### Standard parameterisation {#NegBinStandard}
...
The mean number of mites per leaf is 1.14 witha variance of 3.57; The Poisson distribution has an equal mean and variance, the Poisson distribution may not model these count data well.
...
Consider a simple example: A bag contains six light-coloured balls and four dark-coloured balls (Fig. \@ref(fig:HypergeometricHelp)).
...
But suppose there are 10,000 balls in the bag, of which 6000 are light-coloured.
...
```{r EggsSimulation, fig.cap="The number of fertile eggs laid, over 5000 simulations", fig.height=4, fig.width=5, out.width='56%', fig.align="center"}
...
1. Take logarithms of \@ref(eq:GeomCapture), and hence write this equation in the form of a linear regression equation $\hat{y} = \beta_0 + \beta_1 x$.
1. From the regression equation, identify *how* to estimate $p$ from the slope of the regression line ($\beta_1$), and then $N$ from the $y$-intercept ($\beta_0$).
...
:::{.exercise}
A study of crashes at intersections [@lord2005poisson] examined high risk intersections, and modelled the number of crashes with a Poisson distribution using $\lambda = 11.5$ per day.
...
:::{.exercise #LayDate}
A *negative binomial* distribution was used to model the day on which eggs were layed by glaucous-winged gulls [@zador2006balancing].
...
the overdispersion parameter was 20.6 in 1999 and 8.9 in 2000.
...
The negative binomial distribution was defined in Sect \@ref(NegativeBinomialDistribution) for the random variable $X$, which represented the number of failures until the $r$th success.
...
Consider the mean and variance for the hypergeometric distribution.
...
In this exercise, we find the variance of the discrete uniform distribution, using the definitions of $Y$ and $X$ in Sect. \@ref(DiscreteUniform).

1. First find $\text{E}(Y^2)$ 
1. Then find $\text{var}(X)$.
:::


:::{.exercise #GeometricPropertiesProof}
Prove the results in Theroem \@ref(thm:GeometricProperties), by first finding the mgf.
:::


:::{.exercise #PoissonZeroTruncated}
Prove these results from Sect. \@ref(PoissonExtensions): $\text{E}(X) = \lambda + 1$ and $\text{var}(X) = \lambda$ for the zero-truncated Poisson distribution.
:::
