...
In **R**, many functions are built-in for computing the pf, df and other quantities for common distributions (see Appendix \@ref(UseRDistributions)).
...
::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
The four **R** functions for working with the negative binomial distribution are based on the paramaterisation in \@ref(eq:NegativeBinomialPMF), and have the form `[dpqr]nbinom(size, prob)`, where `prob` is $p$ and `size` is $r$ (see Appendix \@ref(UseRDistributions)).
:::
