...
:::{.exercise #CountingBirds}
In a study of counting birds [@white1996analysis], the parameterisation of the negative binomial distribution used by the authors is

\begin{equation}
   p_X(x) 
   = \frac{\Gamma(k + x)}{\Gamma(k)\,\Gamma(x + 1)} 
     \left( \frac{m}{k} \right)^x
     \left( 1 - \frac{m}{k} \right)^{ -(k + x)}
   (\#eq:NegativeBinomialPMFAlternative)
\end{equation}
for $x = 0, 1, 2, \dots$ and $k > 0$ and $m > 0$.
Here, $X$ is the number of birds observed.
...
1. On the same graph, plot both distributions.
   Comment.
1. Compute the probability of observing fewer than 2 birds in each environment.
1. Compute the probability of observing fewer than 2 birds in each environment, *given* that the bird was seen.
:::
