...
In **R**, these function start with the letter `r`,; for example, to generate random numbers from a Poisson distribution, use:
...
cases <- 0
for (m in (1:12)){
   cases <- cases + rpois(1, 0.5)  
   cat("Month ", m, "; Number of cases so far: ", cases, "\n")
}
```

The code can be repeatedly run, as each run will produce different random numbers.
If we run the above code 1000 times, we can record how many cases are recorded at the end of each year:
...
```{r echo=TRUE}
numberCasesEndOfYear <- array( dim = 100)
...
The negative binomial distribution was defined in Sect \@ref(NegativeBinomialDistribution) fpr teh random variable $Y$, which represented the number of failures until the $r$th success.
...
1. Deterine the mean, variance and mgf of $X$, using the results already available for $Y$.
:::
