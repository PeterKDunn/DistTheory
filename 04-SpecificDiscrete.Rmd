...
* recognise the probability functions and underlying parameters of uniform, binomial, geometric, negative binomial, Poisson, and hypergeometric random variables.
...
The probability of each outcome remains constant on each trial.
...
A binomial distribution is used in a situation where the same random process is repeated many times, and one of two outcomes is observed.
...
Now the number of distinct arrangements of the $r\,S$'s and $(n - r)$ $F$'s is $\binom{n}{r}$, so
\begin{equation}
...
Assuming that the number entering the line in any given time interval has a Poisson distribution, determine the probability that at least one customer enters the line in a given $\frac{1}{2}$-minute interval, we have (since $\mu = 2$):
\[ 
...
\begin{equation}
   p_X(x) = \binom{x - 1}{r - 1}(1 - p)^{x - r} p^r
...
::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
In **R**, the negative binomial distribution functions have the form `[dpqr]nbinom(size, prob)`.
The alternative parameterisation uses `mu`...

CHECK PARAMETERISATIONS CORRECT.
:::
