...
\begin{align*}
  M_X(t)
  &= \sum_{i = a}^{b} \exp(it) \frac{1}{b - a + 1}\\
  &= \frac{1}{b - a + 1} \sum_{i = a}^{b} \exp(it)\\
...
\[
   p_X(x) =  (1 - p)^{x - 1} p = q^{x - 1} p\quad\text{for $x = 0, 1, 2, \dots$}.
\]
...
We can repeat this over many simulations:

```{r}
# Many simulations
set.seed(1238721)

NumberOfEggs <- array( dim = 1000 )
NumberFertileEggs <- array( dim = 1000 )
...
```{r EggsSimulation, fig.cap="The number of fertile eggs laid, over 1000 simulations", fig.height=4, fig.width=5, out.width='56%', fig.align="center"}
...
```{r echo=TRUE}
sum( NumberFertileEggs > 10) / 1000
```

The chance that more than 10 eggs are fertile is about `r round(sum( NumberFertileEggs > 10) / 1000, 1)`%.
