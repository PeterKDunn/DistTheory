...
\[
   ...
   \end{cases}
\]
where $\lfloor z \rfloor$ is the *floor function* (i.e., round $z$ towards $-\infty$ to the nearest integer).
:::
...
:::{.proof}
The mean and variance are easier to find by working with  $Y = X - a$ rather than $X$ itself (as $Y$ is defined on $0 < y < (b - a)$), using that $\text{E}(X) = \text{E}(Y) + a$ and $\text{var}(Y) = \text{var}(X)$.
...
Oz Lotto randomly picks numbers between 1 and 47 (inclusive).
Each number should have an equal chance of selection, so the discrete uniform distribution $U(1,47)$ is appropriate.

If the data follow a discrete uniform distribution exactly, the mean number drawn should be  $(1 + 47) / 2 = 24$, with variance $(46 \times 48)/12 = 184$.
:::
...
Let $X$ be the number of successes in a single trial with $\Pr(\text{Success}) = p$ ($0\le p\le 1$), constant in each trial.
Then $X$ has a *Bernoulli probability distribution* with parameter $p$ and pf given by \@ref(eq:BernoulliPMF).
We write $X\sim\text{Bern}(p)$.
:::
...
1. $\text{E}(X) = p$.
2. $\text{var}(X) = p(1 - p) = pq$ where $q = 1 - p$.
3. $M_X(t) = pe^t + q$.
:::
...
A binomial distribution is used to model the number of successes in $n$ independent Bernoulli trials.
...
There are $n = 4$ Bernoulli trials with $p = 1/6$.
...
\end{align*}
The summation is 1, since it is equivalent to summing over all values of $y$ for the binomial pf with $y$ successes in $(n - 1)$ Bernoulli trials (and hence represents a probability function with a sum of one).
In the second line, the sum is over $x$ from $1$ to $n$ because, for $x = 0$, the binomial coefficient has the form $\binom{b}{a}$ with $a < 0$, which is defined to be zero. 
Thus,
...
Since the first success may occur on the first trial, or second trial or third trial, and so on, $X$ is a random variable with range space $\{0, 1, 2, 3, \dots\}$.
...
A random variable $X$ has a *geometric distribution* if the pf of $X$ is

\begin{equation}
   p_X(x) =  (1 - p)^x p = q^x p\quad\text{for $x = 0, 1, 2, \dots$}
   (\#eq:GeometricPMF)
\end{equation}
...
::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
In **R**, the geometric distribution functions have the form `[dpqr]geom(prob)`, where `prob`${} = p$.
:::
...
\[
   p_X(x) =  q^{x - 1} p\quad\text{for $x = 0, 1, 2, \dots$}.
\]
The only way to distinguish which parameterisation is being used is to check the range space or the pf.
...
Suppose a netball goal shooter has a probability of $p = 0.8$ of *scoring* a goal.
Let $Y$ be the number of goals made until the shooter *misses*; in this case a 'success' refers to *missing* the shot with $p = 0.2$.
Notice that $Y = \{0, 1, 2, \dots \}$), so the parameterisation corresponds to that used by **R**, and $Y\sim \text{Geom}(0.2)$.
...
pgeom(4, prob = 0.2)
```
The expected number of shots until the first miss is $\text{E}(Y) = (1 - p)/p  = 4$.
...
To determine the probability that at least one customer enters the line in a given $\frac{1}{2}$-minute interval, use (since $\lambda = 2$ oer half-hour):
...
Computing binomial probabilities is tedious if the number of trials $n$ is very large and the probability of success in a single trial is very small. For example, consider $X\sim \text{Bin}(n = 2000, p = 0.005)$; the pf is
...
For example, consider modelling the number of nights patients spend in hospital; patients must spend at least one night in hospital. 
Then, the probability functions can be expressed as

\[
   p_X(x) = \frac{\exp(-\lambda) \lambda^{x - 1}}{(x - 1)!}\quad   \text{for $x = 1, 2, 3, \dots$}
\]
where the parameter is $\lambda > 0$.
...
\[
   ...
   \begin{cases}
      ...
      \displaystyle
      \sum_{x = K + 1}^\infty \frac{\exp(-\mu) \mu^x}{x!}   & \text{for $x = K + 1, K + 2, \dots, \infty$};
   \end{cases}
\]
...
To observe the $r$th success after $y$ failures, $y$ failures and $r - 1$ successes are observed first in the $x + r - 1$ trials.
...
\begin{equation}
   p_X(x) = \binom{x + r - 1}{r - 1}(1 - p)^{x} p^{r - 1}
...
The mean number of failures made will is $\text{E}(Y) = \{r(1 - p)\}/p = \{10 \times (1 - 0.15)\}/0.15 \approx  56.66667$.
...
In this expression, $\Gamma(r)$ is the *gamma function* (Def. \@ref(def:GammaFunction)), and is related to factorials when $r$ is integer;
...
:::{.proof}
For the first propoerty, integration by parts gives
...
From the data, the mean number of mites per leaf is
...
The Poisson distribution has an equal mean and variance; the Poisson distribution may not model these count data well.
...
In general, suppose a bag contains $N$ balls, and $m$ of them are red and $n$ and *not* red; then $N = m + n$.
Suppose a sample of size $k$ is drawn from the bag *without replacement*; then the probability of finding $x$ red balls in the sample of size $k$ is
...
\]
where $X$ is the number of red balls in sample of size $k$.
(In the example, $k = 3$, $m = 6$ and $N = 10$.)

In the formula, $\binom{m}{x}$ is the number of ways of selecting $x$ red balls from the $m$ red balls in the bag; $\binom{n}{k - x}$ is the number of ways of selecting all the remaining $k - x$ to be the other colour (and there are $n = N - m$ of those in the bag); and $\binom{N}{k}$ is the number of ways of selecting a sample of size $k$ if there are $N$ balls in the bag in total.
...
\begin{equation}
   p_X(x) = \frac{ \binom{m}{x}\binom{n}{k - x}}{\binom{m + n}{k}}
...
If $X$ has a hypergeometric distribution with pf \@ref(eq:HypergeometricPMF), then (writing $N = m + n$)

1. $\text{E}(X) = km/N$.
2. $\displaystyle
    \text{var}(X) = {k \left(\frac{m}{N}\right)\left(\frac{N - k}{N - 1}\right)\left(1 - \frac{m}{N}\right)}$.
:::
The moment generating function is difficult and will not be considered.


::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
In **R**, the hypergeometric distribution functions have the form `[dpqr]hyper(m, n, k)`, where `k`${} = k$, `m`${} = m$ and `m`${} + {}$`n`${} = N$ (so that `n`${}={}$`N`${}-{}$`m`).
:::
...
But suppose there are 10,000 balls in the bag, of which 6000 are red.
The probability of drawing a red ball initially is $6000 / 10,000 = 0.6$, and the probability that the next ball is red becomes $5999/9999 = 0.59996$; the probability is almost the same.
...
Consider, for example, if the Poisson distribution was used to model the number of new cases of a disease in a given town each month.
However, the mean number of cases each month depends on external factors; the mean is either $\lambda = 3.5$ (with probability $0.17$) or $\lambda = 0.5$ (with probability $0.73$) following a binomial distribution. 
The number of cases in one year could be modelled using:

```{r echo=TRUE}
set.seed(1072908) # So you can repeat what I do

cases <- 0  # Initialise
for (m in (1:12)){   # For each month...

  whichLambda <- rbinom(1, # Give one random observation
                        1, # Number of trials: That is, 0 or 1 here
                        prob = 0.17) 
  lambda <- ifelse(whichLambda == 0, 
                   3.5,
                   0.5)
  newCasesThisMonth <- rpois(1, lambda = lambda)
  cases <- cases + newCasesThisMonth  # Add new cases for each month
  cat("Month ", m, "; New cases:", newCasesThisMonth, 
      "; Total number of cases so far: ", cases, "\n")
}
```

The code can be repeatedly run, as each run will produce different random numbers (provided the `set.seed()` line is removed).
...
```{r SimAPlot, echo=FALSE, fig.cap="The number of cases per year", fig.align="center", fig.width=5, fig.height=4}
<<SimA>>
```
...
```{r echo=TRUE}
sum( numberCasesEndOfYear > 40) / 1000
```

The chance that more than 40 cases are observed is about `r round(sum( numberCasesEndOfYear > 40) / 1000 * 100, 1)`%.
...
1. Explain how \@ref(eq:GeomCapture) arises.
...
* in lodgepole pine stands (again, riparian sites): $m = 0.04$ and $k= 0.275$.
...
Consider the mean and variance for the hypergeometric distribution, and write the hypergeometric distribution in terms of the sample size $n$ rather than $k$.

1. Write $p = m/N$, and express the mean and variance of the hypergeometric distribution in terms of $p$ and $n$.
1. Show that the mean of the hypergeometric and binomial distributions are the same.
1. Show that the variance of the hypergeometric and binomial distributions are by connected a factor of

\[
 \sqrt{ \frac{N - n}{N - 1} },
\]
called the *Finite Population Correction* factor (Def. \@ref(def:FPC)).
:::