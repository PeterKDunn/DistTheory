...
Working with order statistics is easier with continuous random variables than for discrete random variables, because ties are not possible (since $\Pr(X = x)$ for some value\ $x$ is zero when $X$ is a continuous random variable).
As a result, the formulas for discrete random variables are more complicated since they need to allow for the presence of ties.
