...
Consider a sample of size\ $n$ from a continuous distribution with density function\ $f_X(x)$ and distribution function\ $F_X(x)$.
Then, the $k$th order statistic $X_{(k)}$ has the distribution function $F_{X_{(k)}}(x) = \Pr(X_{(k)}\le x)$. 
By definition, this is the probability that the $k$th smallest value in the sample is less than (or equal to) some value\ $x$.

Writing $X_{(k)} \le x$ means that there must be *at least*\ $k$ of the\ $n$ observations smaller than the value\ $x$; that is, $k, k+1, k+2, \dots, n$ values values smaller than $x$.
We can denote the probability than a single observation has a value less than\ $x$ as $p = F_X(x)$, and then think of a 'success' as an observation having a value less than or equal to\ $x$.
Then, the number of successes (the number of values less than or equal to\ $x$, say\ $Y$) is
$$
   Y \sim \text{Binom}(n, p).
$$
Then, since $X_{(k)} \le x$ means *at least*\ $k$ of the\ $n$ observations are smaller than the value\ $x$, this means $k, k + 1, \dots n$ of the observations are smaller than\ $x$.

Then,
\begin{align*}
  \Pr(X_{(k)} \le x) 
  &= \Pr(\text{at least $k$ successes}) \\
  &= \Pr( Y\ge k) \\
  &= \Pr(Y = k) + \Pr(Y = k+1) + \cdots + \Pr(Y = n),
\end{align*}
where each probability in the final line is a binomial probability.
That is, adding up the corresponding binomial probability mass functions,
$$
  F_{X_{(k)}}(x) 
  = \Pr(X_{(k)} \le x)
  = \Pr( Y\ge k)
...
::: {.example #UniformkOrderStats name="Other order statistics"}
...
For continuous random variables, discussing order statistics is easier than for discrete random variables, because ties are not possible (since $\Pr(X = x)$ for some value\ $x$ is zero).
As a result, the formulas for discrete random variables are more complicated to accommodate the non-zero probability of ties.
...
$$
   F_X(x) = \Pr(X \le x).
$$
In the discrete case, defining $F(x^{-}) = \Pr(X < x)$ is convenient.
...
????\textbf{Step 3: Special case for integer-valued \(X\).}  
If the support is \(a_1 < a_2 < \dots\), then
\[
p_{X_{(1)}}(a_j) 
= \big[ 1 - F_X(a_{j-1}) \big]^n - \big[ 1 - F_X(a_j) \big]^n,
\]
with the convention \(F_X(a_0) := 0\).
---

### General order stat from c

Let \(X_1, X_2, \dots, X_n\) be i.i.d.\ from a discrete distribution with
\[
p_X(x)=\Pr(X=x), \qquad F_X(x)=\Pr(X\le x), \qquad F_X(x^-)=\Pr(X<x).
\]
Denote the \(k\)-th order statistic by \(X_{(k)}\).

\textbf{Step 1: CDF of \(X_{(k)}\).}
Define the count of observations \(\le x\) as
\[
Y=\sum_{j=1}^n I\{X_j\le x\} \sim \mathrm{Binomial}\!\left(n,\,F_X(x)\right).
\]
Then
\begin{align*}
F_{X_{(k)}}(x)
...
\textbf{Step 2: PMF (discrete “pdf”) of \(X_{(k)}\).}
For a discrete \(x\), the PMF is the jump of the CDF at \(x\):
\begin{align*}
p_{X_{(k)}}(x)
  &= \Pr\bigl(X_{(k)}=x\bigr)
   = F_{X_{(k)}}(x) - F_{X_{(k)}}(x^-) \\
...
\textbf{Step 3: Explicit multinomial form (optional).}
Write
\[
a=F_X(x^-)=\Pr(X<x),\quad b=p_X(x)=\Pr(X=x),\quad c=1-a-b=\Pr(X>x).
\]
Let \((R,S,T)\) be the counts of observations that are \(<x\), \(=x\), and \(>x\), respectively; then
\((R,S,T) \sim \mathrm{Multinomial}\bigl(n; a,b,c\bigr)\).
The event \(\{X_{(k)}=x\}\) occurs iff \(R\le k-1\) and \(R+S\ge k\).
Thus
\[
p_{X_{(k)}}(x)
  = \sum_{r=0}^{k-1}
...
\begin{itemize}
  \item \(k=1\): recovers the discrete minimum
    \[
    p_{X_{(1)}}(x)
    = \bigl[1-F_X(x^-)\bigr]^n - \bigl[1-F_X(x)\bigr]^n.
    \]
  \item If \(X\) is continuous (so \(b=0\) and \(F_X(x^-)=F_X(x)\)), the PMF vanishes and one uses the usual density
    \(f_{X_{(k)}}(x)=\dfrac{n!}{(k-1)!(n-k)!}\,[F_X(x)]^{k-1}[1-F_X(x)]^{n-k} f_X(x)\).
\end{itemize}
...
:::{.exercise #OrderStatisticsSim1}
aaa
:::

:::{.exercise #OrderStatisticsSim2}
bbb
:::
