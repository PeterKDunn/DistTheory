...
## Distribution of the maximum value {#OrderStatisticsMaxDistribution}
...
$$
Using the density function of the maximum value as given above, the expected value and the variance of the maximum value can be found too (Exercise); for example,
$$
...
Equivalently, this means that no values have a value greater than\ $x$.

For example, if $X_{(1)}$ (the smallest value) is less than or equal to\ $x = 3$, then there must be *at least* one value in the sample smaller\ $3$.
That is, one value in the sample may be less than (or equal to)\ $3$; however, the smallest value is less than \ $3$ when *two* values in the sample are less than (or equal to)\ $3$ also.
Indeed, the smallest value is less than\ $3$ when *three* values in the sample are less than (or equal to)\ $3$ also... and so on.
In fact, the smallest value is less than (or equal to)\ $3$ provided *at least one value* has a value less\ $3$; in other words, provided *none* of the\ $n$ values have a value greater than\ $3$.
...
$$
  F_{(1)}(0.2) = 1 - (1 - 0.3)^{10} = 0.8926\dots
$$
