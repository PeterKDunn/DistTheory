...
$$
for $x \in R_X$.
The density function is found by differentiating $F_{X_{(n)}}(x)$ with respect to\ $x$ (Sect.\ \@ref(DistributionFunction)), to obtain
$$
...
For the case where $n = 10$, Fig.\ \@ref(fig:OrderStatsMax) shows the probability density function (left panel) and the distribution function (right panel).
...
For the case where $n = 10$, Fig.\ \@ref(fig:OrderStatsMin) shows the probability density function (left panel) and the distribution function (right panel).
For example, the probability that the smallest value in a sample of size\ $10$ is smaller than\ $0.3$ is
$$
...
$$
 ...
 =
 \begin{cases}
    n !\, f_{X_{(1)}} \times f_{X_{(2)}}\times \cdot \times f_{X_{(n)}} & \text{for $x_{(1)} < x_{(2)} < \cdots < x_{(n)}$}\\
    0 & \text{elsewhere}
 \end{cases}
$$
