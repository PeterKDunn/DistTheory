...
```{r OrderStatsDiscreteDefine, echo=FALSE, fig.align="center", fig.cap="Explaining the meaning of $F_X(x^-)$.", fig.width=8, fig.height=4, out.width="90%"}
...
axis(side = 2,
     at = c(1, 0),
     las = 1,
     label = c( expression(italic(F)[italic(X)](italic(x))),
                expression(italic(F)[italic(X)](italic(x^"\u2013"))) ) )
axis(side = 1,
     at = 0,
     label = expression(italic(x)) )
box()
...
text(x = -0.5,
     y = 0.9,
     label = expression(italic(F)[italic(X)](italic(x))),
     pos = 2)
...
text(x = 0.5,
     y = 0.1,
     label = expression(italic(F)[italic(X)](italic(x^"\u2013"))), #\u2013 is en-dash; \u2014 is em-dash
     pos = 4)
...
\begin{align}
  p_{X_{(k)}}(x)
    ...
    &\qquad \qquad
       \sum_{i=k}^n \binom{n}{i}\,[F_X(x^-)]^{i}\,[1-F_X(x^-)]^{\,n-i}.
       (\#eq:OrderStatsDiscrete)
\end{align}



:::{.example #DiscreteOrderStatsDice name="Rolling a fair die"}  
Suppose a fair die is rolled $n = 10$ times, and the number appearing is recorded is denoted by\ $X$.
...
$$
  ...
  \begin{cases}
    0                   & \text{for $x < 1$}\\
    \lfloor x/6 \rfloor & \text{for $1 \le x \le 6$}\\
    1                   & \text{for $x > 6$.}
  \end{cases}
$$
...
\begin{align*}
  ...
  \end{cases}
\end{align*}
If we focus on the integer outcomes only,
$$
  F_{X_{(1)}}(x) 
  = 
  1 - \left(\frac{7 - x}{6}\right)^{10} \qquad\text{at $x = 1, 2\dots, 6$}.
$$
Hence the pmfs (for $k=1,\dots,6$):
$$
  \Pr\left(X_{(1)} = k\right) = \left(\frac{7-k}{6}\right)^{10} - \left(\frac{6-k}{6}\right)^{10},
  \qquad
  \Pr\left(X_{(10)} = k\right) = \left(\frac{k}{6}\right)^{10} - \left(\frac{k-1}{6}\right)^{10}.
$$

The general $r$-th order statistic $X_{(r)}$, for $r\in\{1,\dots,10\}$ and $k\in\{1,\dots,6\}$, has ????
$$
  \Pr\big(X_{(r)} \le k\big)
  = \sum_{i = r}^{10} \binom{10}{i}\,\left(\frac{k}{6}\right)^{i}\,\left(1 - \frac{k}{6}\right)^{10 - i},
$$
and the pmf is the discrete difference
$$
  \Pr\big(X_{(r)} =  k\big) 
  =
  \Pr\big(X_{(r)}\le k\big) - \Pr\big(X_{(r)}\le k - 1\big).
$$
So, for instance, the third order statistic (i.e., where $r = 3$) is
$$
  \Pr\big(X_{(3)} \le k\big)
  =
  \sum_{i = 3}^{10}\binom{10}{i}\left(\frac{k}{6}\right)^{i}\left(1 - \frac{k}{6}\right)^{10 - i},
  \quad
  \Pr\big(X_{(3)} = k\big) 
  = 
  \Pr\big(X_{(3)}\le k\big) - \Pr\big(X_{(3)}\le k - 1\big).
$$
:::
...
barplot(table(min_Roll)/num_Sims,
