...
\[
   \text{E}[Z] = \int_1^{-\infty} z \frac{1}{z^2}\, dz = \int_1^\infty \frac{1}{z} = \left. -\log z \right|_1^\infty.
\]
However, $\displaystyle\lim_{z\to\infty} -\log z \to \infty$.
This expected value is undefined.
:::

CONTINUOUS.
TWEEDIE ONLY KNOWN BY MGF




```{r NoMean, echo=FALSE, fig.align="center", fig.cap="The probability function for the random variable Z", fig.height = 3.5, fig.width=4}
z <- seq(1, 6, 
         length = 100)
plot(x = z,
     y = z^(-2),
...
:::{.proof}
Assume $X$ is a discrete rv with probability function $p_X(x)$.
...
We have also seen situation where the mean is undefined (Example \@ref(exm:InfiniteMean)).


### The mode and median {#ModeMedian}

Other measures of centre may be more appropriate.
...
\begin{align*}
   ...
   &= \frac{70}{24}.
\end{align*}
The standard deviation is then given by $\sigma = \sqrt{\frac{70}{24}} \approx 1.71$.
...
* Discrete case: $\Pr(X = \mu + x) = \Pr(X = \mu - x)$.
* Continuous case: $\Pr(X = \mu + x) = f_X(\mu - x)$.
:::
...
For a symmetric distribution, the odd central moments are zero (Exercise ???).
...
```{r ProbFunSkewRight, echo=FALSE, fig.align="center", fig.cap="The probability function", fig.width=4, fig.height=3.5}
plot( x = c(-1, 8),
      y = c(0, 0.5),
      type = "n",
      las = 1,
...
Monypenny and Middleton (@monypenny1998analysiswinds, @monypenny1998analysisgust) use the skewness and kurtosis to analyse wind gusts at Sydney airport.
:::
...
The moment generating function can be used to derive the distribution of a function of a rv (see Sect. \@ref(TransformationMoments)).
...
MAYBE WE SHOULD PRESENT IT HERE!
