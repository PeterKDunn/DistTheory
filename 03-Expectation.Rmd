...
If $Y = g(X)$, we would write $\mu_Y = \text{E}(Y) = \text{E}(g(X))$.
...
(An analogous proof applies in the continuous case, replacing the probability function with a pdf and summations with integrals.)
...
An important special case of a  function of a random variable is the *variance* of a random variable.
...
Because of this, using the variance is less popular than the standard deviation in practice to describing spread.
In theoretical work however, because of the square root function, the variance is easier to work with than standard deviation, and you will find the variance rather than standard deviation featuring in many results throughout the study of statistics.
...
NOTES ABOUT USE OF COMP FORMULA IN COMPUTING AMD ROUND OFF
...
GIVE SOME ITEMIZED FACTS ABOUT VAR? SOME REPETED:
...
THEY WERE JUST DEFINED. REORGANISE?
...
The median, $\nu$, is any value of $X$ in the range $3 < x < 4$, since *any* number in this range satifies $\Pr(X \le x) = 0.5$ and $\Pr(X \ge x) = 0.5$. 
...
Consider the rabdom variable $X$ with probability function
\[
   f_X(x) = 
   \begin{cases}
...
If $\gamma_1 >0$ we say the distribution is positively (or right) skewed, and it is 'drawn out' in the positive (negative) direction.
...
WAIT!!!     The interpretation of 'peakedness' is somewhat vague!
...
Note that in the special case when all the rvs are independently and identically distributed in Theorem \@ref(thm:MGFLinear),
\[
   M_Y(t) = [M_{X_i}(t)]^n.
\]
