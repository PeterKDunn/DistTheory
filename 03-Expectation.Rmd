...
```{r DiscreteMedian2, echo=FALSE, fig.align="CAPTION", fig.align="center", fig.height = 8}
par( mfrow = c(2, 1))
...
Provided the mgf is defined for some values of $t$ other than zero, it *uniquely* defines a probability distribution, and we can use it to generate the moments of the distribution as described in Theorem~\ref{TM:mgfmoments} below.
...
Replacing $e^{xt}$ by its series expansion (see Sect. \@ref(Series)) in the definition of the mgf gives
\begin{align*}
     M_X(t) 
...
\end{align*}
where $\sum_{y = 1}^\infty a^y = \frac{a}{1 - a}$ for $a < 1$ has been used (Sect. \@ref(Series)); here $a = \exp(t)/3$.

Next consider finding the mgf of $Y = (X - 2)/3$.
From Theorem~\ref{TM:3:mgf1} with $a = 1/3$ and $b = -2/3$, 
\[
   M_Y(t) 
   = \exp(-2t/3) M_X(t/3)
   = \frac{2\exp\{(-t)/3\}}{3 - \exp(t/3)}.
\]
In practice, rather than identify $a$ and $b$ and remember Theorem~\ref{TM:3:mgf1}, problems like this are best solved directly from the definition of the mgf:
\begin{align*}
   M_Y(t) 
   =\text{E}(\exp(tY))
   &= \text{E}(\exp\{t(X - 2)/3\})\\
...
Nonetheles, using Tchebysheff's inequality we can find a useful *bound* to either the probability outside or inside of $\mu \pm k\sigma$.
