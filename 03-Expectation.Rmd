...
Because random variables are *random*, knowing the outcome on any realisation of the random process is not possible.
...
Often we write $\mu = \text{E}(X)$, or $\mu_X$ to distinguish between random variable.
:::
...
\[
   ...
   \end{cases}
\]
as shown in Fig. \@ref(fig:NoMean).
The expected value of $Z$ is
\[
   \text{E}[Z] = \int_1^{-\infty} z \frac{1}{z^2}\, dz 
   = \int_1^\infty \frac{1}{z} =  -\log z \Big|_1^\infty.
\]
However, $\displaystyle\lim_{z\to\infty} \log z \to \infty$.
This expected value is not defined.
:::

```{r NoMean, echo=FALSE, fig.align="center", fig.cap="The probabilityfunction for the random variable Z"}
z <- seq(1, 5, 
         length = 100)
z2 <- seq(5, 6, 
         length = 100)
plot(x = z,
     ...
     main = "The probability function for Z")
lines(x = z2,
      y = z2^(-2),
      lwd = 2,
      lty = 2)
...
While means can be expressed in terms of mathematical expectation, mathematical expectation is a more general concept.
...
* For a discrete random variable $X$ with pmf $p_X(x)$:
  \[
     \text{E}(g(X)) = \sum_{x\in R_X} g(x) p_X(x).
  \]
...
If $Y = g(X)$, write $\mu_Y = \text{E}(Y) = \text{E}(g(X))$.
...
(The proof in the continuous case replaces the probability function with a pdf, and summations with integrals.)
...
\]
where we have used that $\sum_x p_X(x) = 1$.
:::


:::{.example #ExpectationFunction name="Expectation of a function of a random variable"}
Consider the random variable $Y = 2x$ where $X$ is defined in Example \@ref(exm:ExpectationContinuous).
...
Two general features of a distribution are the:
...
The mean is the *balance point* of the distribution:  the mean deviation of the random variable from $\mu$ is zero.
...
We have also seen situation where the mean is  NOT DEFINED/INFINIGE (Example \@ref(exm:InfiniteMean)).
...
* at which $p_X(x)$ attains its maximum (for the discrete case); or 
...
If $X$ is a discrete random variable with probability function defined in Table \@ref(tab:ProbDeg1r) (and shown in Fig. \@ref(fig:DiscreteMedian)).
We see this distribution is bimodal (i.e., has two modes), with modes at 3 and 4.
...
```{r DiscreteMedian, echo=FALSE, fig.cap="Finding the median, when it is not unique", fig.align="center", fig.height = 8}
par( mfrow = c(2, 1))
...
:::{.example #ExpectationContinuous name="Expectation for continuous variables"}
Let $X$ be a discrete random variable with the probability function shiown in Table \@ref(tab:ProbDeg2r) (and Fig. \@ref(fig:DiscreteMedian2)).
...
```{r DiscreteMedian2, echo=FALSE, fig.align="center", fig.cap="The probability function and distribution function for $H$", fig.height = 8}
par( mfrow = c(2, 1))
...
Apart from the mean, the most important description of a random variable is the *dispersion*: How much the values of the random variable are dispersed.
The most important measure of dispersion is the *variance*.


### The variance and standard deviation {#VarianceStdDev}

The *variance* can be expressed as a function of a random variable.
(More correctly, we should say "the variance of the distribution of the random variable").

The variance is a measure of how variable or dispersed the values of a random variable are.
...
The variance of $X$ is commonly denoted by $\sigma^2$, or $\sigma^2_X$ if it is necessary to distinguish amongst variables.
:::


The variance is the *expected value* of the the squared distance of the vaklues of the random variable from the mean, weighted by the probability function.

The unit of measurement for variance is the original ($x$) unit of measurement *squared*.
That is, if $X$ is measured in metres, the variance of $X$ is in $\text{metres}^2$.

Describing the 'spread' in terms of the original units is more natural, by taking the square root of the variance.
...
Using the variance is less popular than the standard deviation in practice to describing dispersion.
In theoretical work, however, the variance is easier to work with than standard deviation (due to the square root), and the variance, rather than standard deviation, features in many results throughout the study of statistics.
...
\[
   \mu = \text{E}(X) = \sum x\Pr(X = x) = (1 + 2 + 3 + 4 + 5 + 6 )/6 = 7/2.
\]
...
\begin{align*}
   ...
   &= \left.-\frac{x^4}{4} + \frac{2x^3}{3} - \frac{x^2}{12}\right|_0^2 = 1.
\end{align*}
To use the computational formula,
\begin{align*}
   \text{E}(X^2)
   &= \int_0^2 x^2(-x^2 + 2x - 1/6)\, dx \\
   &= \left.-\frac{x^5}{5} + \frac{x^4}{2} - \frac{x^3}{18}\right|_0^2 = \frac{52}{45},
\end{align*}
and so
\[
...
\begin{align*}
   ...
   &= \left.-\frac{x^5}{5} + x^4 - \frac{31}{18}x^3 + \frac{7}{6}x^2 - \frac{1}{6}x\right|_0^2 = 7/45.
\end{align*}
...
NOTES ABOUT USE OF COMP FORMULA IN COMPUTING, AND ROUND OFF

The variance represents the expected value of the squared distance of the values of the random variable from the mean.
The variance can never be negative, and is only zero when all the values of the random variable are identical (that is, there *is* no variation).

GIVE SOME ITEMIZED FACTS ABOUT VAR? SOME REPEATED:


:::{.example #InfiniteVar name="Infinite variance"}
In Example \@ref(exm:InfiniteMean), we found $\text{E}(X)$ was not defined.
...
:::{.definition #MAD name="Mean absolute difference (OR: deviation??)"}
...
\begin{align*}
   ...
   &= \text{var}(X) \\
   &= \sum(x-\mu)^2 \Pr{X=x}\\
   &= \frac{1}{6}\left[ \left(1 - \frac{7}{2}\right)^2 + \left(2 - \frac{7}{2}\right)^2 +
       \dots + \left(6 - \frac{7}{2}\right)^2\right]\\
   &= \frac{70}{24}.
\end{align*}
...
\begin{align*}
   \text{E}(|X - \mu|) 
   &= \sum |x - \mu| \Pr(X = x)\\
   &= \frac{1}{6}\left[ \left|1 - \frac{7}{2}\right| +\left|2 - \frac{7}{2}\right|+
      \dots + \left|6 - \frac{7}{2}\right|\right]\\
   &= 1.5
\end{align*}
:::
...
* the mean $\mu'_1 = \mu$ is the *first moment about the origin*, or the *first raw moment*; and
...
The ratio $(\mu_r)^p / (\mu_p)^r$ is such an expression, and the simplest form of it is $\mu^2_3/\mu_2^3$.
...
:::{.example #Skewness nane="Skewness"}
Consider the random variable $X$ with probability function
\[
   f_X(x) = 
   \begin{cases}
      \frac{2x}{5} - \frac{2}{5} & \text{for $1 \le x < 2$}\\
      \frac{x}{10} + \frac{3}{5} & \text{for $2 \le x < 6$}\\
      0                          & \text{elsewhere}.
   \end{cases}
\]
To compute the skewness, we first compute the necessary moments
\[
   x.
\]
...
```{r ProbFunSkewRight, echo=FALSE, fig.align="center", fig.cap="The probability function"}
plot( x = c(-1, 8),
      y = c(0, 0.5),
      type = "n",
      las = 1,
      main = "The probability function",
      xlab = expression(italic(x)),
...
WAIT!!!     The interpretation of 'peakedness' is somewhat vague apparently.
...
Notice that once the initial computations to find $\mu'_r$ have been done, the evaluation of any raw moment is simple.
:::
...
So far, the distribution of a random variable has been described using a probability function, pdf or a distribution function.
...
There are other uses of the moment generating function that are seen later (see Sect. \@ref(TransformationMoments)).


:::{.definition #MGF name="Moment generating function (MGF)"}
The *moment generating function* (or MGF) $M_X(t)$ of the random variabale $X$ is defined as:

* Discrete case:
\[
   M_X(t)  = \text{E}(\exp(tX)) = \sum_x \exp(tx) p_X(x).
\]
* Continuous case:
\[
   M_X(t)  = \text{E}(\exp(tX)) = \int_{-\infty}^\infty \exp(tx) f_X(x).
\]
:::
...
Provided the mgf is defined for some values of $t$ other than zero, it *uniquely* defines a probability distribution, and we can use it to generate the moments of the distribution as described in Theorem \@ref(thm:Moments) below.
...
Consider the pf of $X$, the outcome in tossing a fair die (Example \@ref(exm:VarianceDice)).
The mgf of $X$ is
\begin{align*}
   M_X(t)
...
Replacing $e^{xt}$ by its series expansion (see Sect. \@ref(UsefulSeries)) in the definition of the mgf gives
\begin{align*}
     M_X(t) 
     & = {\sum_x} (1 + xt + \frac{x^2t^2}{2!} + \dots) \Pr(X = x)\\
     & = 1 + \mu'_1t + \mu'_2 \frac{t^2}{2!} +\mu'_3 \frac{t^3}{3!} + \dots
\end{align*}
To obtain the result, the order of summations has been interchanged, and in cases where this interchange is permitted, the $r$th moment of a distribution about the origin is the coefficient of $t^r/r!$ in the series expansion of $M_X(t)$:
\begin{align*}
     M_X'(t) 
     & = \sum_x x\,e^{xt}\Pr(X = x)\\
     M''_X(t) 
...
\]
On setting $t = 0$ we find $M'_X(0) = \text{E}(X)$, $M''_X(0) = \text{E}(X^2)$, and for each positive integer $r$,
\begin{equation}
     M^{(r)}_X (0) = \text{E}(X^r).
\end{equation}
...
The moment generating function can be used to derive the distribution of a function of a random variable (see Sect. \@ref(SEC:transforms:mgf)).
...
:::{.theorem #MGFIndependent name="MGF of independent random variables"}
...
\end{align*}
where $\sum_{y = 1}^\infty a^y = \frac{a}{1 - a}$ for $a < 1$ has been used (Sect. \@ref(UsefulSeries)); here $a = \exp(t)/3$.
...
It can be proven that the mgf (if it exists) completely determines the distribution of a random variable.
...
\end{align*}
Putting $\sqrt{c} = k\sigma$, we obtain (\@ref(eq:Tchebysheff)).
:::
...
2. Compute the mean and variance of $M$.
3. Find the mgf for $M$.
4. Compute the mean and variance of $M$ from the mgf.
:::
...
2. Find $\text{E}((X + 3)^2)$ using the previous answer.
...
1. Show that the mean of the distribution does not exist.
