...
The expected value of $X$ is
\[
   \text{E}(X) = \int_0^2 x(-x^2 + 2x - 1/6)\, dx = 1.
\]
To use the computational formula,
\[
   \text{E}(X^2) = \frac{52}{45},
\]
and so $\text{var}(X) = \text{E}(X^2) - \big(\text{E}(X)\big)^2 = 7/45$.
...
\begin{align*}
   ...
   &= \text{E}\big((X - 1)^2\big)\\
   &= \int_0^2 (x-1)^2(-x^2+2x-1/6)^2\,dx = 7/45.
\end{align*}
...
* $\mu'_r = \text{E}(X^r) = \sum_X x^r p_X(x)$ for the discrete case;
* $\mu'_r = \text{E}(X^r) =  \int_{-\infty}^\infty x^r f_X(x)$ for the continuous case.
:::
...
* $\mu_r = \text{E}((X-\mu)^r) = \sum_x (x-\mu)^r p_X(x)$ in the discrete case;
* $\mu_r = \text{E}((X-\mu)^r) = \sum_x (x-\mu)^r p_X(x)$ in the continuous case.
:::
