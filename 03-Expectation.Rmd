...
Using Theorem~\ref{TM:3.1} with $a = 2$ and $b = 0$, the value of $\text{E}(Y)$ is
\[
...
An important special case of a  function of a random variable is the *variance* of a random variable (or, if you like, of the distribution of the rv).
...
NOTES ABOUT USE IN COMPUTING AMD ROUND OFF

The variance represents the mean of the squared distance of the values of the rv from the mean.
...
GIVE SOME ITEMIZED FACTS ABOUT VAR? SOME REPETAED:
...
If $X$ is a discrete random variable with probability function defined in Table \@ref(tab:ProbDeg1r).
...
:::{.example #ExpectationContinuous name="Expectation for continuous variables"}
Let $X$ be a discrete random variable with probability function (Table \@ref(tab:ProbDeg2r)).

The mode of this distribution is 3.

The median is $\nu = 3$ since this is the only value of $X$ that will satisfy Definition \@ref(def:Median).
The probability function and distribution function are shown in  Fig. \@ref(fig:DiscreteMedian2).
:::
