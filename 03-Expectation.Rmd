...
By Definition \@ref(def:ExpectationFunction) with $g(X) = aX + b$,\medskip
\[
...
Simulate this is **R** to confirm your answer.
:::
...
In Ex. \@ref(exr:PooledTest), the probability distribution of the random variable $X$ was deduced for $n = 4$ and $p = 0.1$, where $X = 0$ means the pooled test returns a negative result, and $X = 1$ means the pooled test returns a positive result.

1. What is the expected number of tests needed when $n = 4$?
1. What is the variance of the number of tests needed when $n = 4$?
1. How many tests are expected to be saved in that pool of four?
1. Compute the expected number of tests needed for values of $n$ from $n = 2$ to $n = 14$.
   How many tests are expected to be saved from each pool of size $n$?
   Comment and use graphs if necessary.
1. Suppose a test costs $15.
   What is the cost-saving for using the pooled-testing method with $n = 4$, if 200 people must be tested?
:::
