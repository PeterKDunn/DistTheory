...
\begin{equation}
   f_X(x) = 
   \frac{1}{2\pi} \int_{-\infty}^{\infty} M_X(it) \exp(-ity)\, dx,
...
\begin{align*}
 f_X(x) 
 &= \frac{1}{2\pi} \int_{-\infty}^{\infty} \exp(t^2/2) \exp(-ity)\, dx,\\
 &= \frac{1}{2\pi} \int_{-\infty}^{\infty} \exp(t^2/2)\left[ \cos(-ty) + i\sin(-ity)\right]\, dx,
\end{align*}
since $y\in\mathbb{R}$ and $t\in\mathbb{R}$.
Extracting just the real components:
\begin{align*}
 f_X(x) 
 &= \frac{1}{2\pi} \int_{-\infty}^{\infty} \exp(t^2/2) \cos(-ty) \, dx\\
