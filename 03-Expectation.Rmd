...
For a *symmetric* distribution, show that the odd central moments are zero.
:::


:::{.exercise #CharFunction}
The *characteristic function* of a random variable $X$, denoted $\varphi(t)$, is defined as $M_X(it) = \text{E}[\exp(i t X)]$, where $i = \sqrt{-1}$.
...
1. Suppose $R_X(t) = \text{E}\left [(1 - tX)^{-1} \right]$ is called the geometric generating function of $X$.
