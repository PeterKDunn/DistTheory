...
In statistics and probability, we rarely work with arbitrary distributions.
Some distributions arise so frequently in practice that they are given names and are studied in detail. 
...
* the *discrete uniform distribution* models situations where every outcome is equally likely, such as rolling a fair die or drawing a card at random from a well-shuffled deck.
* the *Bernoulli* and *binomial distributions* model the number of successes in repeated independent trials, relevant whenever we count how many times an event occurs in a fixed number of attempts, such as the number of defective items in a production run or the number of patients responding to a treatment.
* the *geometric* and *negative binomial distributions* model waiting times: how long until a first success, or until a fixed number of successes occur, arising naturally in quality control, reliability, and queuing problems.
* the *Poisson distribution* models the number of rare events occurring in a fixed interval of time or space; these are used widely in insurance, telecommunications, epidemiology, and traffic modelling.
* the *hypergeometric distribution* models the number of successes when sampling without replacement from a finite population, applicable in quality inspection, ecology, and clinical trials where the population is small enough that each draw meaningfully changes the composition of what remains.
...
```{r DiscreteUniform, echo=FALSE, fig.align="center", fig.cap="The probability function for the discrete uniform distribution $\\text{Uniform}(a, b)$.", fig.width=5, fig.height=3.5, out.width='50%'}
x <- c(1:4, 7)
y <- rep(1, length(x))
...
The discrete uniform distribution is rarely useful by itself, except in trivial scenarios like rolling dice and tossing coins, but forms the foundation for other distributions in statistics.


:::{.example #UsingDiscreteUniform name="Using the dicrete uniform distributuon"}
...
$$
  ...
  \end{cases}
$$
The random-number generator produces an integer value, say\ $U$, such that $U\sim \text{Discrete Uniform}(1, 100)$.
...
$$
   ...
   \begin{cases}
    ...
    \end{cases}
$$
so that the parameter\ $p$ represents the probability of $x = 1$, called a 'success' (while $x = 0$ is called a 'failure').
More succinctly:
\begin{equation}
...
A plot of the probability function for a Bernoulli distribution is very simple, and shown in Fig. \@ref(fig:Bernoulli).
...
```{r Bernoulli, echo=FALSE, fig.align="center", fig.cap="The probability function for the Bernoulli distribution $\\text{Bernoulli}(p = 0.35)$.", fig.width=6, fig.height=3.5, out.width='70%'}
par(mfrow = c(1, 2))
...
### Derivation and definition {#GeometricDerivation}
...
The mean number of failures made is $\operatorname{E}[Y] = \{r(1 - p)\}/p = \{10 \times (1 - 0.15)\}/0.15 \approx  56.66667$.
...
::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
The four **R** functions for working with the negative binomial distribution are based on the paramaterisation in Eq.\ \@ref(eq:NegativeBinomialPMF), and have the form `[dpqr]nbinom(., size, prob)` (see App.\ \@ref(UseRDistributions)):\index{dnbinom()@\texttt{dnbinom()}}
...
\begin{align*}
   M_X(t) = \operatorname{E}[e^{tX}] 
     ...
     &= e^{-\lambda} \sum^\infty_{x = 0} \frac{(\lambda\, \exp t)^x}{x!} \\
     &= e^{-\lambda} \left[ 1 + \lambda\, \exp t + \frac{(\lambda\,\exp t)^2}{2!} + \dots \right]\\
...
This distribution is useful when there is overdispersion relative to the binomial.

The *Conway–Maxwell–Poisson* (COM-Poisson)\index{Conway–Maxwell–Poisson} is a flexible generalisation of the Poisson that can handle both overdispersion and underdispersion, and is increasingly used for modelling count data.
...
2. Write some **R** code to simulate $N = 1\,000$ draws of $U$, map them to $Y$, and output the empirical relative frequencies alongside the theoretical probabilities $p_Y(y)$.
:::
...
3. Write some **R** code to simulate $N = 1\,000$ draws of $U$, map them to $Y$, and output the empirical relative frequencies alongside the theoretical probabilities $p_Y(y)$.
:::
