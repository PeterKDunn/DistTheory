...
```{r echo=FALSE, fig.cap="Empirical probability mass functions from the call-center modelling. Left: the number of calls per hours. Right: the handling time per hour.", out.width='100%', fig.width=8, fig.align="center" }
par(mfrow = c(1, 2))
...
1. A fair die is rolled once. Let X be the outcome.
1. A student answers a multiple-choice question correctly or incorrectly. Let X indicate correctness.
1. The number of calls received by a call centre in a 10-minute interval.
:::

Answer 1
Discrete Uniform — all outcomes 1–6 are equally likely.
...
A hospital records the number of emergency arrivals between 2pm and 3pm on a typical day.
A factory inspects 12 items selected at random from a large production batch without replacement and counts how many are defective.
A machine is tested repeatedly until the 4th failure occurs, and the number of tests required is recorded.
Answer 1
...
The number of emails received by a helpdesk in a day, where emails arrive independently at a constant average rate.
A biased coin is flipped until the first head appears; let X be the number of flips required.
From a shipment of 200 items, 10 are defective. A sample of 20 items is selected with replacement, and the number of defectives is counted.
Answer 2
