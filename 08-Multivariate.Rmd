...
Hence, the range space of $(X, Y)$, written $R_{X \times Y}$, is a subset of the Euclidean plane. 
...
So far, the bivariate cases have included two discrete or two continuous random variables.
However, in it also possible for one variable, say\ $X$, to be continuous and the other, say\ $Y$, to be discrete.


:::{.definition #BivariateProbFunctionCont name="Continuous bivariate distribution function"}
Let $(X, Y)$ be a random vector where\ $X$ is **continuous** and\ $Y$ is **discrete** with possible values $\{y_1, y_2, \dots\}$.

The **joint density–mass function** of $(X, Y)$ is defined by
$$
  f_{X, Y}(x, y_j) 
...
- $p_Y(y_j) = \Pr(Y = y_j)$ is the probability *mass* function of\ $Y$,
- $f_{X \mid Y}(x \mid y_j)$ is the conditional *density* of\ $X$ given\ $Y = y_j$.
:::
...
1. **Nonnegativity**:
$$
  f_{X, Y}(x, y_j) \geq 0 \quad \text{for all valid } (x, y_j).
$$

2. **Normalization**:
$$
  \sum_{j} \int_{-\infty}^{\infty} f_{X,Y}(x, y_j)\, dx = 1.
$$
...
$$
Conditional on $Y = 1$, $X \sim \text{Uniform}(0, 1)$, so that
$$
  f_{X \mid Y}(x \mid 1) = 
  \begin{cases}
    1 & 0 \le x \le 1, \\
  ...
  \end{cases}
$$
and onditional on $Y = 2$, $X \sim \text{Uniform}(0,2)$, so that
$$
  f_{X \mid Y}(x \mid 2) 
  = 
  \begin{cases}
...
This describes a **mixed distribution**, where probabilities are computed by **integrating over the continuous variable**\ $X$ and *summing over the discrete variable*\ $Y$.


Let $(X, Y)$ be a $2$-dimensional random variable where\ $X$ is a continuous random variable, and\ $Y$ is a discrete random variable.
The *mixed joint probability function*, $f_{X, Y}$, 
$$
  f_{X, Y}(x, y_j) 
  = f_{X\mid Y}(x\mid y_j) \cdot \Pr_Y(y_j),
$$
where $\Pr_Y(y_j) = \Pr(Y = y_j)$ and $f_{X\mid Y}(x\mid y)$ is a conditional density of\ $X$ given\ $Y = y_j$, such that 
\begin{align}
   f_{X, Y}(x, y) 
   &\geq 0, \text{ for all } (x, y) \in R, \\
   \int \!\! \int_{R} f_{X, Y}(x, y) \, dx \, dy 
   &= 1.
\end{align}
:::
