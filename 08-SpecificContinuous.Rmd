...
The four **R** functions for working with the continuous uniform distribution have the form `[dpqr]unif(min, max)` (see App.\ \@ref(UseRDistributions)).
:::
...
::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
The four **R** functions for working with the normal distribution have the form `[dpqr]norm(mean, sd)` (see App.\ \@ref(UseRDistributions)).
...
::: {.softwareBox .software data-latex="{iconmonstr-laptop-4-240.png}"}
The four **R** functions for working with the exponential distribution have the form `[dpqr]exp(rate)` where `rate`${}= \lambda = 1/\beta$ (see App.\ \@ref(UseRDistributions)).
:::
...
The four **R** functions for working with the gamma distribution have the form `[dpqr]gamma(shape, rate, scale = 1/rate)`, where `shape`${}= \alpha$ and `scale`${}= \beta$ (see App.\ \@ref(UseRDistributions)).
:::
...
3. The median of\ $X$ is $\exp(\mu)$.
4. The mode of\ $X$ is $\exp(\mu - \sigma^2)$.
5. The MGF of\ $X$ does not exist for $t > 0$.
...
The four **R** functions for working with the beta distribution have the form `[dpqr]beta(shape1, shape2)`, where `shape1`${}= m$ and `shape2`${}= n$ (see App.\ \@ref(UseRDistributions)).
:::
...
The percentage taller than $173\cms$ is:
...
Now, we are seeking women *or* men over $173\cms$ tall (Fig.\ \@ref(fig:HeightSimHistograms), right panel):
