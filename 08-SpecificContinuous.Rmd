...
The distributions discussed in this chapter and standard and commonly-used continuous distributions.

TALK ABOUT WHERE USEFUL.

* Continuous uniform
* Normal: Data defined over the real line (or a long way from 0 so it makes no practical difference)
* Exponential: Strictly positive data, right-skewed; waiting times between Poisson arrivals; time between memoryless events.
* Gamma: Strictly positive data, right-skewed. Insurance claims; rainfall 
* Beta: when modelling proportions (or probabilities), such as cloud cover, fractional resource consumption, etc.

Other useful continuous distributions include:

* The $t$-distribution: sampling; normal-like, but heavy tails
* The $\chi^2$: sampling; sums of squared normal distributions 
* The $F$-distribution: sampling
* The Cauchy: normal-like, but heavy tails; ratios of independent normal variables.
* The Weibull: lifetimes; reliability analysis; survival analysis
* The Pareto: 80/20 rule. Wealth, income, city sizes, heavy tailed.
* Arcsine: lots of prob near 0, 1 (??)
* von Mises: anglel defined on $[-\pi, \pi)$; circular normal distribution; wind directions, animal movements, etc.
* Lognormal; income, stock prices (Black--Scholes equation)
* inverse Gaussian: very skewed right; positive values; survival times

```{r echo=FALSE}
discrete_Table <- array( dim = c(10, 3) )
discrete_Table[, 1] <- c("Arcsine",
                         "von Mises",
                         "inverse Gaussian",
                         "Pareto",
                         "Log-normal",
