...
If $X\sim \text{Gamma}(\alpha, \beta)$, find the distribution of $Y = kX$ for some constant\ $k$.
...
\begin{align*}
   ...
   &= (1 - \beta kt)^{-\alpha}.
\end{align*}
This is just the MGF for random variable\ $X$ with\ $k\beta$ in place of\ $\beta$, so the distribution of\ $Y$ is $\text{Gamma}(\alpha, k\beta)$.
:::
\index{Gamma distribution|)}
