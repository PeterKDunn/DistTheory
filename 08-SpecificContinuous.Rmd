...
The four **R** functions for working with the gamma distribution have the form `[dpqr]gamma(shape, rate, scale = 1/rate)`, where `shape`${}= \alpha$ and `scale`${}= \beta$ (see App.\ \@ref(UseRDistributions)).\index{dgamma()@\texttt{dgamma()}}
:::
...
The *continuous uniform distribution*\index{Continuous uniform distribution} is used to model complete randomness when all values in an interval are equally likely.
Generally, computers can only generate continuous uniform (pseudo-) random numbers; random numbers from other distributions must be produced by transforming (pseudo-) random number produced from a *continuous uniform distribution*.

The *normal distribution*\index{Normal distribution} is the most well-known continuous distribution (often kown as the 'bell-shaped curve')
...
Many natural process are only defined for non-negative values, rather than over the entire real line (such as heights, times and distances).
The *exponential*\index{Exponential distribution} and *gamma* distributions\index{Gamma distribution} are defined over a suitable region.
The *exponential distribution* is right-skewed, and often used for modelling waiting times between Poisson arrivals, and the time between memoryless events.
The *gamma distribution* is more flexible than the exponential distribution, and is often used to model insurance claims, rainfall, and waiting times.
The exponential distribution is a special case of the gamma distribution.

The *log-normal distribution*\index{Log-normal distribution} is also a right-skewed distribution defined on the positive real numbers.
If $\log X\sim N(\mu, \sigma^2)$, then\ $X$ has a log-normal distribution.
...
The chi-squared distribution (Sect.\ \@ref(ChiSquareDistribution)), with $\nu$\ degrees of freedom, is a gamma distribution with $\alpha = \nu/2$ and $\beta = 2$ (Exercise\ \@ref(exr:ChiSquaredGamma)).
The chi-squared distribution appears often in statistics.
The Erlang distribution,\index{Erlang distribution} often used in queuing theory, is a $\text{Gamm}(k, 1/\lambda)$ distribution where\ $k$ is a positive integer.
The Erlang distribution arises as the sum of\ $k$ independent exponential random variables.

The *beta distribution*\index{Beta distribution} is used for modelling proportions (or probabilities) that are not counts of a fixed number (when a *binomial distribution*\index{Binomial distribution} is appropriate), such as cloud cover, fractional resource consumption, etc.

Countless other useful discrete distributions exist; we mentioned a few.
The *Weibull distribution*\index{Weibull distribution} is defined on $[0, \infty)$ (notice this *includes* zero) and is used to model lifetimes and failures, and is used in reliability analysis and survival analysis.

The *von Mises* distribution is used to model angles, and is defined on $[-\pi, \pi)$; it is sometimes called the 'circular normal distribution'. 
...
The *arcsine distribution*\index{Arcsine distribution} is a special case of the beta distribution and hence defined on $(0, 1)$, with probability concentrated near the limits of the range at $x = 0$ and $x = 1$.
...
The *$t$-distribution*\index{t-distribution@$t$-distribution} (Sect.\ \@ref(TDistribution)) is similar to a normal distribution (and defined on the entire real line), but with heavier tails.
The *$\chi^2$ distribution*\index{Chi-squared distribution} (Sect.\ \@ref(ChiSquareDistribution)), with $\nu$\ degrees of freedom and defined on $(0, \infty)$, is related to the sums of independent squared normal distributions.
It is a special case of a gamma distribution with $\alpha = \nu/2$ and $\beta = 2$ (Exercise\ \@ref(exr:ChiSquaredGamma)).
The *$F$-distribution*\index{F-distribution@$F$-distribution} (Sect.\ \@ref(FDistribution)), defined on $(0, \infty)$, is related to the ratio of independent $\chi^2$ distributions.
...
Again, in **R**, random numbers from a specific distribution use functions that start with the letter `r`; for example, to generate random numbers from a normal distribution, use `rnorm()`:\index{rnorm()@\texttt{rnorm()}}
...
Suppose adult males have heights with a mean of\ $175\cms$ with standard deviation of\ $7\cms$, and constitute $44$% of the population.
Now, we are seeking women *or* men over $173\cms$ tall (Fig.\ \@ref(fig:HeightSimHistograms), right panel):\index{rbinom()@\texttt{rbinom()}}\index{sum()@\texttt{sum()}}\index{var()@\texttt{var()}}
...
```{r HeightSimHistograms, echo=FALSE, fig.width=6.5, out.width='90%', fig.height = 3.5, fig.cap="The distribution of heights of women (left panel) and men and women combined (right panel)."}
par( mfrow = c(1, 2))
...
cat("Simulated mean:      ", round(mean(recovery), 2), "\n\n")

cat("--- Pain score ---\n")
cat("Theoretical mean:    ", round(2/(2 + 4) * 10, 2), "\n")
cat("Simulated mean:      ", round(mean(pain), 2), "\n")
cat("Simulated sd:        ", round(sd(pain), 2), "\n\n")

cat("--- Readmission ---\n")
cat("Theoretical rate:    ", 0.15, "\n")
cat("Simulated rate:      ", round(mean(readmit), 3), "\n")
```

Now consider some more complex questions that would be difficult to answer analytically.
For example: What proportion of patients have a recovery time exceeding $7$ days?
...
We may also be interested in the mean recovery time for patients who are readmitted.
...
cat("Mean recovery time, non-readmitted patients:",
...
```{r RecoveryHistograms, echo=FALSE, fig.width=6.5, out.width='90%', fig.height=3.5, fig.cap="Distribution of simulated recovery times (left) and pain scores at discharge (right). The theoretical exponential density is overlaid on the left panel."}
par(mfrow = c(1, 2))
