...
* `dunif(x, min, max)` computes the probability function at $X = {}$`x`;
* `punif(q, min, max)` computes the distribution function at $X = {}$`q`;
...
* `dnorm(x, mean, sd)` computes the probability function at $X = {}$`x`;
* `pnorm(q, mean, sd)` computes the distribution function at $X = {}$`q`;
...
* `dexp(x, rate)` computes the probability function at $X = {}$`x`;
* `pexp(q, rate)` computes the distribution function at $X = {}$`q`;
...
* `dgamma(x, shape, rate)` computes the probability function at $X = {}$`x`;
* `pgamma(q, shape, rate)` computes the distribution function at $X = {}$`q`;
...
The functions for the *scale* paramaterisation have the form `[dpqr]gamma(., shape, scale)`, where `shape`${}= \alpha$ and `scale`${}= \beta = 1/\lambda$ (see App.\ \@ref(UseRDistributions)).\index{dgamma()@\texttt{dgamma()}}\index{Gamma distribution!scale parameterisation}
...
* `dlnorm(x, meanlog, sdlog)` computes the probability function at $X = {}$`x`;
* `plnorm(q, meanlog, sdlog)` computes the distribution function at $X = {}$`q`;
...
* `dbeta(x, shape1, shape2)` computes the probability function at $X = {}$`x`;
* `pbeta(q, shape1, shape2)` computes the distribution function at $X = {}$`q`;
