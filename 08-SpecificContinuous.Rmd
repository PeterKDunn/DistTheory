...
Because\ $X$ is strictly positive and right-skewed, the log-normal is a natural model for quantities such as incomes, rainfall amounts, and survival times
...
The mode is found by differentiating $f_X(x)$ and setting to zero, giving $x^* = \exp(\mu - \sigma^2)$.
...
The *gamma distribution* is more flexible than the exponential distribution, and is often used ot model insurance claims, rainfall, and waiting times.
...
The *log-normal distribution*\index{Log-normal distribution} is the a right-skewed distribution defined on the positive real numbers.
...
Many distributions are special cases of the gamma distribution.
