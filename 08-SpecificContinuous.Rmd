...
The *Erlang distribution* is a $\text{Gamma}(k, 1/\lambda)$ distribution where =\ $k$ is a posotove integer, and arises as the sum of\ $k$ independent exponential random variables.
...
The *Pareto distribution*\index{Pareto distribution} is defined on $(x_m, \infty)$ for some minimum value $m > 0$, and has a heavy right tail; it is used to model income distributions, insurance losses, and other phenomena where extreme values are more common than a normal or exponential distribution would suggest.
