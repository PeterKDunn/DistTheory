...
We will focus on the case where $z^* = 0$.
...
The distribution of the Poisson sum of gamma distributions has a probability function that cannot be written down in closed form, but can be expressed as an infinite sum:
\begin{align*}
  f_X(x) 
...
MASS::truehist(Y,
               ...
               ylab = "Probabiity fn",
               xlab = expression(Observed~variable~italic(X)) )
#MASS::truehist(Y, add=TRUE, col="grey") CANNOT ADD
...
Suppose we seek to model the monthly rainfall at given location for a certain month, where about\ $35$% of months record zero rainfall, and the mean monthly rainfall (when rain is recorded) is about $100\mms$.
This scenarios could be modelled using a censored model.

We could suppose a (censored) normal distribution for the monthly rainfall.
The latent variable\ $Y\sim N(\mu, \sigma)$ perhaps, then rainfall.
...
mean_R <- 100
sd_R <- 20
threshold_R <- mean_R  + qnorm(0.35) * sd_R
...
For example, the rainfall could be modelled using a censored model (e.g., @glasbey1997rainfall).
The latent variable could represent how favourable the conditions are for rainfall.

BUT NOT INDEPENDENT!! SO USE MONTHLY?

*Monthly* hurdle: @kenabatho2012stochastic

*Monthly* censored: @sanso2000nonstationary

*Monthly* P--G: @hasan2010simple
