...
The characteristic function provides the most elegant proof of the Central Limit Theorem.  
The key steps are:

1. Take the log of $\varphi_{\bar X_n}(t)$ (the CF of the standardised sample mean).
2. Expand $\log \varphi_X(t/\sqrt{n})$ in a Taylor series.
3. Show that as $n \to \infty$ the result converges to $-t^2/2$.
4. Conclude $\bar X_n \overset{d}{\to} N(0,1)$ by the continuity theorem for CFs.

This approach works even for distributions with no MGF, which is why it is the
standard proof in mathematical statistics courses.
