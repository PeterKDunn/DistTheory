...
:::{.exercise #SimHurdle}
For zero-modified\index{Zero-modified distribution!Gamma modified} Gamma(2,1)-distribution, $\pi=0.5$: derive $E[\bar{Y}_n]$ and $\text{Var}(\bar{Y}_n)$;
...
:::{.exercise #SimCPG}
For  compound Poisson--gamma distributions\index{Compound Poisson--gamma distribution} with $\lambda=1.5$, $\alpha=2$ and $\beta=1$), state $\operatorname{E}[\bar{S}_n]$ and $\operatorname{Var}(\bar{S}_n)$.
Simulate for $n \in \{5,30,150\}$; overlay normal approximation.
:::
