...
#      ylim = c(0, 0.4),
      ...
      ylab = "Density",
      las = 1,
      main = expression( bold(The~chi^2*~distribution: df==1)))
...
plot( x = xx,
      ...
      ylab = "Density",
      las = 1,
      main = expression(bold(The~chi^2*-distribution: df==6)))
...
plot( x = xx,
      ...
      ylab = "Density",
      las = 1,
      main = expression( bold(The~chi^2*-distribution: df==20)))
```
...
The distribution of the sample mean\ $\bar{X}$ can be approximated by the normal distribution with mean of $\mu = 200$ and standard error of $\sigma_{\bar{X}} = \sigma/\sqrt{n} = 15/\sqrt{36} = 15/6$, according to the CLT.
That is, $\bar{X} \sim N(200, (15/6)^2)$.
Now
$$
   \Pr(\bar{X} \ge 204) \approx \Pr_N\left(Z \ge \frac{204 - 200}{15/6}\right)
   = \Pr(Z \ge 1.6).
$$
In **R**:
...
1. Derive $\operatorname{E}[\bar{Y}_n]$ and $\operatorname{var}(\bar{Y}_n)$.
1. Simulate the distribution of $\operatorname{E}[\bar{Y}_n]$ for $n \in \{10, 50, 200\}$, overlayed with the appropriate normal approximation.
   Comment.
:::
