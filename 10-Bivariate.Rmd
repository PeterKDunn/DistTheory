...
$$
This is a joint density function; the required probability density function for just $Y_1$ is
\begin{align*}
  f_{Y_1}(y_1) 
...
```{r BivariateContinuousXformCoVSupportSingle, echo=FALSE, fig.align="center", fig.cap="The support in the $(y_1, y_2)$-plane (left), and the marginal distribution of $Y_1$ (right).", out.width='100%', fig.width=8}
par(mfrow = c(1, 2))
...
For $m \in \{0, 1, \ldots, \max(n_1, n_2)\}$:
$$
  F_M(m) = \Pr(X_1 \leq m,\, X_2 \leq m) = F_{X_1}(m) \cdot F_{X_2}(m)
$$
by independence (Fig.\ \@ref(fig:BivariateDiscreteBinomialMax), left panel). 
The probability mass function (Fig.\ \@ref(fig:BivariateDiscreteBinomialMax), right panel) is then
$$
  p_M(m) = F_M(m) - F_M(m - 1)
  = 
  F_{X_1}(m)F_{X_2}(m) - F_{X_1}(m - 1)\cdot F_{X_2}(m - 1).
$$
This does not simplify to a standard named distribution in general; the distribution function method can yield the distribution even when it has no closed-form family name.
:::


```{r BivariateDiscreteBinomialMax, echo=FALSE, fig.lign="center", fig.cap="A bivariate transformation: the maximum of two independent binomial distributions. Left: The DF; right: the PMF.", fig.width = 9, fig.height = 4, out.width="100%"}
n1 <- 10; n2 <- 8; p <- 0.4
m_vals <- 0:max(n1, n2)
...
For two random *continuous* variables\ $X_1$ and\ $X_2$  with a joint probability density function $f_{X_1, X_2}(x_1, x_2)$ defined on the two-dimensional set of points\ $R^2_X$, the distribution of $Y = u(X_1, X_2)$ (a scalar function of\ $X_1$ and\ $X_2$), find
$$
...
Consider Exercise\ \@ref(exr:TransformBivariate).
Find the marginal distribution of\ $Y_1$. 
:::


:::{.exercise #BivariateContinuousXformCoVExercise}
Consider Exercise\ \@ref(exr:BivariateContinuousXformCoV).
Find the marginal distribution of\ $Y_1$. 
:::
::::
