...
## Types of sets {#FiniteInfiniteSets}
...
No bijection exists with $\mathbb{N}$.
:::
...
plot(0, 0, 
     ...
     ylab = "", 
     axes = FALSE,
     main = expression( bold(Intersection~of~italic(A)~and~italic(B)) ) )
...
plot(0, 0, 
     ...
     ylab = "", 
     axes = FALSE,
     main = expression(bold(Union~of~italic(A)~and~italic(B)) ) )
...
plot(0, 0, 
     ...
     ylab = "", 
     axes = FALSE,
     main = expression(bold(Complement~of~italic(A)) ) )
...
plot(0, 0, 
     ...
     ylab = "", 
     axes = FALSE,
     main = expression(bold(Proper~subset~of~italic(A)) ) ) 
...
plot(0, 0, 
     ...
     ylab = "", 
     axes = FALSE,
     main = expression(bold(Difference~italic(A)~~"\u2216"~italic(B) ) )) 
...
plot(0, 0, 
     ...
     ylab = "", 
     axes = FALSE,
     main = expression(bold(Difference~italic(B)~~"\u2216"~italic(A) ) )) 
...
plot(0, 0, 
     ...
     main = expression( atop(
               bold(Symmetric~difference~of~italic(A)~and~italic(B)),
...
Then use **R** to find the intersection and union:\index{intersect()@\texttt{intersect()}}\index{union()@\texttt{union()}}\index{sort()@\texttt{sort()}}
...
The function `setdiff()` finds the difference between two sets:\index{setdiff()@\texttt{setdiff()}}
...
\begin{align*}
  A &= \{\text{Countries whose names have `a' as the } \textit{second} \text{ letter}\};\\
...
The list of country names are in the **R** vector `list_countries()`:\index{head()@\texttt{head()}}

```{r}
head( list_countries() ) # Show the first five values only
```

First, we find the countries whose names have an `a` as the second letter (assuming it is always lower case); that is, we identify the elements in set\ $A$:\index{which()@\texttt{which()}}\index{substr()@\texttt{substr()}}\index{length()@\texttt{length()}}
...
Now we can find the countries whose names have an `a` as the second and as the last letters by finding the intersection between the two sets:\index{intersect()@\texttt{intersect()}}
