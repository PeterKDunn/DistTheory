...
$\operatorname{E}[X]$ is finite only if, and only if, $\operatorname{E}[\,|X|\,] < \infty$.\index{Expected value!finite}
...
More generally, $\operatorname{E}[X]$ exists (and is possibly infinite) provided $\operatorname{E}[X^+]$ and $\operatorname{E}[X^-]$ are not *both* infinite. If both are infinite, then $\operatorname{E}[X]$ *does not exist* (Example\ \@ref(exm:ExpectationDoesNotExist)).\index{Expected value!does not exist}
:::
...
\begin{align*}
  \sum_{n=1}^\infty x_n\,p_X(x_n)
    &= \Big[(x_1 p_X(x_1) + x_3 p_X(x_3)) + x_2 p_X(x_2)\Big]\\
    &\quad + \Big[(x_5 p_X(x_5) + x_7 p_X(x_7)) + x_4 p_X(x_4)\Big] + \cdots\\
...
\begin{align*}
  \sum_{n=1}^\infty x_n\,p_X(x_n)
  &= \Big[(x_1 p_X(x_1) + x_2 p_X(x_2) ) + x_4 p_X(x_4\Big] \\
  &\quad + \Big[(x_3 p_X(x_3) + x_6 p_X(x_6) ) + x_8 p_X(x_8)\Big] + \cdots\\
...
The expected value of $\operatorname{E}[Z]$ is *infinite*, or is *not finite*.
:::
\index{Expected value|)}
...
```{r ConvexConcave, echo=FALSE, fig.align="center", fig.cap="A convex function (left) and a concave function (right). The dashed lines are the tangents at a fixed point $x = a$.", out.width='100%', fig.width=7.5, fig.height=3.5}
par(mfrow = c(1, 2))
