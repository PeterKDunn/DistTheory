...
Mathematical expectation goes far beyond just computing means, but since this idea is easily understood, we begin here.


## Expected values {#ExpectedValue}

The *expected values* of a random variables is a numerical expression of the average value that a random variable can take.
...
$\operatorname{E}[X]$ is finite if, and only if, $\operatorname{E}[\,|X|\,] < \infty$.\index{Expected value!finite}

If $X\ge 0$ (or $X\le 0$) and $\operatorname{E}[X] = \infty$, the expectation is *infinite* (Example\ \@ref(exm:InfiniteMean)).\index{Expected value!infinite}

More generally, $\operatorname{E}[X]$ exists (and is possibly infinite) provided $\operatorname{E}[X^+]$ and $\operatorname{E}[X^-]$ are not *both* infinite (where $X^+ = \max(X, 0)$ and $X^- = \max(-X, 0)$; both are non-negative). 
If both are infinite, then $\operatorname{E}[X] = \operatorname{E}[X^+] - \operatorname{E}[X^-]$ *does not exist* (Example\ \@ref(exm:ExpectationDoesNotExist)).\index{Expected value!does not exist}
:::
...
$$
  ...
  \end{cases}
$$
so $p_W(w) = 1/2$ for $w = 0$, and $f_W(w) = \exp(-2w)$ for $w > 0$.
...
\begin{align*}
  \operatorname{E}[T] &= \sum_{t=0}^{1} t \cdot p_T(t) \\[4pt]
  &
  \begin{array}[t]{r@{{}={}}l@{{}+{}}l}
  ...
  \end{array}
\end{align*}
Of course, $0.5$\ tails can never actually be observed in practice on one toss; however, rounding up (or down), and saying the expected number of tails on one toss of a coin is one (or zero), would be silly.
...
:::{.example #ExpectationDoesNotExist name="Expectation does not exist"}
Let\ $X$ be a discrete random variable such that
$$
  X = (-1)^{n+1} \, \frac{2^n}{n}\quad\text{for $n = 1, 2, 3, \dots$}.
$$
The probability mass function is
$$
  p_X(x) = 2^{-n}\quad\text{for $n = 1, 2, 3, \dots$}.
$$
More explicitly, the probability mass function is the last two columns shown in Table\ \@ref(tab:InfiniteExp).
This is a valid probability distribution since $\sum_{n = 1}^{\infty} 2^{-n} = 1$.
First, check if the expected value exists:
$$
  \operatorname{E}[\, |X|\, ] 
  = \sum_{n=1}^\infty \frac{2^n}{n} \cdot \frac{1}{2^n} 
  = \sum_{n=1}^\infty \frac{1}{n} = \infty.
$$
Thus, $\operatorname{E}[\, |X|\, ]$ is infinite and so $\operatorname{E}[X]$ *does not exist*.
Now, consider naively summing the series
\begin{equation}
  \operatorname{E}[X]
  = \sum_{n=1}^\infty (-1)^{n+1} \cdot \frac{2^n}{n}\cdot\frac{1}{2^n}
  = \sum_{n=1}^\infty \frac{(-1)^{n+1}}{n}
  = \log 2,
   (\#eq:NotTheMean)
\end{equation}
and this summation converges.
However, *this is not $\operatorname{E}[X]$*; since $\operatorname{E}[\,|X|\,] = \infty$, $\operatorname{E}[X]$ is not defined.
:::



```{r InfiniteExp, echo=FALSE}
n <- 1:6
X <- as.character(
  MASS::fractions(
    (-1)^(n + 1) * (2^n)/n
...
X <- c("2", "\\llap{$-$}2", "\\tfrac{8}{3}", 
       "\\llap{$-$}4", "\\tfrac{32}{5}", "\\llap{$-$}\\tfrac{32}{3}")
pX <- 
  c("\\tfrac{1}{2}",
    "\\tfrac{1}{4}",
    "\\tfrac{1}{8}",
...
knitr::kable( InfExpTab,
              ...
              align = "c",
              booktabs = TRUE,
              caption = "The probability mass function for the random variable $X$."
)
```


The reason why the second summation Eq.\ \@ref(eq:NotTheMean) is *not* the mean is that the values of\ $X$ form a *set* (i.e., unordered), not a *sequence* (i.e., ordered); there is no natural first, second, third value. 
So $\sum x\cdot p_X(x)$ is only a legitimate definition if it gives the *same* value regardless of the *order* in which the terms are added. 
But this is not the case.
For example, Eq.\ \@ref(eq:NotTheMean) could be written to take two positive outcomes ($n$ odd) for every one negative outcome ($n$ even):
\begin{align*}
  \sum_{n=1}^\infty x_n\,p_X(x_n)
    &= \Big[(x_1\cdot p_X(x_1) + x_3\cdot p_X(x_3)) + x_2\cdot p_X(x_2)\Big]\\
    &\qquad + \Big[(x_5\cdot p_X(x_5) + x_7\cdot p_X(x_7)) + x_4\cdot p_X(x_4)\Big] + \cdots\\
    &= \Big[1            + \tfrac{1}{3} - \tfrac{1}{2}\Big] 
     + \Big[\tfrac{1}{5} + \tfrac{1}{7} - \tfrac{1}{4}\Big] + \cdots\\
    &= \tfrac{3}{2}\log 2.
\end{align*}
Alternatively, Eq.\ \@ref(eq:NotTheMean) could be written to take one positive outcome for every two negative outcomes:
\begin{align*}
  \sum_{n=1}^\infty x_n\,p_X(x_n)
  &= \Big[(x_1\cdot p_X(x_1) + x_2\cdot p_X(x_2) ) + x_4\cdot p_X(x_4\Big] \\
  &\qquad + \Big[(x_3\cdot p_X(x_3) + x_6\cdot p_X(x_6) ) + x_8\cdot p_X(x_8)\Big] + \cdots\\
  &= \Big[1              - \tfrac{1}{2} - \tfrac{1}{4}\Big] 
    +  \Big[\tfrac{1}{3} - \tfrac{1}{6} - \tfrac{1}{8}\Big] + \cdots \\
  &= \tfrac{1}{2} \log 2.
\end{align*}
So, depending on how the terms are organised, a different value for the sum seems possible.
The proof for these sums is not straightforward, but can be demonstrated in **R**:

```{r}
log2 <- log(2)

## Original order: 1 - 1/2 + 1/3 - 1/4 + ... -> log(2)
n <- 1:200000
orig_sum <- sum((-1)^(n + 1) / n)

## Rearrangement 1: two positive terms, then one negative 
k <- 1:100000
r1_sum <- sum(1 / (4 * k - 3) + 1 / (4 * k - 1) - 1 / (2 * k))

## Rearrangement 2: one positive term, then two negative
r2_sum <- sum(1 / (2 * k - 1) - 1 / (4 * k - 2) - 1 / (4 * k))

cat("Original     ->", orig_sum, " (target: log 2      =", log2,       ")\n")
cat("2 pos, 1 neg ->", r1_sum,   " (target: 1.5 * log2 =", 1.5 * log2, ")\n")
cat("1 pos, 2 neg ->", r2_sum,   " (target: 0.5 * log2 =", 0.5 * log2, ")\n")
```


:::{.example #ExpectationDoesNotExist2 name="Expectation does not exist"}
Consider the random variable $X$ with probability density function
$$
  f_X(x)
  =
  \begin{cases}
    \frac{1}{3 x^2}   & \text{for $x \ge 1$};\\[4pt]
    \frac{2}{3x^2}    & \text{for $x \le -1$};\\[4pt]
    0                 & \text{for $-1 < x < 1$}.
  \end{cases}
$$
This is a valid density (check!), but the expected value $\operatorname{E}[X]$ does not exist.
See that
\begin{align*}
  \operatorname{E}[X^+] &= \int_1^\infty x   \cdot \frac{1}{3x^2}\, dx = \frac{1}{3} \int\frac{1}{x}\, dx = \infty;\\
  \operatorname{E}[X^-] &= \int_1^\infty |x| \cdot \frac{2}{3x^2}\, dx = \frac{2}{3} \int\frac{1}{x}\, dx = \infty.
\end{align*}
Thus, both tails diverge, so $\operatorname{E}[X] = \operatorname{E}[X^+] - \operatorname{E}[X^-] = \infty - \infty$, which is *undefined*.
Thus, $\operatorname{E}[X]$ does not exist.
:::


:::{.example #InfiniteMean name="Infinite mean"}
Consider the distribution of\ $Z$, with the probability density function\index{Expected value!infinite}
$$
...
$$
However, $\displaystyle\lim_{z\to\infty}\, \log z = \infty$.
Hence, $\operatorname{E}[Z]$ exists and is *infinite*: $\operatorname{E}[Z] = \infty$.
:::
\index{Expected value|)}
...
$$
For a *mixed* random variable\ $X$ with probability mass function $p_X(x)$ and probability density function $f_X(x)$, the expected value of $g(X)$ is
$$
   \operatorname{E}\big[g(X)\big]
   =
   \sum_{x\in \mathcal{R}_X} g(x)\, p_X(x)
   +
   \int_{-\infty}^{\infty} g(x)\, f_X(x)\,dx,
$$
where $\mathcal{R}_X$ is the set of values having positive probability mass.
...
\begin{align*}
  ...
  &= \tfrac{1}{2} + \left(0 + \tfrac{1}{4}\right) = \tfrac{3}{4}.
\end{align*}
...
```{r ConvexConcave, echo=FALSE, fig.align="center", fig.cap="A convex function (left) and a concave function (right). The dashed lines are the tangents at a fixed point $x = a$.", out.width='100%', fig.width=8.5, fig.height=3.5}
par(mfrow = c(1, 2))
...
:::{.example #ExpectationMixedVarX name="Variance for a mixed variable"}
Consider the mixed random variable\ $W$ with probability density function shown in Example\ \@ref(exm:ExpectationMixedX).
The expected value of\ $W$ is $\operatorname{E}[W] = 1/4$.
The variance of\ $W$ is then $\operatorname{var}[W] = \operatorname{E}[W^2] - \operatorname{E}[W]^2$, and so
\begin{align*}
   \operatorname{E}[W^2]
...
In Example\ \@ref(exm:InfiniteMean), $\operatorname{E}[X]$ did not exist\index{Variance!does not exist}
For that reason, the variance also does not exist.
:::
...
:::{.example #Tchebysheffmarks name="Tchebysheff inequality"}
Suppose students' exam marks\ $X$ have mean\ $70$ and standard deviation $10$.
Tchebysheff's inequality can be used to bound the probability that a student's mark differs from the mean by *at least*\ $20$ marks.

Since $|X - \mu| \ge 20$ and 
$$
  k = 20/10 = 2,
$$
Tchebysheff's inequality gives
$$
  \Pr( |X - \mu| \ge k\sigma) \le 1/k^2;
$$
hence
$$
  \Pr( |X - 70| \ge 20) \le 1/2^2 = 1/4.
$$
Therefore
$$
  \Pr(| X - 70 | \ge 20) \le 0.25,
$$
or, equivalently,
$$
  \Pr(50 < X < 90) \ge 0.75.
$$
Even if we know nothing about the distribution of marks, and only know the mean and standard deviation, we can still guarantee that at least $75$% of marks lie between\ $50$ and\ $90$.
:::
...
$$
   M_X(t) = \operatorname{E}\big[\exp(tX)\big], 
$$
...
$$
When\ $X$ is a mixed random variable with probability mass function $p_X(x)$ and probability density function $f_X(x)$,
$$
  M_X(t)
  =
  \operatorname{E}\big[\exp(tX)\big]
  =
  \sum_{x\in \mathcal{R}_X} \exp(tx)\, p_X(x)
  +
...
$$
  f_X(x \mid X > 0) = exp(-x)\quad\text{for $x > 0$)}.
$$

1. Find the expected biomass.
2. Find the variance of the the biomass.
3. Find the MGF of\ $X$.
4. Compute the mean and variance of\ $X$ from the MGF.
:::
...
<!-- labmda = 1/20 -->
Suppose the true daily rainfall\ $X$ follows a distribution with probability function $f_X(x) = \exp( -x/20)/20$.
...
3. Find the MGF of\ $X$, and write down the expression for evaluating the MGF of\ $R$.
:::
...
:::{.exercise #SimHiking}
Five hikers are walking together. 
Since they stay together as a group, their walking speed is determined by the slowest hiker.

The walking speed (km/h) of each hiker has the following distribution:
...
Assume the hikers' speeds are independent.

Use a simulation of $10\,000$ hiking groups to estimate:

* the average speed of the group;
* the probability the group walks faster than 3.5 km/h;
* the probability the group walks exactly 4.0 km/h.
:::



:::{.exercise #SimHospitalBeds}
A small hospital ward has 20 beds.

Each morning, the number of new patients admitted is a random variable with the following distribution:

| New admissions | Probability   |
_----------------+---------------|
| 0 |	0.10 |
| 1	| 0.20 |
| 2	| 0.35 |
...
Every patient currently in the ward has a\ $0.15$ probability of being discharged that day, independently of every other patient.
If there are no empty beds, arriving patients are turned away.
Initially, the ward contains\ $10$ patients.

Use a simulation of 365 days to estimate:

* the average number of occupied beds;
* the probability the ward is full at the end of a day;
* the proportion of arriving patients who are turned away.

Repeat the simulation\ $100$ times and comment on the variability of your estimates.
:::
::::
