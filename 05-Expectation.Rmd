...
For example, consider $t > 0$: as $x\to\infty$, we see $\exp(tx)\to\infty$, while $1/(1 + x^2)\to 0$ quite slowly; the integrand diverges (see Fig.\ \@ref(fig:MGFdiverges) (left panel) for an example when $t = 1/2$). 

Now consider $t < 0$: as $x\to-\infty$, we see $\exp(tx)\to\infty$, while $1/(1 + x^2)\to 0$ quite slowly; the integrand again diverges (see Fig.\ \@ref(fig:MGFdiverges) (right panel) for an example when $t = -1/2$). 

The integral only converges for $t = 0$ (Fig.\ \@ref(fig:MGFdiverges), centre panel).
...
```{r echo=FALSE, MGFdiverges, fig.width=6.5, fig.height=3, out.width="100%", fig.align="center", fig.cap="The MGF cannot be computed, as the integrand diverges for $t > 0$ (left panel) and for $t < 0$ (right panel), but converges only when $t = 0$ (center panel)."}
par( mfrow = c(1, 3))
...
plot(x = x_Pos,
     ...
     xlab = expression(italic(x)),
     ylab = "Integrand",
     main = expression(bold(Integrand~with~italic(t)==-0.5)),
     type = "l")
abline(h = 0,
       col = "grey")
# t negative
ig0 <- igrand(x_Zero, t = 0)
plot(x = x_Zero,
     y = ig0, 
     las = 1,
     lwd = 2,
     ...
     xlab = expression(italic(x)),
     ylab = "Integrand",
     main = expression(bold(Integrand~with~italic(t)==0)),
     type = "l")
abline(h = 0,
       col = "grey")

# t negative
ig2 <- igrand(x_Neg, t = -0.5)
plot(x = x_Neg,
     y = ig2, 
     las = 1,
     lwd = 2,
...
### Useful results {#UsefulMFGResults}
...
$$
(Later, this will be identified as a geometric distribution (Sect.\ \@ref(GeometricDistribution)).)\index{Geometric distribution}
:::
