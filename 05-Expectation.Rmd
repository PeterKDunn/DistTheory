...
Consider the mixed random variable\ $X$ with probability density function shown in Example\ \@ref(exm:ExpectationMixedX).
The expected value of\ $W$ is $\operatorname{E}[W] = 1/4$.
...
The expected biomass is
$$
  \operatorname{E}(X) = 0\times 0.6 + 0.4\times \operatorname{E}(X\mid X > 0).
$$
Since
$$
  \operatorname{E}(X\mid X>0) = \alpha\beta = 3\times 2 = 6,
$$
then
$$
  \operatorname{E}(X) = 0.4\times6 = 2.4.
$$

This illustrates a useful shortcut: if a mixed random variable consists of a point mass at zero and a continuous distribution otherwise, then
$$
  \operatorname{E}(X) = \Pr(X > 0) \times \operatorname{E}(X \mid X > 0).
$$
::::
...
The recorded rainfall\ $R$ is a mixed random variable because:

* a point mass exists at\ $R = 0$;
* a continuous component exists on\ $(0.1, 70)$;
* a point mass exists at\ $R=70$.
...
$$
  ...
  0\times P(R=0)
  +
  \int_{0.1}^{70} r\, f_X(r)\, dr
  +
  70\times \Pr(R = 70).
$$

The first term contributes nothing because the recorded rainfall is zero.

For a mixed random variable,

$$
  E(X) =
  \text{(discrete contribution)}
  +
  \text{(continuous contribution)}.
$$
The calculation combines summation and integration in the same expression.
:::
