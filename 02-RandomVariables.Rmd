...
For example, we could define $s_1$ as the sample point $(1, 1)$.\smallskip
...
```{r DFCont, echo=FALSE, fig.align="center", fig.cap="The probability function (left) and the distribution function (right) for $V$", fig.width=7, out.width='100%', fig.height=3.5}
par(mfrow = c(1, 2))

fv <- function(v){
  y <- array( 0, dim = length(v))
  fy <- ifelse( (v > 0) & ( v < 2 ),
                v/2,
                0)
  fy
...
plot( x = c(-2, 4),
      y = c(0, 1),
      type = "n",
      main = expression(paste("The distribution function for"~italic(V))),
      xlab = expression(italic(v)),
...
1. Suppose a producer requires at least 50 mm of rain in June to plant a crop.
...
1. Compare the probabilities of obtaining 30mm, 50mm, 100mm and 150mm of rain in the months of June and December.
...
rownames(RainArray) <- c("Zero rainfall", 
           "$\\phantom{20}0 < R < 20\\phantom{0}$",
           "$\\phantom{0}20 \\le R < 40\\phantom{0}$",
           "$\\phantom{0}40 \\le R < 60\\phantom{0}$",
           "$\\phantom{0}60 \\le R < 80\\phantom{0}$",
           "$\\phantom{0}80 \\le R < 100$",
           "$100 \\le R < 120$",
           "$120 \\le R < 140$",
           "$140 \\le R < 160$",
           "$160 \\le R < 180$",
...
In the alternative method, the blood from $n$ individuals is combined, and one test is conducted.
If the test returns a negative result, then all $n$ people do not have the disease; if the test returns a positive result, all $n$ individuals are then tested.

Suppose a disease occurs in an unknown proportion of people $p = 0.1$ of people, and blood from $n = 4$ people is pooled and tested. 
Deduce the probability distribution of the random variable 'whether the pooled test returns a negative test' $X$, where $x = 0$ means the pooled test returns negative, and $x = 1$ means the pooled test returns positive.
:::
...
:::{.exercise #Arcsine} 
Consider the random variable $X$ with pdf\smallskip <!-- arcsine distribution -->

\[ 
   f(x) =
   \begin{cases} 
      \frac{k}{\sqrt{x(1 - x)}} & \text{for $0 < x < 1$};\\
      0                         & \text{elsewhere}.
   \end{cases}
\]
...
:::{.exercise #Cards4}
Suppose a dealer deal four cards from a standard 52-card pack.
Define $Y$ as the number if suits in the four cards.
