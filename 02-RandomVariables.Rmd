...
The *domain* (i.e., inputs) of a random variable is the sample space, and the *range space* or *value set* is the set of real numbers taken by a function.\smallskip
...
\[ 
   (1, 3), (2, 2), (3, 1).
\]
Notice that many elements of the sample space can be assigned to the same value of the random variable.\smallskip
...
:::{.example}
In Example \@ref(exm:TwoDiceRV), exactly 11 values of the random variable $Y$ are possible: $2, 3, \dots 12$.\smallskip

In Example \@ref(exm:DrawTwoCards), the random variable $D$ can take one of three possible values: $-2$, $0$ or $2$.\smallskip

In Example \@ref(exm:TossCoinTillHead), the random variable $N$ takes a *countably infinite* number of possible values: $1, 2, 3, \dots$\smallskip
:::
...
Consider the time spent by vehicles waiting at a set of traffic lights before proceeding through the intersection.\smallskip
...
However, if the light is red on arrival, the vehicle needs to  wait a continuous amount of time before it turns green.\smallskip

The time spent waiting is a *mixed* random variable.\smallskip
:::
...
Since a random variable is a *real-valued function*, simply observing the outcome as $\{H, T\}$, for example, does not define a random variable. \smallskip
...
\]
where all 10 elements are equally likely.\smallskip
...
$H$ is a (discrete) random variable, and the range of $H$ is $R_H = \{0, 1, 2\}$, representing the values that $H$ can take.\smallskip
...
The ideas from Example \@ref(exm:ContinuousProbs) are relevant here: The probability of throwing a cricket ball within two distances.
...
\[
   0.5 \times 1 /2 = 0.25,
\]
and the answer is the same as before.\smallskip
...
```{r HeadsDF, echo=FALSE, fig.cap="A graphical representation of the distribution function for the tossing-heads example. The filled circles contain the given point, while the empty circles omit the given point.", fig.align="center", fig.width=4,fig.height=3.5}
plot( x = c(-1, 4),
      y = c(0, 1.05),
      type = "n",
      las = 1,
...
\[
   f(x; a, b, c) = 
   \begin{cases}
      \frac{2(x - a)}{(b - a)(c - a)} & \text{for $a\le x\le c$};\\[6pt]
...
\[
   p(x) = 
   \begin{cases}
      \log_{10}\left(1 + \frac{1}{x} \right),& \text{for $x = 1, 2, 3, \dots, 9$}\\
      0 & \text{otherwise}.
   \end{cases}
\]

1. Show that $p(x)$ is a probability function.
1. Determine an expression for the corresponding distribution function.
:::



:::{.exercise}
The quartic polynomial model

\[
   f_Y(y) = 3\times 10^{-9} y^2 (100 - y)^2\quad \text{for $0 \le y \le 100$}
\]
is used for describing mortality where $Y$ denotes age at death in years.

1. Is a person more likely to die at age between 60 and 70 or live past 70?
1. According to this model, what is the mean age at death?
1. What is the standard deviation of the age at death?
:::
...
1. Determine that moment generating function (mgf) of $X$.
1. Use the mgf to verify that $\text{E}(X) = \text{var}(X)$.
1. Suppose that $Y = 1 - X^2$.
   Determine $\text{E}(Y)$.
:::

:::{.exercise}
In a deck of cards, consider an **Ace** as low (i.e., value of one), and all picture cards (i.e., **Jack**, **Queens** and *Kings**) as tens.

Deduce the probability distribution of the absolute difference between the value two cards drawn at random from a well-shuffled deck of 52 cards.
:::
...
Numbers are said to satisfy *Benford's law* if the *leading digit*, say $d$ (for $d\in \{1, ..., 9\}) has the probability mass function
