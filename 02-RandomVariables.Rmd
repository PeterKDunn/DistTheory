...
Subsets these sample space elements are usually of greater interest and more convenient to work with.
...
* '... the value 3.' (that is, the specific value $3$).
...
\begin{align*}
      ...
      \Pr(H = 1):&\enskip 1/2\\
      \Pr(H = 2):&\enskip 1/4\\
      \Pr(H = h):&\enskip 0\quad \text{for all other values of $h$}.
\end{align*}
:::
...
Then $R_X = \{2, 3, 4, 5\}$.
We could list the probabilities:
\begin{align*}
     \Pr(X = 2) &= \Pr((1, 2)) &= 1/10\\
...
The ideas from Example \@ref(exm:ContinuousProbs) sre relevant here.
