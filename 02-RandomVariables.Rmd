...
In the alternative method, blood from $n$ individuals is combined, and one test is conducted.
If the test returns a negative result, then none of the $n$ people have the disease; if the test returns a positive result, all $n$ individuals are then tested to identify which individual(s) have the disease.

Suppose a disease occurs in an unknown proportion of people $p$ of people.
Blood from $n = 4$ people is pooled and tested.
Deduce the probability distribution of the random variable $X$ when $n = 4$ and $p = 0.1$, where $X = 0$ means the pooled test returns a negative result, and $X = 1$ means the pooled test returns a positive result.
:::
