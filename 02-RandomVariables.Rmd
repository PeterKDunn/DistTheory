...
In Chapter \@ref(Probability), the tools of probability were used to assist in answering questions about phenomena involving uncertainty.
...
\begin{align*}
     & \Pr(\text{roll\ a\ \epsdice{1}}) = \Pr(\text{roll\ a\ \epsdice{2}}) 
   = &\cdots = \Pr(\text{roll a \epsdice{6}}) = 1/6.
\end{align*}
Define the random variable $X$ as the number of pips showing on the die.
Then, $X = 1$ for the outcome $\epsdice{1}$. 
Consequently:
\[
...
ADD THING ABOUT MY HIEGHT THE SAME... BUT HTS VARY FROM OINDIVIDUAL TO INDIVIDUAL
...
Since a random variable is a *real-valued function*, simply observing the outcome as $\{\head, \tail\}$, for example, is not a rv. 
...
\[
   S = \{ (\tail\tail), (\tail\head), (\head\tail), (\head\head)\}.
\]
...
knitr::kable(TwoDieTable,
             caption = "A table showing the distribution of $H$, the number f heaDS in two rolls of a die")
```
...
plot( x = 0:2,
      ...
      las = 1,
      col = "grey",
      main = "The probability distribution of $H$",
...
```{r DFCont, echo=FALSE, fig.align="center"}
vb <- seq(0, 2,
         length = 100)
vbDF <- ( vb ^ 2 ) / 4
