...
The concepts of the [*sample space*](#def:SampleSpace) was introduced, which lists all the possible outcomes of a [*random process*](#def:Process).
Often, however, the elements of the sample space are often not of direct interest... especially if the sample space contains many elements.
Often, combinations of these sample space elements are of greater interest and more convenient to work with.
...
More generally, it proves useful to collect elements of the sample space together and assign a real number to that collection.
...
A *random variable* is some function that assigns a real number to each value in the sample space.
...
For the example above (the sum of rolling two dice), the sample space cold be defined as $\{r_1, r_2\}$, where $r_1$ and $r_2$ are the results of roll 1 and 2 respectively. 
...
\[
   Y = r_1 + r_2.
\]
Then, for example, the elements of $S$ that are associated with $Y = 4$ are
\[ 
   (1, 3), (2, 2), (3, 1).
\]
...
:::{.example}
Consider the random process 'observing the outcome of tossing a coin three times'.

The sample space is
\begin{align*}
   \Omega 
   &= \{(H, H, H), (H, H, T), (H, T, H), (H, T, T)\\
   &\phantom{{}= \}}(T, H, H), (T, H, T), (T, T, H), (T, T, T) \}.
\end{align*}
We could then define the random variable $F$ as 'the number of heads of the *first two* tosses'.
Then each element of the sample space can be assigned to a real number:
\begin{align*}
   (T, T, T), (T, T, H)\quad&\text{are assigned to}\quad F = 0;\\
   (H, T, T), (H, T, H), (T, H, H), (T, H, T) \quad&\text{are assigned to}\quad F = 1;\\
   (H, H, T), (H, H, H)\quad&\text{are assigned to}\quad F = 2.
\end{align*}
Writing $F = 0$ means 'the number of heads in the first two tosses has the value $0$'.
:::
...
A capital letter is usually used to describe the random variable (such as $Y$), while lower-case letters are used to describe the *value* of the random variable (such as $y$).
For example, if we write $Y = 3$, this means:

* 'The sum of the roll of two dice...' (that is, $Y$)
* '...is equal to...'
* '... the value 3.' (that is, $y = 3$)
...
A **discrete random variable** contains a finite of countably infinite number of values.
:::

NEED YO CONNECT THESE EXAMPLES TO THE DEFINITION ABOVE.
See older text below.
...
:::{.example}
Consider the time spent by vehicles waiting at a set of traffic lights before proceeding.

If the light is green on arrival, the wait tune is exactly zero (i.e., discrete): the vehicle can drive straight through the intersection.
...
## Random variables {#RandomVariablesa}
