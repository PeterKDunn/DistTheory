...
See Fig. \@ref(fig:ShowProbCont), wich shows some probability density function $f(x)$.
...
\] 
as $\Delta x \to 0$.
This is shown by the shaded area, which can be approximated by the rectangle shown.
That is,  
\begin{align*}
  \Pr(X = x^*)
  &\approx \Pr\left(x^* - \frac{\Delta x}{2} < X < x^* - \frac{\Delta x}{2} \right) \\
...
Rearranging,  
\[
  f(x) = \frac{\Pr(X = x^*)}{\Delta x}.
\]
That is, the density function $f(x)$ is *not* a probability.
...
```{r ContinuousPDF, echo=FALSE, fig.align="center", fig.cap="The probability function for $W$", fig.height=3.5,fig.width=6, out.width='55%'}
plot( x = c(-1, 2),
      y = c(0, 2),
      type = "n",
      las = 1,
...
Using this ,odel, is a person more likely to die at age between 60 and 70 or live past 70?
:::
