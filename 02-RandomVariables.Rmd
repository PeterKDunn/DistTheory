...
Often, however, the elements of the sample space are not of direct interest... especially if the sample space contains many elements.
...
For example, if we write $Y = 3$, this could mean:
...
\[
   \Omega = \{ (B, B), (B, R), (R, R), (R, B)\}.
\]
Many random variable coud be defined on this sample space; for example:
\begin{align*}
   T&: \text{The number of black cards drawn};\\
   M&: \text{The number of red cards drawn on the first draw};\\
   D&: \text{The number of black cards drawn, minus the number of red cards drawn}.
\end{align*}
...
\begin{align*}
   ...
   (T, H)\quad&\text{is assigned to}\quad N = 2\\
   (T, T, H)\quad&\text{is assigned to}\quad N = 3,
\end{align*}
and so on.
...
**THIS NEEDS RETHINKING, DEPENDING ON HOW CONT DEFINED**
...
\[ 
   ...
   \begin{cases}
      ...
      0 & \text{elsewhere}.
\]
The probability function could also be given in a table (Table \@ref(tab:LargestDie)) or as a graph as in Fig. \@ref(fig:ProbDist1).
:::
