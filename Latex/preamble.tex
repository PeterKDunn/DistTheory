...
\usepackage{needspace} % Fix shaded boxes starting at bottom of page x, but text starting page x+1
...
\usepackage{enumitem}% http://ctan.org/pkg/enumitem. Remove some space between itemise are preceeding text
...
\newenvironment{linkBox}[1]
  ...
  \setlength{\fboxsep}{1em}
  \begin{linkcolourbox}\setlength{\parskip}{2mm}
  \item
  }
  {
  \end{linkcolourbox}
...
% Notation Box
\definecolor{NotationColour}{RGB}{249, 242, 236}
\newtcolorbox{notationcolourbox}{
  colback = NotationColour,
  colframe = NotationColour,
  coltext = black,
  boxsep = 5pt,
  arc = 4pt}
\newenvironment{notationBox}[1]
  ...
  \setlength{\fboxsep}{1em}
  \begin{notationcolourbox}\setlength{\parskip}{2mm}
  \item
  }
  {
  \end{notationcolourbox}
  \end{itemize}
  } 
...
\newenvironment{answer}
  {\vspace{-2pt}\footnotesize\parindent=0pt
   \setlength{\topsep}{0pt}
...
\renewenvironment{example}%
  ...
  \begin{mdframed}[
    ...
    innertopmargin=2pt,% Gap between start of grey area, and start of text in the box
...
\renewenvironment{exercise}%
  {\small\begingroup\setlist{nosep,topsep=-0.8\parskip}\vspace{3mm}% That final 5mm influences the gap between exercises
