...
The *discrete uniform distribution*\index{Discrete uniform distribution} is used for equally-likely outcomes (like coin tosses and rolls of a die).
The *Bernoulli distribution*\index{Bernoulli distribution} is used to model single dichotomous (e.g., yes/no; true/false) outcomes.
These two distributions are not often used in practice, but the basis for developing other distributions.

The *binomial distribution*\index{Binomial distribution} is used to model the number of successes in a fixed number of Bernoulli repetitions.
The *geometric distribution*\index{Geometric distribution} is used to model the number of Bernoulli trials until the first success.
The *negative binomial distribution*\index{Negative binomial distribution} can be used to model the number of Bernoulli trials until the $r$th success, but it also has mider uses for modelling count data (with no theoretical upper limit); it has greater flexibility for modelling counts than the Poisson distribution.
The *Poisson distribution*\index{Poisson distribution} is commonly used to model count data (with no theoretical upper limit).

The *hypergeometric distribution*\index{Hypergeometric distribution} is used to model the number of successes in a sample of size\ $n$ n drawn *without replacement* from population size of size\ $N$, with\ $K$ successes.
The *multinomial distribution*\index{Multinomial distribution} is used to model counts of outcomes in\ $n$ independent categorical trials (whereas the binomial is used for two independent categorical trials).
...
### Simulating a call centre

A call centre receives calls throughout the day.
...
- Whether each call is *resolved* on the first attempt, modelled as $\text{Bernoulli}(p = 0.7)$.
  Thus, the *number of calls resolved* in an hour is $R \sim \text{Bin}(C, 0.7)$, but since\ $C$ is random, this is not a simple binomial.
- The *number of calls until the first unresolved call*, modelled as $X \sim \text{Geom}(p = 0.7)$.

We simulate $n = 10\,000$ hours of call centre operation:
...
The simulated summaries canbe compared to theoretical values:

```{r}
cat("--- Number of calls ---\n")
cat("Theoretical mean:    ", lambda, "\n")
cat("Simulated mean:      ", round(mean(n_calls), 2), "\n\n")

cat("--- First unresolved call ---\n")
cat("Theoretical mean:    ", (1 - p_res) / p_res, "\n")


cat("Simulated mean:      ", round(mean(first_fail), 2), "\n\n")
cat("--- Resolved calls per hour ---\n")
cat("Simulated mean:      ", round(mean(n_resolved), 2), "\n")
cat("Simulated variance:  ", round(var(n_resolved), 2), "\n")
```
...
```{r}
cat("Theoretical P(C > 15):", 
    round(1 - ppois(15, lambda = lambda), 3), "\n")
cat("Simulated P(C > 15):  ", 
...
cat("Simulated P(no unresolved calls):", 
...
Analytically this requires inclusion-exclusion over many cases; simulation is far more straightforward using `rle()`:
...
cat("P(3+ consecutive unresolved in 20 calls):",
    round(mean(streak3), 3), "\n")
```


Suppose we also seek the probability that total handling time in an hour exceeds $60\mins$.
Suppose each resolved call takes $H \sim \text{Geom}(p = 0.5)$ minutes to handle.
The total handling time depends on both the random number of resolved calls and their random handling times, and this compound randomness makes an analytic answer intractable:

```{r}
total_time <- sapply(n_resolved, function(r) {
  if (r == 0) 0 else sum(rgeom(r, prob = 0.5))
})
cat("P(total handling time > 60 min):",
    round(mean(total_time > 60), 3), "\n")
cat("Mean total handling time (min): ",
...
cat("Simulated probability:  ", round(mean(busy_day), 3), "\n")
cat("Theoretical probability:", 
    round(1 - ppois(20, lambda = lambda)^8, 3), "\n")
```


```{r echo=FALSE, fig.cap="XXXXXX"}
par(mfrow = c(1, 2))
