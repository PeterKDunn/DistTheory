...
The *zero-truncated Poisson distribution*,\index{Poisson distribution!zero-truncated} with range space $\mathcal{R}_X = \{1, 2, \dots\}$, is used to model counts where counts of zero are impossible (i.e., the number of night of hospital stay for in-patients).
In contrast, the *zero-inflated Poisson distribution*,\index{Poisson distribution!zero-inflated} with range space $\mathcal{R}_X = \{0, 1, 2, \dots\}$, is used to model counts where the proportion of zeros is higher than expected when using the Poisson distribution.
