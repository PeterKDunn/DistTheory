...
The main interest is in the *mean* of the sample.\smallskip
...
Assume that the sample is of size $n = 2$, and sampling is *without* replacement.\smallskip
...
For example, the sample $\{2, 3\}$ has a probability of 1/10 of being selected, and the mean of this sample is 2.5.\smallskip
...
The mean observed for any one sample depends on which sample is taken.\smallskip

From this table, the distribution of the sample mean itself can be determined (Table \@ref(tab:SamplingDistMean)).
This distribution is known as the *sampling distribution of the sample mean*, recognition that the distribution is based on sampling data.\smallskip

The mean and variance of the sampling distribution are $\mu_{\bar{X}} = 4.4$ and $\sigma^2_{\bar{X}} = 1.89$.\smallskip
...
\]
If sampling is done *without* replacement, the sampling distribution of the mean has mean $\mu_{\bar{X}} = \mu = 4.4$ and variance $\text{var}(\bar{X}) = 1.89 = \frac{5.04}{2} \times \text{FPC}^2$, in agreement with the note following Theorem \@ref(thm:CLT).\smallskip
...
What is the probability that a pile of 25 will *not* be counted as such?\smallskip
...
For a plunger and tube chosen randomly from a day's production run, find the probability that the plunger will not fit into the cylinder.\smallskip

Let $X$ and $Y$ be the diameter of the plunger and cylinder respectively.
Then $X\sim N\big(2.1, (0.1)^2\big)$ and $Y\sim N\big( 2.3, (0.05)^2\big)$, and we seek $\Pr(Y < X)$.\smallskip
...
\begin{align*}
   ...
   &= \sum_{i = 1}^n \left( \frac{X_i - \mu}{\sigma} \right)^2,\\
   U
   &= n\frac{(\bar{X} - \mu)^2}{\sigma^2}, \quad{and}\\
   V
   &= \sum_{i = 1}^n \left(\frac{X_i - \bar{X}}{\sigma}\right)^2.
\end{align*}
Then see that:
...
What is the probability that the variance of this sample exceeds 10?\smallskip
...
(Notice lower-case symbols are used for specific values of statistics, and upper-case symbols for the random variables.)\smallskip
...
In **R**, the $F$-distribution functions have the form `[dpqr]f(df1, df2)`, where `df1`${} = \nu_1$ and `df2`${} = \nu2$. 
:::
...
What is the probability that the *average* amount dispensed in a random sample of size 36 is at least 204mL?\smallskip
...
For $n = 12$, find the probability that the sum of the faces is at least 52.\smallskip

Let the random variable $X_i$ be the number showing on the $i$th throw.
Then, define $Y = X_1 + \dots + X_{12}$. 
We seek $\Pr(Y\geq 52)$.\smallskip

In order to use Theorem \@ref(thm:CLT), note that the event '$Y\geq 52$' is equivalent to '$\bar{X}\geq 52/12$', where $\bar{X} = Y/12$ is the mean number showing from the 12 tosses.\smallskip

Since the distribution of each $X_i$ is rectangular with $\Pr(X_i = x) = 1/6$ (for $x = 1, 2, \dots, 6$), then $\text{E}(X_i) = 7/2$ and $\text{var}(X_i) = 35/12$.\smallskip
...
Define $Y = a_1X_1 + a_2X_2 + \dots + a_nX_n$.\smallskip
...
For $n = 20$, find $\Pr(V > 105)$.\smallskip

This is an example of Theorem \@ref(thm:ConvergeToN) with each $a_i = 1$. 
To find $\Pr(V > 105)$, the distribution of $V$ must be known.\smallskip
...
The normal approximation to the binomial distribution (seen in Sect. \@ref(NormalApproxBinomial)can be seen as an application of the [Central Limit Theorem](thm:CLT).
...
:::{.exercise #EggsBroken}
In a carton of a dozen eggs, the number broken has a Poisson distribution with mean $0.2$. Find the probability that the sample mean of the number of broken eggs per carton is more than one.
:::
