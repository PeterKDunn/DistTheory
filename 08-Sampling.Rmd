...
Mean  |  2.5  |  3.0  |  4.0  |  4.5  |  5.0  |  5.5  |  7.0
------|------|--------|-------|-------|-------|-------|------
Probability  |  0.2  |  0.1  |  0.1  |  0.2  |  0.1  |  0.2  |  0.1


Notice we can determine the mean and variance of the distribution as $\mu_{\bar{X}} = 4.4$ and $\sigma^2_{\bar{X}} = 1.89$.

This example can be repated, but *sampling randomly with replacement*.
...
In terms of distribution theory we should see Table \@ref(tab:DistSampMeanIntro) as just the distribution of a random variable, since a sample mean is a random variable. 
...
(The exact relationship between $\sigma^2_{\bar{X}}$ and $\sigma^2$ is expanded on in Theorem \@ref(thm:meandist) below.)
...
:::{.proof}
This is a direct application of Corollary \@ref(cor:varlincomb) with $a_i = 1/n$.
:::


Theorem \@ref(thm:CLT) applies to a random sample as defined in Def. \@ref(def:randomsample). 
The distribution of the population from which the sample is drawn is irrelevant.

It is interesting to note what happens if a sample is randomly selected *without replacement* from a finite population as described in \@ref(def:randomsampling).
...
For large samples it turns out that it doesn't as we see in Sect.~\ref{CLT}.
...
:::{.proof}
In Theorem \@ref(thm:normal) we showed the mgf of the random variable $X_i$ to be
\[ 
   M_{X_i} (t)
...
Corollary~\ref{CR:normmeasum} relies only on the population from which the sample is drawn being normally distributed and on the properties of expectation.
...
\begin{align*}
     1 - \Pr{70 < Y < 80}   
       &= 1 - \Pr{\frac{70 - 75}{\sqrt{9}} < Z < \frac{80 - 75}{\sqrt{9}}}\\
...
The required result follows directly from Theorem \@ref(thm:chin).
:::
...
:::{.theroem #FProperties name="Propoerties of $F$-distribution"}
If $X\sim F(\nu_1, \nu_2)$ then
...
Sampling distributions for various statistics of interest are well developed when sampling is from a normal distribution as shown in Sect. \@ref(normsamp).
...
In Sect. \@ref(meandist) general results are given describing the mean and variance of the sample mean which hold for any population distribution.
...
The 'convergence in distribution' bit is saying that $Z_n$ approaches normality as $n\to \infty$. 
...
:::{.example #FairDiceSum name="Thropwing dice"}
...
This is clearly an example of Theorem \@ref(thm:CLTgen) with each $a_i = 1$. 
In order to find $\Pr(V > 105)$ we need to know the distribution of $V$. 
We have $\text{E}(V_i) = 5$ and $\text{Var}(V_i) = 100/12$ so that from Theorem \@ref(thm:CLTgen), we can say that $V$ is distributed approximately normal with mean $20\times 5 = 100$ and variance $20\times 100/12$.
...
\]
Theorems \@ref(thm:CLT) and \@ref(thm:CLTgen) are applicable to $\bar{X}$ and $Y$ respectively. 
Hence
\[
