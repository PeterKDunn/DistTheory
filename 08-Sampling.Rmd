...
In practice, of course, we do not have infinite populations; instead, we observe finite (usually relatively small) samples (i.e., *statistics*) from unknown distributions.

Consider taking a sample of size\ $n$ from some possibly-unknown distribution: $x_1, x_2, \ldots, x_n$.
Suppose we wish to use the sample to estimate the unknown mean of the population, and the unknown variance of the population.
How do we proceed?
...
It turns out that a 'good' estimate of the unknown population mean $\mu$ is
$$
  \bar{X} = \frac{1}{n}(X_1 + X_2 + \cdots X_n) = \frac{1}{n}\sum_{i=1}^n X_i,
$$
the familiar *sample mean*.
This is a 'good' estimate in the sense that is an *unbiased* estimate of $\mu$.
...
This means that an unbiased estimator produces estimates that, on average, have the correct value.
This seems a sensible property for a 'good' estimator.
(There are other properties of 'good' estimators, we but do not explore these here.)
When we say that $\bar{X}$ is an unbiased estimator of $\mu$ then, we mean that $\text{E}[\bar{X}] = \mu$,
...
:::{.proof}
Using the definition of unbiased:
\begin{align*}
  \text{E}[\bar{X}] 
  &= \text{E}\left[ \frac{1}{n}(X_1 + X_2 + \cdots X_n \right]\\
  &= \frac{1}{n} \text{E}[ X_1 + X_2 + \cdots X_n) ]\\
  &= \frac{1}{n} \left( \text{E}[ X_1 ] + \text{E}[X_2] + \cdots \text{E}[X_n)] \right) \\
...
By extension, if a population consists of\ $N$ values $X_i, X_2, \ldots X_n$, then the population mean is
$$
...
We can also define an unbiased estimator of the variance.
...
\begin{align*}
    \text{E}[S^2_n] 
    &= \text{E}\left[ \frac{1}{n} \sum_{i=1}^n (X_i - \mu)^2\right]\\
    &= \text{E}\left[ \frac{1}{n} (X_1 - \mu)^2 + (X_2 - \mu)^2 + \cdots + (X_n - \mu)^2\right]\\
    &= \frac{1}{n} \left( \text{E}[  (X_1 - \mu)^2] + \text{E}[(X_2 - \mu)^2] + \cdots + \text{E}(X_n - \mu)^2\right]\\
...
Again by extension, if a population consists of\ $N$ values $X_i, X_2, \ldots X_n$, then the population variance is
$$
...
Knowing the value of $\mu$ but not the value of $\sigma^2$ seems an unusual situation.
More likely, we would only have an estimate of the population mean $\bar{X}$.

Since $\bar{X}$ is estimated imprecisely (from just one of the many possible samples), this introduces an extra source of variation into the estimation of $\sigma^2$, and a new estimate is needed.


:::{.theorem #SampleVarianceUnbiased name="An unbiased estimator for $\sigma^2$ ($\mu$ unknown)"}
...
:::{.proof}
Starting again with the definition of an unbiased estimator:
\begin{align*}
  \text{E}[S^2_{n-1}]
  &= \frac{1}{n - 1}\text{E}\left[ \sum_{i=1}^n (X_i - \bar{X})^2\right]\\
  &= \frac{1}{n - 1}\text{E}\left[ \sum_{i=1}^n \left(X^2_i - 2X_i\bar{X} + \bar{X}^2\right)\right]\\
  &= \frac{1}{n - 1}\text{E}\left[ \sum_{i=1}^n X^2_i - \bar{X}\sum_{i=1}^n 2X_i + \sum_{i=1}^n \bar{X}^2\right].
\end{align*}
Now observe that since $\bar{X} = \sum_i X_i/n$, then $\sum_i X_i = n \bar{X}$; hence
\begin{align*}
  \text{E}[S^2_{n-1}]
  &= \frac{1}{n - 1}\text{E}\left[ \sum_{i=1}^n X^2_i - 2n\sum_{i=1}^n \bar{X}^2 + n \bar{X}^2\right]\\
  &= \frac{1}{n - 1}\text{E}\left[ \sum_{i=1}^n X^2_i - n\sum_{i=1}^n \bar{X}^2 \right]\\
  &= \frac{1}{n - 1}\left(\text{E}\left[ \sum_{i=1}^n X^2_i\right] - n\text{E}\left[\sum_{i=1}^n \bar{X}^2 \right]\right).
\end{align*}
This expression can be simplified by noting that
$$
  \text{var}[\bar{X}]
  = \text{E}[\bar{X}^2] - \text{E}(\bar{X})^2 
  = \text{E}[\bar{X}^2] - \mu^2, 
$$
and so $\text{E}[\bar{X}^2] = \text{var}{\bar{X}} + \mu^2 = \frac{\sigma^2}{n} + \mu^2$.
Also, 
$$
  \text{var}[X] 
  = \text{E}[X^2] - \text{E}[X]^2 
  = \text{E}[X^2] - \mu^2,
$$
so that $\text{E}[X^2] = \sigma + \mu^2$.
Hence,
\begin{align*}
  \text{E}[S^2_{n-1}]
  &= \frac{1}{n - 1}\left(\text{E}\left[ \sum_{i=1}^n X^2_i\right] - n\text{E}\left[\sum_{i=1}^n \bar{X}^2 \right]\right)\\
  &= \frac{1}{n - 1}\left\{ \sum(\sigma^2 + \mu^2) - n\left(\frac{\sigma^2}{n} + \mu^2\right)\right\}\\
  &= \frac{1}{n - 1}\left\{ n\sigma^2 + n\mu^2 - \sigma^2 - n\mu^2\right\}\\
  &= \frac{1}{n - 1}\left\{ n\sigma^2  - \sigma^2\right\} = \sigma^2.
\end{align*}
Thus $S^2_{n-1}$ is an unbiased estimator of $\sigma^2$.
:::
...
Suppose a simple random sample (SRS) of size\ $n$ is taken from a population.
...
$$
  \sigma^2 = \frac{1}{N} \left( \sum_{i = 1}^n  (X_i - \mu)^2 \right) = 4.4
$$
```{r}
N <- length(Population)
...
The factor $(n - 1)/n$ comes from  the difference between the formulas.
...
The mean *of the sampling distribution* in Fig.\  \@ref(fi:SampleMeanDistn) is $\mu_{\bar{X}} = 4$:

```{r}
mean(MeanAllSamples)
```

The variance *of the sampling distribution* in Fig.\  \@ref(fi:SampleMeanDistn) is $\sigma^2_{\bar{X}} = 1.65$:
...
Using Definition\ \@ref(def:TDistribution) then, $T$ has a $T$ distribution.
:::
