...
A simple example demonstrates how these two are related.

## Examples

### Some results for samples

Suppose a simple random sample (SRS) is taken from a population.
...
$$
We have seen that the theoretical mean of a discrete random variable $X$ is $\mu = \text{E}[X] = \sum_{i = 1}^n p_X(x) x$ where $p_x(x)$ is the probability function.

Suppose we take a *simple random sample* of observations from some population of size $N$.
This means that every observation in the population has the sample chance of being selected in our *sample* of size $n$.
That is, each of the $n$ observations in the sample has the same probability of $1/n$ of being selected in the sample.
That is,
$$
  \text{E}[X] = \sum_{i = 1}^n \frac{1}{n} X_i. 
$$
For *observed* values of $X_i$ we write the mean of the sample (the *smaple mean*) as $\bar{x}$:
$$
  \bar{x} = \frac{1}{n} \sum_{i = 1}^n x_i. 
$$
This is the mean of a sample of observations.
Similarly, the theoretical variance of a discrete random variable $X$ is $\sigma^2 = \text{E}[X] = \sum_{i = 1}^n p_X(x) (x - \bar{X})^2$ where $p_X(x)$ is the probability function.
Taking a *simple random sample* of observations again from some population of size $N$, every observation in the population has the sample chance of being selected in our *sample* of size $n$.
That is, each of the $n$ observations in the sample has the same probability of $1/n$ of being selected in the sample.
$$
  \text{var}[X] 
  = \sum_{i = 1}^n \frac{1}{n} (X_i - \bar{X})^2
  = \frac{1}{n} \left( \sum_{i = 1}^n  X^2_i - \mu^2 \right).
$$
(PROOF?????)
For *observed* values of $X_i$ we write the mean of the sample (the *smaple mean*) as $\bar{x}$:
FIX: THEIS OS VAR OF WHOLE POPULATION OF SIZE N
$$
  \sigma^2 = \frac{1}{n} \left( \sum_{i = 1}^n  X^2_i - \mu^2 \right).
$$
This is the variance of the *sample*, which is written in terms of the *population* mean.
However, if the variance of the population is unknown, then the mean of the population is likely to be unknown also.
IN that situation (PROVE!!!)
$$
  s^2 = \frac{1}{n - 1} \left( \sum_{i = 1}^n  X^2_i - \mu^2 \right).
$$
PROOF?



### Distribution of the sample mean: $n = 2$, sampling *without* replacement

The mean of this population is $\mu = 4$:
...
The variance of this population is $\sigma^2 = 4.4$:
...
(This cumbersome formula for the variance is necessary because the **R** function `var()` computes the variance of a *sample*, rather than the variance of a *population* as we have here.^[The formula for the variance of a *sample* is $s^2 = \frac{1}{n - 1}\sum (x - \bar{x})^2$; the formula for the variance of a *population* is $s^2 = \frac{1}{n}\sum (x - \bar{x})^2$... thus the factor of $(n - 1)/n$.])
Now, assume we take a sample of size $n = 2$ from this population, *without* replacement.
...
The mean of a selected sample depends on *which* sample is selected at random.

From this table, the *distribution* of the sample mean itself can be determined (Fig.\ \@ref(fig:SampleMeanDistn)):
...
The mean *of the sampling distribution* is $\mu_{\bar{X}} = 4$:

```{r}
mean(MeanAllSamples)
```

The variance *of the sampling distribution* is $\sigma^2_{\bar{X}} = 1.65$:
...
### Distribution of the sample mean: larger samples, sampling *without* replacement
...
The mean of all possible samples is the same ($\mu_x = 4$), but the variance is smaller ($\sigma^2_{\bar{X}} = 0.7333$ rather than $1.65$).
...
### Distribution of the sample mean: $n = 2$, sampling *with* replacement {#SamplingWithReplacement}

All the above example use sampling *without replacement*.
However, *sampling randomly with replacement* could be used.
...
```{r SampleMeanDistnWithReplacement, fig.align="center", fig.cap="The distribution of the sample means", fig.width=5, fig.height=3, out.width="60%"}
...
Then, the mean and variance of the sampling distribution in this case are $\mu_{\bar{X}} = 4$ and $\sigma^2_{\bar{X}} = 2.2$ respectively.
...
In principle, the distribution of *any* statistic could be found (for a mean, median, variance, range, etc.) using the approach in the example above.
For example, we can display the distribution of the sample range:

```{r SampleRangeDistnWithReplacement, fig.align="center", fig.cap="The distribution of the sample range", fig.width=5, fig.height=3, out.width="60%"}
## Population range:
...
This means that sample means, sample standard deviations, sample medians all are random variables.
:::


This knowledge is essential in extracting information from samples. 
...
The weight of an envelope is distributed normally with mean 3\ g and standard deviation $0.6$\ g. 
Any weighed pack of envelopes is declared as containing $25$ if it weighs between $70$\ g and $80$\ g.
...
```{r EnvelopeDist, echo=FALSE, out.width='80%', fig.width=7, fig.height=3, fig.align="center", fig.cap="The envelope question. Left panel: The distribution of the weight of individual envelopes; right panel: The distribution of the weight of packs of $25$ envelopes."}
par( mfrow = c(1, 2))
...
The diameter of the plunger can be considered a normal random variable with mean $2.1$\ cm and standard deviation $0.1$\ cm.
The inside diameter of the cylindrical tube is a normal random variable with mean $2.3$\ cm and standard deviation $0.05$\ cm. 
...
$$
follows a $t_{n - 1}$ distribution where $\bar{X}$ is the sample variance and $S^2$ is the sample variance as defined in\ \@ref(eq:SampleVarDist).
:::
...
:::
SO WHAT? THAT NEEDS SOME EXTRA EXPLANATION TO MAKE THE CONNECTIONS.
...
$$
  ...
  = \frac{Z^2/1}{V/n}
$$
and the numerator has a $\chi^2_1$ distribution; hence, $T^2$ has a $F_{1, n}$ distribution.
...
::: {.importantBox .important data-latex="{iconmonstr-warning-8-240.png}"}
The *Central Limit Theorem* states that, for *any* random variable $X$, the distribution of the *sample means* will have an:
...
A soft-drink vending machine is set so that the amount of drink dispensed is a random variable with a mean of $200$\ mL and a standard deviation of $15$\ mL.
What is the probability that the *average* amount dispensed in a random sample of size\ $36$ is at least $204$\ mL?
...
1. Find the probability that, in a sample of $20$ cartons,  the sample mean weight will exceed $59.5$ grams. 
