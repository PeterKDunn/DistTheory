...
$$
   2\qquad 3\qquad 3\qquad 6\qquad 8.
$$
The mean of this population is $4.4$:

```{r}
Population <- c(2, 3, 3, 6, 8)
mean(Population)
```

The variance of this population is $5.04$:
...
(This cumbersome formula for the variance is necessary because the **R** function `var()` computes the variance of a *sample*, rather than the variance of a *population* as we have here.)
...
We can use **R** to determine all possible samples of size $n = 2$:
...
This distribution is known as the *sampling distribution of the sample mean*, recognising that the distribution is that of the sampling mean.

The mean *of the sampling distribution* is $\mu_{\bar{X}} = 4.4$:

```{r}
mean(MeanAllSamples)
```

The variance *of the sampling distribution* is $\sigma^2_{\bar{X}} = 1.89$:
...
This example can be repeated, assuming *sampling randomly with replacement*.
In this case, there are $5\times 5 = 25$ equally-likely samples possible:

```{r}
AllSamplesWithReplacement <- expand.grid(Population, Population)
names(AllSamplesWithReplacement) <- c("First", 
                                      "Second")
MeanAllSamplesWithReplacement <- rowMeans(AllSamplesWithReplacement)

cbind(AllSamplesWithReplacement,
...
```{r SampleMeanDistnWithReplacement, fig.align="center", fig.cap="The distribution of the sample means"}
...
Then, the mean and variance of the sampling distribution in this case are $\mu_{\bar{X}} = 4.4$ and $\sigma^2_{\bar{X}} = 2.52$ respectively.
...
Observe that the mean of the sampling distribution in the example (when taken *without replacement*) is the same as the mean of the population itself: $\mu = (2 + 3 + 3 + 6 + 8 )/5 = 4.4$.
