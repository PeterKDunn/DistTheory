...
But there are ${N\choose n}$ samples of size $n$ sampling without replacement from $N$ objects. 
Hence in our example there are ${5\choose 2} = 10$ equally likely possible samples.
For example, the sample $\{2,6\}$ has a probability of 1/10 of being selected. 
Further, the mean of this sample, is 4.0.

We can systematically list all 10 samples as shown in Table~\ref{TB:simpsamp1}.

\begin{table}
$$\vbox{\offinterlineskip
\def\jump{\omit&height3truept&&&\cr}
\halign{\strut\enskip\hfil#\hfil\enskip&&\vrule#&\enskip\hfil#\hfil\enskip\cr
Sample&&Mean&&Probability\cr
\jump
\noalign{\hrule}
2,3&&2.5&&1/10\cr
2,3&&2.5&&1/10\cr
...
$$
\caption{List of samples in Example~\ref{EG:simpsamp}.}\label{TB:simpsamp1}
\end{table}

From this table we can obtain the distribution of the mean itself---see Table~\ref{TB:simpsamp2}.
...
\def\jump{\omit&height3truept&&&&&&&\cr}
\halign{\strut\enskip\hfil#\hfil\enskip&\vrule#&&\enskip\hfil#\hfil\enskip\cr
Mean&&2.5&3.0&4.0&4.5&5.0&5.5&7.0\cr
\jump\noalign{\hrule}\jump
Probability&&0.2&0.1&0.1&0.2&0.1&0.2&0.1\cr
}}$$
\caption{Sampling distribution of the mean in Example~\ref{EG:simpsamp}.}
\label{TB:simpsamp2}
\end{table}


Notice we can determine the mean and variance of the distribution as $\mu_{\bar{X}} = 4.4$ and $\sigma^2_{\bar{X}} = 1.89$.

It is instructive to repeat this example, this time sampling randomly *with replacement*.
...
In principle we could obtain the distribution of any statistic, whether it be a mean, median, variance, range or whatever, using the approach in Example~\ref{EG:simpsamp}.
...
In terms of distribution theory we should see Table~\ref{TB:simpsamp2} as just the distribution of a random variable, since a sample mean is a random variable. 
...
(The exact relationship between $\sigma^2_{\bar{X}}$ and $\sigma^2$ is expanded on in Theorem~\ref{TM:meandist} below.)
...
The point is, a rv is a numeric variable which is fully determined by the values of all, some or even no members of the sample.
...
In this definition, the distribution function has been used to cater for both discrete and continuous rvs.

It should be pointed out that this definition of a random sample is standard in theoretical statistics.
In the practice of statistics however there are many sampling designs involving randomness; eg simple random sampling, stratified sampling, systematic sampling. 
These are often said to give rise to 'random samples' even though the samples produced don't necessarily satisfy Definition~\ref{DF:randomsample}.
It is usually clear from the context whether the strict or loose meaning of 'random sample' is intended.

When a sample of size $n$ is assumed to be chosen 'at random' *without replacement* from a population of size $N$, does this sample constitute a random sample in the sense of Definition~\ref{DF:randomsample}?
...
For example, in Example~\ref{EG:simpsamp}, consider say the second member of the sample, $X_2$.
...
Note also in Example~\ref{EG:simpsamp}, if the sampling is *with replacement*, we have that $\Pr(X_2 = 2 \mid X_1 = 2) = \Pr(X_2 = 2) = \Pr(X_1 = 2)$ and a similar statement can be made for all sample members and all values of the population. 
...
:::{.proof}
This is a direct application of Corollary~\ref{CR:varlincomb} with $a_i = 1/n$.
:::


Theorem~\ref{TM:meandist} applies to a random sample as defined in~\ref{DF:randomsample}. 
The distribution of the population from which the sample is drawn is irrelevant.

It is interesting to note what happens if a sample is randomly selected without replacement from a finite population as described in~\ref{SS:randomsampling}.
...
:::{.example #Exprain name="Exponential distributions"}
In Example~\ref{EG:simpsamp} we have that $\mu = 4.4$ and $\sigma^2 = 5.04$ for the population.

If sampling is done without replacement, the sampling distribution of the mean has mean $\mu_{\bar{X}} = \mu = 4.4$ and variance $\text{Var}(\bar{X}) = 1.89 = \frac{5.04}{2}\frac{(5 - 2)}{(5 - 1)}$ in agreement with the note following Theorem~\ref{TM:meandist}.

If sampling is done with replacement we see $\mu_{\bar{X}} = \mu=4.4$ and $\text{Var}(\bar{X}) = 2.52 = \ds{\frac{5.04}{2}}$ in agreement with the theorem.
:::


Theorem~\ref{TM:meandist} describes the location and spread of the sampling distribution of the mean.
...
For large samples it turns out that it doesn't as we see in Section~\ref{SC:CLT}.
...
:::{.theorem}
Let $X_1, X_2, \dots, X_n$ be a set of independent rvs where $X_i\sim N(\mu_i,\sigma^2_i)$. 
...
:::{.proof}
In Theorem~\ref{TM:normal} we showed the mgf of the rv $X_i$ to be
\[ 
   M_{X_i} (t)
   =\exp\{\mu_it+\frac 12 \sigma^2_it^2\}\quad i=1,2,\dots,n.
\]
So, for a constant $a_i$,
\begin{align*}
     M_{a_iX_i}(t) & = M_{X_i}(a_it)\quad\text{(using Theorem 3.3 with} \quad\beta =0, \ \alpha = a_i)\\
     & = \exp\{\mu_ia_it+\frac 12 \sigma^2_ia^2_it^2\}.
\end{align*}
Since the mgf of a sum of independent rv's is equal to the product of their mgf's we have
\begin{align*}
     M_Y(t) & = \prod^n_{i=1} \exp\{\mu_ia_it+\frac 12
\sigma^2_ia^2_it^2\}\\
     & = \exp\{t\Sigma a_i\mu_i +\frac 12 t^2\Sigma a^2_i
\sigma^2_i\}.
\end{align*}
This is of the form of the mgf of a normal rv with mean, $\text{E}(Y) = \Sigma a_i\mu_i$ and variance, $\text{Var}(Y) = \Sigma a^2_i\sigma^2_i$.
:::


The sampling distribution of the sum and mean of a random sample from a normal population follows directly from
Theorem~\ref{TM:normlinsum}.


:::{.corollary}
Let $X_1, X_2, \dots, X_n$ be a random sample of size $n$ from $N(\mu,\sigma^2)$. 
Define the sum $S$ and mean respectively as
\begin{align*}
   S
   &=X_1+X_2 + \dots + X_n\\
   \bar{X}
...
Corollary~\ref{CR:normmeasum} (Theorem~7.1 in \WMS) relies only on the population from which the sample is drawn being normally distributed and on the properties of expectation.
It forms the basis for inference about the population mean of a normal distribution with known variance.

:::{.example #Exprain name="Exponential distributions"}
Suppose that envelopes are counted out in packets of 25 by weighing them, and that the weight of an envelope is distributed normally with mean 3\,gm and standard deviation 0.6\,gm. 
Any weighed pile of envelopes is 'counted' as 25 if it weighs between 70 and 80\,gm.
What is the probability that a pile of 25 will not be counted as such?

Let rv $X_i$ be the weight of the $i$th envelope. 
Then $X_i$ is distributed $N(3,0.36)$. 
Let $Y=X_1+X_2+\dots +X_{25}$. 
Then $Y$ is distributed $N(75, 9)$. 
We require
\begin{align*}
     1-\Pr{70<Y<80} & = 1-\Pr{\frac{70-75}{\sqrt{9}}<Z<\frac{80 - 75}{\sqrt{9}}}\\
...
We have
\[ 
   \Pr{|\bar{X}-110|\leq 5}=.90 .
\]
That is
\[ 
   \Pr{\frac{|\bar{X}-110|\sqrt{n}}{12} \leq \frac{5\sqrt{n}}{12}} = 0.90 . 
\]
That is,
\[ 
...
:::{.example #Exprain name="Exponential distributions"}
A certain product involves a plunger fitting into a cylindrical tube and the 2 items are manufactured in such a way that the diameter of the plunger can be considered a normal rv with mean 2.1\,cm and s.d. $0.1$\,cm while the inside diameter of the cylindrical tube is a normal rv with mean 2.3\,cm and s.d. $0.05$\,cm. 
...
\begin{align*}
     \Pr(Y - X < 0) 
     & = \Pr(Z <\frac{0 - 0.2}{\sqrt{.0125}}) \quad\text{where }Z\sim N(0,1)\\
     & = \Pr(Z < -1.78)\\
     & = 0.0375
\end{align*}
:::
...
Firstly a preliminary result which is little more than a rewording of Theorem~\ref{TM:chin}.

:::{.theorem}
Let $X_1,X_2,\dots,X_n$ be a random sample of size $n$ from $N(\mu,\sigma^2)$.
Then $\sum_{i=1}^n \left(\ds{\frac{X_i-\mu}{\sigma}}\right)^2$ has a chi-square distribution with $n$ degrees of freedom.
:::


:::{.proof}
Putting $Z_i = \ds{\frac{X_i-\mu}{\sigma}}$ we have
\[
...
Also the rvs $Z_i$ are independent because the $X_i$ are independent ($i = 1,2,\dots,n$).
The required result follows directly from Theorem~\ref{TM:chin}.
:::
...
We have, using (\ref{EQN:sampvar}),
\[
   \frac{(n - 1)S^2}{\sigma^2}
   = \sum_{i = 1}^n \left(\frac{X_i - \bar{X}}{\sigma}\right)^2
\]
which looks much like the conditions for Theorem~\ref{TM:chin2}. 
...
\end{align*}
(Check the algebra for yourself.)
...
\begin{align*}
   ...
   &= \sum_{i=1}^n \left(\frac{X_i-\bar{X}}{\sigma}\right)^2
\end{align*}
Now $T\sim\chi^2(n)$ (by Theorem~\ref{TM:chin2}) and $V\sim\chi^2(1)$ because $V$ is the square of a standard normal rv (since $\bar{X}\sim N(\mu,\sigma^2/n)$.
...
By Theorem~\ref{TM:variancedist},
\[
   \frac{(n - 1)S^2}{\sigma^2}
...
The basis of the independence assumed in the proof of Theorem~\ref{TM:variancedist} is the rather surprising result that the mean and variance of a random sample from a normal population are independent rvs as stated in the following
theorem.
...
The pdf of the $t$ distribution is very similar to the standard normal distribution, being bell-shaped and symmetric about zero. 
...
\]
follows a $t(n - 1)$ distribution where $\bar{X}$ is the sample variance and $S^2$ is the sample variance as defined in (\ref{EQN:sampvar}).
:::
...
The derivation of the pdf of $\ds{\frac{Z}{Y/(n - 1)}}$ is addressed in \DGS, Section~7.4 and \WMS, Exercise~7.56. 
This proof is not examinable.
:::
...
2. In random samples from this population, what is the probability that $T$ is less than the value found in~(a)?

From the sample $\overline{x} = 19.0$ and $s^2 = 12.0$. 
Therefore $t = \frac{19.0 - 20}{\sqrt{12.0/5}} = -0.645$.
(Notice we have used lower-case symbols for specific values of statistics and upper-case for the rvs, but don't lose any sleep over the distinction!)
...
Definition~\ref{DF:Fdist} introduces a distribution which describes the ratio of two sample variances from normal populations and is used in inferences concerning the comparison of two variances.
This distribution is also used in *analysis of variance*, a technique used to test the equality of several means.
...
* For $\nu_2 > 2$, $\text{E}(F)=\ds{\frac{\nu_2}{\nu_2-2}}$.
* For $\nu_2>4$, $\text{Var}(F)=\ds{\frac{2\nu_2^2(\nu_1+\nu_2-2)} {\nu_1(\nu_2-2)^2(\nu_2-4)}}$.
...
\begin{align*}
   U_1 
   &= \ds{\frac{(n_1-1)S_1^2}{\sigma_1^2}}$\\
   \nu_1 
   &= n_1 - 1, \\
   U_2 
   &= \ds{\frac{(n_2-1)S_2^2}{\sigma_2^2}}$, 
   \nu_2 
   &= n_2 - 1.
\end{align*}
...
Quantiles of the $F$ distribution are available in most statistics texts including \WMS\ and \DGS. 
...
Sampling distributions for various statistics of interest are well developed when sampling is from a normal distribution as shown in Section~\ref{SC:normsamp}.
Although these results are important, the question remains, what say we don't know, as is usually the case, the distribution of the population from which the random sample is drawn?

In Section~\ref{SS:meandist} general results are given describing the mean and variance of the sample mean which hold for any population distribution.
Can we say more than this about the sampling distribution of the mean in general?

The answer is yes of course---why else would we mention it!  
...
:::{.proof}
Let the rv $X_i$ $(i = 1, \dots, n)$ have mgf $M_{X_i}(t)$. 
Then
\[
   M_{X_i}(t) 
...
\begin{align*}
  ...
  &= e^{-\sqrt{n}\mu t / \sigma} \left[ M_{X_i}\left(\frac{\sqrt{n}t}{\sigma n}
  \right)\right]^n\\
  &= e^{-\sqrt{n}\mu t /\sigma}\left[ M_{X_i}(t / \sigma \sqrt{n})\right]^n
\end{align*}
Then
\begin{align*}
     \log M_{Z_n}(t) =\,&\frac{-\sqrt{n}\mu t}{\sigma}+n\log \left[ 1+\frac{\mu'_1t}{\sigma\sqrt{n}}+\frac{\mu'_2t^2}{2!n\sigma^2}
...
The 'convergence in distribution' bit is saying that $Z_n$ approaches normality as $n\to \infty$. So when $n$ is 'large' we can expect $Z_n$ to approximate a standard normal distribution and, transforming $Z_n$ back to the sample mean, we can expect the $\bar{X}$ to approximate a $N(\mu,\sigma^2/n)$ distribution.
...
($P_N(A)$ denotes the probability
of event $A$ involving a rv assumed to be normal in distribution.)
:::
...
Let the rv $X_i$ be the number showing on the $i$th throw and define $Y = X_1 + \dots + X_{12}$. 
We want $\Pr(Y\geq 52)$.

In order to use Theorem~\ref{TM:CLT} note that the event '$Y\geq 52$' is equivalent to '$\bar{X}\geq 52/12$' where $\bar{X} = Y/12$ is the mean number showing from the 12 tosses. 
...
Then from Theorem~\ref{TM:CLT},
\begin{align*}
  \Pr{Y\geq 52} &\simeq \Pr{\bar{X}\geq 52/12}\\
...
Example~\ref{EG:dieclt} can also be solved using a generalisation of the Central Limit Theorem.
This generalisation indicates why the normal distribution plays such an important role in statistical theory. 
It says that a large class of rv's converge in distribution to the standardized normal.

:::{.theorem}
Let $X_1, X_2, \dots, X_n$ be a sequence of independent rv's with $\text{E}(X_i) = \mu_i$, $\text{Var}(X_i) = \sigma^2_i$.
...
This is clearly an example of Theorem~\ref{TM:CLTgen} with each $a_i = 1$. 
In order to find $\Pr(V > 105)$ we need to know the distribution of $V$. 
We have $\text{E}(V_i) = 5$ and $\text{Var}(V_i) = 100/12$ so that from Theorem~\ref{TM:CLTgen}, we can say that $V$ is distributed approximately normal with mean $20\times 5 = 100$ and variance $20\times 100/12$.
...
The normal approximation to the binomial distribution has already been considered in Section~\ref{SEC:speccont:normalbin}; it is seen here again as an application of the Central Limit Theorem.
...
\]
Theorems~\ref{TM:CLT} and~\ref{TM:CLTgen} are applicable to $\bar{X}$ and $Y$ respectively. 
Hence
\[
